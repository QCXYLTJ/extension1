window.XJZHimport(function (lib, game, ui, get, ai, _status) {
  ...
  game.xjzh_filterEligible = function () {
    if (!_status.xjzhDebug) {
      if (!game.me || !get.isXHwujiang(game.me)) return false;
      if (get.mode() == 'identity') {
        if (game.countPlayer2() < 5 || game.countPlayer2() > 12) return false;
      }
  ...
  lib.onover.push(function (ret) {
    if (ret) {
      //女巫<地狱之火>成就
      if (get.mode() == 'identity' && game.me && get.is.playerNames(game.me, 'xjzh_poe_nvwu')) {
        let storage = game.me.storage.xjzh_poe_zhaohuan;
      ...
      //左幽<微妙玄通>成就
      if (get.mode() == 'identity' && game.me && get.is.playerNames(game.me, 'xjzh_sanguo_zuoyou')) {
        let storage = game.me.storage.xjzh_sanguo_tongxuan;
      ...
      if (get.mode() == 'identity' && game.me && get.is.playerNames(game.me, 'xjzh_sanguo_espliuxie')) {
        ...
        var obj = new Object(),
          list = ['xjzh_sanguo_tiance', 'xjzh_sanguo_tianming', 'xjzh_sanguo_moubian', 'xjzh_sanguo_zhongxing'];
  ...
  game.xjzhAchi = {
    ...
    ofName(name) {
      ...
      return [{ character: 'character', game: 'game', special: 'special' }[names[0]], names[1]];
    },
    //获得成就文本
    getInfoName(name) {
    ...
    openAchievementEquipPage() {
      ...
      duihuanBox.listen(function () {
        ...
        var inputDiv = ui.create.div(blank, {
          ...
          textAlign: 'center',
          backgroundSize: '100%',
          backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/duihuanma.png)'
        });
      ...
      var state = {
        ...
        refreshPage() {
          //刷新书签
          if (state.zhuanshu_on) zhuanshu.style.backgroundImage = 'url(extension/仙家之魂/css/images/qishuyaojian/zhuanshu_on.png)';else
          zhuanshu.style.backgroundImage = 'url(extension/仙家之魂/css/images/qishuyaojian/zhuanshu.png)';
          if (state.tongyong_on) tongyong.style.backgroundImage = 'url(extension/仙家之魂/css/images/qishuyaojian/tongyong_on.png)';else
          tongyong.style.backgroundImage = 'url(extension/仙家之魂/css/images/qishuyaojian/tongyong.png)';
          //刷新奇术要件
          for (var i = 0; i < state.showing.length; i++) {
            state.showing[i].remove();
          }
          state.showing.length = 0;
          if (state.pageNum == 0) arrow2.style.display = 'none';else
          arrow2.style.display = 'inline';
          state.equipNum = 0;
          for (var i in lib.xjzh_qishuyaojians) {
            if (state.filter(i)) state.equipNum++;
          }
          if (state.equipNum / 8 - 1 <= state.pageNum) arrow.style.display = 'none';else
          ...
          list.sort(function (a, b) {
            var level1 = get.xjzh_equipInfo(a).level || 1,
              level2 = get.xjzh_equipInfo(b).level || 1;
          ...
          for (var i = 0; i < 8 && num + i < list.length; i++) {
            ...
            equipShow.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cards/' + equip + '.jpg)';
            var index = i;
            if (index < 4) equipShow.style.top = '15%';else
            equipShow.style.top = '52%';
            switch (i) {
              case 0:
              case 4:
            ...
            equipShow.onmouseover = function (event) {
              ...
              if (!item) return;
              var str = '';
              str += '<span style="font-family:shousha;"><span style="font-size:18px;font-weight:600">' + item.translate + '</span><br>';
    ...
    openShowRunePack(item, type, fuwen, state) {
      var bookWindow = ui.create.div('.xjzh-bookWindow', {
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'center',
        alignItems: 'center',
        height: '100vh', // 设置 bookWindow 高度为视口高度
        width: '100vw', // 设置 bookWindow 宽度为视口宽度
        position: 'fixed',
        top: 0,
        left: 0,
        backgroundColor: 'rgba(0, 0, 0, 0.5)', // 添加背景遮罩
        zIndex: 1000 // 确保 bookWindow 在最上层
      });
      document.body.appendChild(bookWindow);
      var runePack = ui.create.div('.xjzh-runePack', bookWindow);
      var textPack = ui.create.div(runePack, {
        width: '100%',
        height: '100%',
        fontSize: '30px',
        fontFamily: 'xinwei',
      ...
      var runes = get.xjzh_runeListName(type);
      runes.forEach((rune, index) => {
        // 设置容器为 Flexbox 容器
        runePack.style.display = 'flex';
        runePack.style.flexWrap = 'wrap';
        runePack.style.justifyContent = 'space-between';
        // 创建 runeDiv 元素,移除绝对定位
        var runeDiv = ui.create.div(runePack, {
          width: '30%',
          height: '30%',
          ...
          margin: '5px',
          position: 'relative',
          backgroundImage: `url('extension/仙家之魂/css/images/runes/${type === 'pray' ? 'prayRunes' : 'ritualRunes'}.png')`
        });
        runeDiv.listen(function () {
          if (game.xjzh_hasAllEquipRunes(item, rune)) {
            window.xjzhOpenLoading('你已经装备相同组合的符文,无法再次装备!');
          } else {
            game.xjzh_equipRune(item, rune);
            game.xjzhAchi.openAchievementEquipIntro(item, state);
          }
          bookWindow.delete();
          game.resume2();
        });
        var runDivNumber = ui.create.div(runeDiv, {
          position: 'absolute',
          top: '55%',
          left: '38%',
          color: 'rgb(38, 218, 14)',
        ...
        runDivNumber.innerHTML = `✘${get.xjzh_runeListNumber(rune)}`;
        runeDiv.onmouseover = function (event) {
          var str = '';
          str += '<span style="font-family:shousha;"><span style="font-size:18px;font-weight:600">' + get.xjzh_runeTranslate(rune, type) + '(' + get.xjzh_runeTypeTranslate(rune) + ')</span><br>';
        ...
        runeDiv.onmousemove = function (event) {
          intro.style.left = (event.clientX + 10) / game.documentZoom + 'px';
          intro.style.top = (event.clientY + 10) / game.documentZoom + 'px';
        };
        runeDiv.onmouseout = function () {
          intro.hide();
        };
        var removeIntroOnWheel = function (event) {
          if (event.deltaY !== 0) {
            intro.remove();
            runeDiv.removeEventListener('wheel', removeIntroOnWheel);
          }
        };
        runeDiv.addEventListener('wheel', removeIntroOnWheel);
      });
    },
    //打开奇术要件信息页面
    openAchievementEquipIntro(item, state) {
      if (!game.getExtensionConfig('仙家之魂', 'xjzh_qishuyaojianOptions') || game.getExtensionConfig('仙家之魂', 'xjzh_qishuyaojianOptions') === 'close') return;
      //覆盖图层
      var bookWindow = ui.create.div('.xjzh-bookWindow');
      document.body.appendChild(bookWindow);
      //背景图层
      var bk = ui.create.div('.xjzh-bookWindow-bk', bookWindow, {
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/bk.png)'
      });
      //退出按钮
      var exit = ui.create.div('.xjzh-bookWindow-return', bk);
      exit.listen(function () {
      ...
      var nameDialog = ui.create.div(bk, {
        ...
        left: '53%',
        width: '20%',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/kuang.png)',
        backgroundSize: '100%'
      });
      ...
      var mengban = ui.create.div(bk2, {
        ...
        width: '33%',
        backgroundRepeat: 'no-repeat',
        backgroundImage: 'url(extension/仙家之魂/image/qishuyaojian/lihui/' + item + '.jpg)',
        backgroundSize: '100%'
      });
      //奇术要件介绍
      ...
      var changeBtm = ui.create.div(bk, {
        ...
        width: '3%',
        backgroundRepeat: 'no-repeat',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/changeBtm.png)',
        backgroundSize: '100%'
      });
      ...
      var line = ui.create.div(bk, {
        ...
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/xian.png)',
        backgroundSize: '100%'
      });
      //装配文字
      var numText = ui.create.div(bk, {
        ...
        color: 'rgb(41, 25, 1)',
        fontFamily: 'xingkai',
        fontSize: ui.window.offsetHeight * 0.04 + 'px',
        textShadow: 'none'
      });
      ...
      if (num >= 1) {
        var qishudisen = ui.create.div(bk, {
          ...
          width: '3%',
          backgroundRepeat: 'no-repeat',
          backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/disintegrate.png)',
          backgroundSize: '100%'
        });
      ...
      //装备符文按钮
      var qishuEquip = ui.create.div(bk, {
        top: '81.8%',
        height: '12%',
        left: '53%',
        width: '30%',
      ...
      if (ritualList.length) {
        ...
        var ritualFuwen = ui.create.div(qishuEquip, {
          ...
          backgroundRepeat: 'no-repeat',
          backgroundSize: '80%',
          backgroundImage: `url('extension/仙家之魂/css/images/runes/${game.xjzh_hasEquipRunes(item, 'ritual') ? 'ritualRunes' : 'ritualRunesImage'}.png')`
        });
        ritualFuwen.listen(function () {
          ...
          if (game.xjzh_hasEquipRunes(item, 'ritual')) {
            game.xjzh_unEquipRune(
              item,
              runesEquips.find((i) => get.xjzh_runeType(i) == 'ritual')
            );
            ritualFuwen.setBackgroundImage(`extension/仙家之魂/css/images/runes/ritualRunesImage.png`);
            game.xjzhAchi.openAchievementEquipIntro(item, state);
          } else {
        ...
        if (game.xjzh_hasEquipRunes(item, 'ritual')) {
          let rune = runesEquips.find((i) => get.xjzh_runeType(i) == 'ritual'),
            type = get.xjzh_runeType(rune);
      ...
      if (prayList.length) {
        ...
        var prayFuwen = ui.create.div(qishuEquip, {
          ...
          backgroundRepeat: 'no-repeat',
          backgroundSize: '80%',
          backgroundImage: `url('extension/仙家之魂/css/images/runes/${game.xjzh_hasEquipRunes(item, 'pray') ? 'prayRunes' : 'prayRunesImage'}.png')`
        });
        prayFuwen.listen(function () {
          ...
          if (game.xjzh_hasEquipRunes(item, 'pray')) {
            game.xjzh_unEquipRune(
              item,
              runesEquips.find((i) => get.xjzh_runeType(i) == 'pray')
            );
            prayFuwen.setBackgroundImage(`extension/仙家之魂/css/images/runes/prayRunesImage.png`);
            game.xjzhAchi.openAchievementEquipIntro(item, state);
          } else {
        ...
        if (game.xjzh_hasEquipRunes(item, 'pray')) {
          let rune = runesEquips.find((i) => get.xjzh_runeType(i) == 'pray'),
            type = get.xjzh_runeType(rune);
      ...
      state.refreshDialog = function () {
        ...
        if (num > 0) {
          var addPlayer = ui.create.div(characterDialog, {
            ...
            left: i * 30 + 2 + '%',
            width: '25%',
            backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/touxiang.png)',
            backgroundSize: '100%'
          });
      ...
      var left = ui.create.div(bk, {
        ...
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/left.png)',
        backgroundSize: '115% 99%'
      });
    ...
    choosePlayer(item, state, bk) {
      ...
      if (!state.xjzh_onOpen) {
        ...
        if (info.filter) {
          ...
          xianhunCharacter = game.getExtensionConfig('仙家之魂', 'xjzh_qishuyaojianOptions') === 'own' ? lib.characterPack.XWTR : { ...lib.character };
        ...
        var xianhunCharacterGroup = Object.keys(xianhunCharacter),
          groups = [];
          ...
          if (node.classList.contains('thundertext')) {
            ...
            for (var i = 0; i < characterD.buttons.length; i++) {
              if (characterD.currentcapt && characterD.buttons[i].capt != characterD.getCurrentCapt(characterD.buttons[i].link, characterD.buttons[i].capt)) {
                ...
                if (characterD.buttons[i]._changeGroup || characterD.buttons[i].group == 'ye') characterD.buttons[i].classList.remove('nodisplay');else
                characterD.buttons[i].classList.add('nodisplay');
              } else {
        ...
        if (lib.characterDialogGroup[lib.config.character_characterD_tool] || lib.config.character_characterD_tool == '自创') {
    ...
    openAchievementChoujiang() {
      ...
      //覆盖图层
      var bookWindow = ui.create.div('.xjzh-bookWindow', {
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/choujiang.png)'
      });
      document.body.appendChild(bookWindow);
      //背景图层
      var bk = ui.create.div('.xjzh-bookWindow-bk', {
        backgroundSize: 'contain',
        backgroundRepeat: 'no-repeat',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/choujiang2.png)'
      });
      //退出按钮
      var exit = ui.create.div(bookWindow, {
        ...
        backgroundRepeat: 'no-repeat',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/exit.png)',
        backgroundSize: '80%'
      });
      exit.listen(function () {
        bookWindow.delete();
      ...
      var tokens = ui.create.div(bk, {
        ...
        height: '7%',
        backgroundSize: '100%',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/jingpolan.png)'
      });
      ...
      var suipian = ui.create.div(bk, {
        ...
        height: '7%',
        backgroundSize: '100%',
        backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/suipianlan.png)'
      });
      ...
      //抽奖
      var map = {
        6: ['jingpo', 'xjzh_cailiao_mingyushi'],
      ...
      var chou = function () {
        ...
        for (var i = 0; i < cishu; i++) {
          var num = Math.random();
          if (num < 0.07) {
            var item = map[6].randomGet();
            item == 'jingpo' ? game.xjzh_changeTokens(1) : game.xjzh_changeCailiao(item, 1);
            result.push(item == 'jingpo' ? item : [item, 1]);
          } else if (num < 0.1) {
          ...
          if (cishu == 1) {
            ...
            if (Array.isArray(result[0])) {
              if (result[0][0] == 'suipian') {
                show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/suipian.jpg)';
              } else {
                show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/' + result[0][0] + '.jpg)';
              }
              show.innerHTML = '✘' + result[0][1] + '';
            } else if (result[0] == 'jingpo') {
              show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/jingpo.jpg)';
            } else {
              show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cards/' + result[0] + '.jpg)';
            }
          } else {
            var list = [];
            for (var i = 0; i < 10; i++) {
              ...
              if (Array.isArray(result[i])) {
                if (result[i][0] == 'suipian') {
                  ...
                  show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/' + result[i][0] + '.jpg)';
                }
                show.innerHTML = '✘' + result[i][1] + '';
              } else if (result[i] == 'jingpo') {
                show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/jingpo.jpg)';
              } else {
                show.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cards/' + result[i] + '.jpg)';
              }
            }
      ...
      var openShop = function () {
        ...
        var list = [
        {
          name: 'jingpo',
          setImage(btm) {
            btm.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/jingpo.jpg)';
        ...
        {
          name: 'xjzh_cailiao_mingyushi',
          setImage(btm) {
            btm.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/xjzh_cailiao_mingyushi.jpg)';
            btm.style.backgroundSize = '100% 100%';
            btm.style.backgroundPosition = '50% 60%';
            var suipians = ui.create.div(btm, {
              top: '3%',
          ...
          content() {
            game.xjzh_changeCailiao('xjzh_cailiao_mingyushi', 1);
          },
          price: 260
        },
        //等阶1
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (typeof link == 'string') {
            btm.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cards/' + link + '.jpg)';
            let info = get.xjzh_equipInfo(link),
              level = info.level || 1;
            switch (level) {
              case 1:
                price = info.filter ? 50 * (1 + level) : 50;
                break;
              case 2:
                price = info.filter ? 100 * (1 + level) : 100;
                break;
              case 3:
                price = info.filter ? 150 * (1 + level) : 150;
                break;
              case 4:
                price = info.filter ? 230 * (1 + level) : 230;
                break;
              case 5:
                price = info.filter ? 320 * (1 + level) : 320;
                break;
              default:
                price = 50;
            }
          ...
          if (i % 5 == 0) {
            var priceLan = ui.create.div(duihuanWindow, {
              ...
              height: '30%',
              backgroundSize: '100%',
              backgroundImage: 'url(extension/仙家之魂/css/images/qishuyaojian/price.png)'
            });
          }
        }
        //大小调整
      ...
      var openYulan = function () {
        ...
        //奖励列表
        var list = ['suipian', 'jingpo', 'xjzh_cailiao_shijieshi', 'xjzh_cailiao_enianzhixin', 'xjzh_cailiao_gangtie', 'xjzh_cailiao_xianxue', 'xjzh_cailiao_kongju', 'xjzh_cailiao_nianyedan', 'xjzh_cailiao_wawa', 'xjzh_cailiao_jiasuo', 'xjzh_cailiao_mingyushi', 'xjzh_cailiao_kutong'];
        for (var i in lib.xjzh_qishuyaojians) {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (link == 'jingpo') {
            ...
            btm.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/suipian.jpg)';
            btm.listen(function () {
              var boxRemove = ui.create.div(ui.window, {
                zIndex: 10000,
                width: '100%',
          ...
          } else if (link.includes('cailiao')) {
            btm.style.backgroundImage = 'url(extension/仙家之魂/image/qishuyaojian/cailiao/' + link + '.jpg)';
            btm.item = link;
            btm.listen(function () {
              var boxRemove = ui.create.div(ui.window, {
                zIndex: 10000,
    ...
    openAchievementView(type) {
        ...
        refreshList() {
          ...
          for (var name of list) {
            ...
            if (isFirst) {
              ...
              text += "<br><p align='center'><img src=extension/仙家之魂/css/images/achievement/splitLine.png></p><br>";
            }
            //<--
            ...
            if (game.getExtensionConfig('仙家之魂', 'xjzhAchiStorage').got.includes(name2)) {
              text += "<img src='extension/仙家之魂/css/images/achievement/isGained.png' style='height:60px;'/>";
            }
            //<--
            //显示成就名
            ...
            for (var i = 0; i < info.level; i++) {
              text += "<img src='extension/仙家之魂/css/images/achievement/star.png' style='height:30px;'/>&nbsp;&nbsp;";
            }
            text += '&nbsp;&nbsp;&nbsp;';
            //<--
            //显示达成时间
      ...
      filterButton.listen(function () {
        ...
        hiden_done.innerHTML = '<br><img src="extension/仙家之魂/css/images/achievement/isGained2.png" style="height:30px;"/>&thinsp;显示完成成就&emsp;&thinsp;';
        ...
        hiden_lv3.innerHTML = '<br><img src="extension/仙家之魂/css/images/achievement/star3.png" style="height:30px;"/>&thinsp;显示三星成就&emsp;&thinsp;';
        ...
        hiden_lv2.innerHTML = '<img src="extension/仙家之魂/css/images/achievement/star2.png" style="height:30px;"/>&thinsp;显示二星成就&emsp;&thinsp;';
        ...
        hiden_lv1.innerHTML = '<br><img src="extension/仙家之魂/css/images/achievement/star.png" style="height:30px;"/>&thinsp;显示一星成就&emsp;&thinsp;';
  ...
  Date.prototype.format = function (str) {
    ...
    for (var k in o) if (new RegExp('(' + k + ')').test(str)) str = str.replace(RegExp.$1, RegExp.$1.length == 1 ? o[k] : ('00' + o[k]).substr(('' + o[k]).length));
    return str;
  };
  //成就计量技能
});