...
const buffMap = {
  /**
   * @import {Buff,AllBuffName} from "./@types/SKSN_buff.d.ts"
   * 现在定义新的BUFF时，在lib.sksnBUFF中请不要加前缀sksn_buff_
   * 添加buff时，请将buff名添加到集合 {@link AllBuffName} 中
   * @type {SMap<Buff>}
   */
  ///增益
  qianggu: {
  ...
  zhongdu: {
    ...
    intro: {
      name: '中毒',
      content: '「<font color=yellow>毒素缠身</font>」<br><li>自然衰减：<b>是</b> 上限：3<br><li>使用牌有30%几率失效并受到1点无来源毒属性伤害，移除一层中毒；每层中毒使该几率提高10%'
    },
    trigger: {
      player: ['useCard']
    },
    ...
    async content(event, trigger, player) {
      let num = get.xjzhBUFFNum(player, 'zhongdu') * 0.1,
        bool = false;
  ...
  binghuan: {
    ...
    async content(event, trigger, player) {
      if (trigger.name == 'phaseUse' || trigger.name == 'changexjzhBUFF') {
        let num = get.xjzhBUFFNum(player, 'binghuan'),
          time = 10;
        do {
          num--;
          time *= 0.5;
  ...
  bingdong: {
    ...
    intro: {
      name: '冰冻',
      content: '「<font color=yellow>冰霜衰弱</font>」<br><li>自然衰减：<b>是</b> 上限：1<br><li>当你获得此buff时，弃置所有牌，直到此buff移除，你无法使用或打出牌'
    },
    trigger: {
      ...
      if (type == 'delay') {
        ...
        filterPlayer(function (current) {
          return !current.countCards('j', function (card) {
            return trigger.card.name == card.name;
          });
        }).
        randomGet();
      } else {
...
const buffGet = {
  ...
  //获取BUFF的翻译
  xjzhBUFFtranslate(name) {
    if (typeof name != 'string') return;
    let info;
    if (lib.xjzhBUFF[name]) info = lib.xjzhBUFF[name].intro;else
    return;
    return info.name;
  },
  //获取BUFF的层数
  xjzhBUFFNum(player, name) {
    var buff = get.xjzhBUFFName(name);
    if (!player.storage[buff] || player.storage[buff] <= 0) return 0;
    return player.storage[buff];
  },
  //获取BUFF的rank值（给ai判断用）
  xjzhBUFFRank(player, name, income, plies) {
    ...
    player.getSkills(null, false, false).filter(function (i) {
      if (lib.skill[i] && lib.skill[i].ai && lib.skill[i].ai.buffRank_extra && lib.skill[i].ai.buffRank_extra[name]) {
        list.push(lib.skill[i].ai.buffRank_extra[name]);
      }
    });
    if (!plies || typeof plies != 'number') {
      if (income && typeof income == 'number') plies = income;else
      plies = get.xjzhBUFFNum(player, buff);
    }
    var num = 0;
    for (let i = 0; i < list.length; i++) {
      ...
      if (income !== false) {
        ...
        var random2 = 1;
        if (rank.randomPower) {
          if (Array.isArray(rank.randomPower)) random2 = rank.randomPower[0];else
          random2 = rank.randomPower;
        }
      ...
      if (income !== true) {
        ...
        var random2 = 1;
        if (rank.randomPower) {
          if (Array.isArray(rank.randomPower)) random2 = rank.randomPower[1];else
          random2 = rank.randomPower;
        }
  ...
  */
  xjzhBUFFInfo(name, filter) {
    var buff = get.xjzhBUFFName(name, false);
    var info;
    if (lib.xjzhBUFF[buff]) info = lib.xjzhBUFF[buff].xjzhBuffInfo;else
    return null;
...
Object.assign(get, buffGet);
const buffGame = {
  changexjzhBUFF() {
    var next = game.createEvent('changexjzhBUFF');
    for (let i = 0; i < arguments.length; i++) {
      if (get.itemtype(arguments[i]) == 'player') {
        if (!next.player) next.player = arguments[i];else
  ...
  changexjzhBUFFTo() {
    ...
    for (let i = 0; i < arguments.length; i++) {
      if (get.itemtype(arguments[i]) == 'player') {
        ...
        if (!next.num1) next.num1 = arguments[i];else
  ...
  xjzhBuff_init() {
    for (let i in buffMap) {
      let buff = buffMap[i],
        name = 'xjzh_buff_' + i;
      {
        let content = buff.intro.content;
        if (typeof content == 'string') buff.intro.content = () => content;
      }
      lib.skill[name] = buff;
...
Object.assign(lib.element.player, buffPlayer);
