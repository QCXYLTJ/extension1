...
export async function precontent(xjzh) {
	...
	game.xjzh_removeFiles = (files) => {
		...
		else new Promise((resolve, reject) => window.resolveLocalFileSystemURL(`extension/仙家之魂/${files}`, resolve, reject)).then((directoryEntry) => directoryEntry.removeRecursively());
	};
	/**
	 * 异步复制文件或文件夹。
