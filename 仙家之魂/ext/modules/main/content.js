...
export async function content(config, pack) {
	...
	game.playXH = function (fn, dir, sex) {
		if (lib.config.background_speak) {
			...
			else game.playAudio('../extension/仙家之魂/audio/skill', fn);
		}
	};
	// ---------------------------------------背景音乐------------------------------------------//
	//代码借鉴自《金庸群侠传》
	...
	lib.element.player.xjzh_zhaohuan = function (...args) {
		...
		for (const arg of args) {
			...
			else if (typeof arg == 'number') draw = argument;
			else if (Array.isArray(arg)) hpList = argument;
		}
	...
	game.xjzh_Criticalstrike = async function (...args) {
		...
		for (let arg of args) {
			if (get.itemtype(arg) == 'player') player = arg;
			else if (typeof arg == 'number') {
				if (typeof num == 'number') num2 = arg;
				else num = arg;
