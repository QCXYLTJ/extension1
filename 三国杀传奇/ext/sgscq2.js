window.yb_sgscq_lib = {
	// get: {
	//get.函数
	// SC_type:function(card){
	// // if(lib.skill[card]&&lib.skill[card].subtype)return lib.skill[card].subtype;
	// if(lib.card[card]&&lib.card[card].subtype)return lib.card[card].subtype;
	// else return false;
	// },
	nodeintro(node, simple, evt) {
		...
		if (typeof node._customintro == 'function') {
			...
			if (lib.characterYBtext[node.name]) {
				uiintro.addText(get.colorspan(lib.characterYBtext[node.name]));
			}
			...
			if (lib.characterYBtext[node.link]) {
				uiintro.addText(get.colorspan(lib.characterYBtext[node.link]));
			}
