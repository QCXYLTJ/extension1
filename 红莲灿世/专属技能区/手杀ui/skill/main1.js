  ...
  var plugin = {
      ...
      app.waitAllFunction([
        function (next) {
          ...
          if (lib.device == 'ios' || lib.device == 'android') {
            lib.init.css('extension/' + app.name + '/' + plugin.name, 'main1', next);
          } else {
            lib.init.css('extension/' + app.name + '/' + plugin.name, 'main1_window', next);
          }
        },
      ], next);
    },
    ...
    controlElement: {
      add(skill, eSkills) {
        ...
        showSkills.forEach(function (item) {
          ...
          if (lib.skill[item.id].filter) {
            if ((lib.skill[item.id].filter + '').includes('player.group')) {
          ...
          if (lib.skill[item.id].viewAsFilter) {
            if ((lib.skill[item.id].viewAsFilter + '').includes('player.group')) {
          ...
          if (node) return;
          if (item.type === 'enable') {
            if (lib.noskillControl && lib.noskillControl.includes(item.id)) return;
            ...
            if (item.id == 'smh_huoji') {
            ...
            } else if (item.id == 'mbjuejin') {
              var tpm = get.translation(item.id)
                .slice(0, 4);
              ...
              if (cmjian) img.src = 'extension/手杀ui/skill/skillimgs1/' + tpm + '.png';
              else img.src = 'extension/手杀ui/skill/skillimgs/' + tpm + '.png';
              img.style.transform = 'scale(1)';
              img.style.marginTop = '-1px';
              img.style.filter = 'contrast(1.07) brightness(0.93)';
              node.appendChild(img);
            } else if (item.id == 'mbcmjiushi_1') {
              var tpm = get.translation(item.id)
                .slice(0, 4);
            ...
            if (showAddSpan) {
              ...
              if (item.info && typeof item.info.usable == 'number' && (item.info.usable > 1)) {
                ...
                if (item.id && game.me.getStat('triggerSkill')[item.id] < item.info.usable) {
                  addSpan(node, item.info.usable - game.me.getStat('triggerSkill')[item.id]);
                  boolRename = true;
                } else if (item.info.group && game.me.getStat('triggerSkill')[item.id] == item.info.usable) {
                  boolRename = true;
                }
                if (item.info.subSkill && item.info.subSkill.add && item.info.subSkill.add.usable) {
                  if (item.info.group && game.me.getStat('triggerSkill')[item.info.group] < item.info.subSkill.add.usable) {
                    addSpan(node, item.info.usable - game.me.getStat('triggerSkill')[item.id]);
                    boolRename = true;
                  } else if (item.info.group && game.me.getStat('triggerSkill')[item.info.group] == item.info.subSkill.add.usable) {
                    boolRename = true;
                  }
                }
                if (!boolRename) {
                ...
                if (item.info.subSkill && item.info.subSkill.add && item.info.subSkill.add.usable) {
                  if (item.info.group && game.me.getStat('triggerSkill')[item.info.group] < item.info.subSkill.add.usable) {
                    addSpan(node, item.info.subSkill.add.usable - game.me.getStat('triggerSkill')[item.info.group]);
                    boolRename = true;
                  } else if (item.info.group && game.me.getStat('triggerSkill')[item.info.group] == item.info.subSkill.add.usable) {
                    boolRename = true;
                  }
                ...
                if (!game.me.getStat('triggerSkill').dbquedi) game.me.getStat('triggerSkill').dbquedi = 0;
                if (game.me.getStat('triggerSkill')[item.id] < item.info.usable) {
                  addSpan(node, item.info.usable - game.me.getStat('triggerSkill')[item.id]);
                }
              }
            }
          ...
          if (item.id == 'mbcmqingzheng') {
            var tpm = get.translation(item.id)
              .slice(0, 4);
            node = ui.create.div('.skillitem-mbcmqingzheng', self.node.specialTrigger);
            var img = document.createElement('img');
            img.src = 'extension/手杀ui/skill/skillimgs1/' + tpm + '.png';
            img.style.transform = 'scale(0.92)';
            img.style.filter = 'contrast(1.07) brightness(0.93)';
            img.style.marginTop = '4px';
            node.appendChild(img);
            var img2 = document.createElement('img');
            img2.src = 'extension/手杀ui/skill/images/qianlong_btn_unget.png';
            ...
            if (showAddSpan) {
              ...
              if (item.info && typeof item.info.usable == 'number' && (item.info.usable > 1)) {
                ...
                if (item.id && game.me.getStat('triggerSkill')[item.id] < item.info.usable) {
                  addSpan(node, item.info.usable - game.me.getStat('triggerSkill')[item.id]);
                  boolRename = true;
                } else if (item.id && game.me.getStat('triggerSkill')[item.id] == item.info.usable) {
                  boolRename = true;
                }
                if (item.info.subSkill && item.info.subSkill.add && item.info.subSkill.add.usable) {
                  if (item.info.group && game.me.getStat('triggerSkill')[item.info.group] < item.info.subSkill.add.usable) {
      ...
      update() {
        ...
        Array.from(this.node.specialEnable.childNodes).forEach(function (item) {
          var cmjian = document.getElementsByClassName('cmjian')[0];
          if (skills.includes(item.dataset.id) && _status.event.player == game.me/*这里*/) {
            item.classList.add('usable');
          } else {
            item.classList.remove('usable');
          }
        ...
        ui.skillControl.node.trigger.style.width = ui.skillControl.node.enable.childNodes.length > 2
          ? '200px' : '120px';
        var cmjian = document.getElementsByClassName('cmjian')[0];
        if (cmjian && game.me.isAlive()) {
          ui.skillControl.node.enable.style.right = '100px';
          ui.skillControl.node.trigger.style.right = '100px';
    ...
    updateSkillMarks(player, skills1, skills2) {
      ...
      Array.from(node.childNodes).forEach(function (item) {
        if (Object.hasOwn(skills1, item.dataset.id)) return;
      ...
      for (var k in skills1) {
        ...
        if (!item) {
          if (!info.zhuanhuanji && !info.yunlvSkill) {
            ...
            item.setBackgroundImage('extension/手杀ui/skill/images/xiandingji/' + k + '.png');
          }
          //如果不是转换技就调用限定技的标记      
          else {
            //判断图片存在,不存在就用底图
            var url = 'extension/手杀ui/skill/images/' + k + '_yang.png';
            ...
            try {//容错函数,优先执行try的内容,try报错时自动执行catch内容
              ...
              if (a) {
                ...
                item.setBackgroundImage('extension/手杀ui/skill/images/' + k + '_yang.png');
              }
            }
            catch (err) {
              item = ui.create.div('.skillMarkItem.zhuanhuanji', node, get.skillTranslation(k, player));
              item.setBackgroundImage('extension/手杀ui/skill/images/ditu_yang.png');
      ...
      for (var k in skills2) {
        ...
        if (info.dutySkill) {
          ...
          item.setBackgroundImage('extension/手杀ui/skill/images/shimingji/' + k + '.png');
        }
        else {
          item = ui.create.div('.skillMarkItem.juexingji', node, '');
          item.setBackgroundImage('extension/手杀ui/skill/images/juexingji/' + k + '.png');
        }
        item.dataset.id = k;
      }
      //这里结束4}
