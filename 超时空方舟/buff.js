export function initBuff(lib, game, ui, get, ai, _status) {
	...
	lib.element.player.gl_purge = function () {
		...
		for (var i = 0; i < skill.length; i++) {
			...
			if (info.debuff) this.gl_chengeBuff(skill[i], true);
		}
	}
	//是否能获得buff
				...
				for (var skill of skills) {
					if (lib.skill[skill].buffInfo && lib.skill[skill].buffInfo.recover !== false) {
						if (lib.skill[skill].buffInfo.recover === true) {
							player.gl_chengeBuff(skill, true);
						} else {
							player.storage.gl_buff[skill]++;
							if ((player.gl_getBuffTimer(skill) / game.players.length) > 1) player.gl_chengeBuff(skill, true);
						}
					}
				}
			} else {
				var skill = trigger.sourceSkill || trigger.skill;
				player.gl_chengeBuff(skill, -1);
			}
		}
	};
	game.addGlobalSkill('gl_loseBuff');
	//获得与失去Buff
	lib.element.player.gl_chengeBuff = function () {
		var next = game.createEvent('gl_chengeBuff');
		next.player = this;
		for (var i = 0; i < arguments.length; i++) {
			if (typeof arguments[i] == 'number') {
				next.num = arguments[i];
			} else if (typeof arguments[i] == 'string') {
				if (arguments[i].indexOf('_buff') == -1) {
		...
		next.setContent('gl_chengeBuff');
		return next;
	}
	lib.element.content.gl_chengeBuff = function () {
		'step 0'
