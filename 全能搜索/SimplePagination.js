...
export class Pagination {
	...
	switchPage() {
		...
		let pageNumber;
		if (!pCNameList) {
			console.warn(`未找到类名为${"." + state.pCName}的元素`);
			return;
		}
	...
	gotoPage(pageNumber) {
		...
		if (!evaNumberLi) {
			console.warn(`未找到类名为${"." + state.pageNumberCName}的元素`);
			return;
		}
	...
	switchPrevNextAble() {
		...
		if (!prevBtn) {
			console.warn(`未找到上一页按钮的元素`);
			return;
		}
		if (!nextBtn) {
			console.warn(`未找到下一页按钮的元素`);
			return;
		}
	...
	renderPageDOM() {
		...
		if (state.insertAfter) {
			...
			if (!afterElement || !pageContainer.contains(afterElement)) {
				console.warn(`未根据配置找到兄弟元素，元素将添加到父元素结尾`);
