    ...
    */
    hokscs1: {
        ...
        async content(event, trigger, player) {
            ...
            if (skills.length == 1) {
                ...
                const { result } = await player.chooseButton(["获得技能", [list, 'textbutton']])
    ...
    hokscs2: {
        ...
        async content(event, trigger, player) {
            ...
            const { result } = await player.chooseButton([`失去技能`, [skills, "textbutton"]])
    ...
    hokscs4: {
        ...
        async content(event, trigger, player) {
            while (true) {
                const { result: { bool, targets } } = await player.chooseTarget('请选择一名角色替换其武将牌');
                if (bool) {
                    const dialog = ui.create.characterDialog('heightset');
                    const { result: { bool, links } } = await player.chooseButton(dialog)
                    if (bool) {
    ...
    hokscs5: {
        ...
        async content(event, trigger, player) {
            while (true) {
                const { result: { bool, targets } } = await player.chooseTarget("请选择两名角色交换座次", [2, 2]);
                if (bool) {
    ...
    hokscs6: {
        ...
        selectCard: -1,
        async content(event, trigger, player) {
            const { result: { bool, targets } } = await player.chooseTarget('请选择要交换座位的角色');
            if (bool) {
                if (get.event().isMine()) {
    ...
    hokscs7: {
        ...
        async content(event, trigger, player) {
            ...
            const { result: { bool, links } } = await player.chooseButton(['创造卡牌', [list, 'vcard']])
                .set('ai', button => {
            ...
            if (bool) {
                const card = game.createCard({
                    ...
                    number: Array.from({ length: 13 }).map((_, info) => info + 1).randomGet()
                });
                const { result: { bool, targets } } = await player.chooseTarget()
                ...
                if (bool) {
                    ...
                    const { result: { bool } } = await player.chooseBool(`你可以令${get.translation(targets[0])}使用${get.translation(card)}`);
                    if (bool) {
    ...
    hokscs8: {
        ...
        async content(event, trigger, player) {
            const list = get.inpileVCardList();
            const { result: { bool, links } } = await player.chooseButton(['检索卡牌', [list, 'vcard']])
                .set('ai', button => {
                    const player = get.player();
                    return player.getUseValue(button.link);
                });
    ...
    hokscs10: {
        ...
        prompt: "请选择执行拼点的角色",
        async content(event, trigger, player) {
            const { result: { bool, targets } } = await player.chooseTarget('请选择执行拼点的两名角色', 2)
