    ...
    var diy_card_jy = {
      ...
      card: {
        ...
        jydiybiaoche: {
          ...
          forceDie: true,
          onLose() {
            delete player.getStat('skill').jydiybiaoche_skill;
            player.unmarkSkill('jydiybiaoche_skill');
        ...
        jydiy_yueguangbaohe: {
          ...
          onLose() {
            //////////////////
            player.storage.yueguang_countDamage = 0;
            //player.storage.yueguang_sourceDamage=0;
            delete player.storage.yueguang_countDamage;
            // delete player.storage.yueguang_sourceDamage;
            /////////////////
        ...
        jydiy_ruanweijia_re: {
          ...
          onLosex() {
            'step 0';
            var bool1 = player.isDamaged(),
              bool2 = player.hasUseTarget({ name: 'sha' });
            if (bool1 || bool2) {
              if (bool1) player.recover();
              if (bool2) player.chooseUseTarget({ name: 'sha' }, true, false).set('selectTarget', [1, 2]);
            }
          },
          onLose() {
            var next = game.createEvent('jydiy_ruanweijia_recover');
            event.next.remove(next);
            var evt = event.parent;
            if (evt.getlx === false) evt = evt.parent;
            evt.after.push(next);
            next.player = player;
            next.setContent(lib.card.jydiy_ruanweijia_re.onLosex);
          },
