        ...
        var qtpz = {
            ...
            skill: {
                ...
                qtpz_yinian: {
                    ...
                    intro: {
                        content(storage, player) {
                            ...
                            if (['shen', 'jy_jue'].includes(group)) return '阳:摸牌阶段,你可以少摸一张牌,对一名角色发动一次〖行侠〗.';
                            else return '阴:本回合内视为拥有〖魔刀〗,使用杀时可以弃置任意张牌并额外指定等量目标.';
                        },
                    },
                ...
                qtpz_balin: {
                    ...
                    async content(event, trigger, player) {
                        ...
                        const { bool } = await trigger.player.chooseBool(str)
                            .set('ai', (evt, player) => get.equipValue(card, trigger.player) > -get.attitude(trigger.player, player)).forResult();
                        if (bool) {
                            player.draw();
                ...
                qtpz_canyue: {
                    ...
                    content() {
                        ...
                        event.num = Math.min(trigger.num, 9);
                        ('step 1');
                        player.chooseDrawRecover(get.prompt(event.name), 2, 1)
                            ('step 2');
                        if (result.control != 'cancel2') {
                            event.num--;
                            if (event.num > 0) {
                ...
                xajh_jiecheng: {
                    ...
                    content() {
                        'step 0';
                        player
                            .chooseToDiscard([1, Infinity], 'he', get.prompt('xajh_jiecheng', trigger.player), '弃置任意张牌令其多摸等量的牌')
                            .set('ai', function (card) {
                                var att = get.attitude(player, trigger.player);
                ...
                qtpz_fengpo: {
                    ...
                    content() {
                        'step 0';
                        player
                            .chooseToDiscard(get.prompt('qtpz_fengpo', trigger.target), '是否弃置一张牌令其展示手牌并弃置与你弃置的牌花色相同的牌', 'h')
                            .set('ai', function (card) {
                ...
                qtpz_youqin: {
                    ...
                    content() {
                        'step 0';
                        player
                            .chooseToDiscard('h', get.prompt('qtpz_youqin', trigger.player), '弃置一张手牌观看其至多X张手牌并使用其中一张非装备牌(X为其的体力值)？')
                            .set('ai', function (card) {
                                var att = get.attitude(player, trigger.player);
                                if (att < 0) {
                                    if (
            ...
            dynamicTranslate: {
                qtpz_yinian(player) {
                    ...
                    if (['shen', 'jy_jue'].includes(group)) {
                        str2 = '<span class="bluetext">' + str2 + '</span>';
                    } else if (group == 'jy_xie') str3 = '<span class="bluetext">' + str3 + '</span>';
                    else return get.translation('qtpz_yinian_info');
