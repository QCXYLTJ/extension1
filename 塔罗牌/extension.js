	...
	return {
		...
		precontent(TLP) {
				...
				if (result.bool) {
					game.broadcastAll(
						function (player, TLP1, TLP2) {
							...
							player.node.TLP.setBackgroundImage('extension/塔罗牌/image/TLPbackground.jpg');
							...
							if (lib.character[player.name][1] == 'TLP') {
								...
								player.TLP[player.name].setBackgroundImage('extension/塔罗牌/image/' + player.name + '.jpg');
								...
								player.TLP[player.name2].setBackgroundImage('extension/塔罗牌/image/' + player.name2 + '.jpg');
							...
							player.TLP[TLP1].setBackgroundImage('extension/塔罗牌/image/' + TLP1 + '.jpg');
							...
							player.TLP[TLP2].setBackgroundImage('extension/塔罗牌/image/' + TLP2 + '.jpg');
			...
			game.import('character', function () {
				var TLP = {
					...
					skill: {
						...
						_TLP_gainTLP: {
							...
							popup: false,
							filter(event, player) {
								return lib.character[player.name] && lib.character[player.name][1] == 'TLP';
							}, //QQQ
							content() {
						...
						TLP_jianxin: {
							...
							filter(event, player) {
								return (
									...
									game.countPlayer(function (current) {
										return current != player && current.maxHp > current.hp;
									}) > 0
								);
							},
							content() {
								...
								player.chooseCard('h', get.prompt('TLP_jianxin')).set('ai', function (card) {
									if (
										game.countPlayer(function (current) {
											return current != player && get.attitude(player, current) > 0 && current.maxHp > current.hp;
										}) > 0
									)
						...
						TLP_wangni: {
							...
							forced: true,
							filter(event, player) {
								return get.distance(player, event.player, 'attack') <= 1 && player != event.player && player.countCards('he') > 0;
							},
							content() {
						...
						TLP_beizhan: {
							...
							TLP_type: 'ni',
							filter(event, player) {
								return event.card && event.card.name == 'sha' && player.countCards('h') == 0; //QQQ
							},
							check(event, player) {
						...
						_TLP_puyao: {
							...
							filter(event, player) {
								return (
									game.countPlayer(function (current) {
										return get.distance(current, player, 'attack') <= 0 && current.hasSkill('TLP_puyao');
									}) > 0
								);
							},
							check(event, player) {
						...
						TLP_huihan: {
							...
							round: 1,
							filter(event, player) {
								return (get.type(event.card) == 'basic' || get.type(event.card) == 'trick') && player.countCards('h', { suit: event.card.suit }) > 0;
							},
							content() {
								'step 0';
								player.chooseCard('h', '请选择一张手牌', { suit: trigger.card.suit }).set('ai', function (card) {
					...
					translate: {
						...
						TLP_geduan: '隔断',
						TLP_geduan_info: '逆位:每当你和其他角色成为一张牌的目标前,你可令此牌对一名其他角色额外结算一次,此牌对你无效',
						TLP_nvjisi: '女祭司',
						TLP_qianren: '谦忍',
						...
						TLP_zhidu: '执独',
						TLP_zhidu_info: '逆位:每当你和其他角色成为一张牌的目标后,你可令此牌对其他角色无效,此牌对你额外结算一次',
						TLP_jiaohuang: '教皇',
						TLP_jianxin: '兼心',
						TLP_jianxin_info: '正位:每当你回复一次体力或受到一次伤害后,你可以弃置一张牌,令一名其他角色回复一点体力',
						TLP_anyan: '暗言',
						...
						TLP_yinzhe: '隐者',
						TLP_xingxin: '醒心',
						TLP_xingxin_info: '正位:准备阶段开始时,你可以将武将牌翻面,你获得你判定区中的牌',
						TLP_gumi: '孤迷',
						TLP_gumi_info: '逆位:当你的武将牌翻面时,你可以选择一项:1.摸一张牌,2.将场上的一张牌置入弃牌堆',
						TLP_mingyunzhilun: '命运之轮',
						TLP_fulin: '福临',
						TLP_fulin_info: '正位:每当其他角色的♥️️判定牌生效后,你可以摸一张牌',
						TLP_wangni: '罔逆',
						...
						TLP_taiyang: '太阳',
						TLP_puyao: '普曜',
						TLP_puyao_info: '正位:你攻击范围内的角色的结束阶段结束时,该角色可以将武将牌翻面,摸一张牌并回复一点体力',
						_TLP_puyao: '普曜',
						_TLP_puyao_info: '正位:你攻击范围内的角色的结束阶段结束时,该角色可以将武将牌翻面,摸一张牌并回复一点体力',
						TLP_maidang: '霾当',
						TLP_maidang_info: '逆位:每当你攻击范围内的角色受到雷属性以外的伤害时,你可以弃置一张手牌,令此伤害-1',
						TLP_shenpan: '审判',
						TLP_shengcai: '圣裁',
						TLP_shengcai_info: '正位:出牌阶段限一次,你可以选择一名角色,做一次判定,若判定牌为红色,所有角色摸一张牌,否则所有角色弃一张牌,你可以将此判定牌交给目标角色',
						TLP_huanmie: '幻灭',
				...
				for (var i in TLP.character) {
					TLP.character[i][4].push('ext:塔罗牌/image/' + i + '.jpg');
				}
				lib.config.all.characters.add('TLP');
				lib.config.characters.add('TLP');
		...
		help: {
			塔罗牌: '<li>塔罗牌的获得:<ul><li>游戏开始时,塔罗牌武将可以选择两张塔罗牌武将组成塔罗牌组.若你为双将且两个武将均为塔罗牌武将,则第一张武将进入塔罗牌组,第二张武将正逆位技能均可以使用<li>联机模式下随机获得两张塔罗牌组成塔罗牌组</ul><li>正逆位:<ul><li>正位时拥有正位技能,逆位时拥有逆位技能,获得塔罗牌后组成的塔罗牌组中,所有武将均为正位<li>回合开始和回合结束时,你可以调整一张塔罗牌的正逆为</ul><li>特别提醒:<br>非联机模式下,组成塔罗牌组时会切换布局为<手杀>',
		},
		package: {
			intro: "<br><span style='color: gold'>潜水的火修复版<br>『无名杀扩展大全群』:771901025<br></span>",
			author: "<br>武将设计:<br><img src='extension/塔罗牌/image/yoyo_ts_喵.jpg' width='65' height='65'><br>yoyo_ts_喵(百度贴吧)<br>翻译:<br><img src='extension/塔罗牌/image/极光.jpg' width='65' height='65'><br>极光(百度贴吧)<br><div onclick=window.open('https://tieba.baidu.com/p/2703972337?see_lz=1')><span style='cursor:pointer'>查看原帖</span></div>",
		},
	};
});
