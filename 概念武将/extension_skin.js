...
window.zyile_import = function (lib, game, ui, get, ai, _status, config) {
	window.zyile_extension_Menu.zyile_skin = {
		...
		zyile_Skin_load: {
			...
			intro: '<li>顺序:进行跟原有的加载一样是以1、2、3、4....' +
				'<li>检索:皮肤名可以不按照原有的规则,但是不会加载只有 英文、数字 的皮肤',
			init: 'retrieval',
		},
		zyile_Skin_Name_color: {
			...
			item: {
				...
				custom: '自定义:.nodeinput/~type=color~value=' + lib.config.extension_概念武将_zyile_Skin_Name_color + '@input=' + `
				function(){
					var node=this.parentNode.parentNode.parentNode;
	...
	window.zyile_extension_Menu.zyile_skin_js = {
		...
		zyile_skin_js_info: {
			name: '介绍',
			onclick() {
				lib.zyile_layer.openFull([''].concat([
					'命名规则:必须以原武将id后面加_pifu1.jpg后缀必须是.jpg',
					'例:dongman_Kaguya(武将id|武将图id),第一个皮肤则为dongman_Kaguya_pifu1 以此类推放到自己的扩展目录里面\n参考【概念武将】扩展下武将的皮肤命名',
					'若想全部皮肤均有解锁条件则<span style="color:red">添加前缀<span style="color:blue">all</span></span>',
					'货币数量:购买时所需要的货币数量',
					'更新函数:购买完皮肤后所执行的函数,用于更新游戏内显示的货币;配置时会先运行一次,来检查是否右问题.',
					'所需货币:所需要的货币代码,比如富甲天下的货币代码是coin']).join("<li>"), {
					title: '皮肤解锁模式介绍',
				});
			},
			clear: true,
	...
	*/
	...
	game.playAudio = function () {
		...
		if (lib.config.extension_概念武将_zyile_Skin_Audio) {
			let src = Audio.src, num = '', name = src.slice(0), by_name = src.slice(0), AudioName = '';
			...
			for (let i in lib.config.zyile_Skin_Audio) {							//i就是武将名称
				const Audio_Skin = lib.config.zyile_Skin_Audio[i];
				for (let j in Audio_Skin) {										//j是语音id
					if (j === name) {												//如果保存的技能名称和上面的一样,证明是换肤换音保存的.
			...
			if (lib.zyile_common.isNotEmpty(AudioName)) {
				let onerror = Audio.onerror, onerrorNum = 0;
				"use strict";
	...
	game.playSkillAudio = function (name, index) {
		...
		if (lib.config.extension_概念武将_zyile_Skin_Audio) {
			...
			for (var i in lib.config.zyile_Skin_Audio) {			//i就是武将名称
				var Audio_Skin = lib.config.zyile_Skin_Audio[i];
				for (var j in Audio_Skin) {						//循环已保存的语音
					if (j === src) {									//如果技能是这个路径的话
		...
		var str = 'audio/skill/', by_name, onerrorNum = 0;
		...
		var Skin = function (player) {
			...
			var loadImage = function (_0xOqU8) {
				...
				img.onload = function () {
					...
					if (skin_num > 0) {
						if (player.zyile_Skin_Node) player.zyile_Skin_Node.remove();
						stop = true;
						player.zyile_Skin_Node = ui.create.div(player, '.zyile_Skin_Node', event => { event.stopPropagation(); event.preventDefault(); });
			...
			if (lib.config.extension_概念武将_zyile_skin_fluent) {
				if (player.classList && (player.classList.contains('player') || player.classList.contains('button'))) {
					if (player.zyile_Skin_Node) player.zyile_Skin_Node.remove();
					player.zyile_Skin_Node = ui.create.div(player, event => { event.stopPropagation(); event.preventDefault(); });
					var div = ui.create.div('.zyile_skin', player.zyile_Skin_Node);
					get.is.mobileMe(player) && div.css({
						left: '90px',
					});
		...
		window.zyile_charactercard = function (player, resume) {
			...
			var changeskin = function (player, name, avatar) {
				...
				let createButtons = function (num, src, _src, modular) {
					...
					if (!node.content[modular]) (node.content[modular] = ui.create.div(node.content, '.zyile_skin_Button_modular')) && (node.content[modular].onmousewheel = function (evt) {
						let node = this;
						let num = 6;
						let speed = 16;
								...
								if (lib.config.extension_概念武将_zyile_skin_Gradient) {
									var JianBian = ui.create.div('.jbtp', avatar),
										...
										huanfu = [function () {
											...
											fu_num++;
										}], fu_num = 0, IntervalId = setInterval(huanfu.randomGet(), 35);
								} else {
									avatar.style.backgroundImage = that.style.backgroundImage;
								}
							}, avatar, this);
					...
					let Skin_info_div = ui.create.div('.zyile_Skin_div', button, event => { event.stopPropagation() });
					...
					let info_Input_button = ui.create.div(button._info, '.zyile_Skin_info_input_button', '修改', function (event) {
						lib.zyile_layer.prompt({
							...
							maxlength: 10,
							success(layer) {
								var i = layer.querySelector('.layui-layer-input'), textShadow = lib.config.extension_概念武将_zyile_Skin_Name_color;
				...
				img.onload = function () {
					...
					if (skin_num > 0) {
						if (player.classList && (player.classList.contains('player') || player.classList.contains('button'))) {
							...
							playerSkin.zyile_Skin_Node = ui.create.div(playerSkin, event => { event.stopPropagation(); event.preventDefault(); });
		...
		window.zyile_charactercard = function (name, resume, avatar, that) {
			...
			var changeskin = function () {
				var createButtons = function (num, src, _src) {
					...
					var Skin_info_div = ui.create.div('.zyile_Skin_div', button, event => { event.stopPropagation() });
					button._info = Skin_info_div;
	...
	window.zyile_content.push(function (lib, game, ui, get, ai, _status, config) {
		if (game.getExtensionConfig('概念武将', 'zyile_skin_Gradient_Preloading_images')) {
			let onload = function () {
				...
				game.ensureDirectory('extension/概念武将/redskin', function () { ReadFileList('redskin') });
				game.ensureDirectory('extension/概念武将/purpleskin', function () { ReadFileList('purpleskin') });
				game.ensureDirectory('extension/概念武将/yellowskin', function () { ReadFileList('yellowskin') });
			};
			onload();
		}
