...
window.zyile_import = function (lib, game, ui, get, ai, _status) {
    ...
    let downloadCollapse = function () {
        ...
        setTimeout(function uiCommandnode() {
            ...
            buttons[0].addEventListener('click', function (forcecheck, dev) {
                if (this.disabled) {
                    return;
                        ...
                        lib.init.req('game/source.js', function () {
                            ...
                            var finish = function () {
                                ...
                                if (autoRecoverOrReDownload === 'Recover') {
                                    ...
                                    ui.create.zyile_node('a.btn.btn-warning.btn-rounded.btn-sm', fuzhune, '重置')
                                    ...
                                    ui.create.zyile_node('a.btn.btn-info.btn-rounded.btn-sm', fuzhune, '反选')
                                        .addEventListener('click', function () {
                                            QXFX('file');
                                        });
                                    ...
                                    for (let i of pullRightUlLi) {
                                        ...
                                        a.onclick = function (event) {
                                            if (this.parentElement.classList.contains('active'))
                                                return RefreshTable(list.concat(sizeFiles).concat(RecoverFileList), false),
                                        ...
                                        yes(layer) {
                                            ...
                                            for (let i of TableTr) {
                                                ...
                                                for (let e of InputList)
                                                    if (e.checked == true && e.getAttribute('subtype') == 'file')
                                    ...
                                    ui.create.zyile_node('span.pagination-info',
                                        ui.create.div('.pull-left.pagination-detail', page),
                                        `共检下载  ${RecoverFileList.length + sizeFiles.length + list.length}  个文件,下载失败  ${RecoverFileList.length}  个文件,下载成功  ${list.length} 个文件,小于指定数量  ${sizeFiles.length}  个文件.`)
                            ...
                            let multiDownload = (downloadFiles) => {
                                game.multiDownload(downloadFiles, function (length, fileName, fileLocation) {
                                    n1++;
                                    xhr(fileLocation)
                                        .then(value => {
            ...
            buttons[2].addEventListener('click', function (event) {
                ...
                if (this.disabled) {
                    ...
                    lib.init.req('game/asset.js', function () {
                        ...
                        let p = that.parentNode, n1 = 0, n2, flag = true,
                        ...
                        let proceed = () => {
                            ...
                            game.multiDownload(updates, function (length, fileName, fileLocation) {
                                n1++;
