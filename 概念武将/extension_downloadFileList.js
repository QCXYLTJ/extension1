  ...
  window.zyile_extension_Menu.downloadFileList = {
    ...
    downloadFCFileList: {
      ...
      onclick() {
        ...
        lib.zyile_layer.confirm(`约2.06GB是否继续？<br>注:下载完毕后将会将${'加载目录'.fontcolor('red')}自动切换为${'FC'.fontcolor('red')},${'加载方式'.fontcolor('red')}改为${'检索'.fontcolor('red')}`, (layer) => {
          layer.close();
          xhr({
            url: `${uri}FC/Files.js`,
            time: 3e3,
            ...
            .then((value) => {
              try {
                ...
                let download = (DownloadFile) => {
                  ...
                  _status.FCFileListLoad.span = ui.create.zyile_node(
                    _status.FCFileListLoad,
                    {
                      position: 'static',
                      padding: '0 10px',
                      color: '#4cff36',
            ...
            .catch((reason) => {
              this.firstChild.innerHTML = '下载无名杀全皮肤素材';
              delete this.reqested;
              if (reason.status === 404) lib.zyile_layer.alert('资源文件不存在');
