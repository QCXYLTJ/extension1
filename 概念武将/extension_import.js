...
window.zyile_import = function (lib, game, ui, get, ai, _status, config) {
  ...
  window.zyile_extension_Menu.import = {
    zyile_BD_extension: {
      ...
      onclick() {
          ...
          var NextFile = function (i) {
            game.hasDirectory(
              ...
              (data) => {
                ...
                folders.length > 0 ? NextFile(folders.shift()) : (createExtensionButton(ImportList, true, true), createExtensionButton(NoImportList, false, true), createExtensionButton(NoExtension, false), ui.create.zyile_node('span.pagination-info', ui.create.div('.pull-left.pagination-detail', page), '共检索到  ' + (ImportList.length + NoImportList.length + NoExtension.length) + '  个文件夹,共导入  ' + ImportList.length + '  个扩展,未导入  ' + NoImportList.length + '  个扩展,  ' + NoExtension.length + ' 个非扩展(缺少extension.js文件) '), zyile_syscn_layer.remove());
              },
              (onerror) => {
                NoExtension.add(i);
        ...
        ui.create.zyile_node('a.btn.btn-warning.btn-rounded.btn-sm', fuzhune, '重置').addEventListener('click', function () {
          (RefreshTable(ImportList, false), RefreshTable(NoImportList, false), RefreshTable(NoExtension, false));
        ...
        for (let i of pullRightUlLi) {
          ...
          a.onclick = function (event) {
            if (this.parentElement.classList.contains('active')) return (RefreshTable(ImportList.concat(NoImportList).concat(NoExtension), false), this.parentElement.classList.remove('active'));
        ...
        let layerFull = lib.zyile_layer.openFull(
          openImExt,
          {
            ...
            yes(layer) {
              ...
              lib.zyile_layer.confirm(
                "注:每次都是清空扩展重新导入选定扩展<br>新增<span style='color:blue'>:" + addExt.join('、') + '</span><br>移除:<span style="color:red">' + remExt.join('、') + "</span><br>总共导入:<span style='color:magenta'>" + extensions.join('、') + '</span>',
                function (layerX) {
                  layerX.close();
        ...
        get.zip(function () {
          let Imporclick = function () {
            var files = [],
              folders = [],
              finishLoading = false,
              dir = game.getExtensionConfig('概念武将', 'zyile_import_URL') || '无路径',
              page,
              extObj = {},
              NoImportList = [],
              ImportList = [],
            ...
            if (lib.zyile_common.isMobile() && ![].includes) {
              ...
              var zyile_syscn_layer = lib.zyile_layer.load(),
                callback = function callback(folders, files) {
                  ...
                  get.zip(function () {
                    var fn = function () {
                      (createExtensionButton(ImportList, true, true), createExtensionButton(NoImportList, false, true), createExtensionButton(NoExtension, false), zyile_syscn_layer.close());
                    };
                    var NextFile = function (i) {
                      if (finishLoading) return void 0;
                  ...
                  var loadFile = function loadFile(file) {
                    ...
                    impFn = function (zip, data) {
                      ...
                      try {
                        ...
                        console.warn(e);
                        (NoExtension.add(zip.link), sliceList.add(zip.link), (NoExtension[i] = zip.link), (sliceList[i] = zip.link), createExtensionButton(sliceList));
                      } finally {
                        if (!lib.config.dev) delete window.game;
                        fn(i);
                      }
            ...
            let AutoDelete = ui.create.zyile_node(
              ui.create.div(
                fuzhune,
                {
                  right: 0,
            ...
            function createExtensionButton(list, isImp, isExt) {
              for (var i of list) {
                ...
                ('use strict');
                {
                  edit.onclick = function () {
                    ...
                    for (var i in config) {
                      ...
                      node.appendChild(
                        ui.create.zyile_node('span', String(config[i].name) + ':', {
                          position: 'absolute',
                        }),
                      );
                      if (config[i].item) {
                        ...
                        let initx = true;
                        if (Object.hasOwn(config[i], 'initx')) (this.classList.toggle('zyile_menu_skin_on', config[i].initx), (initx = false));
                ...
                if (!isExt) {
                  ...
                  Universal_Import.addEventListener(
                    'click',
                    function (event) {
                      var node = this,
            ...
            var layerFull = lib.zyile_layer.openFull(
              openImExt,
              {
                title: '指定路径导入zip',
                icon: -1,
                yes(layer) {
                  ...
                  layer.close();
                  setTimeout(function () {
                    implExtList.length > 0 &&
                      lib.zyile_layer.confirm(
                        `${AutoDelete.enable ? `${'警告:你已开启导入完后自动删除源文件'.fontcolor('red')}<br>` : ''}<span style='color:red'>注:请勿在导入时刷新或离开游戏,否则将导入失败丢失素材等,后果自负!</span><br/>本次将导入以下扩展:  ${implExtList.join('、')}`,
                        function (layerX) {
                          layerX.close();
                          ('use strict');
                          {
                            (function implNext(i) {
                              ...
                              if (
                                game.importExtension(zip.generate({ type: 'arraybuffer' }), function () {
                                  let config = zip.config;
                                  for (var j in config) {
                                    if (config && Object.hasOwn(config[j], 'init')) {
                                  ...
                                  implExtList.length > 0
                                    ? implNext(implExtList.shift())
                                    : setTimeout(
                                      () =>
                                        lib.zyile_layer.msg('导入完毕', {
                                          icon: 1,
                                          time: 25e2,
                                          protype: 2,
                                        }),
    ...
    zyile_import_URLx: {
      ...
      onclick(...args) {
        let callback;
        for (var i of args) if ('function' === typeof i) callback = i;
        var node = this;
          ...
          function (val, index, input) {
            if (lib.zyile_common.isMobile()) {
              game.hasDirectory(
                val,
                function () {
                  (lib.zyile_layer.msg('配置成功', { icon: 1, protype: 2 }), game.saveExtensionConfig('概念武将', 'zyile_import_URL', val));
              ...
              if (lib.node.fs.existsSync(val))
                (lib.zyile_layer.msg('配置成功', {
                  icon: 1,
                  protype: 2,
                }),
