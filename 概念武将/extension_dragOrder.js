...
window.zyile_import = function (lib, game, ui, get, ai, _status) {
  ...
  ui.zyile_sortCard.addEventListener(
    ...
    function (event) {
      ...
      hs.sort(function (b, a) {
        if (a.name != b.name) return lib.sort.card(a.name, b.name);else
        if (a.suit != b.suit) return lib.suit.indexOf(a) - lib.suit.indexOf(b);else
  ...
  ui.create.control = function () {
    ...
    var i, controls;
    var nozoom = false;
    if (Array.isArray(arguments[0])) controls = arguments[0];else
    controls = arguments;
