		...
		content(config, pack) {
			...
			game.playSu = function (fn, dir, sex) {
				if (lib.config.background_speak) {
					...
					else game.playAudio('../extension/群英会/audio', fn);
				}
			};
			lib.skill._zhengwangpeiyin = {
				...
				forced: true,
				content() {
					game.playAudio('../extension/群英会/audio', trigger.player.name);
				},
			};
				...
				content() {
					for (var x = 0; x < game.players.length; x++) {
						...
						if (playerx.classList.contains('out') && playerx.hasSkill('xwj_xqunying_mingxiang')) {
							...
							playerx.update();
							ui.backgroundMusic.src = 'extension/群英会/audio/wms_backgroundmusic.mp3';
			...
			if (config.wuxianyuedu) {
				lib.skill._jieshihuanhun = {
					...
					content() {
						...
						game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_wxyd_background.jpg');
						ui.backgroundMusic.src = 'extension/群英会/audio/wms_backgroundmusic.mp3';
						huanhun.phase('nodelay');
					},
				};
			}
			if (config.xwansha) {
			...
			if (config.xjisha) {
				lib.skill._xwj_jisha = {
					...
					_priority: 2019,
					content() {
						player.storage.xwj_jisha = player.storage.xwj_jisha + 1 || 1;
						if (player.storage.xwj_jisha == 1) {
							player.$fullscreenpop('一血  卧龙出山', 'fire');
							game.playSu('xwj_jisha1');
						}
						if (player.storage.xwj_jisha == 2) {
							player.$fullscreenpop('双杀  一战成名', 'water');
							game.playSu('xwj_jisha2');
						}
						if (player.storage.xwj_jisha == 3) {
							player.$fullscreenpop('三杀  举世皆惊', 'thunder');
							game.playSu('xwj_jisha3');
						}
						if (player.storage.xwj_jisha == 4) {
							player.$fullscreenpop('四杀  天下无敌', 'fire');
							game.playSu('xwj_jisha4');
						}
						if (player.storage.xwj_jisha == 5) {
							player.$fullscreenpop('五杀  诛天灭地', 'thunder');
							game.playSu('xwj_jisha5');
						}
						if (player.storage.xwj_jisha == 6) {
							player.$fullscreenpop('六杀  癫狂杀戮', 'water');
							game.playSu('xwj_jisha6');
						}
						if (player.storage.xwj_jisha == 7) {
							player.$fullscreenpop('无双  万军取首', 'fire');
							game.playSu('xwj_jisha7');
						}
					},
				};
			...
			if (config.xinjishatexiao) {
				lib.skill._xin_jisha = {
					...
					_priority: 2019,
					content() {
						player.storage.xin_jisha = player.storage.xin_jisha + 1 || 1;
						if (player.storage.xin_jisha == 1) {
							player.$skill('卧龙出山', 'fire', 'red', 'avatar');
							game.playSu('xwj_jisha1');
						}
						if (player.storage.xin_jisha == 2) {
							player.$skill('一战成名', 'fire', 'red', 'avatar');
							game.playSu('xwj_jisha2');
						}
						if (player.storage.xin_jisha == 3) {
							player.$skill('举世皆惊', 'thunder', 'red', 'avatar');
							game.playSu('xwj_jisha3');
						}
						if (player.storage.xin_jisha == 4) {
							player.$skill('天下无敌', 'fire', 'red', 'avatar');
							game.playSu('xwj_jisha4');
						}
						if (player.storage.xin_jisha == 5) {
							player.$skill('诛天灭地', 'fire', 'red', 'avatar');
							game.playSu('xwj_jisha5');
						}
						if (player.storage.xin_jisha == 6) {
							player.$skill('癫狂杀戮', 'thunder', 'red', 'avatar');
							game.playSu('xwj_jisha6');
						}
						if (player.storage.xin_jisha == 7) {
					...
					lib.storage.stage.火影剧情 = {
						...
						scenes: [
								...
								players: [
									{
										...
										handcards: [],
										equips: [
											['xwj_xus_xuelunyang', 'random', 'random'],
											['xwj_xus_mianju', 'random', 'random'],
										],
										judges: [],
									},
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '卡卡西班vs再不斩&&白',
								players: [
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_bai', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zaibuzhan', name2: 'none', identity: 'fan', position: 0, hp: 6, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '春野樱vs山中井野',
								players: [
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jinye', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '手鞠vs天天',
								players: [
									{ name: 'xwj_xhuoying_shouju', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_tiantian', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '鸣人vs犬冢牙',
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_quanzhongya', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '宁次vs雏田',
								players: [
									{ name: 'xwj_xhuoying_ningchi', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_chutian', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '我爱罗vs李洛克',
								players: [
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_liluoke', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_ningchi', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '佐助vs我爱罗',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '油女志乃vs勘九郞',
								players: [
									{ name: 'xwj_xhuoying_zhinai', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kanjiulang', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '鹿丸vs手鞠',
								players: [
									{ name: 'xwj_xhuoying_luwan', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_shouju', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dashewan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_yuanfeirizhan', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dou', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_asima', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kai', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_itachi', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guijiao', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_itachi', name2: 'none', identity: 'zhu', position: 1, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guijiao', name2: 'none', identity: 'zhong', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhilaiye', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_dingchi', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_chilangfang', name2: 'none', identity: 'fan', position: 2, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_ningchi', name2: 'none', identity: 'zhong', position: 3, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guitongwan', name2: 'none', identity: 'fan', position: 4, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_quanzhongya', name2: 'none', identity: 'zhong', position: 5, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zuojinyoujin', name2: 'none', identity: 'fan', position: 6, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_luwan', name2: 'none', identity: 'zhong', position: 7, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_tayuya', name2: 'none', identity: 'fan', position: 8, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_liluoke', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_junmalv', name2: 'none', identity: 'fan', position: 1, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_didala', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiezi', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_didala', name2: 'none', identity: 'fan', position: 1, hp: 8, maxHp: 8, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_feiduan', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jiaodu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_asima', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_luwan', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '大和班潜入大蛇丸的基地',
								players: [
									{ name: 'xwj_xhuoying_dahe', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zuojin', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dou', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dashewan', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_dashewan', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_luwan', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dingchi', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_feiduan', name2: 'none', identity: 'fan', position: 0, hp: 6, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jiaodu', name2: 'none', identity: 'fan', position: 0, hp: 6, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 1, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dashewan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_didala', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhilaiye', name2: 'none', identity: 'zhu', position: 1, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_changmen', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaonan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_itachi', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_shuiyue', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhongwu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_qilabi', name2: 'none', identity: 'fan', position: 0, hp: 8, maxHp: 8, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_changmen', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaonan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
							...
							},
							{
								...
								players: [
									...
									{ name: 'xwj_xhuoying_zhongwu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_shuiyue', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xianglin', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daluyi', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_leiying', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kanjiulang', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dayemu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhaomeimeng', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'xwj_xhuoying_xianglin', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_tuanzang', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_shuimen', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jiuxinnai', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_jiuwei', 'random', 'random']], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_qilabi', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guijiao', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_kai', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guijiao', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaonan', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_dou', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_didala', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dayemu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dahe', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_qilabi', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_itachi', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
									{ name: 'xwj_xhuoying_changmen', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_huanyue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_sanlei', name2: 'none', identity: 'fan', position: 0, hp: 6, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_dingchi', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_luwan', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_asima', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiezi', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_duan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jiaodu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daluyi', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jinye', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zuojin', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_tiantian', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_ningchi', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_liluoke', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_bai', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zaibuzhan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_didala', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_chutian', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_quanzhongya', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhinai', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_ban', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dou', name2: 'none', identity: 'zhong', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_wuren', name2: 'none', identity: 'zhong', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dayemu', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_leiying', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_woailuo', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhaomeimeng', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_itachi', name2: 'none', identity: 'zhu', position: 1, hp: 7, maxHp: 7, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhong', position: 0, hp: 7, maxHp: 7, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
									{ name: 'xwj_xhuoying_dou', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_chilangfang', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_guitongwan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_tayuya', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zuojinyoujin', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhu', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_dashewan', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhongwu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_feijian', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_shuimen', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_xianglin', name2: 'xwj_xhuoying_shuiyue', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhujian', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_yuanfeirizhan', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								intro: '',
								players: [
									{ name: 'xwj_xhuoying_zhujian', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_ban', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_ban', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daluyi', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_shouju', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kanjiulang', name2: 'none', identity: 'fan', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_ban', name2: 'none', identity: 'zhu', position: 1, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_daitu', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'zhong', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_liluoke', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kai', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'fan', position: 0, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									...
									{ name: 'xwj_xhuoying_xiaoying', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'zhong', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_huiye', name2: 'none', identity: 'fan', position: 0, hp: 7, maxHp: 7, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [['xwj_xus_xuelunyang', 'random', 'random']], judges: [] },
									{ name: 'xwj_xhuoying_jue', name2: 'none', identity: 'fan', position: 0, hp: 6, maxHp: 6, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_kakasi', name2: 'none', identity: 'zhu', position: 1, hp: 4, maxHp: 4, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
							...
							},
							{
								...
								players: [
									{ name: 'xwj_xhuoying_mingren', name2: 'none', identity: 'zhu', position: 0, hp: 3, maxHp: 3, linked: false, turnedover: false, playercontrol: true, handcards: [], equips: [], judges: [] },
									{ name: 'xwj_xhuoying_zhuozhu', name2: 'none', identity: 'fan', position: 0, hp: 5, maxHp: 5, linked: false, turnedover: false, playercontrol: false, handcards: [], equips: [], judges: [] },
								],
								cardPileTop: [],
								cardPileBottom: [],
								discardPile: [],
			...
			game.import('character', function () {
				var xwugeng = {
					...
					character: {
						...
						xwj_xu_xiaohuan: ['female', 'shen', 3, ['xwj_xu_lixing', 'xwj_xu_choumou'], []],
						...
						xwj_xsanguo_qiaoguolao: ['male', 'wu', 3, ['xwj_xsanguo_naxian', 'xwj_xsanguo_zexu'], []],
					...
					characterIntro: {
						...
						xwj_xhuoying_chiwan: '赤丸是日本动漫<火影忍者>中的一只忍犬,是犬冢牙的得力助手,经常和牙呆在一起.当牙作战时,赤丸可以起到很好的辅助作用.曾经把鸣人的裤子连同臀部咬烂,牙齿十分锋利.赤丸是犬冢牙永不分离的伙伴,亦为忍术道具,具有灵敏的嗅觉,能感受敌人的查克拉力量及危险性.因为吃下兵粮丸后身体会变成赤红色,故名赤丸. 它能化成犬冢牙的分身,合作使出忍术--‘牙通牙’,化成两股龙卷风攻击敌人.及后更能与犬冢牙进行人兽混合变身,化成双头一身的双头狼,并使出超必杀技--<牙狼牙>,形成具有极大破坏力、可将敌人切开的龙卷风.小时候不会说话,只有犬冢牙听得懂赤丸的叫声所表达的意思.',
						xwj_xhuoying_itachi: '宇智波鼬,木叶村宇智波一族的天才忍者,7岁忍校毕业,13岁当上暗部队长,忍术高超,擅长手里剑.宇智波一族少数开了万花筒写轮眼的人,左眼月读,能瞬间让对手陷入鼬控制的幻术之中,包括时间、空间、质量皆由鼬控制,让其受到极重的精神攻击,属精神攻击类幻术.右眼天照,聚焦即发出能烧毁一切的黑色火焰,烧毁目标前永不熄灭.双眼须佐能乎,拥有极强的攻击与防御力.少年鼬是名双重间谍,为了村子,在宇智波一族叛变木叶前选择灭族,后逃离加入并暗中调查晓组织.后与其弟佐助决战而死,佐助因此开了万花筒写轮眼.第4次忍界大战被兜用秽土转生术复活,与鸣人相遇发动最强幻术<别天神>,摆脱控制,与兜一战,发动禁术<伊邪那美>,让兜陷入循环的幻术中.',
						...
						xwj_xhuoying_dashewan: '大蛇丸,日本动漫<火影忍者>中的人物.火之国木叶村的叛忍,木叶村的<三忍>之一,与自来也、纲手同为第三代火影猿飞日斩的弟子.具有独身攻陷一个小国的强大实力,和近乎不老不死的回复力,被认为比蛇还难缠.擅长研究忍术且渴望得到写轮眼,闻名于忍界.野心勃勃,由于目睹了太多人的死亡、知道生命是脆弱的而误入歧途,他认为人体中蕴含着一生都无法使用的力量,因此他想获得长生不老从而学习世间所有忍术,掌握世间的真理.其野心被多次阻断,在佐助与鼬一战中被鼬的十拳剑封印.后在第四次忍界大战中,从御手洗红豆和药师兜的身上看见了药师兜的失败,有所醒悟.之后被佐助复活,与历代火影和鹰小队前往战场支援忍者联军.',
						...
						xwj_xhuoying_feiduan: '飞段,晓组织成员,邪教教徒,拥有不死之躯.飞段信仰邪神教,是邪神教的忠实教徒.他和角都合称<不死二人组>,被鬼鲛戏称为<僵尸二人组".飞段体术灵活,且拥有不死之身的能力,只需尝到对手的血,就能够通过诅咒使对手和自己承担同等的伤害!曾经和角都联手击败二尾人柱力二位由木人,击杀守护忍十二士之一的地陆和猿飞阿斯玛,最终被奈良鹿丸用计谋活埋于鹿林的深洞里.',
						...
						xwj_xhuoying_shuimen: '波风水门,四代目火影,鸣人的父亲,木叶村的英雄,天才忍者,外号<黄色闪光>,年纪轻轻便当上火影.自创忍术螺旋丸,凝聚查克拉无需结印即可发动.改良二代火影的<飞雷神>之术,只需在目标地留下术式,便能在术式之间瞬间移动或转移物体,速度极快,千里之外取人首级只在须臾之间,瞬间又遁迹千里之外,来无影去无踪,敌皆闻风丧胆,遇见可放弃任务,只管逃跑,曾在忍界大战以一己之力秒杀对方五十名上忍,名声由此大震.鸣人出生当晚,被宇智波带土掳走妻子,放出九尾狐,后发动禁术<尸鬼封尽>、<八卦封印>将九尾封印进鸣人体内后死去.第四次忍界大战中被大蛇丸切开<死神>腹部,释放出灵魂,用秽土转生术复活,后参与对抗宇智波带土和宇智波斑的战斗,发动短暂的仙人模式和九尾模式,帮助鸣人...',
						...
						xwj_xhuoying_xiezi: '赤砂之蝎,日本动漫<火影忍者>中的人物.<晓>成员之一,代号玉(玉女).被誉为天才傀儡师,人称<赤砂之蝎>.风之国砂隐村顾问千代之孙,幼年时其父母在执行任务时被<木叶白牙"所杀,内心无比孤独,后来在千代的教导下习得傀儡术,曾经制作名为<父>与<母>的两具傀儡以抚慰自己内心的孤独.从小因为内心的孤独寂寞,而对傀儡有着超乎寻常的爱,是一个天才傀儡师,在自己青年的时期里就已经染指了人傀儡,把自己唯一的好友做成了傀儡.后来甚至将自己也做成了傀儡,后与春野樱和千代婆婆战斗被击杀.在秽土转生的时候,受到了勘九郎的话语感化,找到了自己内心深处真正的艺术,被唤醒了热爱傀儡的心,也找到了自己作为人类的一丝意义,明白了作为傀儡师的意义,把父母的两具傀儡托付给勘九郎,灵魂得以解脱.',
						...
						xwj_xhuoying_xiaonan: '小南,日本动漫<火影忍者>中的人物.<晓>组织的创立者之一,而且是其中唯一一个女性成员,代号<白>(白虎),被称为<天使>.她是第二次忍界大战时雨隐村的孤儿,曾经和弥彦、长门一起接受过自来也的忍术指导.后来和长门一起被漩涡鸣人的真诚所感动,重拾保护世界和平的信心.曾设计六千亿张起爆符诛杀宇智波带土,但失败.最终为了保护长门的遗体的轮回眼而被自称为<宇智波斑>的<面具男>(宇智波带土)击杀.',
						...
						xwj_xhuoying_asima: '阿斯玛,日本动漫<火影忍者>中的人物,是一个性格随和的正派角色.猿飞阿斯玛在火之国木叶忍者村,拥有罕见的风属性查克拉,是近战中首屈一指的上忍. 同时他也是第三代火影猿飞日斩的儿子,猿飞木叶丸的叔叔,第十班(奈良鹿丸、山中井野、秋道丁次)的老师,上忍夕日红的情侣.后来在与<晓>的战斗中被飞段击杀,临终前将意志托付给第十班.夕日红后于第四次忍界大战前夕生下阿斯玛的遗腹女猿飞未来. 第四次忍界大战中,阿斯玛被药师兜以秽土转生形式复活,后被其弟子第十班合力击败并封印,最后随秽土转生的解除而灵魂回归净土.',
						xwj_xhuoying_leiying: '夜月艾,日本动漫<火影忍者>中雷之国云隐村的四代目雷影,夜月一族,八尾人柱力奇拉比的义兄[1]. 性格暴躁冲动,身体健壮并以速度著称,全身在<雷遁护体>的保护下,速度和防御力都堪称强劲,在第四次忍界大战中担任忍者联军的统帅,后来在第四部队战场中与其余四影大战宇智波斑,结果被斑击败,回复伤势后前往十尾战场支援前线,与其余联军合力击败十尾.',
						...
						xwj_xhuoying_xianglin: '香燐,日本漫画<火影忍者>系列及其衍生作品中的女性角色.漩涡一族的族人,拥有强大的查克拉和出色的感知能力,而且伤者只要咬她的皮肤就会回复查克拉和伤势.曾经是大蛇丸的部下,后成为宇智波佐助<鹰>小队的一员.',
						...
						xwj_xwugeng_shixing: '<br><li>攻击★★★★★<br><li>防御★★★☆☆<br><li>辅助★★☆☆☆<br><li>控场★★☆☆☆<br><li>爆发★★★★★<br>十刑,<封神纪>(动画<武庚纪>)中的人物,天之子,阎部大神,号称<猎神之神>,主修罗界神力,后得到不死鸟的帮助使用元始界神力.被大剑士子羽用诛天剑击杀后再次复活,但这次复活也回复了神志和记忆,从而踏上向神族复仇之路',
					...
					skill: {
									...
									next.ai = function (button) {
										...
										if (!player.storage.tian) {
											for (var i = 0; i < tian.length; i++) {
												if (lib.character[button.link][3].includes(tian[i])) {
													return Math.random();
												}
											}
										} else {
											for (var i = 0; i < di.length; i++) {
												if (lib.character[button.link][3].includes(di[i])) {
													return Math.random();
												}
											}
										}
							...
							content() {
								...
								for (var i of game.players) {
									if (i != player) {
										...
										if (i.identity == 'nei') player.storage.countnei++;
									}
								}
								('step 1');
						...
						xwj_xu_qingshang: {
							...
							content() {
								...
								if (Array.isArray(event.cards))
									for (var i of event.cards) {
										if (!event.type.includes(get.type(i))) {
						...
						xwj_xsanguo_naxian: {
							...
							audio: 'ext:群英会/audio:2',
							init(player) {
								player.storage.xwj_xsanguo_naxian = [];
							},
							filter(event, player) {
							...
							content() {
								'step 0';
								player
									.chooseCard([1, Infinity], 'h', get.prompt('xwj_xsanguo_naxian'), function (card) {
										return true;
									})
								...
								if (result.bool) {
									...
									if (!player.storage.xwj_xsanguo_naxian) {
										player.storage.xwj_xsanguo_naxian = result.cards;
									} else {
										player.storage.xwj_xsanguo_naxian = player.storage.xwj_xsanguo_naxian.concat(result.cards);
									} //QQQ
									player.lose(result.cards, ui.special);
									player.markSkill('xwj_xsanguo_naxian');
								}
							},
							intro: {
								content: 'cards',
								onunmark(storage, player) {
									if (storage && storage.length) {
										...
										delete player.storage.xwj_xsanguo_naxian;
									}
								},
							},
							mod: {
								targetEnabled(card, player, target, now) {
									if (target.storage.xwj_xsanguo_naxian && target.storage.xwj_xsanguo_naxian.length) {
							...
							ai: {
								threaten(player, target) {
									if (target.storage.xwj_xsanguo_naxian && target.storage.xwj_xsanguo_naxian.length) return 2;
									return 1;
								},
							},
						},
						xwj_xsanguo_zexu2: {
							...
							silent: true,
							forced: true,
							popup: 'xwj_xsanguo_zexu',
							content() {
								for (var i of game.players) {
									if (i.hasSkill('xwj_xsanguo_zexu1')) {
										i.storage.xwj_xsanguo_zexu1 = [];
										i.removeSkill('xwj_xsanguo_zexu1');
									}
								}
							},
						},
						xwj_xsanguo_zexu1: {
							init(player) {
								player.storage.xwj_xsanguo_zexu1 = [];
							},
						},
						xwj_xsanguo_zexu: {
							audio: 'ext:群英会/audio:2',
							...
							forced: true,
							group: 'xwj_xsanguo_zexu2',
							init(player) {
								player.storage.xwj_xsanguo_naxian = [];
							},
							filter(event, player) {
								return player.storage.xwj_xsanguo_naxian?.length;
							}, //QQQ
							content() {
								'step 0';
								event.cards = [];
								...
								if (result.bool) {
									...
									player.storage.xwj_xsanguo_naxian.remove(result.links);
									if (!player.storage.xwj_xsanguo_naxian.length) {
										player.unmarkSkill('xwj_xsanguo_naxian');
									} else {
										player.markSkill('xwj_xsanguo_naxian');
									}
								} else event.finish();
								...
								player
									.chooseControl()
									.set('choiceList', ['令' + get.cnNumber(event.cards.length) + '名其他角色各摸一张牌', '交给' + get.cnNumber(event.cards.length) + '名其他角色各一张<贤>,其须弃置一张与之不同类别的手牌,否则失去一点体力'])
									.set('ai', function () {
										var num = game.countPlayer(function (current) {
								...
								('step 4');
								player
									.chooseTarget(get.prompt('xwj_xsanguo_zexu'), event.cards.length, true, function (card, player, target) {
								...
								if (result.bool) {
									...
									event.targets[event.num].addSkill('xwj_xsanguo_zexu1');
									event.targets[event.num].storage.xwj_xsanguo_zexu1 = get.type(result.links[0]);
								...
								event.targets = game.filterPlayer(function (current) {
									return current != player && current.hasSkill('xwj_xsanguo_zexu1');
								});
								event.targets.sort(lib.sort.seat);
								('step 11');
								if (event.num < event.targets.length) {
									event.targets[event.num].chooseCard('弃置一张与' + get.translation(event.targets[event.num].storage.xwj_xsanguo_zexu1) + '牌类型不同的手牌,否则失去一点体力', 'h', function (card) {
										return get.type(card) != event.targets[event.num].storage.xwj_xsanguo_zexu1;
									}).ai = function (card) {
						...
						xwj_xsanguo_qianzhi: {
							...
							content() {
								...
								player
									.chooseCardButton('潜志:选择要移动的牌(还能移动' + event.num + '张)', event.cards)
						...
						xwj_xsanguo_longhun1: {
							...
							filter(event, player, card) {
								return event.parent.skill == 'xwj_xsanguo_longhun2' && event.cards.length > 1;
							},
							content() {
								trigger.num++;
							},
						...
						xwj_xsanguo_longhun3: {
							...
							forced: true,
							filter(event, player, card) {
								return event.parent.skill == 'xwj_xsanguo_longhun4' && event.cards.length > 1;
							},
							content() {
								trigger.num++;
							},
						...
						xwj_xsanguo_longhun4: {
							...
							viewAs: {
								...
								number: 10,
								cards: [
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'heart', number: 11, name: 'wuzhong', cardid: '7612547293', _transform: 'translateX(0px)', _mouseentercreated: false, clone: { name: 'wuzhong', suit: 'heart', number: 11, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _onEndDelete: true, timeout: 554, _transitionEnded: true }, timeout: 513, original: 'h' },
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'heart', number: 12, name: 'shengdong', cardid: '2156409290', _transform: 'translateX(224px)', _mouseentercreated: false, clone: { name: 'shengdong', suit: 'heart', number: 12, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _onEndDelete: true, timeout: 555, _transitionEnded: true }, timeout: 514, original: 'h' },
								],
							},
							filter(event, player) {
						...
						xwj_xsanguo_longhun5: {
							...
							content() {
								...
								var str = '请选择一张还是两张♣️️牌当【闪】使用或打出';
								player.chooseControl(controls, ui.create.dialog(str, 'hidden')).ai = function () {
									return Math.floor(Math.random() * controls.length);
								};
								('step 1');
						...
						xwj_xsanguo_longhun7: {
							...
							viewAs: {
								...
								number: 5,
								cards: [
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'spade', number: 7, name: 'nanman', cardid: '4314694258', _transform: 'translateX(112px)', _mouseentercreated: false, clone: { name: 'nanman', suit: 'spade', number: 7, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _transitionEnded: true, timeout: 1067 }, timeout: 960, original: 'h' },
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'spade', number: 8, name: 'sha', cardid: '4490253259', _transform: 'translateX(0px)', _mouseentercreated: false, clone: { name: 'sha', suit: 'spade', number: 8, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _transitionEnded: true, timeout: 1068 }, timeout: 961, original: 'h' },
								],
							},
							viewAsFilter(player) {
						...
						xwj_xsanguo_longhun6: {
							...
							forced: true,
							filter(event, player, card) {
								return event.parent.skill == 'xwj_xsanguo_longhun7' && event.cards.length > 1;
							},
							content() {
								player.discardPlayerCard('he', _status.currentPhase, true);
							},
						...
						xwj_xsanguo_yizan1: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt('xwj_xsanguo_yizan1'), 'he', function (card) {
						...
						xwj_xsanguo_yizan5: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt('xwj_xsanguo_yizan5'), 'he', function (card) {
										return get.type(card) == 'basic';
									})
									.set('ai', function (card) {
						...
						xwj_xsanguo_yizan3: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt('xwj_xsanguo_yizan3'), 'he', function (card) {
										return get.type(card) == 'basic';
									})
									.set('ai', function (card) {
										if (!_status.event.player.countCards('h', 'shan')) {
						...
						xwj_xsanguo_yizan6: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt('xwj_xsanguo_yizan6'), 'he', function (card) {
										return get.type(card) == 'basic';
									})
									.set('ai', function (card) {
										if (!_status.event.player.countCards('h', 'sha')) {
						...
						xwj_xsanguo_wuniang: {
							...
							content() {
								...
								player
									.chooseTarget(
										get.prompt('xwj_xsanguo_wuniang'),
										1,
										function (card, player, target) {
						...
						xwj_xsanguo_zhengnan: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择【镇南】的目标', lib.translate.xwj_xsanguo_zhengnan_info, function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										var player = _status.event.player;
						...
						xwj_xsanguo_xushen: {
							...
							filter(event, player) {
								for (var i of game.players) {
									if (i.name == 'guansuo') return false;
								}
						...
						xwj_xsanguo_chuanshu: {
							...
							content() {
								...
								game.broadcastAll() + trigger.player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xsanguo_zhangjiao.jpg');
						...
						xwj_xsanguo_badao: {
							...
							content() {
								player.chooseToUse({ name: 'sha' }, '霸刀:是否使用一张杀？');
							},
						},
						xwj_xsanguo_zhenjun: {
							audio: 'jieyue',
							...
							content() {
								'step 0';
								player
									.chooseTarget(get.prompt('xwj_xsanguo_zhenjun'), function (card, player, target) {
						...
						xwj_xsanguo_chengmou: {
							...
							forced: true,
							filter(event, player) {
								return player.storage.xwj_xsanguo_jugong?.length;
							}, //QQQ
							content() {
								'step 0';
						...
						xwj_xqinshi_hengjian: {
							...
							check(card) {
								...
								for (var i of players) {
									if (lib.filter.targetEnabled({ name: 'sha' }, player, i) && get.effect(i, { name: 'sha' }, player) > 0) {
										num++;
							...
							viewAs: {
								name: 'sha',
								cards: [
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'spade', number: 4, name: 'sha', nature: 'thunder', cardid: '2771745310', clone: { name: 'sha', suit: 'spade', number: 4, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _transitionEnded: true, timeout: 1375 }, timeout: 1313, original: 'h' },
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'spade', number: 8, name: 'sha', cardid: '6226101294', clone: { name: 'sha', suit: 'spade', number: 8, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, _transitionEnded: true, timeout: 1376 }, timeout: 1314, original: 'h' },
								],
							},
								...
								effect: {
									player(card, player) {
										...
										if (card.name == 'sha' && player.countCards('h', 'sha') < 2 && !player.needsToDiscard()) {
											...
											var players = game.filterPlayer();
											for (var i of players) {
												if (lib.filter.targetEnabled({ name: 'sha' }, player, i) && get.attitude(player, i) < 0) {
													num++;
							...
							content() {
								...
								if (maxh) {
									player.chooseCard('选择一张手牌交给' + get.translation(event.current), true).ai = function (card) {
										if (get.attitude(player, event.current) > 0) return get.value(card);
										return -get.value(card);
									};
								}
								('step 3');
						...
						xwj_xhuoying_jihuai: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择弃置一名未被封印、白板的其他角色令其与你组成双将', lib.translate.xwj_xhuoying_jihuai_info, function (card, player, target) {
						...
						xwj_xhuoying_jinkui: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择弃置一名已横置的角色令其重置武将牌', lib.translate.xwj_xhuoying_jinkui_info, function (card, player, target) {
						...
						xwj_xhuoying_luosheng: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择弃置一名角色弃置其一张装备区的牌', lib.translate.xwj_xhuoying_luosheng_info, function (card, player, target) {
						...
						xwj_xhuoying_huanyin: {
							...
							content() {
								...
								player.chooseTarget(
									get.prompt2('xwj_xhuoying_huanyin'),
									[1, num],
									function (card, player, target) {
						...
						xwj_xhuoying_xundao: {
							...
							content() {
								...
								for (var i of game.players) {
									if (i != player) {
										if (i.getEquip(1) && !controls.includes('武器栏')) controls.push('武器栏');
										if (i.getEquip(2) && !controls.includes('防具栏')) controls.push('防具栏');
										if (i.getEquip(3) && !controls.includes('防御马')) controls.push('防御马');
										if (i.getEquip(4) && !controls.includes('进攻马')) controls.push('进攻马');
										if (i.getEquip(5) && !controls.includes('宝物栏')) controls.push('宝物栏');
										if (!controls.includes('cancel2')) controls.push('cancel2');
									}
								}
								var str = '请选择一个装备栏';
								player.chooseControl(controls, ui.create.dialog(str, 'hidden')).ai = function () {
						...
						xwj_xhuoying_daoji2: {
							...
							filter(event, player) {
								...
								if (Array.isArray(event.cards))
									for (var i of event.cards) {
										if (i.original == 'e' && get.position(i) == 'd') return true;
									}
								return false;
							},
							forced: true,
							content() {
								...
								if (Array.isArray(trigger.cards))
									for (var i of trigger.cards) {
										if (get.type(i) == 'equip' && i.original == 'e' && get.position(i) == 'd') {
											cards.push(i);
										}
									}
								if (cards.length) {
						...
						xwj_xhuoying_fenglian: {
							...
							content() {
								...
								player
									.chooseTarget(get.prompt('xwj_xhuoying_fenglian'), [1, Math.min(num, player.countCards('h'))], function (card, player, target) {
						...
						xwj_xhuoying_chongyu: {
							...
							audio: 'ext:群英会/audio:2',
							filter(event, player) {
								if (!player.storage.xwj_xhuoying_chongyu || player.storage.xwj_xhuoying_chongyu.length >= player.hp) return false;
								if (event.player == player) return false;
								if (Array.isArray(event.cards))
									for (var i of event.cards) {
							...
							content() {
								...
								if (cards.length) {
									...
									player.say(chat);
									if (!player.storage.xwj_xhuoying_chongyu) {
										player.storage.xwj_xhuoying_chongyu = cards;
									} else {
						...
						xwj_xhuoying_chongyu2: {
							...
							filter(event, player) {
								return event.player != player && player.storage.xwj_xhuoying_chongyu?.length;
							}, //QQQ
							forced: true,
							content() {
								'step 0';
						...
						xwj_xhuoying_ganzhi: {
							...
							content() {
								...
								player.say(chat);
								for (var i of game.players) {
									if (i.countCards('h', 'tao') > 0 || i.countCards('h', { type: 'equip' }) > 0) {
										player.draw();
									}
								}
							},
						...
						xwj_xhuoying_nishou: {
							...
							content() {
								...
								('step 1');
								setTimeout(function () {
									player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xhuoying_chiwan.jpg');
								}, 500);
						...
						xwj_xhuoying_fengwang: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择【风网】的目标', lib.translate.xwj_xhuoying_fengwang_info, function (card, player, target) {
						...
						xwj_xhuoying_anqi: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择发动【暗器】的目标', lib.translate.xwj_xhuoying_anqi_info, function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return -get.attitude(_status.event.player, target);
						...
						xwj_xhuoying_weishou: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择发动【伪兽】的目标', lib.translate.xwj_xhuoying_weishou_info, function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return -get.attitude(_status.event.player, target);
						...
						xwj_xhuoying_wodi: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择发动【卧底报信】的目标', lib.translate.xwj_xhuoying_wodi_info, function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return get.attitude(_status.event.player, target);
						...
						xwj_xhuoying_renzong: {
							...
							content() {
								...
								player.maxHp = 6;
								player.hp = 6;
								game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xhuoying_yuyi.jpg');
								ui.backgroundMusic.src = 'extension/群英会/audio/wms_backgroundmusic.mp3';
						...
						xwj_xhuoying_tianyan: {
							...
							content() {
								'step 0';
								player
									.chooseTarget(get.prompt2('xwj_xhuoying_tianyan'), function (card, player, target) {
										var names = [];
										if (target.name && !target.isUnseen(0)) names.add(target.name);
										if (target.name1 && !target.isUnseen(0)) names.add(target.name1);
										if (target.name2 && !target.isUnseen(1)) names.add(target.name2);
						...
						xwj_xhuoying_zhoushu: {
							...
							content() {
								...
								setTimeout(function () {
									game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xhuoying_xieshen.jpg');
									player.node.avatar.show('extension/群英会/image/xwj_xhuoying_xieshen.jpg');
								}, 100);
							},
						},
						xwj_xhuoying_zhoushu3: {
							...
							content() {
								...
								if (!player.hasSkill('xwj_xhuoying_zhoushu')) {
									...
									game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xhuoying_feiduan.jpg');
									player.update();
								}
							},
							forced: true,
						...
						xwj_xhuoying_qianshou: {
							...
							content() {
								'step 0';
								player
									.chooseTarget(get.prompt('xwj_xhuoying_qianshou'), [1, trigger.targets.length], function (card, player, target) {
						...
						xwj_xhuoying_guazhang: {
							...
							filter(event, player) {
								...
								return player.storage.xwj_xhuoying_guazhang.suit == event.cards[0].suit;
							},
							content() {
								player.say('八卦•六十四掌');
								player.draw();
						...
						//当你需要使用或打出牌时,你可以观看牌堆顶的两张牌,若其中有你可以使用或打出的牌,则你可以使用或打出之(初代火影的木遁能瞬间凭空创造出一片森林)
						xwj_xhuoying_mudun: {
							mod: {
								cardUsable(card, player, num) {
									if (!player.getCards('h').includes(card)) return Infinity;
								},
								targetInRange(card, player) {
							...
							forced: true,
							filter(event, player) {
								for (var i of Array.from(ui.cardPile.childNodes).slice(0, 2)) {
									if (!event.filterCard(i, player, event)) {
										continue;
									}
									if (!player.hasUseTarget(i, false, false)) {
										//但是空城这种不能成为目标的还是不能过,需要这步判断
										continue;
									}
									return true;
								}
							...
							async content(event, trigger, player) {
								...
								if (list.length) {
									const { links } = await player.chooseButton(['使用或打出牌堆顶的牌', list]).set('ai', (button) => {
									...
									if (links?.length) {
										...
										game.log(`${get.translation(player)}使用或打出牌堆顶的${get.translation(links)}`);
									}
								}
							},
							ai: {
						...
						xwj_xhuoying_renfa: {
							...
							content() {
								...
								player
									.chooseButton([get.prompt('xwj_xhuoying_renfa'), [list1.concat(list2).concat(list3), 'vcard']])
									.set('filterButton', function (button) {
										var player = _status.event.player;
										if (player.storage.xwj_xhuoying_renfa2 && player.storage.xwj_xhuoying_renfa2.includes(button.link[2])) return false;
										return true;
									})
									.set('ai', function (button) {
							...
							mod: {
								maxHandcard(player, num) {
									if (player.storage.xwj_xhuoying_renfa2) {
						...
						xwj_xhuoying_renfa2: {
							...
							filter(event, player) {
								...
								return player.storage.xwj_xhuoying_renfa2 && player.storage.xwj_xhuoying_renfa2.includes(event.card.name);
							},
							forced: true,
							content() {
								'step 0';
						...
						xwj_xhuoying_renfa3: {
							...
							filterCard: true,
							filter(event, player) {
								return player.storage.xwj_xhuoying_renfa.length;
							},
							...
							content() {
								...
								for (var i of result.links) {
									player.storage.xwj_xhuoying_renfa.remove(i);
								}
						...
						xwj_xhuoying_aiyuan: {
							...
							content() {
								...
								if (player == trigger.player) {
									player
										.chooseControl('你摸两张', '其摸两张', 'cancel2', function () {
											return '其摸两张';
										})
									...
									player
										.chooseTarget(get.prompt('xwj_xhuoying_aiyuan'), function (card, player, target) {
						...
						xwj_xhuoying_jiaoji: {
							...
							content() {
								...
								('step 1');
								player.chooseDrawRecover(get.prompt('xwj_xhuoying_jiaoji'))
									('step 2');
						...
						xwj_xhuoying_shuilao: {
							...
							logTarget: 'player',
							filter(event, player) {
								return player.hp <= player.countCards('e') && event.player != player && player.countCards('h') > 0 && event.player.countCards('h') > 0;
							},
							content() {
								'step 0';
						...
						xwj_xhuoying_jiaodan3: {
							...
							silent: true,
							filter(event, player) {
								return event.reason && event.reason.parent.name == 'xwj_xhuoying_jiaodan';
							},
							content() {
								player.removeSkill('xwj_xhuoying_jiaodan3');
							},
						...
						xwj_xhuoying_zhimou_shan: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt2('xwj_xhuoying_zhimou'), 'h', function (card) {
										return get.type(card) == 'basic';
									})
									.set('ai', function (card) {
										if (!_status.event.player.countCards('h', 'shan')) {
						...
						xwj_xhuoying_yiliao: {
							...
							content() {
								...
								player
									.choosePlayerCard(trigger.player, get.prompt('xwj_xhuoying_yiliao', trigger.player), 'he')
									.set('ai', function (button) {
						...
						xwj_xhuoying_jinshu: {
							...
							content() {
								...
								player.say(chat);
								if (player.storage.xwj_xhuoying_jinshu) {
									player.removeSkill(player.storage.xwj_xhuoying_jinshu);
								} //QQQ
								switch (Math.floor(Math.random() * 3)) {
									case 0:
										if (lib.skill.xwj_xhuoying_fenshen) {
											player.addSkill('xwj_xhuoying_fenshen');
											player.storage.xwj_xhuoying_jinshu = 'xwj_xhuoying_fenshen';
											player.popup('xwj_xhuoying_fenshen');
										}
										break;
									case 1:
										if (lib.skill.xwj_xhuoying_shanguang) {
											player.addSkill('xwj_xhuoying_shanguang');
											player.storage.xwj_xhuoying_jinshu = 'xwj_xhuoying_shanguang';
											player.popup('xwj_xhuoying_shanguang');
										}
										break;
									case 2:
										if (lib.skill.xwj_xhuoying_zhuansheng) {
											player.addSkill('xwj_xhuoying_zhuansheng');
											player.storage.xwj_xhuoying_jinshu = 'xwj_xhuoying_zhuansheng';
											player.popup('xwj_xhuoying_zhuansheng');
										}
										break;
								}
							},
						...
						xwj_xhuoying_baofu2: {
							...
							content() {
								...
								player.chooseCardTarget({
									...
									position: 'he',
									prompt: '互乘起爆符之术:弃置一张♦️️牌对目标或与其距离为1以内的目标造成等量火焰伤害',
								});
								('step 1');
						...
						xwj_xhuoying_reshuilao: {
							...
							logTarget: 'player',
							filter(event, player) {
								return player.isTurnedOver() && event.player != player && player.countCards('h') > 0 && event.player.countCards('h') > 0;
							},
							content() {
								'step 0';
								player.chooseToCompare(trigger.player);
								('step 1');
								if (result.bool) {
									var chat = ['呼～终于回复自由身,差点闷死自己', '给我老实呆着吧,水遁•水牢之术', '既然那么想进牢里,那就别想出来了'].randomGet();
									player.say(chat);
									player.turnOver();
									trigger.player.addTempSkill('zishou2');
								}
						...
						xwj_xhuoying_reyiliao: {
							...
							content() {
								...
								player
									.choosePlayerCard(trigger.player, get.prompt('xwj_xhuoying_reyiliao', trigger.player), 'he')
									.set('ai', function (button) {
										if (!_status.event.check) return 0;
										if (_status.event.target.isUnderControl(true, _status.event.player)) {
											if (get.type(button.link) == 'equip') {
						...
						xwj_xhuoying_yuanyuwind: {
							...
							//出牌阶段,你可弃置所有其他角色区域内的一张牌
							async content(event, trigger, player) {
								//QQQ
								player.$fullscreenpop('风遁•压害', 'thunder');
								game.playSu(['xwj_xhuoying_zhongquan1', 'xwj_xhuoying_zhongquan2'].randomGet());
								for (var i of game.players.filter((q) => q.countCards('he') && q != player)) {
									await player.discardPlayerCard(i, 'he');
								}
						...
						xwj_xhuoying_newkuilei: {
							...
							content() {
								...
								player
									.chooseTarget(
										get.prompt('xwj_xhuoying_newkuilei'),
										[1],
										function (card, player, target) {
						...
						xwj_xhuoying_duoyang: {
							...
							forced: true,
							//每当你造成伤害后,你可以选择一项:1、获得受到伤害的角色装备区里的一张牌(你的装备区相应位置不能有此类装备牌);2、摸一张牌
							content() {
								'step 0';
								var next;
								if (
									...
									next = player
										.chooseControl('夺取写轮眼', 'draw_card', 'cancel2', function (event, player) {
						...
						xwj_xhuoying_chengyi: {
							...
							filter(event, player) {
								...
								if (Array.isArray(event.cards))
									for (var i of event.cards) {
										if (get.type(i) == 'equip' && i.suit != 'club' && get.position(i) == 'd') {
											return true;
										}
									}
								return false;
						...
						xwj_xhuoying_tianyu: {
							...
							content() {
								...
								var list = [];
								for (var i of game.players) {
									if (player.isAlive()) list.push(i);
								}
								var players = [];
								for (var i of game.players) {
									if (player.isAlive()) {
										i.storage.xwj_xhuoying_tianyu = list.randomGet();
										players.push(i);
									}
								}
								for (var i of players) {
									if (player.isAlive()) {
										game.swapSeat(i, i.storage.xwj_xhuoying_tianyu);
										delete i.storage.xwj_xhuoying_tianyu;
									}
								}
							},
							ai: {
								...
								result: {
									player(player) {
										if (get.attitude(player, player.next) <= 0) return 10;
										return 0;
									},
								},
							},
						...
						xwj_xhuoying_zibao: {
							...
							content() {
								...
								for (var i of game.players) {
									if (i != player) i.damage('fire');
								}
								('step 1');
								player.unmark('xwj_xhuoying_zibao');
								for (var i of game.players) {
									if (i != player && get.distance(player, i) <= 1) i.damage('fire');
								}
								player.die();
							},
							intro: {
						...
						xwj_xhuoying_xfengyin: {
							...
							content() {
								...
								('step 2');
								player
									.chooseTarget('请选择一名目标,令其技能失效', get.prompt('xwj_xhuoying_xfengyin'), function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return get.damageEffect(target, player, player);
						...
						xwj_xhuoying_yan: {
							...
							intro: {
								content: '<font color=#f00>锁定技</font> 开始阶段,若你有<黑炎>标记,你须进行判定,若判定结果为♠️️,你受到一点无来源的火焰伤害',
							},
						},
						...
						xwj_xhuoying_tiancheng: {
							...
							content() {
								'step 0';
								player
									.chooseControl('超重岩之术', '超轻岩之术', 'cancel2')
									.set('prompt', get.prompt('xwj_xhuoying_tiancheng'))
									.set('ai', function () {
						...
						xwj_xhuoying_zhishu: {
							...
							content() {
								...
								if (
									...
									next = player
										.chooseControl('式纸之舞', 'draw_card', 'cancel2', function (event, player) {
											var source = _status.event.source;
											var att = get.attitude(player, source);
											if (source.hasSkillTag('noe')) {
												if (att > 0) {
						...
						xwj_xhuoying_xmudun: {
							...
							subSkill: {
								use: {
									...
									chooseButton: {
										dialog(event, player) {
											player.storage.xwj_xhuoying_xmudun = [ui.cardPile.childNodes[0]];
						...
						xwj_xhuoying_jinshao: {
							...
							content() {
								...
								target
									.chooseToDiscard(num, 'he', '弃置' + get.cnNumber(num) + '张牌或受到两点火焰伤害')
									.set('ai', function (card) {
						...
						xwj_xhuoying_tianyin: {
							...
							check(event, player) {
								...
								var num = 0;
								for (var i of game.players) {
									if (i.countCards('he') && i != player && get.attitude(player, i) <= 0) {
										num++;
									}
									if (i.countCards('j') && i != player && get.attitude(player, i) > 0) {
										num++;
									}
								...
								selectTarget(card, player, range) {
									...
									if (type != 'delay') {
										if (player.hp == 1 || player.countCards('h') == 1) {
											if (Array.isArray(range) && range[1] != -1) range[1] += Infinity;
										}
									}
								},
							},
							...
							subSkill: {
								less: {
									...
									content() {
										...
										trigger.player
											.chooseToDiscard('绝防:弃置一张基本牌,否则此牌对' + get.translation(player) + '无效', function (card) {
												return get.type(card) == 'basic';
											})
											.set('ai', function (card) {
						...
						xwj_xhuoying_reshenwei: {
							...
							prompt: '若你的武将牌背面朝上,你可以选择翻面,视为你使用一张【无懈可击】',
							check() {
								return 1;
							},
							ai: {
							...
							subSkill: {
								move: {
									...
									content() {
										'step 0';
										player
											.chooseToDiscard('he', get.prompt('xwj_xhuoying_reshenwei'), '弃置一张牌并移动场上的一张牌(神威左眼远距扭曲空间)', lib.filter.cardDiscardable)
											.set('ai', function (card) {
												if (!_status.event.check) return 0;
												return 7 - get.value(card);
											})
						...
						xwj_xhuoying_xishou: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('请选择一名目标并弃置你所有的手牌,获得其所有牌', get.prompt('xwj_xhuoying_xishou'), function (card, player, target) {
										return target != player && target.countCards('he') > 0;
									})
									.set('ai', function (target) {
										return get.damageEffect(target, player, player);
							...
							subSkill: {
								move: {
									...
									content() {
										'step 0';
										player
											.chooseToDiscard('he', get.prompt('xwj_xhuoying_kkxshenwei'), '弃置一张牌并移动场上的一张牌(神威左眼远距扭曲空间)', lib.filter.cardDiscardable)
											.set('ai', function (card) {
												if (!_status.event.check) return 0;
												return 7 - get.value(card);
											})
						...
						xwj_xhuoying_refengyin: {
							...
							content() {
								...
								('step 2');
								player
									.chooseTarget('请选择一名目标,令其技能失效', get.prompt('xwj_xhuoying_refengyin'), function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return get.damageEffect(target, player, player);
						...
						xwj_xhuoying_zhuansheng: {
							...
							enable: 'phaseUse',
							filter(event, player) {
								return game.dead.length && player.countCards('h', 'tao') > 0;
							},
							content() {
								'step 0';
											...
											if (get.value(hs[i]) <= 6) {
												switch (hs[i].number) {
													case 13:
														return -1;
													case 12:
														if (player.storage.xwj_xhuoying_fenshen + num <= 8) return -1;
														break;
													case 11:
														if (player.storage.xwj_xhuoying_fenshen + num <= 7) return -1;
														break;
													default:
														if (hs[i].number > 5 && player.storage.xwj_xhuoying_fenshen + num <= 8) return -1;
												}
											}
										}
										return 0;
						...
						xwj_xhuoying_lunmu: {
							...
							content() {
								player.gainPlayerCard([1, trigger.num], get.prompt('xwj_xhuoying_lunmu', trigger.player), trigger.player, get.buttonValue, 'he')
							},
							ai: {
								expose: 0.8,
								order: 0.8,
						...
						xwj_xhuoying_rebusi: {
							...
							content() {
								...
								for (var i = 0; i < player.storage.xwj_xhuoying_rebusi.length - 1; i++) {
									if (event.card.number && event.card.number == player.storage.xwj_xhuoying_rebusi[i].number) return;
								}
								trigger.cancel();
								if (player.hp <= 0) {
									player.hp = 2;
						...
						xwj_xhuoying_zhenxing: {
							...
							viewAs: {
								name: 'wanjian',
								cards: [
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'heart', number: 11, name: 'sha', cardid: '6788264292', clone: { name: 'sha', suit: 'heart', number: 11, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, cardid: '24684222150', _transitionEnded: true, timeout: 16135 }, original: 'h', timeout: 8930 },
									{ node: { image: {}, info: {}, name: {}, name2: {}, background: {}, intro: {}, range: {} }, storage: {}, vanishtag: [], _uncheck: [], suit: 'diamond', number: 13, name: 'sha', cardid: '7368636302', clone: { name: 'sha', suit: 'diamond', number: 13, node: { name: {}, info: {}, intro: {}, background: {}, image: {} }, fixed: true, _transitionEnded: true, timeout: 17132 }, original: 'h' },
								],
							},
							ai: {
						...
						xwj_xhuoying_yiyuan: {
							...
							content() {
								...
								player
									.chooseTarget(get.prompt('xwj_xhuoying_yiyuan'), function (card, player, target) {
								...
								if (result.bool) {
									...
									ui.backgroundMusic.src = 'extension/群英会/audio/wms_backgroundmusic.mp3';
									game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xhuoying_liudaoban.jpg');
									game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_wxyd_background.jpg');
						...
						xwj_xqunying_fengyuan2: {
							...
							content() {
								...
								if (targets.length == 1) {
									...
									player
										.chooseTarget(true, '选择【逢源】的目标', function (card, player, target) {
						...
						xwj_xqunying_jusha: {
							...
							content() {
								...
								('step 1');
								if (trigger.target.countCards('h', { suit: trigger.card.suit })) {
									trigger.directHit = true;
								}
							},
							ai: {
						...
						xwj_xqunying_jiaohua: {
							...
							content() {
								...
								('step 2');
								player.chooseToUse({ name: 'sha' }, trigger.player, '是否对' + get.translation(trigger.player) + '使用一张杀？');
							},
							ai: {
								threaten: 0.8,
								order: 8,
						...
						xwj_xqunying_bianshen: {
							...
							content() {
								...
								if (player.maxHp == 4) {
									...
									player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xqunying_frieza1.jpg');
									player.update();
								}
								if (player.maxHp == 6) {
									player.recover();
									...
									player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xqunying_frieza2.jpg');
									player.update();
								}
								if (player.maxHp == 8) {
									player.recover();
									...
									player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xqunying_frieza3.jpg');
						...
						xwj_xwugeng_anyue: {
							...
							filter(event, player) {
								...
								if (event.targets.includes(player)) return false;
								return game.hasPlayer(function (current) {
									return current.isDisabled(2) && !event.targets.includes(current);
								});
							},
							content() {
								'step 0';
						...
						xwj_xwugeng_anxie: {
							...
							content() {
								...
								player
									...
									.set('filterButton', function (button) {
										if (player.storage.xwj_xwugeng_anxie && player.storage.xwj_xwugeng_anxie.includes(button.link[2])) return false;
										return true;
									})
									.set('ai', function (button) {
										var rand = _status.event.rand * 2;
						...
						xwj_xwugeng_anxie2: {
							mod: {
								targetEnabled(card, player, target) {
									if (target.storage.xwj_xwugeng_anxie.includes(card.name)) return false;
								},
							},
						},
						xwj_xwugeng_dunkong: {
						...
						xwj_xwugeng_qinhe: {
							...
							content() {
								...
								player.storage.xwj_xwugeng_qinhe = card.suit;
						...
						xwj_xwugeng_qinhe2: {
							...
							filter(event, player) {
								...
								return event.card.suit == player.storage.xwj_xwugeng_qinhe;
							},
							content() {
						...
						xwj_xwugeng_fanji1: {
							...
							content() {
								...
								switch (Math.floor(Math.random() * 2)) {
									case 0:
										if (lib.skill.xwj_xwugeng_mingpao) {
											player.addSkill('xwj_xwugeng_mingpao');
											player.storage.xwj_xwugeng_fanji = 'xwj_xwugeng_mingpao';
											player.popup('xwj_xwugeng_mingpao');
										}
										break;
									case 1:
										if (lib.skill.xwj_xwugeng_lianqi) {
											player.addSkill('xwj_xwugeng_lianqi');
											player.storage.xwj_xwugeng_fanji = 'xwj_xwugeng_lianqi';
											player.popup('xwj_xwugeng_lianqi');
										}
										break;
								}
							},
						...
						xwj_xwugeng_fanji2: {
							...
							content() {
								...
								switch (Math.floor(Math.random() * 2)) {
									case 0:
										if (lib.skill.xwj_xwugeng_wuse) {
											player.addSkill('xwj_xwugeng_wuse');
											player.storage.xwj_xwugeng_fanji = 'xwj_xwugeng_wuse';
											player.popup('xwj_xwugeng_wuse');
										}
										break;
									case 1:
										if (lib.skill.xwj_xwugeng_xuwu) {
											player.addSkill('xwj_xwugeng_xuwu');
											player.storage.xwj_xwugeng_fanji = 'xwj_xwugeng_xuwu';
											player.popup('xwj_xwugeng_xuwu');
										}
										break;
								}
							},
						...
						xwj_xwugeng_anyu: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择【暗狱】的目标', lib.translate.xwj_xwugeng_anyu_info, function (card, player, target) {
						...
						xwj_xwugeng_zhutian: {
							...
							content() {
								...
								player.chooseToUse({ name: 'sha' }, '诛天:是否使用一张杀？');
							...
							content() {
								...
								if (event.targets.length) {
									...
									if (event.current.countCards('e', { subtype: 'equip1' }) && target.isAlive()) {
										player
											.discardPlayerCard(event.current, 1, 'e', true)
											.set('filterButton', function (button) {
						...
						xwj_xwugeng_zhengnu: {
							...
							content() {
								...
								('step 1');
								event.current
									.chooseControl('随机弃牌', '令其摸牌')
									.set('ai', function () {
										if (get.attitude(event.current, player) > 0) return '令其摸牌';
										if (get.attitude(event.current, player) <= 0 && event.current.countCards('he') > 2) return '随机弃牌';
										return '令其摸牌';
									})
						...
						xwj_xwugeng_baofeng: {
							...
							content() {
								...
								('step 2');
								player
									.chooseTarget('选择【救护】的目标', lib.translate.xwj_xwugeng_baofeng_info, function (card, player, target) {
										return target != player;
									})
									.set('ai', function (target) {
										return get.attitude(_status.event.player, target);
						...
						xwj_xwugeng_zhengkong: {
							...
							content() {
								'step 0';
								player
									.chooseTarget('选择发动神技【真空】的目标', lib.translate.xwj_xwugeng_zhengkong_info, function (card, player, target) {
						...
						xwj_xwugeng_xuemao: {
							...
							//出牌阶段限一次,你可以展示牌堆顶的一张牌,若该牌为:装备牌,你回复一点体力并使用之;锦囊牌,你弃置此展示的牌后再摸两张牌;基本牌,你获得此牌,视为对任意一名其他角色使用一张不计距离、不计入次数限制的【杀】
							async content(event, trigger, player) {
								//QQQ
						...
						xwj_xwugeng_baiqian1: {
							...
							content() {
								...
								('step 2');
								game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_tian_background.jpg');
						...
						xwj_xwugeng_chenjie: {
							...
							prepare(cards, player) {
								...
								for (var i of game.players) {
									if (i != player && i.storage.xwj_xwugeng_yinguo > 0) {
										i.addTempClass('target');
									}
								}
							},
							...
							content() {
								for (var i of game.players) {
									if (i.storage.xwj_xwugeng_yinguo <= 0) {
										event.finish();
									} else {
										if (i.storage.xwj_xwugeng_yinguo >= 1) {
											i.damage();
											i.storage.xwj_xwugeng_yinguo--;
											if (i.storage.xwj_xwugeng_yinguo < 1) {
												i.unmarkSkill('xwj_xwugeng_yinguo');
											}
										}
									}
									...
									game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xwugeng_eshan.jpg');
									player.update();
						...
						xwj_xwugeng_chenzhui: {
							...
							content() {
								...
								game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xwugeng_zhengshan.jpg');
						...
						xwj_xwugeng_zhuzhan: {
							...
							//当一名角色处于濒死状态时, 你可以亮出牌堆顶的一张牌, 若该牌为红色牌, 则该角色回复一点体力, 将这张红色牌置入弃牌堆; 若该牌为黑色牌, 你获得之
							async content(event, trigger, player) {
								//QQQ
								const card = ui.cardPile.firstChild;
								player.showCards(card);
						...
						xwj_xwugeng_fuhuo: {
							...
							content() {
								var chat = ['想要击杀我？你还得加把劲……不好,被包围了', '好小子,你成功地让我死了一次,哈哈……'].randomGet();
								player.say(chat);
								setTimeout(function () {
									player.gainMaxHp();
									player.update();
									ui.backgroundMusic.src = 'extension/群英会/audio/wms_backgroundmusic.mp3';
									game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_shixing_background.jpg');
									game.broadcastAll() + player.node.avatar.setBackgroundImage('extension/群英会/image/xwj_xwugeng_xinshixing.jpg');
					...
					translate: {
						...
						xwj_xu_huhua: '护花',
						xwj_xu_huhua_info: '<li>当一名女性角色受到来源不为你的伤害后,你可与其组成双将.其与伤害来源的原武牌先后随机替换为一张女性角色的武将牌<li>当你受到伤害后,你可令伤害来源的武将牌随机替换为一张女性角色的武将牌',
						xwj_xu_huhua2: '护花',
						xwj_xu_huhua2_info: '当你受到伤害后,伤害来源的武将牌随机替换为一张女性角色的武将牌',
						xwj_xu_shuangsu: '双宿',
						xwj_xu_shuangsu_info: '<span class=greentext>觉醒技</span> 当你进入濒死状态时,你须失去技能【护花】,获得技能【栋梁】,与【小焕】组成双将,回复体力至3',
						xwj_xu_dongliang: '栋梁',
						...
						xwj_xu_dingju: '定局',
						xwj_xu_dingju_info: '<span class=yellowtext>限定技</span> 你可回收所有其他角色的武将牌,重新分配武将牌(原体力上限和体力均不变)',
						xwj_xu_cheng: '小诚',
						xwj_xu_tiandun: '天遁',
						xwj_xu_tiandun_info: '<font color=#f00>锁定技</font> 当你受到其他角色使用卡牌所造成的伤害时,你随机获得伤害来源的一项技能,检索所有的武将牌牌堆,选择一张令伤害来源替换之,你再摸X张牌(X为对你造成伤害的牌的点数的三分之一进位取整)',
						xwj_xu_xiaoxu: '小徐',
						xwj_xu_tuiyin: '退隐',
						...
						xwj_xu_qingshang: '情殇',
						xwj_xu_qingshang_info: '当你受到伤害后,你可以弃置一张牌并声明一种牌的类型,从牌堆顶亮出五张牌,获得其中与你所声明类型相同的牌,将其余的牌置入弃牌堆.',
						xwj_xu_yunchou: '运筹',
						xwj_xu_yunchou_info: '<span class=yellowtext>限定技</span> 你可回收除主公外的其他角色的身份牌,重新分配身份牌(限身份局)',
						xwj_xsanguo_qiaoguolao: '乔国老',
						xwj_xsanguo_naxian: '纳贤',
						xwj_xsanguo_naxian_info: '回合开始时,你可将任意张手牌置于武将牌上,称为<贤>.若你有<贤>,你不能成为基本牌的目标',
						xwj_xsanguo_zexu: '择婿',
						xwj_xsanguo_zexu_info: '回合结束时,你可选择一项:1、弃置任意张<贤>并令等量的其他角色各摸一张牌;2、将任意张<贤>交给等量的其他角色,该角色须弃置一张与之不同类别的手牌,否则其失去一点体力',
						...
						xwj_xsanguo_juli: '诛戾',
						xwj_xsanguo_juli_info: '出牌阶段开始时,若你已受伤且你的体力为全场最低,你可指定任意名<魏国>角色,令各摸X张牌(X为你损失的体力值),你的体力上限调整至当前体力值',
						xwj_xsanguo_xunyou: '荀攸',
						xwj_xsanguo_houlve: '后略',
						xwj_xsanguo_houlve_info: '若你没有手牌,你可以摸一张牌,视为你使用一张【无懈可击】',
						xwj_xsanguo_shenzhaoyun: '神赵云',
						xwj_xsanguo_longhun1: '龙魂',
						xwj_xsanguo_longhun1_info: '',
						xwj_xsanguo_longhun2: '龙魂•火杀',
						xwj_xsanguo_longhun2_info: '♦️️当火【杀】',
						xwj_xsanguo_longhun3: '龙魂',
						xwj_xsanguo_longhun3_info: '',
						xwj_xsanguo_longhun4: '龙魂•桃',
						xwj_xsanguo_longhun4_info: '♥️️当【桃】',
						xwj_xsanguo_longhun5: '龙魂•闪',
						xwj_xsanguo_longhun5_info: '♣️️当【闪】',
						xwj_xsanguo_longhun6: '龙魂',
						xwj_xsanguo_longhun6_info: '',
						xwj_xsanguo_longhun7: '龙魂•无懈',
						xwj_xsanguo_longhun7_info: '♠️️当【无懈可击】',
						xwj_xsanguo_longhun: '龙魂',
						xwj_xsanguo_longhun_info: '你可将至多两张同花色的牌按以下规则使用或打出:♥️️当【桃】;♦️️当火【杀】;♣️️当【闪】;♠️️当【无懈可击】.若你此法使用了两张红色牌,则此牌回复值伤害值+1.若你此法使用了两张黑色牌,则你弃置当前回合角色一张牌',
						...
						xwj_xsanguo_wuniang_info: '你使用或打出【杀】时,你可以获得一名其他角色的一张牌,该角色摸一张牌;若<关索>在场,你可令<关索>也摸一张牌',
						xwj_xsanguo_zhengnan: '镇南',
						xwj_xsanguo_zhengnan_info: '当你成为【南蛮入侵】的目标时,你可令一名其他角色随机受到一至三点伤害',
						xwj_xsanguo_xushen: '许身',
						xwj_xsanguo_xushen_info: '当其他男性角色令你脱离濒死状态时,若<关索>不在场,其可以选择是否用<关索>替换其武将牌,你回复一点体力并获得技能【镇南】',
						...
						xwj_xsanguo_oldguhuo_info: '你可以说出任何一种基本牌或非延时类锦囊牌,并正面朝下使用或打出一张手牌.体力值不为0的其他角色依次选择是否质疑.若无人质疑,则该牌按你所述之牌结算.若有人质疑则亮出验明:若为真,质疑者各失去1点体力;若为假,质疑者各摸一张牌.无论真假,弃置被质疑的牌.仅当被质疑的牌为♥️️且为真时,该牌仍然可以进行结算',
						...
						xwj_xsanguo_guanxing_info: '每当你受到一次伤害后,你可以观看牌堆顶的4张牌,并将其以任意顺序置于牌堆项或牌堆底,你摸一张牌.',
						...
						xwj_xsanguo_xiongqi_info: '<span class=yellowtext>非限定技</span>当你处于濒死状态时,你可以丢弃你判定区里的牌,并重置你的武将牌,摸1张牌且体力回至体力上限,加一体力上限.',
						...
						xwj_xsanguo_tishen_info: '<span class=yellowtext>限定技</span> 准备阶段开始时,你可以将体力回复至体力上限,你每以此法回复1点体力,便摸一张牌.',
						xwj_xsanguo_yujin: '于禁',
						xwj_xsanguo_zhenjun: '镇军',
						xwj_xsanguo_zhenjun_info: '准备阶段,你可以弃置一名手牌数多于体力值的角色的X张牌(X为其手牌数和体力值之差),选择一项:1.你弃置等同于其中非装备牌数量的牌;2.其摸等量的牌.',
						...
						xwj_xsanguo_xiuzheng_info: '出牌阶段限一次,你可选择一名其他角色,展示牌堆顶的两张牌,若同为红色,则其受到一点火焰伤害;若同为黑色,其受到一点雷电伤害;若颜色不相同,你弃置其一张牌.你获得这两张展示的牌后再弃置两张牌',
						xwj_xqinshi_mozi: '墨子',
						...
						xwj_xqinshi_jianai_info: '当一名角色进入濒死状态,你可以弃置当前装备区的所有牌,令其回复体力至1,你可切换一次装备区',
						...
						xwj_xqinshi_renzhong_info: '<font color=#F0F>万物回春</font> 出牌阶段限一次,你可以获得所有其他角色区域内的一张牌,你须为所有存活的角色每人逐个派发一张手牌',
						xwj_xhuoying_xinyiliao: '医疗',
						xwj_xhuoying_xinyiliao_info: '当一名角色进入濒死状态时,若你手牌中有基本牌,你可以弃置之,其回复体力至1',
						...
						xwj_xhuoying_jinkui_info: '<font color=#F0F>粘金之铠</font> 每名角色的回合限一次,当你受到伤害时,你可选择一名武将牌已横置的角色,令其重置武将牌,此伤害减一',
						...
						xwj_xhuoying_jihuai_info: '<font color=#F0F>寄生鬼坏</font> 回合开始阶段,你可选择一名未被<封印>、<白板>的其他角色,你与其组成双将,若你的体力值不小于3,其原武将非锁定技失效(被封印),否则你咒印化,其所有技能失效,直到其受到伤害后',
						xwj_xhuoying_luosheng: '罗生',
						xwj_xhuoying_luosheng_info: '<font color=#F0F>通灵罗生门</font> 每名角色的回合限一次,当你受到伤害时,你可以弃置一名角色的一张装备区的牌,此伤害值减一',
						...
						xwj_xhuoying_bingdun_info: '<font color=#F0F>魔镜冰晶</font> <li>出牌阶段限一次,若你的武将牌正面朝上,你可以令至多两名其他角色翻面,你翻面(<font color=#F0F>配合再不斩</font>)<li>你与已翻面的角色的距离为1,若你的武将牌背面朝上,你的防御距离为无限<li>你对其他角色使用唯一目标的牌(借刀杀人除外)时,可令此牌额外指定所有其他已翻面的角色',
						...
						xwj_xhuoying_xundao_info: '准备阶段,你可以选择一个装备栏名,获得所有其他角色该装备栏里的装备牌',
						...
						xwj_xhuoying_chongyu_info: '当其他角色的♠️️牌因弃牌进入弃牌堆时,若你武将牌上的<虫>数量不大于你的体力值,你可将其改为置于你的武将牌上,称为<虫>(<font color=#F0F>唤虫</font>).当一名其他角色回复体力时,你可以移去一张<虫>,改为你回复一点体力,若你未受伤,则改为你摸一张牌(<font color=#F0F>吸食查克拉</font>)',
						...
						xwj_xhuoying_tongya_info: '出牌阶段限一次,若你有牌,你可以选择一名有牌的其他角色并弃置任意张牌,该角色须弃置等量张牌(不足则全弃且你补摸两者差值张牌),若如此做,视为你对其使用一张【决斗】(<font color=#F0F>配合油女志乃</font>)',
						xwj_xhuoying_tiantian: '天天',
						xwj_xhuoying_jiju: '集具',
						xwj_xhuoying_jiju_info: '当其他角色使用武器牌或进攻马时,你可令视为你使用之',
						xwj_xhuoying_anqi: '暗器',
						xwj_xhuoying_anqi_info: '当你失去一张装备区的牌时,你可视为对任意一名其他角色使用一张无距离【杀】,你摸一张牌',
						xwj_xhuoying_zuojin: '佐井',
						xwj_xhuoying_weishou: '伪兽',
						xwj_xhuoying_wodi: '卧底',
						xwj_xhuoying_wodi_info: '结束阶段,若你的装备区有牌(有情报),你可令一名其他角色与你各摸一张牌,你回复一点体力,若如此做,你须将装备区的牌收进手牌区',
						xwj_xhuoying_weishou2: '虎眈',
						xwj_xhuoying_weishou2_info: '不能使用或打出牌',
						xwj_xhuoying_weishou_info: '当你受到伤害后,你可以选择一名其他角色,令其直到下个出牌阶段开始,不能使用或打出牌(<font color=#F0F>封印术•虎视眈弹</font>)你视为对其使用一张【杀】,结算后其随机使用一张装备牌(<font color=#F0F>配合团藏</font>)',
						...
						xwj_xhuoying_xianti_info: '<font color=#f00>锁定技</font> 结束阶段,你可以将手牌数补至体力上限,回复一点体力',
						xwj_xhuoying_liudaoxianren: '六道仙人',
						xwj_xhuoying_renzong: '忍宗',
						xwj_xhuoying_renzong_info: '<font color=#f00>锁定技</font> 当一名其他角色进入濒死状态,你选择技能【天眼】或【仙体】令其永久获得之,其回复体力至1并摸两张牌,你失去技能【忍宗】,体力上限和体力值改为6.当该被授予【忍宗】的角色造成或受到一次伤害后,你随机获得一张基本牌',
						...
						xwj_xhuoying_tianyan_info: '回合开始阶段,你可以选择一名其他角色,获得其一项技能,直到回合结束',
						...
						xwj_xhuoying_xinbaiyan_info: '出牌阶段限一次,你可以观看一名角色的手牌,你可以获得其中一张黑色手牌',
						...
						xwj_xhuoying_zhoushu_info: '<font color=#F0F>死司凭血</font> 当你对任意一名其他角色造成伤害后,你可施展咒术诅咒该角色,技能【咒术】进入冷却状态,直到该被诅咒的角色死亡后方可再次发动.当你受到伤害后,同时该被诅咒的角色视为受到来源为你的等量的伤害,直到其死亡为止',
						...
						xwj_xhuoying_yiyuan_info: '<font color=#F0F>完全体须佐能乎</font>  <span class=greentext>觉醒技</span> 濒死阶段,你摸一张牌,将所有手牌交给一名体力上限大于你的其他角色,调整你的体力上限至与该角色相同,回复体力至体力上限,并获得技能<轮墓>',
						...
						xwj_xhuoying_resizhan_info: '<font color=#F0F>八门遁甲</font> <span class=yellowtext>限定技</span> 出牌阶段开始前,你可以摸8张牌,你的体力上限调整至1',
						...
						xwj_xhuoying_xianfa_info: '<font color=#f00>锁定技</font> 当你失去最后的手牌时,你可以摸一张牌,回复一点体力.',
						xwj_xhuoying_zhuansheng: '转生',
						xwj_xhuoying_zhuansheng_info: '<font color=#F0F>秽土转生</font> 出牌阶段限一次,你可弃置一张【桃】并选择一名已阵亡角色,令其复活,其体力回复至2,摸两张的牌,并且若为身份局,其身份阵营与你一致(若你为主公则视阵营为忠臣),你失去一点体力上限',
						...
						xwj_xhuoying_leique_info: '<li>出牌阶段,你可以将一张普通【杀】当雷【杀】使用,你使用的雷【杀】可无视目标角色的防具.<li>当你击杀一名角色后,你获得技能【神威】',
						xwj_xhuoying_chutian: '日向雏田',
						xwj_xhuoying_baiyan: '白眼',
						xwj_xhuoying_baiyan_info: '出牌阶段限一次,你可以观看一名角色的手牌,你可以获得其中一张红色手牌',
						...
						xwj_xhuoying_kkxshenwei_info: '<li>当你即将受到伤害时,若你的武将牌正面朝上,你可翻面并取消此伤害(右眼虚化)<li>若你的武将牌背面朝上,你可以选择翻面,视为使用了一张【无懈可击】(右眼虚化)<li>当你从背面翻至正面时,你可以弃置一张牌,移动场上的一张牌(左眼远距离扭曲空间转移物体)',
						xwj_xhuoying_daitu: '宇智波带土',
						xwj_xhuoying_reshenwei: '神威',
						xwj_xhuoying_reshenwei_info: '<li>当你即将受到伤害时,若你的武将牌正面朝上,你可翻面并取消此伤害(右眼虚化)<li>若你的武将牌背面朝上,你可以选择翻面,视为使用了一张【无懈可击】(右眼虚化)<li>当你从背面翻至正面时,你可以弃置一张牌,移动场上的一张牌(左眼远距离扭曲空间转移物体)',
						xwj_xhuoying_xishou: '吸收',
						xwj_xhuoying_xishou_info: '回合结束阶段,你可指定一名其他角色,弃置你所有的手牌并将你的武将牌翻面,若如此做,你获得其所有的牌.',
						xwj_xhuoying_xianyan: '献眼',
						xwj_xhuoying_xianyan_info: '当你死亡时,你可将所有手牌交给一名其他角色,该角色随机获得你的除此技能外的另一项其他技能,并回复一点体力.',
						...
						xwj_xhuoying_xianshu_info: '<font color=#f00>锁定技</font> 当你失去最后的手牌时,你可以摸牌补至你当前体力的张数,回复一点体力.',
						xwj_xhuoying_fenshen: '分身',
						xwj_xhuoying_fenshen_info: '<font color=#F0F>影分身之术</font> 出牌阶段限一次,你可以用一张手牌与一至X名角色同时拼点,依次结算拼点结果,若你赢,没赢的角色随机弃置一张牌;若你拼点没赢,你摸一张牌,并获得一个<分身>标记.你的进攻距离+X(X为你的<分身>标记数,若你有:①至少6个分身标记,你获得技能【螺旋】;②至少9个分身标记,你立即死亡)',
						...
						xwj_xhuoying_refengyin_info: '<font color=#F0F>尸鬼封尽</font> <span class=yellowtext>限定技</span> 出牌阶段开始时,你可令任意一名角色永久失去当前的所有技能(若其体力上限大于4则调整为4),你进入濒死状态',
						...
						xwj_xhuoying_tianyin_info: '<font color=#F0F>万象天引</font> 回合结束阶段限一次,你可以随机获得每名其他角色的一张手牌,你将你的武将牌翻面',
						...
						xwj_xhuoying_reguaili_info: '每两轮限一次,你可以失去任意点体力并摸X张牌,直到回合结束,你使用的【杀】造成的伤害+1,你的进攻距离+X,且你可以额外使用X张【杀】(X为你以此法失去的体力值),若你在发动此技能后,每击杀一名其他角色,你增加一点体力上限',
						...
						xwj_xhuoying_baihao_info: '<font color=#F0F>阴封印•百豪之术</font> 出牌阶段结束或濒死状态时,若你已受伤,你可选择回复体力至体力上限,失去一点体力上限(创造再生•百豪之术是能提高自愈能力但会减少寿命的禁术)',
						xwj_xhuoying_xianren: '仙人',
						xwj_xhuoying_xianren_info: '<font color=#F0F>蛤蟆仙人</font> <span class=greentext>觉醒技</span> 准备阶段开始时,若你的<忍>的数量不小于3,你减1点体力上限,选择一项:1、回复1点体力;2、摸两张牌.你获得技能<仙术>',
						xwj_xhuoying_renfa: '忍法',
						xwj_xhuoying_renfa_info: '你每受到一点伤害时,你可以摸一张牌并将一张手牌移出游戏,称为"忍".为"忍"记录一个基本牌或锦囊牌名称(须与其他"忍"记录的名称均不同).出牌阶段,你可以用任意数量的手牌与等量的<忍>交换,每阶段限一次.你的回合外,当有其他角色使用与你记录的"忍"牌名相同的牌时,你可以令此牌无效,移去该"忍",你的手牌上限+X(X为<忍>的数量)',
						xwj_xhuoying_renfa2: '忍法',
						xwj_xhuoying_renfa2_info: '你每受到一点伤害时,你可以摸一张牌并将一张手牌移出游戏,称为"忍".为"忍"记录一个基本牌或锦囊牌名称(须与其他"忍"记录的名称均不同).你的回合外,当有其他角色使用与你记录的"忍"牌名相同的牌时,你可以令此牌无效,移去该"忍"',
						xwj_xhuoying_citan: '刺探',
						xwj_xhuoying_citan_info: '(偷窥美女、潜入雨忍村)出牌阶段开始时,你可以潜入偷窥其他角色的手牌',
						xwj_xhuoying_changsheng: '永生',
						xwj_xhuoying_changsheng_info: '濒死阶段,你可摸一张牌,你可以与一名有手牌的其他角色拼点,若你赢,你与该角色交换体力值(伤害来源转为你)并且你增加一点体力上限(不得超过5);若你拼点没赢,你回复体力至体力上限,你失去一点体力上限并翻面(一个像蛇一样难缠的家伙)',
						...
						xwj_xhuoying_rexianshu_info: '<font color=#f00>锁定技</font> 当你失去最后的手牌时,你可以摸牌补至你当前体力的张数,回复一点体力.',
						...
						xwj_xhuoying_reshouhu_info: '<font color=#F0F>别天神</font> <span class=yellowtext>限定技</span> 出牌阶段,你可以把所有手牌交给一名存活的非主公角色,若为身份局,其身份与你一致(你不为主公),并且其之后的一切行动受你控制,你(止水)自杀身亡(无伤害来源)',
						xwj_xhuoying_linghua: '灵化',
						xwj_xhuoying_linghua_info: '<font color=#F0F>灵化之术</font> 你的防御距离+X(X为你的体力值).出牌阶段,你可弃置一张基本牌,选择一名存活角色,令其与你交换角色(仅游戏玩家交换,直到该角色的回合结束阶段你才能<回魂>).你失去此技能(直到你<回魂>重新获得【灵化】),并对目标造成一点伤害,结算完后当前回合立即结束',
						...
						xwj_xhuoying_huihun_info: '<font color=#f00>锁定技</font> 回合结束阶段,你与【加藤断】交换角色魂魄(其须存活),你失去此技能,其获得技能【灵化】',
						...
						xwj_xhuoying_baofu2_info: '<font color=#F0F>互乘起爆符</font> 当一名角色受到火焰伤害后,你可以弃置一张♦️️牌,对该角色或与其距离为1以内的一名角色造成等量的火焰伤害.(单张起爆符即可无限连续通灵出起爆符集中于一点无限连续爆炸)',
						xwj_xhuoying_baofu: '爆符',
						xwj_xhuoying_baofu_info: '<font color=#F0F>互乘起爆符</font> 出牌阶段限一次,你可以对一名其他角色造成1点火焰伤害.每当一名角色受到火焰伤害后,你可以弃置一张♦️️牌,对该角色或与其距离为1的一名角色造成等量的火焰伤害.(单张起爆符即可无限连续通灵出起爆符集中于一点无限连续爆炸)',
						...
						xwj_xhuoying_ansha_info: '出牌阶段限一次,你可以选择一名有牌的其他角色,令其弃置一张牌,若如此做,你与其依次将武将牌翻面,直到该回合结束,其非锁定技失效.(水龙弹之术)你可以弃置一张手牌视为对任意一名角色使用一张不计入次数限制的无距离限制的【杀】',
						...
						xwj_xhuoying_beihua_info: '<font color=#F0F>三色药丸 倍化之术</font> 出牌阶段限3次,你可以展示牌堆顶的2X张牌,其中每有一张♣️️牌,你回复1点体力(♣️️引蝶)你将这些牌收入手牌,并且弃牌阶段你的手牌上限－X(X为你发动倍化的次数)',
						xwj_xhuoying_beihua2: '倍化',
						xwj_xhuoying_beihua2_info: '',
						xwj_xhuoying_zhuanxin: '转心',
						xwj_xhuoying_zhuanxin_info: '<font color=#F0F>心转心之术</font> 出牌阶段限一次,你可以弃置一张♥️️手牌,选择一名存活的其他角色,令其与你交换手牌',
						xwj_xhuoying_reyiliao: '医疗',
						xwj_xhuoying_reyiliao_info: '当有角色进入濒死状态时,你可以展示该角色的一张牌:若此牌为装备牌,则该角色弃掉这张牌并回复体力至1,若为非装备牌,你获得之.',
						xwj_xhuoying_yuanyu_info: '<font color=#F0F>地怨虞</font> 你每击杀一名角色时,可随机获得其一项技能,并增加一点体力上限(不得超过5)和回复一点体力.你根据该角色的势力获得相应的限定技:<li>魏国:风遁-出牌阶段,你可弃置所有其他角色区域内的一张牌<li>蜀国:火遁-出牌阶段,你可选择至多五名其他角色,对其各造成一点火属性伤害<li>吴国:水遁-当你回复体力时,你可将体力回复至体力上限,并摸一张牌<li>群雄:土遁-当你即将受到任何伤害时,你可防止之,并回复一点体力<li>其它:雷遁-出牌阶段,你可选择一名其他角色,对其造成一点雷属性伤害并令其武将牌背面朝上',
						...
						xwj_xhuoying_huomeng_info: '<font color=#F0F>伊邪那歧</font> 结束阶段,你可以弃置一张装备区的牌,对自己施加终极幻术,进入伊邪那歧梦境:直到下回合开始,你防止受到非属性伤害(佐助可破之)',
						...
						xwj_xhuoying_chengyi_info: '(继承赤砂之竭的傀儡)当其他角色的牌因弃置而进入弃牌堆时,若其中含有非♣️️装备牌,你可以获得所有这些非♣️️的牌.',
						...
						xwj_xhuoying_yinmou_info: '<span class=yellowtext>限定技</span> 出牌阶段,你可以弃置一张【闪电】,选择一名存活的其他角色,令其将武将牌更换为大筒木辉夜,你与其各回复一点体力(黑绝阴阳遁•精神附体•同化,谋划复活母亲辉夜)',
						...
						xwj_xhuoying_baodun_info: '出牌阶段限一次,你可以选择一名有手牌的其他角色,声名一种花色,并展示该角色随机一张手牌,若两者花色一致,你获得该手牌;若花色不一致,其受到一点火焰伤害,并且你可弃置其一张牌',
						xwj_xhuoying_zibao: '自爆',
						xwj_xhuoying_zibao_info: '<span class=yellowtext>限定技</span> 出牌阶段,你可令所有其他角色受到一点火焰伤害,若如此做,与你距离为1的角色额外受到一点火焰伤害,你死亡.',
						xwj_xhuoying_huigu: '灰骨',
						xwj_xhuoying_huigu_info: '<font color=#F0F>共杀灰骨</font> 出牌阶段限一次,你可以令攻击范围内的一名有手牌的其他角色展示一下其手牌,该角色陷入瞳术中:直到回合结束,其不能使用或打出牌.你须弃置其一张牌',
						...
						xwj_xhuoying_reguaili3_info: '你每击杀一名角色后,你增加一点体力上限',
						xwj_xhuoying_bizhu: '臂助',
						xwj_xhuoying_bizhu_info: '每当一名角色流失体力后,你与其可以各摸一张牌.',
						xwj_xhuoying_landun: '岚遁',
						xwj_xhuoying_landun_info: '<font color=#F0F>杀的集大成者</font>出牌阶段限一次,你可以进行一次判定,若判定结果为:♠️️,你使用的【杀】造成的伤害+1;♥️️,你使用的【杀】可以额外指定一名角色成为目标;♣️️,你使用【杀】后可弃置对方一张牌;♦️️,你使用【杀】的上限次数+1.你本回合使用的【杀】无距离限制且无法闪避',
						...
						xwj_xhuoying_zhengbao_info: '<font color=#F0F>蒸危暴威</font> <span class=greentext>觉醒技</span> 当你处于濒死状态时,你丢弃你所有判定区的牌,并重置你的武将牌,摸两张牌,体力上限改为4点并回复体力至3点(<font color=#F0F>令二代土影感到头痛的术,故调上限克尘遁</font>),你使用一张<蒸危暴威>卡牌,获得技能<循爆>、<水炮>',
						...
						xwj_xhuoying_xfengyin_info: '<font color=#F0F>尸鬼封尽</font> <span class=yellowtext>限定技</span> 回合结束阶段你可令任意一名角色永久失去当前的所有技能(若其体力上限大于4则调整为4),你进入濒死状态.',
						...
						xwj_xhuoying_yan: '黑炎',
						xwj_xhuoying_yan_info: '<font color=#f00>锁定技</font> 开始阶段,若你有此<黑炎>标记,你须进行判定,若判定结果为♠️️,你受到一点无来源的火焰伤害(直到该角色阵亡,否则黑炎不会消失)',
						...
						xwj_xhuoying_daiban_info: '<font color=#f00>锁定技</font> 身份模式下,当你存活时,你先获得当前主公的主公技,主公的主公技失效',
						...
						xwj_xhuoying_rongdun_info: '<font color=#F0F>溶怪之术</font> 每名角色的回合限一次,当其他角色使用装备牌时,你可取消之,你摸一张牌',
						xwj_xhuoying_feidun: '沸遁',
						xwj_xhuoying_feidun_info: '<font color=#F0F>沸遁•巧雾之术</font> 出牌阶段限一次,你可以观看一名其他角色的牌,你可以用一张手牌替换其中的一张牌,若如此做,该角色受到一点火焰伤害',
						zbfs: '蒸危暴威',
						zbfs_info: '延时性锦囊牌,若判定结果为♦️️,则目标角色受到X点无来源的火焰伤害并随机弃置X张牌(X为此锦囊判定结果为♦️️的次数).判定完成后,将此牌移动到下家的判定区里.',
						xwj_xhuoying_shouju: '手鞠',
						xwj_xhuoying_fengwang: '风网',
						xwj_xhuoying_fengwang_info: '每当你翻面时,你可以弃置一名其他角色的一张牌',
						xwj_xhuoying_lianyou: '镰鼬',
						xwj_xhuoying_lianyou_info: '结束阶段时,你可令所有有牌的角色选择:弃置一张牌或令你获得其一张牌(<font color=#F0F>配合勘九郞</font>),你将武将牌背面朝上',
						xwj_xhuoying_jiuxinnai: '玖辛奈',
						xwj_xhuoying_fenglian: '封链',
						xwj_xhuoying_fenglian2: '链',
						xwj_xhuoying_fenglian3: '链',
						xwj_xhuoying_fenglian_info: '回合开始阶段,你可选择一至X名角色(X为你的手牌数),令其横置武将牌.若其下回合没跳过回合开始阶段,其下个回合的出牌阶段不能使用或打出牌',
						...
						xwj_xqunying_haodu_info: '出牌阶段限一次,你可以声明且令其他角色声明点数大(8~K)的牌较多(押大)或点数小(A~6)的牌较多(押小)(点数为7的不计).你亮出牌堆顶的七张牌比较两者点数范围内的数量:猜对的角色各摸一张牌,猜错的角色各须弃置一张牌;你获得所有相应点数数量较少的牌并弃置其余的牌;若数量一致,则仅执行你获得所有的这些牌 ',
						xwj_xqunying_fengyuan1: '逢源',
						xwj_xqunying_fengyuan1_info: '场上的女性角色每回复一点体力时,你若你已受伤,你可回复一点体力,否则摸一张牌.',
						xwj_xqunying_fengyuan: '逢源',
						xwj_xqunying_fengyuan_info: '场上的男性角色的回合可以交给你一张手牌,你交还其一张手牌(交换一张手牌);场上的女性角色每回复一点体力时,你若你已受伤,你可回复一点体力,否则摸一张牌.',
						xwj_xqunying_fengyuan2: '逢源',
						xwj_xqunying_fengyuan2_info: '场上的男性角色的回合可以交给你一张手牌,你交还其一张手牌(交换一张手牌)',
						xwj_xqunying_fengyuan3: '逢源',
						xwj_xqunying_fengyuan3_info: '场上的男性角色的回合可以交给你一张手牌,你交还其一张手牌(交换一张手牌);场上的女性角色每回复一点体力时,你若你已受伤,你可回复一点体力,否则摸一张牌.',
						...
						xwj_xqunying_jiaohua_info: '当你每回合首次成为【杀】的目标后,你可以令此【杀】不计入使用次数,你可对该【杀】使用者使用一张【杀】',
						xwj_xqunying_kunxi: '困袭',
						xwj_xqunying_kunxi_info: '结束阶段,你可以视为对一至X名其他角色使用一张【杀】(X为你装备区的牌数),你将装备区的所有牌收入手牌',
						...
						xwj_xwugeng_tongming_info: '统一冥族.</font><font color=#f00>锁定技</font> 场上每有一名角色阵亡,你回复一点体力并选择回复一个已被废除的装备栏',
						xwj_xwugeng_poji: '破极',
						xwj_xwugeng_poji_info: '限定技,出牌阶段,你可废除你的判定区和所有的装备栏并将体力值减至1并摸等同失去的体力值张牌,选择任意名其他角色,令其废除防具栏,你获得技能【聚义】、【暗月】,直到回合结束',
						xwj_xwugeng_juyi: '聚义',
						xwj_xwugeng_juyi_info: '</font><font color=#f00>锁定技</font> 你每使用一张牌后摸一张牌',
						xwj_xwugeng_anyue: '暗月',
						xwj_xwugeng_anyue_info: '暗月凶光.</font><font color=#f00>锁定技</font> 你对一名其他角色使用牌(借刀杀人除外)时,额外指定所有已废除防具栏的其他角色',
						xwj_xwugeng_bailian: '白莲圣王',
						xwj_xwugeng_xuelian: '血莲',
						xwj_xwugeng_xuelian2: '血莲',
						xwj_xwugeng_xuelian_info: '<span class=yellowtext>限定技</span> 当你受到伤害后,若伤害来源未获得<血莲>标记,你令其获得九个<血莲>标记,其回合结束时,你摸等同其<血莲>标记个数的一半(向下取整)张牌,其须弃置一个<血莲>标记,并随机弃置一张装备区的牌,否则受到一点伤害.若其<血莲>标记不大于0或已阵亡,你重置本技能',
						xwj_xwugeng_siling: '死灵',
						xwj_xwugeng_siling_info: '当你受到伤害时,若场上有角色有<血莲>标记,你可取消此伤害,该角色的<血莲>标记数量减一',
						xwj_xwugeng_fuxi: '伏羲',
						...
						xwj_xwugeng_dongshi_info: '当你使用【杀】／成为【杀】的目标时,你可以观看目标角色／来源的手牌,你可以获得其中一张奇数点数／偶数点数的手牌',
						...
						xwj_xwugeng_dunkong_info: '当你受到伤害时,若你有手牌,你可选择一种花色并弃置该花色的所有手牌,伤害值减一',
						...
						xwj_xwugeng_tianqi_info: '<span class=greentext>觉醒技</span> 当你进入濒死状态时,你令所有其他角色失去所有技能直到其受到伤害且不能使用或打出牌直到其回合开始,你须失去技能【咒文】,获得技能【暗狱】、【繁技】,增加一点体力上限并回复体力至体力上限,且直到你的下个回合开始,你不能成为【杀】的目标',
						...
						xwj_xwugeng_anyu: '暗狱',
						xwj_xwugeng_anyu_info: '当你受到伤害时,你可令一名武将牌正面朝上的其他角色将其武将牌翻面,直到其受到伤害后,其所有技能失效且不能使用或打出牌',
						xwj_xwugeng_anyu2: '暗',
						...
						xwj_xwugeng_qijian_info: '【剑气流星】出牌阶段限一次,你可以指定任意一名其他角色,你可依次弃置场上所有装备区有武器牌的角色的武器牌,并视为其对你指定的角色使用一张【杀】',
						...
						xwj_xwugeng_suiyue_info: '<font color=#F0F>神技•天斗碎岳</font>出牌阶段限一次,若你的体力值不为全场最高之一,你可选择一名技能合理的体力值不小于你的其他角色,你失去一点体力,视为对其连续使用无视防具的【杀】,直到其体力值比你的小为止',
						...
						xwj_xwugeng_chenjie_info: '<font color=#F0F>因果转轮</font>出牌阶段限一次,你可令场上所有拥有<因果>标记的角色受到一点来源为你的伤害,若如此做,这些角色弃掉一个<因果>标记,若此时你已装备防具,你须受到一点伤害,你变身为<恶禅>状态,失去技能<font color=#F0F>惩戒</font>、<font color=#F0F>空镜</font>、<font color=#F0F>助战</font>,获得技能<font color=#F0F>惩罪</font>、<font color=#F0F>金身</font>',
						xwj_xwugeng_chenzhui: '惩罪',
						xwj_xwugeng_chenzhui_info: '<font color=#F0F>罪业转轮</font>出牌阶段限一次,你可令场上任意一名拥有<因果>标记的角色受到X点伤害,若如此做,其<因果>标记数量清零(X为该角色的<因果>标记个数),你变身为<真禅圣王>,失去技能<font color=#F0F>惩罪</font>、<font color=#F0F>金身</font>获得技能<font color=#F0F>惩戒</font>、<font color=#F0F>空镜</font>、<font color=#F0F>助战</font> 结算后立即结束当前回合',
						xwj_xwugeng_zhuzhan: '助战',
						xwj_xwugeng_zhuzhan_info: '<font color=#F0F>长生界神力•生死轮回限</font>当一名角色处于濒死状态时,你可以亮出牌堆顶的一张牌,若该牌为红色牌,则该角色回复一点体力,将这张红色牌置入弃牌堆;若该牌为黑色牌,你获得之',
				...
				for (var i in xwugeng.character) {
					xwugeng.character[i][4].push('ext:群英会/image/' + i + '.jpg');
				}
				lib.config.all.characters.add('xwugeng');
				lib.config.characters.add('xwugeng');
				lib.translate.xwugeng_character_config = '<span class=yellowtext>群英会</span>';
				return xwugeng;
			});
			game.import('card', function () {
				var xwj_xus_equip = {
					...
					skill: {
						...
						xwj_xus_xuelunyang: {
							...
							content() {
								'step 0';
								player
									.chooseTarget(get.prompt2('xwj_xus_xuelunyang'), function (card, player, target) {
										var names = [];
										if (target.name && !target.isUnseen(0)) names.add(target.name);
										if (target.name1 && !target.isUnseen(0)) names.add(target.name1);
										if (target.name2 && !target.isUnseen(1)) names.add(target.name2);
								...
								if (event.isMine()) {
									event.dialog = lib.skill.xwj_xus_xuelunyang.createDialog(player, target);
									event.switchToAuto = function () {
										event._result = event.skillai(event.list);
										game.resume();
									};
					...
					translate: {
						...
						xwj_xus_xuelunyang_info: '回合开始阶段,你可以选择一名角色,获得其一项技能,直到回合结束',
						xwj_xus_jiuwei: '九尾',
				...
				lib.config.all.cards.add('xwj_xus_equip');
				lib.config.cards.add('xwj_xus_equip');
				return xwj_xus_equip;
			});
		},
		help: {
			群英会: '<li>此扩展原名为:新武将,始创于2017年8月,汇集了部分三国新将和<火影忍者>、<秦时明月>、<武庚纪>等作品的部分角色,技能强度略高,可联机.若想关闭某个扩展小包,可在相应武将栏内关闭并重启,开启同理.<li>若发现BUG可到贴吧或扩展交流②群:852740627 反馈,有技能设计(尤其是玄机动画<武庚纪>的角色)的建议也可联系作者<li>新增卡牌:【手里剑】2张,【写轮眼】、【九尾】、【漩涡面具】、【苦无】、【猴子】各1张.<li>须关闭<配音扩展>的<连杀开关>或者直接删了audio-skill目录下的liansha1至liansha7和jiuren1、jiuren2的九个配音文件,否则可能会与<配音扩展>一起播放击杀与回复体力的音效.<li>目前为止,已解决绝大部分已知的可能会导致游戏卡死的BUG,暂时未发现任何导致游戏卡死、弹窗警告的技能<li>【编码】Sukincen<li>【配图】Sukincen<li>【录制配音】Sukincen',
		},
		config: {
			xwjhelp: {
				name: '群英会',
				init: '1',
				item: { 1: '查看介绍', 2: '<li>此扩展原名为:新武将.若发现BUG可到贴吧或无名杀扩展交流群:852740627 反馈,有技能设计的建议也可联系作者', 3: '<li>本扩展汇集了部分三国新将和<火影忍者>、<秦时明月>、<武庚纪>等作品的人物(可在菜单→武将界面处关闭任意一个扩展小包,关闭重启后会隐藏武将图片且玩家禁选、ai禁用),技能强度略高.有技能特效,Ai智商较高,还可联机!', 4: '<li>本扩展能在关闭兼容模式情况下流畅运行', 5: '<li>更多介绍详看:其它→帮助' },
			},
			xjisha: {
				name: '击杀特效',
				intro: '击杀特效:开启此项(建议关闭新杀特效项)后重启游戏生效.任意一名角色击杀一名其他角色后,会记录此为其在本局共击杀过几名角色,并播放相应击杀人次的文字动画和配音',
				init: true,
			},
			xinjishatexiao: {
				name: '新杀特效',
				intro: '新杀特效:开启此项(建议关闭击杀特效项)后重启游戏生效.任意一名角色击杀一名其他角色后,会记录此为其在本局共击杀过几名角色,并播放相应击杀人次的武将动画和配音',
				init: false,
			},
			wuxianyuedu: {
				name: '无限月读',
					...
					switch (item) {
						...
						case '2':
							...
							ui.backgroundMusic.addEventListener('ended', function () {
								ui.backgroundMusic.src = 'extension/群英会/audio/' + lib.config.wms_backgroundmusic + '.mp3';
				...
				onclick(item) {
					switch (item) {
						case '1':
							...
							break;
						case '2':
							game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_background.jpg');
							break;
						case '3':
							game.broadcastAll() + ui.background.setBackgroundImage('extension/群英会/image/wms_wxyd_background.jpg');
							break;
