...
qyhcCL.arenaReady.push((lib, game, ui, get, ai, _status, config) => {
	if (config.jiangechange) {
		...
		qyhcCL.jiange_event = function (name) {
			...
			if (name[0] == 's') {
				for (var i of game.players)
					if (qyhcCL.isJianger(i)) {
						var skill = qyhcCL.jiange_eventMap[name][baseMap[i.identity]];
						if (!i.hasSkill(skill, null, false, false)) i.addSkill(skill);
						else i.addSkill('clyl_dengjie_chongsheng');
					}
			} else if (name[0] == 'b') {
		...
		qyhcCL.$jiange_event = function (name, logger) {
			...
			} else if (name == 'a_baiguiyexing') game.trySkillAudio(lib.character[logger.name][3].randomGet(), logger, true);
			else if (name == 'a_yuanshenqidong') game.trySkillAudio('luanji', logger, true);
		};
		...
		lib.mode.versus.config.update = function (config, map) {
			...
			if (config.versus_mode == 'three') map.reset_character_three.show();
			else map.reset_character_three.hide();
			if (config.versus_mode == 'jiange') {
				map.jiange_moshi.show();
				map.jiange_shili.show();
				map.jiange_zhen.show();
				map.jiange_event.show();
				if (config.jiange_event == 'NONE') map.jiange_eventer.hide();
				else map.jiange_eventer.show();
	...
	qyhcCL.jiangeSkills = {
		...
		cljg_fengjian2: {
			...
			ai: {
				...
				skillTagFilter(player, tag, arg) {
					if (arg && arg.card && arg.card.targets);
					else return false;
		...
		cljg_jiaoxie: {
			...
			content() {
				...
				if (!he.length) event.finish();
					...
					if (he.length > 1) target.chooseCard('he', true, '###' + get.translation(player) + '对你发动了〖缴械〗###<center>请将一张牌交给其</center>');
			...
			ai: {
				...
				result: {
					player(player, target) {
						...
						for (i = 0; i < es.length; i++) if (sgnatt != 0 && att2 != 0 && sgnatt != att2 && get.sgn(get.value(es[i], ui.selected.targets[0])) == -att2 && get.sgn(get.effect(target, es[i], player, target)) == sgnatt && target.canEquip(es[i])) return Math.abs(att);
						if (
							i == es.length &&
		...
		cljg_bashi: {
			...
			filter(event, player) {
				...
				if (event.card && (event.card.name == 'sha' || get.type(event.card) == 'trick') && !player.isTurnedOver());
				else return false;
		...
		cljg_qiwu: {
			...
			content() {
				...
				if (trigger.card.suit == 'heart') {
					player
						.chooseTarget('〖栖梧〗你可以对一名角色造成1点火焰伤害', function (card, player, target) {
					...
					player
						.chooseTarget('〖栖梧〗你可以令一名角色回复1点体力', function (card, player, target) {
			...
			content() {
				...
				if (event.targets.length) {
					...
					if (current && current.isIn())
						...
						else current.chooseToDiscard(true, '〖天狱〗请弃置一张牌');
					event.redo();
				}
			},
		},
		...
		cljg_leili: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('〖' + get.translation(event.name) + '〗由于你使用的【' + get.translation(trigger.card.name) + '】对' + get.translation(trigger.player) + '造成伤害', function (card, player, target) {
						if (target == _status.event.notPlayer) return false;
						return target.isEnemiesOf(player);
					})
					.set('ai', ai.thunderEffect)
					.set('prompt2', '<center>你可以对不为其的一名敌方角色造成1点雷电伤害</center>')
					.set('notPlayer', trigger.player);
				('step 1');
				if (result.targets?.length) {
		...
		cljg_tanjiang: {
			...
			content() {
				'step 0';
				player
					.chooseUseTarget({ name: 'sha' }, false, '〖探江〗视为对所有距离为1的敌方角色(' + get.translation(player.getEnemies((current) => get.distance(player, current) == 1 && player.canUse('sha', current, false))) + ')使用一张【杀】', 'nodistance')
					.set('prompt2', '或点取消令你计算与其他角色的距离-1')
					.set('filterTarget', (event, player, current) => get.distance(player, current) == 1 && current.isEnemiesOf(player)).selectTarget = [-1, -1];
				('step 1');
				if (!result.bool) {
		...
		cljg_xuanlei: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('〖玄雷〗你可以选择一名判定区内有牌的敌方角色', function (card, player, target) {
						return target.countCards('j') && target.isEnemiesOf(player);
					})
					.set('ai', ai.thunderEffect)
					.set('prompt2', '<center>对其造成1点雷电伤害</center>');
				('step 1');
				if (result.targets?.length) {
					result.targets[0].damage('thunder');
				}
		...
		cljg_beijiang: {
			...
			content() {
				'step 0';
				trigger.player
					.chooseCard('〖悲江〗请重铸一张牌', 'he', true, lib.filter.cardRecastable)
		...
		cljg_benlei: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('你可以发动〖奔雷〗选择一名敌方角色', lib.filter.enemies)
		...
		cljg_yibei: {
			...
			content() {
				var ccC = 'yiyi';
				player
					.chooseUseTarget({ name: ccC }, '〖逸备〗是否视为对所有己方角色使用一张【' + get.translation(ccC) + '】？', 'nodistance', 'notargetDelay')
					.set('filterTarget', (event, player, current) => current.isFriendsOf(player))
					.set('forced', !lib.config.autoskilllist.includes('cljg_yibei')).selectTarget = [-1, -1];
			},
		},
		...
		cljg_qizhen: {
			...
			content() {
				...
				player
					.judge('cljg_qizhen', (result) => {
						return result.color == 'red' ? 1.5 : -0.5;
					})
			...
			content() {
				...
				player
					.chooseToUse(
						...
						(event, player, target) => target.isEnemiesOf(player) && lib.filter.targetEnabledx(event, player, target),
					)
					.set('addCount', false)
					.set('openskilldialog', '〖避炎〗请将一张手牌当冰【杀】使用<br><span class=text>目标为所有能合法成为此【杀】目标的敌方角色(' + get.translation(game.filterPlayer((target) => target.isEnemiesOf(_status.event.player) && _status.event.player.canUse({ name: 'sha', nature: 'ice' }, target, false))) + ')</span>')
					.set('norestore', true)
		...
		cljg_gongshen: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('〖工神〗请令一名己方器械回复1点体力', true, (card, player, target) => {
		...
		cljg_runjiang: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('〖润江〗请令一名己方角色回复1点体力', true, (card, player, target) => {
		...
		cljg_tianjiang: {
			...
			filter(event, player) {
				...
				if (target && target.isIn() && target.isFriendsOf(player) && event.card) var evt = event.getParent('useCard', true);
				else return false;
		...
		cljg_lingfeng: {
			...
			content() {
				...
				if (get.color(event.cards[0]) != get.color(event.cards[1])) {
					...
					player.chooseCardButton(event.cards, '〖灵锋〗请获得以下一张牌', true).set('ai', (button) => {
						var player = _status.event.player,
							js = player.getCards('j'),
							val = get.buttonValue(button) / 114;
						if (js.length) {
		...
		cljg_tianyun: {
			...
			content() {
				'step 0';
				player
					.chooseTarget('你可以发动〖天陨〗选择一名敌方角色', lib.filter.enemies)
					.set('ai', function (target) {
						if (get.attitude(player, target) > -3) return 0;
		...
		if (config.boss_init) {
			...
			if (lib.config.characters.includes('qyhc_boss')) {
				for (var i in lib.character) {
					if (Array.isArray(lib.character[i]) && Array.isArray(lib.character[i][4]))
						if (lib.character[i][4].includes('jiangeboss') || lib.character[i][4].includes('jiangemech')) {
		...
		game.chooseCharacterJiange = function () {
			...
			switch (jiange_moshi) {
				case 'PVP':
					...
					switch (jiange_zhen) {
						case 'A':
							var list = [zhens[0] + 'mech1', zhens[0], zhens[0] + 'boss', zhens[0], zhens[1] + 'mech', zhens[1], zhens[1] + 'boss', zhens[1]];
							break;
						case 'B':
							var list = [zhens[0] + 'mech1', zhens[0], zhens[1] + 'boss', zhens[1], zhens[1] + 'mech', zhens[1], zhens[0] + 'boss', zhens[0]];
							break;
						case 'C':
							var list = [zhens[0] + 'mech1', zhens[1] + 'mech', zhens[0], zhens[1], zhens[0] + 'boss', zhens[1] + 'boss', zhens[0], zhens[1]];
							break;
						case 'D':
							var list = [zhens[1] + 'mech1', zhens[0] + 'mech', zhens[1], zhens[0], zhens[1] + 'boss', zhens[0] + 'boss', zhens[1], zhens[0]];
							break;
						default:
							var list = [zhens[0] + '1', zhens[1], zhens[1] + 'mech', zhens[0] + 'mech', zhens[0], zhens[1], zhens[1] + 'boss', zhens[0] + 'boss'];
							break;
					}
				...
				case 'EVE':
					...
					switch (jiange_zhen) {
						case 'A':
							var list = [zhens[0] + 'mech1', zhens[0] + 'boss', zhens[0] + 'boss', zhens[0] + 'mech', zhens[1] + 'mech', zhens[1] + 'boss', zhens[1] + 'boss', zhens[1] + 'mech'];
							break;
						case 'B':
							var list = [zhens[0] + 'boss1', zhens[0] + 'mech', zhens[1] + 'mech', zhens[1] + 'boss', zhens[1] + 'boss', zhens[1] + 'mech', zhens[0] + 'mech', zhens[0] + 'boss'];
							break;
						case 'C':
							var list = [zhens[0] + 'mech1', zhens[1] + 'boss', zhens[0] + 'boss', zhens[1] + 'mech', zhens[0] + 'mech', zhens[1] + 'boss', zhens[0] + 'boss', zhens[1] + 'mech'];
							break;
						case 'D':
							var list = [zhens[1] + 'mech1', zhens[0] + 'boss', zhens[1] + 'boss', zhens[0] + 'mech', zhens[1] + 'mech', zhens[0] + 'boss', zhens[1] + 'boss', zhens[0] + 'mech'];
							break;
						default:
							var list = [zhens[0] + 'mech1', zhens[1] + 'boss', zhens[1] + 'mech', zhens[0] + 'boss', zhens[0] + 'mech', zhens[1] + 'boss', zhens[1] + 'mech', zhens[0] + 'boss'];
							break;
					}
					while (list[idx].indexOf(zhens[0]) == -1) {
						var current = list.shift();
				...
				default:
					switch (jiange_zhen) {
						case 'A':
							var list = [_status.meShi + '1', _status.meShi + 'boss', _status.meShi, _status.meShi + 'mech', _status.diShi + 'mech', _status.diShi + 'boss', _status.diShi + 'boss', _status.diShi + 'mech'];
							break;
						case 'B':
							var list = [_status.meShi + '1', _status.meShi + 'mech', _status.diShi + 'mech', _status.diShi + 'boss', _status.diShi + 'boss', _status.diShi + 'mech', _status.meShi, _status.meShi + 'boss'];
							break;
						case 'C':
							var list = [_status.meShi + '1', _status.diShi + 'boss', _status.meShi + 'boss', _status.diShi + 'mech', _status.meShi, _status.diShi + 'boss', _status.meShi + 'mech', _status.diShi + 'mech'];
							break;
						case 'D':
							var list = [_status.diShi + 'boss1', _status.meShi, _status.diShi + 'mech', _status.meShi + 'boss', _status.diShi + 'boss', _status.meShi, _status.diShi + 'mech', _status.meShi + 'mech'];
							break;
						default:
							var list = [_status.meShi + 'mech1', _status.diShi + 'mech', _status.diShi + 'boss', _status.meShi, _status.meShi, _status.diShi + 'boss', _status.diShi + 'mech', _status.meShi + 'boss'];
							break;
					}
			...
			for (var i of game.players) {
				...
				if (i.identity != _status.meShi) {
					switch (get.config('jiange_dengjieND')) {
						case '3':
							i.dengjie = [3, 3, 3, 4, 4, 4, 4, 4, 6, 6].randomGet();
							break;
						case '2':
							i.dengjie = [2, 2, 2, 2, 2, 2, 3, 3, 3, 4].randomGet();
							break;
						default:
							i.dengjie = 1;
							break;
			...
			next.setContent(function () {
				...
				if (jiange_event != 'NONE') {
					...
					ui.jiange_event = ui.create.system(
						'本局事件:' + get.translation(jiange_event),
						function () {
				...
				for (var i of game.players)
					if (i != game.me && (indexM == 2 || (indexM == 1 && (jiange_xjsx[0] == 'F') ^ (i.identity == _status.diShi)))) {
				...
				for (var i of game.players)
					if (!i.name1 || i.name.includes('qyhcCL_hiddenJG_')) {
