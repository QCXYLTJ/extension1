...
const extensionInfo = await lib.init.promises.json(`extension/雷金阴洪石/info.json`);
			...
			game.import('character', function () {
				var ljyhsl = {
					...
					character: {
						...
						ljyhsl_hyrs_quaxiaojiang: ['male', 'wu', "4<img src='theme/style/hp/image/official4.png' width='12' height='12'>4", ['ljyhsl_hyrs_zhanhun'], []],
					...
					characterIntro: {
						...
						ljyhsl_lrfj_shamoke: '沙摩柯(?-222年),东汉末三国时期五溪蛮首领.蜀章武元年(221年)初,为报关羽被东吴杀害之仇,刘备亲自领兵攻孙权,以金锦爵赏诱沙摩柯助战.沙摩柯率一万余人响应蜀军,表现活跃,很是让吴军头痛.章武二年(222年),吴大都督陆逊以火攻破刘备,率诸军齐击,蜀军四十多个营寨被攻破,沙摩柯在大乱中匹马奔逃,被乱军击杀',
						...
						ljyhsl_nzry_fanjiangzhangda: '范疆,本名范强,小说<三国演义>中误写作范疆.初为蜀汉车骑将军张飞帐下将.先主伐吴,飞当率兵万人,自阆中会江州.临发,其帐下将强、张达杀飞,持其首,顺流而奔孙权.三国时蜀张飞部将.蜀章武元年,刘备伐吴,张飞率军从阆中前往江州,出发前,张达与范疆击杀张飞,带着张飞的首级投奔了东吴.演义里刘备率军讨伐东吴,东吴交出了范疆与张达,张达与范疆被张飞之子张苞所杀祭父',
						...
						ljyhsl_lyb_simashi: '司马师(208-255年3月23日),字子元,河内温县(今河南温县西)人.三国时期曹魏权臣,官至大将军,西晋奠基人之一.他是司马懿与张春华的长子,司马昭的兄长,西晋开国皇帝司马炎的伯父.司马师沉着坚强,雄才大略.与夏侯玄、何晏齐名.与其父司马懿谋划诛杀曹爽,以功封长平乡侯食邑千户,旋加卫将军.司马懿死后,以抚军大将军辅政,独揽朝廷大权.掌权后,制定选拔官吏的法规,命百官推荐贤才,整顿纲纪,任命文武大臣使各有职掌,朝野肃然.司马师也有卓越的军事才能,曾用计击灭吴国诸葛恪的大军.公元254年,魏帝曹芳与中书令李丰等密谋除司马师,事情泄露,司马师击杀参与者,迫郭太后废掉魏帝曹芳,从太后命以高贵乡公曹髦为帝.次年,司马师亲率兵平定毋丘俭、文钦之乱,途中病死.晋朝建立后,追尊为景皇帝,庙号世宗',
					...
					skill: {
						...
						ljyhsl_drlz_yanju: {
							...
							forced: true,
							filter(event, player) {
								return event.card && event.card.name == 'sha' && event.player != player && _status.currentPhase != player;
							},
							content() {
								'step 0';
						...
						ljyhsl_drlz_zaizheng: {
							...
							forced: true,
							content() {
								if (player.storage.ljyhsl_drlz_zaizheng >= 10) {
						...
						ljyhsl_drlz_gousheng: {
							...
							content() {
								'step 0';
								if (player.storage.ljyhsl_drlz_gousheng < 2 || player.storage.ljyhsl_drlz_gousheng == undefined) event.finish();
						...
						_ljyhsl_drlz_siji1: {
							...
							forced: true,
							filter(event, player) {
								return event.card && event.card.skill == 'ljyhsl_drlz_siji';
							},
							content() {
						...
						ljyhsl_drlz_shichun: {
							...
							round: 1,
							filter(event, player) {
								if (event.parent.name == 'phaseUse') return lib.filter.filterCard({ name: 'jiu' }, player, event) && game.zhu && game.zhu.hasSkill('ljyhsl_drlz_shichun');
						...
						ljyhsl_drlz_renhou: {
							...
							check(event, player) {
								return (get.attitude(player, event.player) <= 0 && event.player.countCards('e') > 0) || (get.attitude(player, event.player) > 0 && player.countCards('he', { type: 'equip' }) > 0) || player.hp == 1;
							},
							content() {
								'step 0';
								trigger.untrigger();
									...
									player(player) {
										if (
											game.countPlayer(function (current) {
												return current.hp < player.hp && get.attitude(player, current) < 0;
											}) > 0 &&
											game.countPlayer(function (current) {
												return current.hp < player.hp && get.attitude(player, current) > 0;
											}) == 0 &&
						...
						ljyhsl_drlz_xiandeng: {
							...
							content() {
								...
								if (get.color(event.card) == 'black') {
									player.chooseTarget(function (card, player, target) {
										return player != target && player.canUse('sha', target);
						...
						ljyhsl_drlz_yimou: {
							enable: 'phaseUse',
							filterTarget(card, player, target) {
								return target != player && get.distance(player, target) <= 1 && target.countCards('h') > 0 && player.countCards('h') > 0 && (player.storage.ljyhsl_drlz_yimou == undefined || (player.storage.ljyhsl_drlz_yimou && !player.storage.ljyhsl_drlz_yimou.includes(target)));
							},
							selectTarget: 1,
							content() {
								'step 0';
						...
						ljyhsl_lrfj_longji: {
							...
							content() {
								...
								('step 1');
								target
									.chooseControl(['视为对一名角色使用【杀】', '令' + lib.translate[player.name] + '回复一点体力'], true)
						...
						ljyhsl_lrfj_chuoyue: {
							...
							content() {
								...
								if (result.bool) {
									trigger.player
										.chooseControl(['此【杀】造成的伤害-1', '令' + lib.translate[player.name] + '指定使用【杀】'], true)
										.set('ai', function (event) {
						...
						ljyhsl_lrfj_mingyu2: {
							mod: {
								cardEnabled(card, player) {
									if (player.storage.ljyhsl_lrfj_mingyu1 != undefined && get.color(card) == get.color(player.storage.ljyhsl_lrfj_mingyu1)) return false;
								},
						...
						ljyhsl_lrfj_dizong: {
							...
							content() {
								'step 0';
								trigger.player
									.chooseControl(['令' + lib.translate[player.name] + '摸一张牌', '取消'])
									.set('ai', function (event) {
						...
						ljyhsl_lrfj_zhiyong: {
							...
							content() {
								'step 0';
								event.c = get.cards(3);
								player.viewCards('牌堆顶的三张牌', event.c);//QQQ
								player
									.chooseControl(['亮出其中的非基本牌并获得之', '你得到其中的一张牌'])
									.set('ai', function (event) {
										var card = [];
										for (let i = 0; i < event.c.length; i++) {
											if (get.type(event.c[i]) != 'basic') card.push(event.c[i]);
										}
										if (card.length || (trigger.source && get.attitude(player, trigger.source) > 0)) return '亮出其中的非基本牌并获得之';
										return '你得到其中的一张牌';
									})
									.set('prompt', '请选择一项');
								('step 1');
						...
						ljyhsl_lrfj_duanxie: {
							...
							content() {
								...
								if (Array.isArray(trigger.cards)) for (const i of trigger.cards) {
									if (i.original == 'e') event.c.push(i);
								}
								('step 1');
						...
						ljyhsl_lrfj_xiaji: {
							...
							content() {
								...
								var card1 = get.cardPile(function (card) {
									return get.type(card) == 'equip' && get.subtype(card) == 'equip1' && lib.card[card.name].distance != undefined && lib.card[card.name].distance.attackFrom == -1;
								});
								if (card1 != undefined) {
						...
						_ljyhsl_lrfj_weisha: {
							mod: {
								cardSavable(card, player) {
									if (
										game.countPlayer(function (current) {
											return current.storage.ljyhsl_lrfj_weisha != undefined;
										}) > 0 &&
										card.name != 'tao'
									)
										return false;
						...
						_ljyhsl_lrfj_weisha1: {
							...
							content() {
								for (const i of game.players) {
									if (i.storage.ljyhsl_lrfj_weisha != undefined) delete i.storage.ljyhsl_lrfj_weisha;
								}
							},
						},
						ljyhsl_lrfj_anshi: {
						...
						ljyhsl_nzry_xieji: {
							...
							content() {
								...
								if (card != undefined) {
									...
									player.draw(2);
									if (player.storage.ljyhsl_nzry_xieji.length) event.goto(1);
								}
							},
						},
						ljyhsl_nzry_shexin: {
						...
						ljyhsl_nzry_zhajiang: {
							...
							ai: {
								...
								result: {
									target(player, target) {
										if (player.hp > 2 && target.countCards('he') > 2) return -1;
										return;
									},
								},
							},
						...
						ljyhsl_nzry_weishi: {
							...
							subSkill: {
								gainMark: {
									...
									filter(event, player) {
										if (Array.isArray(event.cards)) for (const i of event.cards) {
											if (get.color(i) == 'red' && _status.currentPhase != player && _status.currentPhase.countCards('he') > 0) return true;
										}
										return false;
									},
									check(event, player) {
								...
								clearMark: {
									...
									forced: true,
									filter(event, player) {
										return player.storage.ljyhsl_nzry_weishi.length;
									},
									content() {
										var cards = [];
										for (let i = 0; i < player.storage.ljyhsl_nzry_weishi.length; i++) {
						...
						ljyhsl_nzry_lianji: {
							...
							content() {
								...
								var cards = [];
								if (Array.isArray(event.cards)) for (const i of event.cards) {
									if (get.type(i) == 'trick' || get.type(i) == 'delay') cards.add(i);
								}
						...
						_ljyhsl_nzry_liuxian: {
							...
							filter(event, player) {
								return (
									player.hasSkill('ljyhsl_nzry_liuxian') &&
									game.countPlayer(function (current) {
								...
								result: {
									target(player, target) {
										if (
											player.hp <= 2 &&
											game.countPlayer(function (current) {
												return get.distance(player, current, 'attack') <= 1 && current != player && get.attitude(player, current) < 0;
											}) > 0 &&
											game.countPlayer(function (current) {
						...
						ljyhsl_gmrh_weijiang: {
							...
							selectCard: 2,
							filterTarget(card, player, target) {
								return player != target && ((player.storage.ljyhsl_gmrh_weijiang != undefined && !player.storage.ljyhsl_gmrh_weijiang.includes(target)) || player.storage.ljyhsl_gmrh_weijiang == undefined);
							},
							content() {
						...
						ljyhsl_gmrh_zhaoxin: {
							...
							filter(event, player) {
								return player.maxHp > player.hp && event.source;
							},
							check(event, player) {
							...
							content() {
								...
								if (cards.length) {
									player.viewCards(lib.translate[trigger.source.name] + '的手牌', cards.randomGets(player.maxHp - player.hp));
								}
								('step 1');
								var num = player.maxHp - player.hp;
						...
						ljyhsl_gmrh_shijun: {
							...
							content() {
								...
								event.targets = [];
								for (const i of game.players) {
									if (get.distance(i, target, 'attack') <= 1 && i != target) event.targets.push(i);
								}
							...
							ai: {
								...
								result: {
									target(player, target) {
										...
										if (
											game.countPlayer(function (current) {
												return get.distance(current, target, 'attack') <= 1 && current != target;
											}) > 0
										)
											return -1;
									},
							...
							content() {
								...
								if (result.bool) {
									...
									if (get.color(result.links[0]) == 'red') {
										player.gainPlayerCard(get.prompt('fankui', trigger.source), trigger.source, get.buttonValue, 'he');
										event.finish();
									}
								} else {
									event.finish();
						...
						ljyhsl_gmrh_fenji: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.prompt('ljyhsl_gmrh_fenji'), 1, function (card) {
						...
						ljyhsl_gmrh_xiandao: {
							...
							content() {
								'step 0';
								player
									.chooseCard(get.translation(trigger.player) + '的' + (trigger.judgestr || '') + '判定为' + get.translation(trigger.player.judging[0]) + ',' + get.prompt('ljyhsl_gmrh_xiandao'))
									.set('ai', function (card) {
						...
						ljyhsl_gmrh_chuanshu: {
							...
							content() {
								...
								('step 1');
								if (result.card.suit == 'heart') target.recover();
							},
							ai: {
								order: 9,
								result: {
						...
						ljyhsl_gmrh_qimen: {
							...
							filter(event, player) {
								if (Array.isArray(event.cards)) for (const i of event.cards) {
									if (_status.currentPhase != player && (i.original == 'e' || (get.color(i) == 'black' && i.original == 'h'))) return true;
								}
								return false;
							},
							content() {
						...
						ljyhsl_gmrh_shuwu: {
							...
							content() {
								...
								('step 1');
								if (result.card.suit == 'spade') event.bool = 'loseHp';
								if (result.card.suit == 'club') event.bool = 'draw';
								if (result.card.suit == 'heart') event.bool = 'viewAs';
								if (result.card.suit == 'diamond') event.bool = 'link';
								('step 2');
								var num = 1;
						...
						ljyhsl_gmrh_qunru: {
							...
							filter(event, player) {
								return get.type(event.card) == 'trick' && event.targets && event.targets.length > 1 && event.targets.includes(player);
							},
							forced: true,
							content() {
								'step 0';
						...
						ljyhsl_hyrs_changqu: {
							...
							content() {
								...
								player.chooseToDiscard(get.prompt('ljyhsl_hyrs_changqu'), 1, 'h', { color: get.color(trigger.card) }).ai = function (card) {
									if (get.attitude(player, trigger.targets[0]) < 0 && trigger.targets[0].countCards('h', 'shan') > 0) return 5 - get.value(card);
									return -1;
								};
								('step 1');
								if (result.bool) {
						...
						_ljyhsl_hyrs_linjia: {
							...
							filter(event, player) {
								return (
									game.countPlayer(function (current) {
										return current.storage.ljyhsl_hyrs_linjia != undefined && current.storage.ljyhsl_hyrs_linjia.length >= 2;
									}) > 0 && !player.hasSkill('ljyhsl_hyrs_linjia')
								);
							},
							filterTarget(card, player, target) {
								return target.storage.ljyhsl_hyrs_linjia != undefined && target.storage.ljyhsl_hyrs_linjia.length >= 2;
							},
							content() {
								'step 0';
								player.chooseControl(['摸两张牌', '对一名拥有【鳞甲】的角色造成一点火焰伤害']).set('ai', function (event) {
									var pl;
									for (const i of game.players) {
										if (i.hasSkill('ljyhsl_hyrs_linjia')) pl = i;
									}
							...
							ai: {
								...
								result: {
									target(player, target, card) {
										if (player.maxHp - player.hp < 2 && target.maxHp - target.countCards('h') >= 2 && get.attitude(player, target) > 0) return 1;
										return;
									},
								},
							},
						...
						ljyhsl_hyrs_guoye1: {
							...
							ai: {
								...
								result: {
									target(player, target) {
										if (get.attitude(player, target) > 0 && player.hp > 2) return 4;//QQQ
										return;
									},
								},
							},
						...
						ljyhsl_hyrs_hanwei: {
							...
							content() {
								...
								player
									.chooseCardButton('【捍卫】:选择要移动的牌(还能移动' + event.num + '张)', event.cards)
						...
						ljyhsl_hyrs_fengdao: {
							...
							forced: true,
							filter(event, player) {
								return event.card && (event.card.name == 'sha' || event.card.name == 'juedou') && get.distance(player, event.player, 'attack') <= 1;
							},
							content() {
								'step 0';
								player.chooseCard(get.prompt('ljyhsl_hyrs_fengdao'), 1, 'h').set('ai', function (card) {
									if ((trigger.source && trigger.source.countCards('he') >= 2 && get.attitude(player, trigger.source) < 0 && get.type(card) != 'basic') || (get.attitude(player, trigger.player) > 0 && trigger.player.isTurnedOver() && get.type(card) == 'basic') || (get.attitude(player, trigger.player) < 0 && trigger.player.hp <= 1 && !trigger.player.isTurnedOver() && get.type(card) == 'basic')) return 6 - get.value(card);
									return -1;
								});
								('step 1');
								if (result.bool) {
						...
						ljyhsl_hyrs_mengzhi: {
							...
							content() {
								...
								player.chooseCard(get.prompt('ljyhsl_hyrs_mengzhi'), 1, 'h').set('ai', function (card) {
									if (player.countCards('h') > 1 && get.attitude(player, trigger.player) < 0 && get.distance(trigger.player, player, 'attack') <= 1) return 5 - get.value(card);
									return -1;
								});
								('step 1');
								if (result.bool) {
						...
						ljyhsl_lyb_juediao: {
							...
							content() {
								...
								trigger.source
									.chooseCard('请弃置一张点数为' + N * (player.maxHp - player.hp + 1) + '的牌', 'he', 1, function (card, player, target) {
										return card.number == N * (player.maxHp - player.hp + 1);
									})
									.set('ai', function (card) {
										return 1;
									});
						...
						ljyhsl_lyb_tuodao: {
							...
							forced: true,
							filter(event, player) {
								return event.result != undefined && event.result.bool == true && event.filterCard({ name: 'shan' }, player) && _status.currentPhase != player && event.source && get.distance(player, event.source, 'attack') <= 1;
							},
							content() {
								'step 0';
						...
						ljyhsl_lyb_dushi: {
							...
							filter(event, player) {
								return event.player.countCards('h') == 0 && (event.player.countCards('he') > 0 || (event.player.maxHp > event.player.hp && player.countCards('he') > 0));
							},
							content() {
								'step 0';
								var list = [];
								...
								player
									...
									.set('ai', function (event) {
										if (get.attitude(player, trigger.player) > 0) {
											...
											if (player.countCards('he') > 0 && trigger.player.maxHp - trigger.player.hp >= 1) return '弃一张牌,令其回复一点体力';
											return 'cancel2';
										}
										if (get.attitude(player, trigger.player) < 0) {
											if (trigger.player.countCards('e') > 0) return '弃置其一张牌';
										}
										return '弃置其一张牌';
									})
									.set('prompt', '【度势】:请选择一项');
					...
					translate: {
						...
						ljyhsl_drlz_kuizhan_info: '你每受到一次伤害后,你可以进行一次判定,若为♥️️,则你再受到伤害来源造成的一点伤害,否则你可以指定任意一名角色立即执行一个你指定的额外的阶段(弃牌阶段除外)',
						...
						ljyhsl_drlz_yimou_info: '出牌阶段对每名角色限一次,你可令与你距离为1的一名角色亮出一张手牌然后其展示你的一张手牌,若花色相同则目标角色获得该牌,否则该回合内你可以将♠️️手牌当【过河拆桥】、♣️️手牌当【火攻】、♥️️手牌当【顺手牵羊】,♦️️手牌当【决斗】使用',
						...
						ljyhsl_nzry_lianji_info: '每当你使用了一张锦囊牌(结算完毕后),你可以亮出牌堆顶两张牌并获得其中的锦囊牌,然后将余下的牌置于牌堆底',
						...
						ljyhsl_gmrh_hemeng_info: '出牌阶段限X次,你可另一名其他角色观看你的手牌并获得其中一张牌,然后你观看该角色的手牌并获得其中一张牌(X为你已损失的体力值且至少为1)',
						...
						ljyhsl_gmrh_chuanshu_info: '出牌阶段限一次,你可以弃置装备区里的一张牌并选择任意一名角色,然后进行一次判定,若为♥️️,目标回复一点体力',
						ljyhsl_gmrh_qimen: '奇门',
						ljyhsl_gmrh_qimen_info: '你的回合外,当你的黑色手牌进入弃牌堆或你的装备区失去牌时,你可以进行一次判定,若为♠️️,你可以对一名角色造成一点雷电属性伤害',
						...
						ljyhsl_gmrh_shuwu_info: '当你于回合外失去牌后,你可以进行一次判定:♠️️:你和一名角色失去一点体力;♣️️:你令一名角色摸两张牌;♥️️:你视为对一名角色使用一张【乐不思蜀】;♦️️:你可以横置最多两名角色的武将牌',
						...
						ljyhsl_hyrs_fugu: '符谷',
						ljyhsl_hyrs_fugu_info: '出牌阶段限一次,你可以弃置一张♥️️手牌并进行一次判定.若非♠️️则视为你使用了一张【五谷丰登】,若是♠️️则视为你使用了一张【无中生有】',
						...
						ljyhsl_lyb_qingjun_info: '锁定技,你死亡时,当前回合角色须进行一次判定,若为♥️️,其回复一点体力,否则其减少一点体力上限并失去一点体力',
				...
				for (var i in ljyhsl.character) {
					ljyhsl.character[i][4].push('ext:雷金阴洪石/image/' + i + '.jpg');
				}
				for (var i in ljyhsl.character) {
					if (i.includes('drlz')) ljyhsl.characterSort.ljyhsl['ljyhsl1'].push(i);
					if (i.includes('lrfj')) ljyhsl.characterSort.ljyhsl['ljyhsl2'].push(i);
					if (i.includes('nzry')) ljyhsl.characterSort.ljyhsl['ljyhsl3'].push(i);
					if (i.includes('gmrh')) ljyhsl.characterSort.ljyhsl['ljyhsl4'].push(i);
					if (i.includes('hyrs')) ljyhsl.characterSort.ljyhsl['ljyhsl5'].push(i);
					if (i.includes('lyb')) ljyhsl.characterSort.ljyhsl['ljyhsl6'].push(i);
				}
				lib.config.all.characters.add('ljyhsl');
				lib.config.characters.add('ljyhsl');
