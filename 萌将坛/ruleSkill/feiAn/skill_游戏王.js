...
window.scqh = function (lib, game, ui, get, ai, _status) {
	var list = {
		skill: {
			_千鹤ygo_怪兽信息: {
				...
				mod: {
					targetEnabled(card, player, target) {
						var get1 = player.getCards('e', { type: 'ygoSpell' });
						var get2 = player.getCards('e', { type: 'ygoTrap' });
						var length = get1.length + get2.length;
						if (length >= 5) {
							if (get.subtype(card) == 'ygoSpell_equip') return false;
							if (get.subtype(card) == 'ygoTrap_equip') return false;
						}
					},
				},
				trigger: {
				...
				filter(event, player, name) {
					return player.name.includes('千鹤ygo_') || (player.name2 && player.name2.includes('千鹤ygo_'));
				},
				content() {
					'step 0';
					...
					for (var mm of list) {
						...
						if (info && info[4]) {
							for (var kk of info[4]) {
								if (kk.indexOf('ygoNature:') == 0) {
									if (!player.storage.ygoNature) {
										player.storage.ygoNature = kk.slice(11);
									}
								}
								if (kk.indexOf('ygoRace:') == 0) {
									if (!player.storage.ygoRace) {
										player.storage.ygoRace = kk.slice(9);
									}
								}
								if (kk.indexOf('ygoType:') == 0) {
									if (!player.storage.ygoType) {
										player.storage.ygoType = kk.slice(9);
									}
								}
								if (kk.indexOf('ygoAtk:') == 0) {
									if (!player.storage.ygoAtk) {
										player.storage.ygoAtk = 0;
										player.storage.ygoAtk += kk.slice(8) - 0;
									}
								}
								if (kk.indexOf('ygoDef:') == 0) {
									if (!player.storage.ygoDef) {
										player.storage.ygoDef = 0;
										player.storage.ygoDef += kk.slice(8) - 0;
									}
								}
							}
						}
					...
					('step 1');
					if (game.ygo_News == true) player.markSkill(event.name);
				},
				markimage: 'extension/萌将坛/ui/mark_ygo.jpg',
				intro: {
					name: '怪兽信息',
					mark(dialog, content, player) {
						var state = '';
						state += player.storage.ygoNature + '<br/>';
						state += player.storage.ygoRace + '<br/>';
						state += player.storage.ygoType + '<br/>';
						state += '攻击力' + player.storage.ygoAtk + '<br/>';
						state += '守备力' + player.storage.ygoDef + '<br/>';
						dialog.addText(state);
						var carded = player.getCards('x', function (card) {
							return card.hasGaintag('_千鹤ygo_卡组');
						});
						if (carded.length && player.isUnderControl(true)) {
							dialog.addSmall(carded);
						} else return '卡组共有' + get.cnNumber(carded.length) + '张牌';
					},
				},
			},
			千鹤ygo_忤逆2: {
				init(player, skill) {
					player.storage.ygoType = '通常';
				},
				silent: true,
				forced: true,
				mod: {
					maxHandcard(player, num) {
						if (player.countCards('e', (card) => card.name.includes('圣剑'))) {
							player.storage.ygoNature = '暗';
							player.storage.ygoType = '效果';
							return num + 1;
						} else {
							player.storage.ygoType = '通常';
							var list = [player.name1, player.name2];
			...
			千鹤ygo_裁决: {
				...
				filterTarget(card, player, target) {
					if (target.name.includes('圣骑士') || (target.name2 && target.name2.includes('圣骑士'))) {
			...
			千鹤ygo_忤逆: {
				...
				subSkill: {
					...
					die: {
						...
						filter(event, player, name) {
							return player.storage.千鹤ygo_忤逆 != undefined;
						},
						content() {
							'step 0';
							var list = [];
			...
			千鹤ygo_圣刻: {
				...
				content() {
					...
					if (skip == false) {
						...
						if (storage.length) {
							...
							if (list.length) {
								for (var i = 0; i < list.length; i++) {
									if (get.subtype(list[i]) == 'ygoSpell_equip') {
				...
				subSkill: {
					round: {
						...
						filter(event, player, name) {
							return player.storage.千鹤ygo_圣刻;
						},
						content() {
							'step 0';
							player.storage.千鹤ygo_圣刻 = [];
						},
					},
					3: {
						name: '3',
			...
			千鹤ygo_不夜: {
				...
				subSkill: {
					use: {
						...
						filter(event, player, name) {
							var card1 = event.card.number;
							var storage1 = player.storage.千鹤ygo_不夜;
							...
							var storage2 = player.storage.千鹤ygo_圣刻;
							if (storage2.length == 0 && (numjia == 9 || numjian == 9)) {
								return true;
							}
							if (storage2.length == 1 && (numjia == 12 || numjian == 12)) {
			...
			千鹤ygo_同胞: {
				...
				subSkill: {
					1: {
						...
						mod: {
							cardEnabled(card, player) {
								var storage = player.storage['千鹤ygo_同胞_1'];
			...
			千鹤ygo_崩毁: {
				...
				subSkill: {
					...
					damage: {
						...
						filter(event, player, name) {
							if (event.parent.name != '千鹤ygo_崩毁') return false;
			...
			千鹤ygo_默识: {
				...
				global: '千鹤ygo_默识_global',
				group: '千鹤ygo_默识_cancel',
				subSkill: {
					cancel: {
						forced: true,
						trigger: {
			...
			千鹤ygo_圣骑士阿托利斯00: {
				derivation: ['千鹤ygo_圣剑_加拉廷', '千鹤ygo_圣剑_石中剑', '千鹤ygo_圣剑_克拉伦特', '千鹤ygo_圣剑_阿隆戴特', '千鹤ygo_圣剑_天命之圣剑', '千鹤ygo_圣剑_桂妮薇儿', '千鹤ygo_圣剑_丹内尔'],
				nobracket: true,
				trigger: {
					player: ['equipBegin'],
				},
				...
				filter(event, player, name) {
					return !event.card.name.includes('千鹤ygo_圣剑');
				},
				content() {
					'step 0';
					trigger.cancel();
				...
				global: ['千鹤ygo_圣骑士阿托利斯00_银', '千鹤ygo_圣骑士阿托利斯00_金'],
				group: ['千鹤ygo_圣骑士阿托利斯00_hujia'],
				subSkill: {
					hujia: {
						forced: true,
						trigger: {
						...
						filter(event, player, name) {
							return event.card && event.card.name.includes('千鹤ygo_圣剑');
						},
						content() {
							'step 0';
							player.changeHujia();
							if (trigger.card.name.includes('石中剑')) player.addMark('千鹤ygo_圣骑士阿托利斯00_银', 1, false);
							if (trigger.card.name.includes('断钢剑')) player.addMark('千鹤ygo_圣骑士阿托利斯00_金', 1, false);
						},
					},
					银: {
						name: '圣剑的指引·银',
						...
						filter(event, player) {
							return player.countMark('千鹤ygo_圣骑士阿托利斯00_银') && player.countCards('e', (card) => card.name.includes('千鹤ygo_圣剑'));
						},
						content() {
							'step 0';
							player.removeMark('千鹤ygo_圣骑士阿托利斯00_银', 1, false);
							var num = player.countCards('e', (card) => card.name.includes('千鹤ygo_圣剑'));
							if (num > 0) {
								event.count = num;
							} else event.finish();
							('step 1');
					...
					金: {
						...
						filter(event, player) {
							return player.countMark('千鹤ygo_圣骑士阿托利斯00_金');
						},
						filterTarget(card, player, target) {
							return target != player;
						},
						content() {
							'step 0';
							player.removeMark('千鹤ygo_圣骑士阿托利斯00_金', 1, false);
							target.losrHp();
			...
			千鹤ygo_圣骑士贝德维尔00: {
				...
				intro: {
					content: 'expansion',
					markcount(storage, player) {
						var storage = player.storage.千鹤ygo_贝德维尔使命;
						if (!storage) storage = 0;
						return storage;
					},
				},
				init(player, skill) {
					player.storage.千鹤ygo_贝德维尔使命 = 0;
					player.storage.千鹤ygo_种族 = '战士族';
					player.storage.千鹤ygo_属性 = '光';
					var atk = 1600;
					var def = 1500;
					if (!player.storage.千鹤ygo_atk || player.storage.千鹤ygo_atk < atk) {
						player.storage.千鹤ygo_atk = atk;
					}
					if (!player.storage.千鹤ygo_def || player.storage.千鹤ygo_def < def) {
						player.storage.千鹤ygo_def = def;
					}
				},
				trigger: {
				...
				group: ['千鹤ygo_圣骑士贝德维尔00_use', '千鹤ygo_圣骑士贝德维尔00_shiming', '千鹤ygo_圣骑士贝德维尔00_shibai'],
				subSkill: {
					use: {
						enable: 'phaseUse',
						trigger: {
						...
						filter(event, player, name) {
							var mark = player.getExpansions('千鹤ygo_圣骑士贝德维尔00');
							var move = game.countPlayer(function (current) {
								return current.countCards('e', (card) => card.name.includes('千鹤ygo_圣剑'));
							});
							if (player.hasSkill('千鹤ygo_圣骑士贝德维尔00_used')) return false;
					...
					shiming: {
						...
						filter(event, player, name) {
							if (event.getParent(2).skill != '千鹤ygo_圣骑士贝德维尔00') return false;
							if (event.parent.card.name != '千鹤ygo_圣剑_断钢剑') return false;
							player.storage.千鹤ygo_贝德维尔使命++;
							return player.storage.千鹤ygo_贝德维尔使命 >= 3;
						},
						content() {
							'step 0';
							player.awakenSkill('千鹤ygo_圣骑士贝德维尔00');
			...
			千鹤ygo_圣骑士珀西瓦尔00: {
				...
				filter(event, player, name) {
					var ccc = player.getCards('e', function (card) {
						return card.name.includes('千鹤ygo_') && card.name.includes('圣剑') && get.subtype(card) == '千鹤ygo_装备魔法';
					});
					return ccc.length;
				},
				content() {
					...
					event.ccc = player.getCards('x', function (card) {
						return card.hasGaintag('_千鹤ygo_墓地') && card.name.includes('千鹤ygo_') && card.name.includes('圣剑') && get.subtype(card) == '千鹤ygo_装备魔法';
					});
					var list = [];
			...
			千鹤ygo_禁手: {
				...
				superCharlotte: true,
				init(player) {
					player.恋姬无双ShunfajiInit('千鹤ygo_禁手');
				},
				clickable(player) {
					player.useSkill('千鹤ygo_禁手_禁手');
				},
			...
			千鹤ygo_玩家武藤游戏00: {
				...
				group: ['千鹤ygo_禁手', '千鹤ygo_玩家武藤游戏00_先攻', '千鹤ygo_玩家武藤游戏00_注定一抽', '千鹤ygo_玩家武藤游戏00_闪现'],
				subSkill: {
					先攻: {
						name: '先攻',
						trigger: {
					...
					注定一抽: {
						...
						content() {
							...
							var list = get.libCard(function (info) {
								return info.type == '千鹤ygo_魔法' || info.type == '千鹤ygo_陷阱' || info.subtype == '千鹤ygo_装备魔法' || info.type == 'delay';
							});
							if (list.length) {
		...
		translate: {
			_千鹤ygo_怪兽信息: '怪兽信息',
			千鹤ygo_忤逆: '忤逆',
			千鹤ygo_忤逆_info: '锁定技,若你的装备区里有「<b><u>圣剑</u></b>」,你视作效果怪兽、暗属性,且手牌上限加１.',
			千鹤ygo_裁决: '裁决',
			千鹤ygo_裁决_info: '出牌阶段限一次,你可以对一名「<b><u>圣骑士</u></b>」造成一点伤害,若你不是目标,则需要弃置一张牌,你从游戏之外获得一张名字含有「<b><u>圣剑</u></b>」的卡牌.',
			千鹤ygo_高昂: '高昂',
			千鹤ygo_高昂_info: '当你造成过伤害的出牌阶段结束时,你可以从游戏之外获得一张名字含有「<b><u>圣骑士</u></b>」或者「<b><u>圣剑</u></b>」的卡牌.',
			千鹤ygo_忤逆: '忤逆',
			千鹤ygo_忤逆_info: '<b><u>军团技</u></b>,出牌阶段各限一次:⒈你可以在未登场的「<b><u>圣骑士</u></b>」中选择一张武将牌加入「<b><u>军团</u></b>」(初始手牌４),你弃置自己装备区里的一张牌;⒉你可以将「<b><u>军团元首</u></b>」替换成「<b><u>军团</u></b>」中的任意一张未阵亡过的武将牌.',
			千鹤ygo_叛袭: '叛袭',
			千鹤ygo_叛袭_info: '隐匿技,当你于其他角色的回合内登场后,你可以让该角色选择手牌区和装备区内的各一张牌,将该角色未选择的牌当做一张任意花色的【出其不意】对其使用.每回合结束时,若你此回合内未造成过伤害,则你进入隐匿状态.',
			千鹤ygo_圣刻: '圣刻',
			千鹤ygo_圣刻_info: '锁定技,当你使用或打出点数是３的倍数的一张牌时,你摸一张牌.若你是按照９、１２、３、６的顺序触发〖圣刻〗,则你从游戏之外将一张「<b><u>圣剑</u></b>」置入你的装备区,否则重新统计(每轮开始时重新统计,每个数字每回合各限一次).',
			千鹤ygo_不夜: '不夜',
			千鹤ygo_不夜_info: '出牌阶段,你可以弃置任意张牌.当你使用或打出一张牌前,若此牌的点数在加Ｘ或者减Ｘ之后可以增加〖圣刻〗的进度,则你将此牌的点数更改为〖圣刻〗当前进度所需的数字,重新储存Ｘ的信息(Ｘ为你以此法弃置过的牌数).',
			千鹤ygo_同胞: '同胞',
			千鹤ygo_同胞_info: '隐匿技,当你登场时,你可以使用至多两张手牌,若如此做,直到此回合结束之前,你不能使用其他类型的牌.只要你的装备区里名字含有「<b><u>圣剑</u></b>」的卡牌少于三张,你不能使用【杀】.出牌阶段限一次,你可以将弃牌堆的至多三张牌放回牌堆顶,你摸一张牌.',
			千鹤ygo_崩毁: '崩毁',
			千鹤ygo_崩毁_info: '出牌阶段限一次,你可以弃置装备区里的一张牌并对一名未翻面的其他角色造成一点伤害,你摸一张牌,且不能使用【杀】,直到回合结束.当你以此法造成伤害后,受伤角色将其装备区里的所有牌置入弃牌堆中.',
			千鹤ygo_悲歌: '悲歌',
			千鹤ygo_悲歌_info: '当有角色受到伤害后,你可以令其进行判定,你可以弃置你区域里的一张牌,若此牌与判定结果:花色相同,则将此牌置于你的判定区;牌名相同,则将判定牌置于你的判定区.最后根据判定结果执行以下的一个选项:<br/>♥️️其回复１点体力<br/>♦️️其摸两张牌<br/>♣️️伤害来源弃置两张牌<br/>♠️️伤害来源翻面<br/>',
			千鹤ygo_陈情: '陈情',
			千鹤ygo_陈情_info: '限定技,当你处于濒死状态时,你可以让另一名其他角色摸四张牌,其弃置四张牌.若其以此法弃置的四张牌均为红色,则视为该角色对你使用一张【桃】.',
			千鹤ygo_默识: '默识',
			千鹤ygo_默识_info: '锁定技,你没有判定阶段;所有角色不能成为其他角色使用点数为Ｘ的牌的目标(Ｘ为你的判定区内任何一张牌的点数).每回合限一次,当有牌进入你的判定区后,你需要选择一名正面朝上的角色,令其弃置其区域内的两张牌或失去一点体力.',
			千鹤ygo_黑帆: '黑帆',
			千鹤ygo_黑帆_info: '限定技,出牌阶段,你可以创造一张【唯一一次谎言】,让一名其他女性角色使用之.',
			千鹤ygo_圣骑士阿托利斯00: '骑士王',
			千鹤ygo_圣骑士阿托利斯00_info: '使命技,你的手牌数不小于你的体力时,你可以将一张牌当做【杀】、【闪】、【决斗】、【无懈可击】使用或打出,你随机获得一张【圣剑】并使用.每个牌名每回合限一次.<br/>❶ 第一条使命:当你以此法使用【圣剑 石中剑】时,你将体力回复至体力上限,你获得技能<集结>.<br/>❷ 第二条使命:当你以此法使用【决斗】造成伤害时,若你的手牌数和体力均大于对方,你需要将你的装备区里的【圣剑 石中剑】置入弃牌堆,且你本局内不能再使用【圣剑 石中剑】.<br/>❸ 第三条使命:',
			千鹤ygo_圣骑士贝德维尔00: '终幕骑士',
			千鹤ygo_圣骑士贝德维尔00_info: '使命技,游戏开始时或准备阶段,你可以将卡组中的一张【圣剑】送去墓地.<br/>①:每回合限一次,出牌阶段或当一名角色成为【杀】的目标后,你可以移动场上的一张【圣剑】,或将墓地里的一张【圣剑】置于一名角色的装备区.<br/>②:使命:当你的墓地中有三张【圣剑 断钢湖中剑】后,本局游戏平局.<br/>③:失败:当<圣骑士阿托利斯>死亡时,使命失败.',
			千鹤ygo_圣骑士珀西瓦尔00: '神圣骑士',
			千鹤ygo_圣骑士珀西瓦尔00_info: '①:锁定技,若【圣剑】在你的装备区里,你的体力上限增加一点并变成暗属性.<br/>②:每当你受到一点伤害后,若【圣剑】在你的装备区里,你可以获得墓地里的一张【圣剑】或从弃牌堆中获得每种类型的牌各一张.',
			千鹤ygo_禁手: '禁手',
			千鹤ygo_玩家武藤游戏00: '魔术专家',
			千鹤ygo_玩家武藤游戏00_info: '<br>「<font color=red>魔术专家</font>」<br><li>出牌阶段,选择任意名角色(若无目标,则默认所有其他角色为目标),对他们执行【挪牌】【翻面】【治疗】【伤害】【流失】【摸牌】【弃牌】【调整体力上限】中的任意一条效果,数值为１～５和无限大.<br><br>「<font color=red>先攻</font>」<br><li>其他角色的回合开始时,你可以立即执行一个额外的回合.<br><br>「<font color=red>闪现</font>」<br><li>出牌阶段,你可以与目标交换座位,或成为目标的下家,或成为目标上家.<br><br>「<font color=red>注定一抽</font>」<br><li>出牌阶段,你可以将一张魔法牌或陷阱牌加入手牌.<br><br>「<font color=red>禁手</font>」<br><li>瞬发技,废除一名角色的手牌区.',
			千鹤ygo_抽卡: '抽卡',
			千鹤ygo_抽卡_info: '开启「自动发动」后,摸牌阶段,你可以放弃摸牌,改为从卡组中随机获得一张牌',
		},
	};
