            ...
            font-weight: 4px;">回合开始时,你可选择一名角色,你摸两张牌其与你装备【标枪*♠️️️7】.</span>`,
					...
					skill: {
						...
						jishijimin: {
							...
							content() {
								player.gain(trigger.player.get('h'));
								trigger.player.$give(trigger.player.num('h'), player);
							},
							ai: {
								threaten: 4.8,
								order: 1,
						...
						魏使节: {
							...
							content() {
								...
								('step 3');
								if (player.num('he')) player.chooseCard(true, 'he', '展示一张牌');
								('step 4');
								if (result.bool) {
									player.showCards(result.cards[0]);
									var list = game.filterPlayer(function (current) {
						...
						灵迅: {
							...
							trigger: { global: 'useCardToBegin' },
							filter(event, player) {
								return event.card.name == 'sha' && event.target.isFriendsOf(player) && event.player.num('he');
							},
							content() {
								'step 0';
								player.gainPlayerCard(trigger.player, 'he', true);
						...
						屯令: {
							...
							content() {
								...
								if (result.bool) {
									...
									if (player.num('h')) player.chooseCard(true);
								}
								('step 2');
								if (result.bool) {
									player.showCards(result.cards[0]);
							...
							content() {
								...
								if (get.color(event.cards[0]) == get.color(event.cards[1])) {
									...
									var suijidirens = game.filterPlayer(function (current) {
										return current.isEnemiesOf(player) && current.num('h');
									});
						...
						荣帻: {
							...
							content() {
								...
								if (result.bool) {
									result.targets[0].draw(result.targets[0].maxHp);
									result.targets[0].recover();
									var hs = player.get('he', (card) => get.type(card) != 'basic');
						...
						荣帻2: {
							...
							trigger: { player: 'damageEnd' },
							content() {
								player.discard(player.get('he', (card) => get.type(card) == 'basic').randomGet());
								if (trigger.source) trigger.source.discard(trigger.source.get('he').randomGets(2));
							},
						},
						桂枪: {
							group: '桂枪2',
						...
						长镖: {
							...
							content() {
								'step 0';
								var hs = player.get('he', (card) => get.tag(card, 'damage'));
								event.num = hs.length;
								var hs2 = hs.map((card) => game.createCard(card));
						...
						劲旅: {
							...
							content() {
								...
								list.sort(lib.sort.seat);
								event.list = list;
								var hs = player.get('he', (card) => get.type(card) == 'basic' || get.tag(card, 'damage'));
								var hs2 = hs.map((card) => game.createCard(card));
								event.hs2 = hs2;
								('step 2');
								if (event.hs2.length) {
						...
						新野壮士2: {
							...
							filter(event, player) {
								return event.card.name == 'huogong' && event.target && event.target.num('h'); //QQQ
							},
							check(event, player) {
								return get.attitude(player, event.target) <= 0;
							},
						...
						太行山匪: {
							...
							content() {
								var n = [1, 2].randomGet();
								var num = 0;
								if (trigger.target.num('h')) num += n;
								if (trigger.target.num('e')) num += n;
								if (num > 0) {
						...
						庐步2: {
							...
							content() {
								...
								player.draw();
								('step 1');
								var cards = get.cards(player.num('h'));
								event.cards = cards;
								player.showCards(event.cards);
						...
						毒枪: {
							...
							group: '毒枪2',
							content() {
								var hs = trigger.target.get('he').randomGets(2);
								var hs2 = hs.map((card) => game.createCard(card));
								if (hs2.length) player.gain(hs2, 'draw');
						...
						冶首: {
							...
							content() {
								...
								('step 3');
								if (result.cards[0].number > player.num('he')) {
						...
						铁骨战车: {
							...
							nobracket: true,
							filter(event, player) {
								return event.player.maxHp > event.player.num('h') || player.maxHp > player.num('h');
							},
							check(event, player) {
								return get.attitude(player, event.player) >= 0;
							},
						...
						龙川: {
							...
							content() {
								...
								var eo = game
									.filterPlayer(function (current) {
										return current.isEnemiesOf(player) && current.num('he');
									})
									.randomGet();
						...
						佞宦: {
							...
							content() {
								...
								if (trigger.player.num('he')) trigger.player.discard(trigger.player.getCards('he').randomGet());
						...
						古铜镇尺: {
							...
							content() {
								...
								if (result.bool) {
									...
									player.gain(game.createCard(result.targets[0].get('he').randomGet()), 'draw');
						...
						年迈武卒: {
							...
							content() {
								'step 0';
								player.chooseToDiscard(true, 'he', [1, player.num('he')]);
								('step 1');
						...
						府兵: {
							...
							content() {
								trigger.target.changeHujia();
								trigger.target.draw();
								var hs = trigger.player.get('he', (card) => get.color(card) == 'red').randomGets(2);
								var hs2 = hs.map((card) => game.createCard(card));
						...
						御诏: {
							...
							content() {
								...
								('step 1');
								if (result.bool) {
									var hs = result.targets[0].get('he').randomGets(2);
									var hs2 = hs.map((card) => game.createCard(card));
									if (hs2.length) player.gain(hs2, 'draw');
									result.targets[0].draw();
									event.t = result.targets[0];
							...
							content() {
								...
								if (result.bool) {
									...
									for (var i = 0; i < player.get('he').length; i++) {
										if (get.color(player.get('he')[i]) == 'black') {
											num++;
										}
									}
