...
game.import('extension', function (lib, game, ui, get, ai, _status) {
	return {
		...
		content(config, pack) {
			...
			if (lib.config.cards.includes('ybslc') && lib.config.cards.includes('ybgod') && lib.config.ybsl_cardPileReplace) {
				if (lib.config.ybsl_cardPileReplace == 'ybslCardPile') {
					lib.arenaReady.push(function () {
						lib.init.js('extension/夜白神略/ext', 'ybslCardPile', function () {
					...
					lib.arenaReady.push(function () {
						lib.init.js('extension/夜白神略/ext', lib.config.ybsl_cardPileReplace, function () {
			...
			lib.config.cards.addArray(['ybslc', 'ybgod']);
			//-----------------------------------//
			if (config.ybsl_skillstrengthen == true) {
				...
				*/
				...
				lib.translate.xinfu_jingxie1_info = '出牌阶段,你可以展示一张未强化过的【诸葛连弩】或标准包/军争包/SP包中的防具牌,<span class=yellowtext>赤兔,王追,闪电及其变种牌,洪水,火山,朱雀扇及其变种牌,倚天剑,毒,青龙刀,铜雀,护心镜</span>,对其进行强化.当你处于濒死状态时,你可以重铸一张防具牌,将体力回复至1点.';
				//----------------------曹金玉
				{
					lib.skill.yuqi = {
						audio: 2,
					...
					lib.translate.yuqi_info = '每回合限<span class=yellowtext>三</span>次.当有角色受到伤害后,若你至其的距离不大于<span class=thundertext>0</span>,则你可以观看牌堆顶的<span class=firetext>3</span>张牌.你将其中至多<span class=greentext>1</span>张牌交给受伤角色,可以获得剩余牌中的至多<span class=yellowtext>1</span>张牌,并将其余牌以原顺序放回牌堆顶.<span class=yellowtext>(红色的数字至多为10,其余的数字至多为5)</span>';
					lib.dynamicTranslate.yuqi = function (player) {
						var info = lib.skill.yuqi.getInfo(player);
						return '每回合限<span class=yellowtext>三</span>次.当有角色受到伤害后,若你至其的距离不大于<span class=thundertext>' + info[0] + '</span>,则你可以观看牌堆顶的<span class=firetext>' + info[1] + '</span>张牌.你将其中至多<span class=greentext>' + info[2] + '</span>张牌交给受伤角色,可以获得剩余牌中的至多<span class=yellowtext>' + info[3] + '</span>张牌,并将其余牌以原顺序放回牌堆顶.<span class=yellowtext>(红色的数字至多为10,其余的数字至多为5)</span>';
					};
				}
				//----------------------神诸葛
				...
				// 	},
				// }
				// lib.translate.qixing_info='游戏开始时,你将牌堆顶的七张牌置于你的武将牌上,称之为<星>.你可用任意数量的手牌等量交换这些<星>;<span class=yellowtext>摸牌阶段结束后,你可以获得武将牌上所有星,选择至多七张手牌置于武将牌上称为星.</span>'
				// lib.skill.kuangfeng={
				// 	unique:true,
				// 	audio:2,
				// 	enable:'phaseUse',
									...
									*/
										...
										if (_status.yebailvcheng.completeNumber % 5 != 0 || _status.yebailvcheng.completeNumber > 30) {
											...
											game.zhu
												.chooseControl()
												.set('choiceList', list)
												.set('prompt', '下一关BOSS战,请选择一项(当前已通过' + _status.yebailvcheng.completeNumber + '关)');
										} else {
											game.zhu.addMark('_YBLC_level');
											game.zhu
												.chooseControl()
										...
										if (_status.yebailvcheng.completeNumber == 5) {
											event._result = { links: ['ybsl_011gaoyuhang'] };
										} else if (_status.yebailvcheng.completeNumber == 10) {
											event._result = { links: ['ybsl_009liyushan'] };
										} else if (_status.yebailvcheng.completeNumber == 15) {
											event._result = { links: ['ybsl_008wuyuxin'] };
										} else if (_status.yebailvcheng.completeNumber == 20) {
											event._result = { links: ['ybsl_010zhouyue'] };
										} else if (_status.yebailvcheng.completeNumber == 25) {
											event._result = { links: ['ybsl_001sunlisong'] };
										} else if (_status.yebailvcheng.completeNumber == 30) {
											event._result = { links: ['ybsl_002chenailin'] };
										} else if (_status.yebailvcheng.completeNumber == 35) {
											event._result = { links: ['ybsl_022salt'] };
		...
		precontent(ybslb) {
			...
			{
				// 读取文件夹
				lib.init.js(`extension/夜白神略/ext`, 'ybslj');
				lib.init.js(`extension/夜白神略/ext`, 'ybslc');
				lib.init.js(`extension/夜白神略/ext`, 'ybgod');
				lib.init.js(`extension/夜白神略/ext`, 'list24');
			...
			//----------出牌阶段计数
			lib.translate.yb_phaseNumber = '出牌阶段计数'; //QQQ
			//话说,我要是整个
			...
			lib.skill._ybsl_huiyi = {
				...
				characterlist() {
					//-------梦势力将池
					return ['ybsl_026can', 'ybsl_027rain', 'ybsl_028crystal', 'ybsl_029dawn', 'ybsl_030book', 'ybsl_031huanqing', 'ybsl_034zhoulianyuan', 'ybnb_034zhoulianyuan', 'ybsl_035stamp', 'ybsl_036bright', 'ybsl_037diamondqueen', 'db_ybsl_038tengwu', 'ybsl_039zhafu', 'ybsl_040ether', 'db_ybsl_067snake', 'ybsl_069xiangzi', 'ybsl_076zhujun', 'ybsl_077yangqixu', 'ybsl_078zhuyahai', 'ybsl_081chenli', 'ybsl_081chensi', 'ybsl_083xiaozhu', 'ybsb_084zhangmi', 'ybsl_084zhangmi', 'ybnb_084zhangmi', 'db_ybsp_038tengwu', 'ybsp_027rain', 'ybsb_077yangqixu'];
				},
				content() {
					'step 0';
					game.log('梦势力初始化为忆!');
				...
				filter(event, player) {
					if (event.cards)
						for (var i = 0; i < event.cards.length; i++) {
							if (event.cards[i].storage && event.cards[i].storage._yb054_caijin && get.position(event.cards[i], true) == 'd') return true;
						} //QQQ
					return false;
				},
				forceDie: true,
			...
			lib.skill._kagari_ybzongsi_card = {
				...
				firstDo: true,
				filter(event, player) {
					return _status.kagari_ybzongsi;
				},
				content() {
			...
			*/
			...
			lib.translate.hairi_zhongxia_info = '使命技,当你需要使用一张基本牌时,你可以弃置所有手牌(至少一张),若你以此法弃置的牌花色各不相同,你视为使用之,若你以此法弃置的牌数等于你已损体力值或体力上限,你加一点体力上限(若未成功使用目标牌,此技能本回合不能再用)<br>成功:准备阶段,若你的体力上限不小于5,你选择一项:①获得技能<炒饭>,并回复所有已废除装备栏.②将体力上限调整为1,并重置[终夏].<br>失败:当你死亡时,你令一名其他角色获得[终夏].';
			...
			lib.translate.YB_snowsha_info = '当你造成雪属性伤害时,你可以令目标摸X张牌,其武将牌翻面(X为目标当前体力值且至多为5).';
			...
			//------------------------------------------------改函数实现属性杀
			{
				...
				lib.element.content.YB_shelie = function () {
					...
					('step 1');
					var list = [];
					for (var i of cards) list.add(i.suit);
					if (event.k == true) {
						var k = list.length;
					} else {
						var k = [0, Infinity];
				...
				lib.element.content.YB_chooseToChongzhu = function () {
					...
					for (var k = 0; k < suits.length; k++) {
						...
						for (var j of cards) {
							if (j.suit == suits[k]) event.list[k][1].add(j);
						}
					}
					var list = [],
						list6 = [];
				...
				lib.element.content.FY_24 = function () {
					...
					event.list2 = [];
					for (var i = 0; i < event.cards.length; i++) {
						event.list2.push(event.cards[i].number);
					}
			...
			//---------------------------------------------------------*/
			/********************更新公告********************/
			// lib.init.js(`extension/夜白神略/ext`, "update");
			lib.init.js(`extension/夜白神略/ext2`, 'update');
			//此处岔开
		...
		help: {
			夜白神略: '总有游客玩我的扩展不开将包,找不到别的武将呢,说没意思.现在我希望你能耐心读指引.' + '<br>首先打开菜单,去武将列表下翻找到夜白将包和六艺篇,打开,去卡牌列表下翻找到夜白牌堆,打开(BOSS搬运看心情开),重启就OK了.',
			六艺篇: '<li>关于六艺系列' + '<br>六艺区是夜白神略包创立的机制.六艺区相当于一个私人的木牛流马,可以如手牌般使用或打出' + '<br>出牌阶段限一次,你可以将一张手牌置入六艺区,若六艺区牌数大于等于3,则获得精艺(精艺:每当你失去一张六艺区的牌时,你摸一张牌.)六艺区至多为6' + '<br>与此同时,作者同时推出了六艺技,其特征是可以通过对六艺区牌的应用来触发效果.需要注意的是,某些技能虽然和六艺区有关,但作者并没有给它定义为六艺技,因为它只是额外的把牌放入六艺区而并没有利用',
			// "结阵技":"<li>关于结阵技"
			// +"<br>结阵是夜白神略包三传分包创立的机制"
			// // +"<br>出牌阶段限一次,若你未处于阵列,你可以选择至多两名其他角色发起结阵,设定你为阵眼;若你处于阵列且不是阵眼,你可以将阵眼转移给你"
		...
		config: {
			...
			// 		"2":"若这段话挤满整个页面,<span style='color: #ff00cc'>请点这</span>点<span style='color: #ff00cc'>查看介绍</span>,",
			...
			ybslb_jiaqun: {
				...
				onclick() {
					if (this.jiaqun == undefined) {
						var more = ui.create.div('.jiaqun', '<div style="border:2px solid gray"><span><img style=width:238px src=extension/夜白神略/ybslb_QQ.jpg></span>');
						this.parentNode.insertBefore(more, this.nextSibling);
						this.jiaqun = more;
			...
			ybsl_wujinshilian: {
				name: '无尽试炼(需重启)',
				intro: '经实验发现,夜白国战和无尽试炼不可兼得,开了无尽试炼玩不了夜白国战,不知为何' + '<br><span class=yellowtext>夜白国战</span>:' + '<br>将夜白包武将加入国战' + '<br>平衡有待调整' + '<br><span class=yellowtext>夜白旅程</span>:' + '<br>已挑战过的敌人会作为可选队友出现在列表中;' + '<br>加入选项:弃置五张牌,凭空印制花中四君子各一张' + '<br><span class=yellowtext>夜白塔防</span>:' + '<br>可以自由点将;' + '<br>其它有待魔改',
				init: false,
				item: {
		...
		package: {
			intro: (function () {
				...
				return `<p style="color: rgb(210,210,000); font-size:12px; line-height:14px; text-shadow: 0 0 2px black;">' + log.join('<br>') + '</p><br><span style='color: gold'>潜水的火修复版<br>『无名杀扩展大全群』:771901025<br></span>`;
			})(),
...
/*			
			...
			'- 新增的一部分十周年美化卡牌已经就绪,烦请手动前往十周年的文件夹,找到ybsl_dafeng,ybsl_tianhuoduan,ybsl_piaoxueruyi的webp文件并删除,重启两次即可正常导入素材.',
			'- 从谏良计,可得自保(听从了狂神的建议,之后的更新包不再附带背景音乐,背景音乐伺机单独分享)',
			'- 代码中部分文字用ASCII和Unicode进行了转化,以避免剧透.',
			'- 本扩展随时欢迎各路萌新及大佬点评和借鉴,引用或转录注明出处即可,最好告知(就是想高兴高兴……)',
			'- 制作仓促,有bug烦请加群反馈.',
...
/*
  var levels = ['普通','精品','史诗','传说','限定','白露','成就','重阳','冬至','端午','儿童','父亲','国庆','劳动','芒种','清明','七夕','虎年','元旦','元宵','中秋','中元','春节','大雪','圣诞','大暑'];
					 ...
					 */
/*
   ['精品','史诗','传说','限定','白露','成就','重阳','冬至','端午','儿童','父亲','国庆','劳动','芒种','清明','七夕','虎年','元旦','元宵','中秋','中元','春节','大雪','圣诞','大暑'].includes(title)){
   var obj = {
	 精品:'jingpin',
	 史诗:'shishi',
	 传说:'chuanshuo',
