...
const YBSL_nature = function () {
  ...
  }
  {
    ...
    lib.translate.YB_snowsha_info = '当你造成雪属性伤害时,你可以令目标摸X张牌,然后其武将牌翻面(X为目标当前体力值且至多为5)';lib.translate.YB_bloodsha_info = '锁定技,造成血属性伤害时,回复等同伤害值的体力值'; // 其实这段貌似没用了↑↑
    lib.skill._YB_snowsha = { //------雪杀
      trigger: { source: 'damageSource' }, equipSkill: false, ruleSkill: true,
      shaRelated: true,
      filter(event, player) {
