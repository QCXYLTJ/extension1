				...
				const QQQ = {
					...
					skill: {
								...
								result: {
									player(player) {
										...
										for (const i of players) {
											...
											if (att < 0) att = -1;
											if (i != player && i.hp <= 3) {
												if (i.num('h') == 0) num += att / i.hp;
												else if (i.num('h') == 1) num += att / 2 / i.hp;
												else if (i.num('h') == 2) num += att / 4 / i.hp;
											}
						...
						qqwz_绝命3: {
							mod: {
								cardSavable(card, player) {
									...
									if (_status.currentPhase.get('s').includes('qqwz_绝命') && _status.currentPhase != player) {
						...
						qqwz_离歌: {
							...
							filter(event, player) {
								return event.card && event.source && event.player.classList.contains('dead') == false && player.num('he');
							},
							forced: true,
							check(event, player) {
						...
						qqwz_皇令: {
							...
							ai: {
								...
								result: {
									...
									target(player, target) {
										...
										var players = game.filterPlayer();
										for (const i of players) {
											if (i.num('h') && i != target && i != player && get.attitude(player, i) < 0) {
												break;
											}
										}
										if (i == players.length) {
							...
							content() {
								...
								var players = game.filterPlayer();
								for (const i of players) {
									if (i.num('h') && i != event.target1 && i != player) {
										break;
									}
								}
								if (i == players.length) {
						...
						qqwz_散财: {
							...
							group: 'qqwz_散财_1',
							filter(event, player) {
								return _status.currentPhase == player && event.player.isAlive() && event.player.num('he') > 0;
							},
							forced: true,
							content() {
								var num = 0;
							...
							ai: {
								...
								result: {
									target(player, target) {
										...
										if (get.attitude(player, target) <= 0) return target.num('he') ? -1.5 : 1.5;
										var js = target.get('j');
										if (js.length) {
						...
						qqwz_鸳鸯: {
							...
							check(card) {
								...
								if (player.num('h') > player.hp) return 8 - get.value(card);
						...
						qqwz_人杰2: {
							...
							content() {
								...
								player
									...
									.set('ai', function (target) {
										if (!ui.selected.targets.length) return -get.attitude(player, target) + target.num('h');
										if (ui.selected.targets.length) {
						...
						qqwz_引敌: {
							...
							content() {
								'step 0';
								player
									.chooseCard([1, player.num('he')], 'he', get.prompt('qqwz_引敌'), function (card) {
										return player.num('he') > 0;
									})
									.set('ai', function (card) {
										return 6 - get.value(card);
						...
						qqwz_招亲: {
							...
							check(event, player) {
								if (event.player.isUnderControl(true, player)) {
									return (
										event.player.get('h', function (card) {
											return get.type(card) != 'food';
										}).length
									);
								}
						...
						qqwz_镇军: {
							...
							content() {
								...
								player.choosePlayerCard(trigger.target, 'he', [1, Math.min(trigger.target.num('he'), trigger.target.hp * 2)], get.prompt('qqwz_镇军', trigger.target));
								('step 1');
						...
						qqwz_缴械: {
							...
							filter(event, player) {
								return player.num('he') > 0 && event.source && event.source.get('e') != undefined && event.card && event.card.name == 'sha';
							},
							check(event, player) {
						...
						qqwz_秉正: {
							...
							content() {
								'step 0';
								player
									.chooseTarget(get.prompt('qqwz_秉正'), [1, player.num('h')], function (card, player, target) {
										return true;
									})
									.set('ai', function (target) {
										return get.attitude(_status.event.player, target);
						...
						qqwz_阵邀_sha: {
							...
							ai: {
								...
								order() {
									...
									if (
										game.hasPlayer(function (current) {
											return get.attitude(player, current) > 0 && current.num('h') < nh;
										})
									) {
										return 2.9;
									}
						...
						qqwz_控局: {
							...
							content() {
								...
								target.chooseControl('展示手牌', '流失体力').ai = function (event, player) {
									var cards = player.get('he', { suit: player.storage.qqwz_控局.suit });
								...
								('step 3');
								target.discard(target.get('he', { suit: target.storage.qqwz_控局.suit }));
						...
						qqwz据江: {
							...
							subSkill: {
								1: {
									...
									forced: true,
									filter(event, player) {
										return player.getEnemies().includes(event.player) && player.num('h') != event.player.num('h');
									},
									content() {
										'step 0';
