    ...
    init() {
        let pack = {
            ...
            skill: {
                    ...
                    content() {
                        ...
                        var list = game.filterPlayer(function (current) {
                            if (current.name.indexOf('daqin_alpha') == 0) {
                                ...
                                var name = (current.name.includes('1') ? ['re_liubei', 're_xiahoudun'] : ['re_zhangjiao', 're_sunquan']).randomGet();
                                current.uninit();
                                current.init(name);
                        ...
                        for (var i of result.links) {
                            event.current.addSkill(i[2].slice(10));
                        }
                ...
                kangqin_lvshichunqiu: {
                    ...
                    filter(event, player, name) {
                        ...
                        return _status.currentPhase != player && player.name == 'daqin_lvbuwei' && (event.animate == 'draw' || event.parent.name == 'draw') && event.getParent(2).name != 'kangqin_lvshichunqiu';
                    },
                    content() {
                ...
                zhangyi_xiongbian: {
                    ...
                    content() {
                        ...
                        ('step 1');
                        if (result.bool) trigger.parent.cancel();
                ...
                baiqi_wuan: {
                    ...
                    forced: true,
                    filter(event, player) {
                        return !event.audioed && player.isAlive() && event.source && event.source.group == 'daqin' && event.card.name == 'sha' && player.countUsed('sha', true) > 1 && event.parent.type == 'phase';
                    },
                    content() {
            ...
            translate: {
                ...
                zhaogao_haizhong: '害忠',
                zhaogao_haizhong_info: '锁定技,非秦势力角色回复体力时,其需要选择:1.弃置一张红色牌,2.受到你造成的X点伤害(X为该角色拥有的<害>标记,且至少为1).该角色获得一个<害>标记.',
                zhaogao_aili: '爰历',
                ...
                daqin_lvbuwei_rerende_info: '出牌阶段,你可以将至少一张手牌交给其他角色,你于此阶段内不能再以此法交给该角色牌;若你于此阶段内给出的牌首次达到两张,你可以视为使用一张基本牌',
        ...
        for (var i in pack.character) {
            ...
            pack.character[i][4].addArray(['forbidal']);
    ...
    content: {
        ...
        chooseCharacterBefore() {
            ...
            var next = {
                ...
                playername: ['月宫亚由', '神尾观铃', '古河渚', '坂上智代', '星野梦美', '枣铃', '库特莉亚芙卡', '神户小鸟', '立华奏', '友利奈绪', '汐奈', '鸣濑白羽', '仲村ゆり', '藤林杏', '烟雨墨染', '诗笺', '苏婆玛丽奥', 'doremy', '楼小楼', '西沉', '叫我蠢直', '骑着二乔上貂蝉', '内奸不会错', '綦薵', '轮回中的消逝者', 'Sukincen', '太上大牛', '镜中尘', '明了', '阿Q', '昨日影', '蒜头王八', '凉茶', '萌新瑟瑟发抖', '情何以堪', '元春二六', '炒饭123℃', '夜渐寒忆往昔', '【物质主义】', '一曲离歌高唱', '绵绵不绝于耳', 'L', '逆曲惜寒', '杜元枫', '三生六十', 'Itsuka士道君', '菁幽', '殇雪飘零', '瓦力', '别来无恙', '大叔viv', '深邃暖爱', '北瓜南瓜东', '染柒', '543(21)0', '萌新转型中', '随性似风', '牢戏志才', '生熏鱼', '宁静致远', '雷', 'Fire win', 'Empty city°', '?', 'lonely patients', '狂神'],
                dieAfter2(source) {
                    if (source) {
