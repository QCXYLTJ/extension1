...
const brawl = {
    name: '新斗地主',
    mode: 'identity',
    intro: ['萌币系统：<br>' + "当前拥有<span class='texiaotext' style='color: #FFD700'>" + lib.config.extension_活动萌扩_decade_Coin + '</span>' + get.bolInformX('萌币', '<li>萌币作为活动萌扩的特殊货币，可在新斗地主模式使用，本模式获胜/失败/逃跑均会改变萌币数<br><li>当前萌币仅用于新斗地主模式作为仿十周年斗地主金票的筹码，未来可能会支持更多功能') + '<br>' + "进行此游戏需要支付10萌币作为门票（<span class='texiaotext' style='color: #FF0000'>若萌币不足则无法进入游戏</span>），游戏获胜后会获得当前游戏倍数的萌币，失败后会失去当前游戏倍数的萌币（逃跑会失去当前游戏倍数的萌币+10）<br>" + '第一次进入无名杀会获得初始1000萌币，从第二天开始每天进入无名杀可以获得300萌币', '游戏规则：<br>' + '游戏开始时，牌局将为玩家分发五张初始武将牌，玩家可以根据武将信息决定是否叫分抢地主。<br>' + '从随机一名玩家开始依次开始叫分抢地主，玩家选择叫分倍数，叫分最多的玩家成为地主，最多为3倍，也可放弃叫分。<br>' + '每位玩家仅有一次叫分机会，且叫分必须大于上家的叫分，否则放弃叫分。<br>' + '叫分过程中，若有玩家叫分3倍则该玩家直接成为地主。<br>' + '若三名玩家都放弃叫分，第一个叫分的玩家以最低倍数成为地主。<br>' + '确认地主后，地主玩家默认成为一号位。其余两位玩家自动成为农民玩家，两位农民玩家的选将框互相知悉。<br>' + '地主玩家的选将框在原有武将上额外增加两个随机武将。<br>' + '发放初始手牌后，三位玩家同时可以选择是否明牌：若地主选择明牌，本局倍数翻1倍；若农民选择明牌，本局倍数翻0.5倍。', '地主额外技能：<br>' + '【飞扬】：锁定技，你的摸牌阶段的摸牌数+1，出牌阶段使用【杀】的额定次数+1。<br>' + '【跋扈】：判定阶段开始时，你可以弃置两张手牌并弃置判定区所有牌。'],
    init() {
        ...
        lib.skill.decade_feiyang = {
            ...
            content() {
                'step 0';
                player
                    .chooseToDiscard(get.prompt2('decade_bahu'), 2)
    ...
    content: {
        ...
        chooseCharacterBefore() {
            ...
            if (lib.config.extension_活动萌扩_use_DDZname) {
                ...
                var map = lib.config.extension_活动萌扩_DDZname || ['shen_zhaoyun', 'shen_ganning', 'liuyan', 'xizhicai', 're_wuyi', 'xin_lingtong', 'zhoushan', 'chengui', 'dc_liuye', 'dc_tengfanglan', 'shen_machao', 'shen_zhangfei', 'shen_zhangjiao', 'shen_dengai', 're_liuzan', 'caojinyu', 're_sunyi', 'caomao', 'xushao', 'zhujianping', 'tenggongzhu', 'zhangxuan', 'dc_zhouxuān', 'zerong', 'dc_luotong', 'ruanji', 'dc_xujing', 'xuelingyun', 'yue_caiwenji', 'star_caoren'];
            ...
            game.checkResult = function () {
                if (
                    ...
                    game.over(
                        (!game.zhu.isAlive() && game.me.identity == 'fan') ||
                        (!game.players.some(function (current) {
                            return current.identity == 'fan';
                        }) &&
            ...
            game.chooseCharacter = function () {
                ...
                next.setContent(function () {
                    ...
                    game.me
                        ...
                        .set('list', fellow ? fellow.characterlist : [])
                        .set('listx', game.me.characterlist);
                    ('step 5');
                    var getNum = function (name) {
                        var num;
                        switch (game.getRarity(name)) {
