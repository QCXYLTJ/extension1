...
export async function content(config, pack) {
    ...
    content() {
      if (player.node.fuka) {
        ...
        if (!skillname) {
          ...
          var info = lib.skill[skillname];
          if (info.spell) {
            for (let i = 0; i < info.spell.length; i++) {
  ...
  lib.element.player.updatelili = function () {
    ...
    if (this.maxlili == Infinity) {
      ...
      lili.classList.add('text');
    } else if (get.is.newLayout() && (this.maxlili > 9 || (this.maxlili > 5 && this.classList.contains('minskin')) || ((game.layout == 'mobile' || game.layout == 'long') && this.dataset.position == 0 && this.maxlili > 7))) {
  ...
  lib.element.player.canUseLili = function (bool) {
    ...
    for (let i = 0; i < skills.length; i++) {
      ...
      if (info && info.ai && info.gezi_cost && !bool) {
        if (info.ai.skillTagFilter && info.ai[tag] && info.ai.skillTagFilter(this, tag) === false) continue;
        if (!info.ai[tag]) {
          var gezi_cost = info.gezi_cost;
          if (typeof gezi_cost === 'number' && this.lili <= gezi_cost) return false;
        }
      }
    } //循环
    return true;
  ...
  if ((config.gezidedongfanglili && get.mode() != 'library' && get.mode() != 'guozhan') || (get.mode() == 'guozhan' && config.gezidedongfangliliguozhan)) {
    lib.skill._gezi_lili = {
      ...
      content() {
        ...
        if (player.hiddenSkills.length == 0 && info && info[4]) {
          //QQQ
          for (var str of info[4]) {
  ...
  if ((config.gezidedongfanglili || get.mode() == 'stg') && get.mode() != 'library') {
    ui.css.fuka_style = lib.init.css('extension/东方project', 'fuka');
    lib.init.css('extension/东方project', 'official');
  ...
  lib.element.player.getJinengpai = function (arg1) {
    ...
    if (arg1) cards = cards.filter((i) => i.name == arg1);
    return cards;
  };
  //添加技能牌
  ...
  lib.element.content.addJudgen = function () {
    ...
    player.node.jinengpai.appendChild(card); //QQQ
    ...
    ('step 2');
    player.removeJudgen(result.links[0]); //QQQ
  };
  //移除技能牌
  ...
  game.addJudgen = function (player, content) {
    if (player && content) {
      ...
      card.classList.add('fakejudge');
      player.node.jinengpai.appendChild(card);
      ui.updatej(player);
    }
  };
  ...
  lib.element.player.addIncident = function (card) {
    ...
    if (lib.config.background_audio && lib.config.background_music != 'music_off' && get.subtype(card) == 'yibianpai') {
      ...
      if ((this.identity == 'zhu' && get.config('gezibackgroundchange') != 'off') || (this.identity == 'nei' && get.config('gezibackgroundchange') == 'luren')) {
        var str = 'extension/东方project/image/' + card.name + '.jpg';
    ...
    if (get.config('incidentoverbool') || config.incidentoverbool) {
      for (let i = 0; i < get.info(card).incidentskills.length; i++) {
    ...
    lib.skill._tanpai = {
      ...
      content() {
        ...
        if (player.identity == 'zhu' || player.identity == 'nei') {
          ...
          player
            .chooseTarget('【忠】身份明置效果:令一名角色摸一张牌', function (card, player, target) {
              return true;
            })
          ...
          player
            .chooseTarget('【反】明置效果:令一名角色选择:弃一张牌或明置身份', function (card, player, target) {
  ...
  lib.element.player.$effectn = function (name, frame, left, top) {
    if (config.dongfangtexiao) {
      ...
      var ID = setInterval(function () {
        ...
        var src = 'extension/东方project/effect/xuliezhen/' + name + '/' + zhen + '.png';
  ...
  lib.element.player.$effectnn = function (name, frame, left, top) {
    if (lib.config.animation && !lib.config.low_performance && config.dongfangtexiao) {
      ...
      Animation.style.backgroundSize = 'cover';
      var src = 'extension/东方project/effect/gif/' + name + '.gif';
  ...
  game.switchModen = function (name, configx) {
    ...
    window.game = game;
    var script = lib.init.js('extension/东方project', name, function () {
      ...
      if (_status.connectMode && lib.mode[name].connect) {
        ...
        if (configx) {
          ...
          for (let i = 0; i < lib.config.connect_cards.length; i++) {
            lib.configOL.cardPack.remove(lib.config.connect_i);
          }
  ...
  game.playlili = function (fn, dir, sex) {
    if (lib.config.background_speak) {
      ...
      else game.playAudio('../extension/东方project/audio', fn);
    }
  };
  //这是角色变装/背景图的全局技能…
    ...
    content() {
      if (
        game.countPlayer(function (current) {
          return current.name == 'aventurinepurple';
        }) > 0
      ) {
  ...
  game.playnBackgroundMusic = function (music, temp, marisa) {
    ...
    if (lib.config.background_music == 'music_off') {
      ...
      if (music == 'music_custom') {
        ...
        ui.backgroundMusic.src = 'extension/东方project/audio/' + music + '.mp3';
      ...
      if (check) next.ai = check;
        ...
        next.ai = function (card) {
          ...
          if (source && source != player && get.attitude(player, source) > 1) {
            return -card.number - get.value(card) / 2 + addi;
          }
          return card.number - get.value(card) / 2 + addi;
        };
  ...
  lib.element.content.chosenPlayerCard = function () {
    ...
    if (event.forced && select[0] >= cs.length) {
      ...
      for (const i of event.result.buttons) {
        event.result.links[event.result.buttons.indexOf(i)] = i.link;
      } //QQQ
    } else {
      if (event.isMine()) {
        event.dialog.open();
  ...
  if (lib.config.mode == 'brawl') {
    ...
    lib.character.kosuzu = ['female', 'shu', 3, [], ['ext:东方project/image/kosuzu.jpg']];
    ...
    lib.skill._brawljinengpaie = {
      ...
      content() {
        if (_status.brawl.scene.skillPileTop && _status.brawl.scene.skillPileTop.length) {
          for (const i of _status.brawl.scene.skillPileTop) player.addJudgen(game.createCard(i[0]));
        }
    ...
    lib.skill._brawljinengpaip = {
      ...
      content() {
        if (_status.brawl.scene.skillPileBottom && _status.brawl.scene.skillPileBottom.length) {
          for (const i of _status.brawl.scene.skillPileBottom) player.addJudgen(game.createCard(i[0]));
        }
      ...
      scenes: [
        {
          ...
          players: [
            ...
            },
            {
              ...
              handcards: [],
              equips: [
                ['gezi_yinyangyu', 'heart', 5, -1],
                ['gezi_bailou', 'club', 6, -1],
              ],
          ...
          cardPileTop: [
            ...
            ['tao', 'diamond', 12],
            ['guohe', 'club', 4],
            ['gezi_gungnir', 'heart', 13],
            ['gezi_simen', 'spade', 2],
            ['gezi_reidaisai', 'heart', 6],
          ],
  ...
  if (get.mode() == 'stone') {
    ...
    for (var i in lib.characterPack.mode_extension_stone_df) {
      ...
      lib.characterPack.mode_extension_stone_df[i][4].push('ext:东方project/image/gezi_' + i + '.jpg');
      ...
      lib.card[name] = {
        image: 'ext:东方project/image/gezi_' + i + '.jpg',
  ...
  if (get.mode() == 'stg') {
    ...
    for (var i in lib.characterPack.mode_extension_stg) {
      ...
      lib.characterPack.mode_extension_stg[i][4].push('ext:东方project/image/' + i + '.jpg');
      lib.characterPack.mode_extension_stg[i][4].push('die:ext:东方project/audio/' + i + '.mp3');
  ...
  if (get.mode() == 'chess') {
    for (var i in lib.characterPack.mode_extension_chess_df) {
      ...
      lib.characterPack.mode_extension_chess_df[i][4].push('ext:东方project/image/' + i + '.jpg');
      lib.characterPack.mode_extension_chess_df[i][4].push('die:ext:东方project/audio/' + i + '.mp3');
      if (lib.config.forbidai_user.includes(i)) {
        lib.config.forbidai.push(i);
      }
    }
  ...
  for (var i in lib.characterPack.mode_extension_library_luanru) {
    ...
    lib.characterPack.mode_extension_library_luanru[i][4].push('ext:东方project/image/' + i + '.jpg');
    lib.characterPack.mode_extension_library_luanru[i][4].push('die:ext:东方project/audio/' + i + '.mp3');
    if (lib.config.forbidai_user.includes(i)) {
      lib.config.forbidai.push(i);
    }
  }
  for (var i in lib.characterPack.mode_extension_library) {
    ...
    lib.characterPack.mode_extension_library[i][4].push('ext:东方project/image/' + i + '.jpg');
    lib.characterPack.mode_extension_library[i][4].push('die:ext:东方project/audio/' + i + '.mp3');
    if (lib.config.forbidai_user.includes(i)) {
      lib.config.forbidai.push(i);
    }
  }
