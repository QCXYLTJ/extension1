...
export async function precontent(config, pack) {
	lib.init.css('extension/东方project', 'lili');
	//—————————————————————————————————————————————————————————————————————————————数据操作相关自定义函数
	...
	game.import('card', function (lib, game, ui, get, ai, _status) {
		const QQQ = {
			...
			card: {
				gezi_library: {
					...
					subtype: 'yibianpai',
					audio: true,
					image: 'ext:东方project/image/library_mengji.png',
					enable: true,
					selectTarget: -1,
					...
					content() {
						...
						if (result.control == '展示手牌并揭露弱点') {
							...
							if (player.storage._enhance && player.storage._enhance > 1) {
								for (let i = 1; i < player.storage._enhance; i++) {
									target.storage.gezi_ruodianjielu++;
								}
							}
							target.markSkill('gezi_ruodianjielu');
					...
					ai: {
						...
						result: {
							target(player, target) {
								if (get.attitude(player, target) <= 0) return target.countCards('h') ? -1.5 : 0;
				...
				gezi_danmakucraze: {
					...
					content() {
						...
						if (player.storage._enhance) {
							for (let i = 0; i < player.storage._enhance; i++) {
								target.draw(1);
							}
						}
				...
				stg_bawu: {
					...
					content() {
						...
						target
							.chooseControl('回复1点体力,获得圣盾', '摸1张牌,获得连击', true)
							.set('ai', function () {
								if (
									target.countCards('h', {
										name: 'sha',
					...
					content() {
						...
						target.chooseControl(['弃置三张牌', get.translation(player) + '以外角色各弃置两张牌'], function (event, player) {
							...
							if (target.identity == 'zhu' && get.mode() == 'identity') return get.translation(player) + '以外角色各弃置两张牌';
								...
								return get.translation(player) + '以外角色各弃置两张牌';
							}
							return '弃置三张牌';
						});
						('step 1');
					...
					ai: {
						...
						result: {
							target(player, target) {
								if (
									game.countPlayer(function (current) {
										if (game.dead.includes(current) && get.attitude(target, current) > 0 && target != current) return 1;
										return 0;
									})
								)
									return -3;
					...
					async content(event, trigger, player) {
						...
						if (result?.links?.length) {
							...
							if (get.type(card) == 'jinengpai') {
								event.target.removeJudgen(card);
							} else {
						...
						result: {
							player(player, target) {
								var num = game.countPlayer(function (current) {
									if (get.attitude(player, current) < 0 && current.countCards('he')) return 2;
									if (get.attitude(player, current) > 0 && current.countCards('he')) return -2;
								});
								if (num > 0) return -100;
								if (num < 0) return 100;
				...
				holysword: {
					fullskin: true,
					image: 'ext:东方project/image/holysword.png',
					type: 'equip',
					subtype: 'equip1',
					gezi_cost: 5,
					ai: {
				...
				spell_wuyashenxiang: {
					image: 'ext:东方project/image/spell_wuyashenxiang.jpg',
					type: 'trick',
					subtype: 'support',
					fullimage: true,
					enable: true,
				...
				spell_xiaoshi: {
					type: 'trick',
					subtype: 'disrupt',
					image: 'ext:东方project/image/spell_xiaoshi.jpg',
					fullimage: true,
					enable: true,
					gezi_cost: -2,
					selectTarget: 1,
				...
				spell_piaoqie: {
					type: 'trick',
					subtype: 'disrupt',
					image: 'ext:东方project/image/spell_piaoqie.jpg',
					fullimage: true,
					enable: true,
					gezi_cost: -1,
					filterTarget(card, player, target) {
				...
				gezi_gw_xinsheng: {
					type: 'trick',
					subtype: 'disrupt',
					image: 'ext:东方project/image/gezi_gw_xinsheng.jpg',
					fullimage: true,
					enable(card, player) {
				...
				gezi_gw_zumo: {
					...
					fullimage: true,
					subtype: 'disrupt',
					image: 'ext:东方project/image/gezi_gw_zumo.jpg',
					enable: true,
					filterTarget(card, player, target) {
				...
				gezi_gw_youer: {
					type: 'trick',
					subtype: 'disrupt',
					image: 'ext:东方project/image/gezi_gw_youer.jpg',
					enable: true,
					fullimage: true,
					filterTarget(card, player, target) {
				...
				sv_dshift: {
					audio: true,
					fullimage: true,
					image: 'ext:东方project/image/sv_dshift.jpg',
					type: 'trick',
					subtype: 'support',
					enable: true,
					gezi_cost: -4,
				...
				sv_alterfate: {
					audio: true,
					fullimage: true,
					image: 'ext:东方project/image/sv_alterfate.jpg',
					type: 'trick',
					subtype: 'support',
					enable: true,
					selectTarget: -1,
				...
				sv_harvest: {
					fullimage: true,
					image: 'ext:东方project/image/sv_harvest.jpg',
					type: 'equip',
					subtype: 'equip5',
					ai: {
						basic: {
				...
				yugioh_megamorph: {
					fullimage: true,
					image: 'ext:东方project/image/yugioh_megamorph.jpg',
					type: 'equip',
					subtype: 'equip5',
					ai: {
						basic: {
				...
				yugioh_mirror: {
					audio: true,
					fullimage: true,
					image: 'ext:东方project/image/yugioh_mirror.jpg',
					type: 'trick',
					subtype: 'defense',
					notarget: true,
					...
					content() {
						...
						if (evt.mirrorinfo) {
							evt.neutralize();
							evt.parent.excluded.add(evt.mirrorinfo.target);
				...
				yugioh_goldsarc: {
					audio: true,
					fullimage: true,
					image: 'ext:东方project/image/yugioh_goldsarc.jpg',
					type: 'trick',
					subtype: 'support',
					enable: true,
					selectTarget: -1,
					...
					content() {
						...
						target
							.chooseCardButton(1, '将一张牌移除,回合开始时获得', cards)
				...
				uno_skip: {
					audio: true,
					fullskin: true,
					image: 'ext:东方project/image/uno_skip.png',
					type: 'trick',
					subtype: 'disrupt',
					enable: true,
					selectTarget: -1,
				...
				uno_plus2: {
					audio: true,
					image: 'ext:东方project/image/uno_plus2.png',
					fullskin: true,
					type: 'trick',
					subtype: 'disrupt',
					enable: true,
				...
				uno_reverse: {
					audio: true,
					image: 'ext:东方project/image/uno_reverse.png',
					fullskin: true,
					type: 'trick',
					subtype: 'disrupt',
					enable: true,
					...
					content() {
						...
						if (players.length % 2 == 0) {
							...
							game.broadcastAll(function () {
								for (let i = 0; i <= num; i++) {
									game.swapSeat(i, players[players.length - i - 1]);
								}
							});
						} else {
				...
				bianda: {
					audio: true,
					image: 'ext:东方project/image/bianda.png',
					fullskin: true,
					type: 'basic',
					subtype: 'support',
					enable: true,
				...
				bianxiao: {
					audio: true,
					fullskin: true,
					image: 'ext:东方project/image/bianxiao.png',
					type: 'basic',
					subtype: 'attack',
					enable: true,
					enhance: 2,
			...
			skill: {
				...
				g_yugioh_mirror: {
					...
					filter(event, player) {
						...
						return event.card && event.card.name == 'sha' && player.countCards('h', { name: 'yugioh_mirror' }) > 0;
					},
					content() {
						'step 0';
						trigger.mirrorinfo = {
						...
						player
							.chooseToUse(
								...
								'是否使用【反射镜力】无效' + get.translation(trigger.player) + '的【杀】并破坏其手牌？'
							)
					...
					content() {
						if (player.storage.gezi_gw_youer) {
							if (trigger.name == 'phase') {
								...
								for (let i = 0; i < player.storage.gezi_gw_youer.length; i++) {
			...
			translate: {
				gezi_library: '平和',
				gezi_library_info: '<u>胜利条件:</u>无.<br/><u>异变效果:</u>这是在没有异变牌记录时填充的牌,没有任何效果',
				gezi_caifang: '突击采访',
				gezi_caifang_info: '出牌阶段,对一名其他角色使用.目标角色选择一项:<br>明置身份牌(可以发动异变效果),若已明置则获得一层弱点揭露;<br>展示所有手牌,你摸一张牌.<br><b>弱点揭露:<br>一层:受到伤害时弃置一张牌;<br>两层:受到的伤害加一;<br>三层及以上:受到伤害时流失X点体力(X为弱点揭露层数-2).<br>受到伤害后清除层数.<b></br><u>强化(-1):改为选择两项.</u>',
				gezi_danmakucraze: '弹幕狂欢',
				gezi_danmakucraze_info: '出牌阶段,对自己使用.目标本回合可使用【杀】的次数加二.</br><u>强化(-1):摸一张牌.</u>',
				gezi_reidaisai: '博丽例大祭',
				gezi_reidaisai_info: '出牌阶段,对所有角色使用.目标角色摸一张牌,可以将一张牌交给其他角色.',
				gezi_reidaisai2: '守矢例大祭',
				gezi_reidaisai2_info: '出牌阶段,对任意名与你身份不同的角色使用.目标角色摸一张牌,你摸X张牌(X为目标角色数+1),你可以任意分配以此法获得的牌.',
				gezi_tancheng: '坦诚相待',
				gezi_tancheng_info: '出牌阶段,对所有其他角色使用.目标角色展示所有手牌,你可以用一张手牌交换其中一张不同类型的牌.',
				gezi_xuyuanshu: '许愿术',
				gezi_xuyuanshu_info: '出牌阶段,对自己使用.目标角色随机获得一张普通锦囊牌(失去时销毁),可以使用之,若如此做,重复此流程直到获得第二张牌.若没有使用或无法使用,则结束流程,摸2-X张牌(X为以此牌获得的牌数).</br> <u>强化(-1):此法获得的牌+1.</u>',
				stg_bawu: '拨雾开天',
				...
				gezi_deathfan: '凤蝶纹扇',
				gezi_deathfan_info: '出牌阶段限一次,你可以将一张延时锦囊牌置于弃牌堆,对至多两名角色各造成一点雷电伤害.',
				gezi_gungnir: '冈格尼尔',
				...
				gezi_zhiyuu: '净颇梨之镜',
				gezi_zhiyuu_info: '出牌阶段限一次,你可以令一名角色展示一张手牌,你可以弃置一张与之花色相同的手牌,对其造成一点雷属性伤害.',
				stg_goldbook: '金魔导书',
				...
				gezi_ibuki: '伊吹瓢',
				gezi_ibuki_info: '出牌阶段限一次,你可以将一张基本牌置于弃牌堆,你获得一点灵力.若你灵力不大于三或者无法获得灵力,你摸一张牌.',
				gezi_pantsu: '蓝白胖次',
				...
				gezi_huanxiang: '幻想之扉',
				gezi_huanxiang_info: '出牌阶段,对所有角色使用.目标角色摸一张牌,随机贴上一张技能牌.</br> <u>追加效果:游戏开始时,或你进入游戏时,你可以展示此牌,令所有角色摸一张牌.</u>',
				gezi_huazhi: '花之祝福',
				gezi_huazhi_info: '出牌阶段,对自己使用.此回合结束时,目标角色摸X张牌(X为其本回合造成的伤害点数且至少为一).</br> <u>强化(-1):获得一张【惊吓派对】.</u>',
				gezi_jingxia: '惊吓派对',
				gezi_jingxia_info: '出牌阶段,对所有能够拼点的其他角色使用.你摸一张牌并与目标角色拼点,若你赢,你对其造成一点雷电伤害.</br> <u>追加效果(-2):出牌阶段,你可以弃置此牌,获得【潜行】直到你的下个回合开始.</u>',
				gezi_lingbi: '令避之间',
				...
				gezi_tianguo: '天国之阶',
				gezi_tianguo_info: '出牌阶段,对所有角色使用.将弃牌堆洗入牌堆,目标角色摸一张牌.</br> <u>追加效果:你获得此牌后,可以展示之,令所有角色回复一点体力.</u>',
				gezi_zuiye: '罪业边狱',
				gezi_zuiye_info: '出牌阶段,对一名其他角色使用;弃置其区域内X张牌(X为其已损失体力值).</br> <u>追加效果:你造成伤害时,可以将手牌中的所有【罪业边狱】交给受伤角色,令此伤害+X(X为你以此法交出的牌数).</u>',
				gezi_baka: '笨蛋',
				gezi_baka_info: '<u>胜利条件:</u>你击杀一名角色时,若你已经击杀过其他角色,你获得胜利.<br/><u>异变效果:</u>所有数字视为⑨进制.',
				gezi_death: '皆杀',
				...
				gezi_shenyou: '神佑',
				gezi_shenyou_info: '锁定技,你的判定牌生效前,你移除一张【神佑】,令判定牌的花色视为♥️️;你受到伤害时,若伤害大于你的体力值,移除一张【神佑】,防止该伤害.',
				gezi_ziheng: '制衡',
				...
				spell_wuyashenxiang_info: '出牌阶段,对自己使用;目标选择一项:法术牌或基本牌,从三张该种类的牌中获得一张',
				spell_xiaoshi: '消失',
				spell_xiaoshi_info: '出牌阶段,对一名角色使用;其收回装备区内的所有牌,并弃置其若干张手牌,直到其手牌数与你相等',
				spell_piaoqie: '剽窃',
				spell_piaoqie_info: '出牌阶段,对一名角色使用;复制其两张手牌,加入你的手牌',
				gezi_gw_xinsheng: '新生',
				gezi_gw_xinsheng_info: '出牌阶段,对一名角色使用;你随机观看12张角色牌,选择一张替代目标角色牌(保留当前体力值和灵力值,且不大于新上限),结束出牌阶段',
				gezi_gw_zumo: '阻魔手铐',
		...
		for (const i in QQQ.card) {
			...
			if (!info.image) {
				if (info.fullskin) {
					info.image = `ext:东方project/image/${i}.png`;
				} else {
					info.image = `ext:东方project/image/${i}.jpg`;
				}
			}
			lib.inpile.add(i);
		...
		lib.config.all.cards.add('东方project');
		lib.config.cards.add('东方project');
		lib.translate.东方project_card_config = '东方project';
		return QQQ;
	});
	const skill = {
		danmaku_skill: {
		...
		deathfan_skill: {
			...
			prompt: '将手牌中的一张延时类锦囊牌置于弃牌堆,对至多2名角色各造成1点雷电伤害',
			check(card) {
		...
		zhiyuu_skill: {
			audio: 'ext:东方project/audio:true',
			equipSkill: true,
			fullskin: true,
			enable: 'phaseUse',
			usable: 1,
					...
					function (card) {
						...
						if (evt.target)
							if (get.damageEffect(evt.target, evt.player, evt.player, 'thunder') > 0) {
								return get.value(card, evt.player) < 6;
							}
						return -1;
					}
				).prompt = false;
		...
		lantern_skill: {
			...
			filter(event, player) {
				...
				if (
					...
					return false;
				return event.card && event.card.name == 'sha' && get.distance(player, event.player, 'attack') <= 1 && player.canCompare(event.player);
			},
			content() {
				'step 0';
				...
				effect: {
					target(card, player, target) {
						...
						if (
							...
							return;
						if (card.name == 'sha' && get.color(card)) return 0.6;
					},
				},
			},
		},
		...
		ibuki_skill: {
			...
			position: 'h',
			prompt: '将一张基本牌置于弃牌堆,获得一点灵力,若你灵力不大于3或不能获得灵力,摸一张牌',
			content() {
				'step 0';
		...
		pantsu_skill: {
			...
			content() {
				var cards = player.getCards('e');
				for (const i of cards) {
					if (i && i.name == 'gezi_pantsu') {
						trigger.directresult = [i];
						break;
					}
				}
			},
		...
		bingyu2: {
			...
			init(player) {
				...
				ui.css.border_stylesheet.sheet.insertRule('#window .player>.framebg,#window #arena.long.mobile:not(.fewplayer) .player[data-position="0"]>.framebg{display:block;background-image:url(extension/东方project/image/snow1.png)}', 0);
				ui.css.border_stylesheet.sheet.insertRule('#window #arena.long:not(.fewplayer) .player>.framebg, #arena.oldlayout .player>.framebg{background-image:url(extension/东方project/image/snow3.png)}', 0);
			...
			onremove(player) {
				...
				ui.css.border_stylesheet.sheet.insertRule('#window .player>.framebg,#window #arena.long.mobile:not(.fewplayer) .player[data-position="0"]>.framebg{display:block;background-image:url("theme/style/player/' + bstyle + '1.png")}', 0);
				ui.css.border_stylesheet.sheet.insertRule('#window #arena.long:not(.fewplayer) .player>.framebg, #arena.oldlayout .player>.framebg{background-image:url("theme/style/player/' + bstyle + '3.png")}', 0);
				ui.css.border_stylesheet.sheet.insertRule('.player>.count{z-index: 3 !important;border-radius: 2px !important;text-align: center !important;}', 0);
			},
			content() {
				var players = game.filterPlayer();
				for (const i of players) {
					i.removeSkill('bingyu1');
					i.node.framebg.dataset.auto = i.storage.bingyu;
					delete i.storage.bingyu;
				}
		...
		lingbi1: {
			mod: {
				cardEnabled(card, player) {
					...
					var cards = [];
					for (const i of players) {
						if (i.storage._lingbi2) cards = cards.concat(i.storage._lingbi2);
					}
		...
		lingbi2: {
			...
			init(player) {
				ui.css.border_stylesheet = lib.init.sheet();
				ui.css.border_stylesheet.sheet.insertRule('#window .player>.framebg,#window #arena.long.mobile:not(.fewplayer) .player[data-position="0"]>.framebg{display:block;background-image:url(extension/东方project/image/lock1.png)}', 0);
				ui.css.border_stylesheet.sheet.insertRule('#window #arena.long:not(.fewplayer) .player>.framebg, #arena.oldlayout .player>.framebg{background-image:url(extension/东方project/image/lock3.png)}', 0);
				ui.css.border_stylesheet.sheet.insertRule('.player>.count{z-index: 3 !important;border-radius: 2px !important;text-align: center !important;}', 0);
			},
			onremove(player) {
				ui.css.border_stylesheet = lib.init.sheet();
			...
			filter(event, player) {
				...
				return player.storage._lingbi2.length;
			},
			content() {
				var players = game.filterPlayer();
				for (const i of players) {
					i.removeSkill('lingbi1');
					i.node.framebg.dataset.auto = i.storage.lingbi;
					delete i.storage.lingbi;
				}
		...
		_lingbi2: {
			...
			filter(event, player) {
				return event.card && event.card.name == 'gezi_lingbi';
			},
			content() {
				'step 0';
				var list = [];
		...
		_tianguo: {
			...
			forced: true,
			filter(event, player) {
				return event.card && event.card.name == 'gezi_tianguo';
			},
			content() {
				var cards = [];
				for (let i = 0; i < ui.discardPile.childNodes.length; i++) {
				...
				for (let i = ui.cardPile.length; i >= 0; i--) {
		...
		gezi_mengxiang: {
			...
			filter(event, player) {
				...
				return player.lili > lib.skill.gezi_mengxiang.gezi_cost;
			},
			content() {
				player.loselili(lib.skill.gezi_mengxiang.gezi_cost);
				player.Fuka();
				player.say('<符卡>梦想封印!');
			},
		},
		...
		gezi_stardust1: {
			...
			mod: {
				...
				selectTarget(card, player, range) {
					if (player.storage.gezi_stardust && card.type != 'equip' && Array.isArray(range) && range[1] != -1 && !get.info(card).multitarget) range[1] += player.storage.gezi_stardust;
				},
			},
		},
		//芙兰
		...
		gezi_jiesha3: {
			...
			content() {
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_jiesha2) i.gainMaxHp(i.storage.gezi_jiesha2);
				}
			},
		},
		gezi_jiesha4: {
		...
		gezi_miingyunleimi: {
			...
			content() {
				...
				if (result.bool) {
					...
					if (Array.isArray(result.cards))
						for (const i of result.cards) {
							if (i.name == 'gezi_gungnir') {
								result.cards.remove(i);
							}
						}
				...
				if (event.cards) {
					if (Array.isArray(event.cards))
						for (const i of event.cards) {
							if (i.name == 'gezi_gungnir') player.lose(i);
							i.fix();
						}
				}
				('step 3');
			...
			prompt2: '出牌阶段开始时,或你受到伤害后,你可以:获得一点灵力同时摸X张牌,并将等量牌置于牌堆顶(X为你的攻击范围),直到结束阶段:一名角色使用非装备牌指定目标时,若该牌不是以此技能使用,其取消目标并判定:若判定牌可以对目标使用(无视距离),其将判定牌对目标使用.(若你装备冈格尼尔,则无视后续效果)',
		},
		gezi_mingyunleimi2: {
			trigger: {
		...
		gezi_huanzang: {
			...
			filter(event, player) {
				...
				return player.countCards('he', function (card) {
					return card.suit == event.card.suit || card.number == event.card.number;
				});
			},
			forced: true,
			content() {
				...
				player
					.chooseCard('he', '你可以用一把与' + get.translation(trigger.card) + '相同花色/点数的飞刀把它砍断', function (card) {
						return card.suit == trigger.card.suit || card.number == trigger.card.number;
					})
					.set('ai', function (card) {
		...
		gezi_huanzang_1: {
			...
			content() {
				...
				player
					.chooseCard(
						'he',
						'你可以用一把与' + get.translation(trigger.card) + '相同花色/点数的飞刀把它砍断',
						function (card) {
							return card.suit == trigger.card.suit || card.number == trigger.card.number;
						},
						true
		...
		gezi_shijing: {
			...
			filter(event, player) {
				...
				var cards = [];
				for (let i = 0; i < player.storage.gezi_shijing.length; i++) {
		...
		gezi_shijing_mark: {
			...
			filter(event, player) {
				...
				if (Array.isArray(event.cards))
					for (const i of event.cards) {
						if (get.type(i) != 'equip' && get.type(i) != 'delay' && get.position(i) == 'd') {
							return true;
						}
					}
				return false;
			...
			content() {
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						if (!player.storage.gezi_shijing) player.storage.gezi_shijing = [i];
						else player.storage.gezi_shijing.push(i);
					}
		...
		gezi_world_skill: {
			...
			content() {
				...
				('step 1');
				player
					.chooseTarget('获得一名角色区域内一张牌,你可以使用之', function (card, player, target) {
		...
		gezi_hanghourai1: {
			...
			filter(event, player) {
				return player.storage.gezi_huanfa.length;
			},
			content() {
				'step 0';
				...
				if (result.bool && result.links.length) {
					...
					var f = [];
					for (const i of players) {
						if (trigger.target.canUse(event.card, i)) f.push(i);
					}
		...
		gezi_shuangjiang: {
			...
			content() {
				...
				var p = [];
				for (const i of players) {
					if (!i.storage.gezi_shuang && i.storage._gezi_mubiao && i != player) p.push(i);
				}
				event.p = p;
		...
		gezi_baofengxue2: {
			...
			intro: {
				content(storage, player) {
					var str = '';
					for (let i = 0; i < player.storage.gezi_baofengxue.length; i++) {
			...
			content() {
				if (trigger.card.suit && !player.storage.gezi_baofengxue.includes(trigger.card.suit)) {
					player.storage.gezi_baofengxue.push(trigger.card.suit);
			...
			mod: {
				cardEnabled(card, player) {
					if (
						...
						game.hasPlayer(function (current) {
							return current.storage.gezi_baofengxue && current.storage.gezi_baofengxue.includes(card.suit);
						})
					)
						return false;
				},
		...
		gezi_shenxuan_viewAs: {
			...
			chooseButton: {
				dialog(event, player) {
					...
					var list = [];
					for (const i of players) {
						if (i.hasSkill('gezi_shenxuan') && i.storage.gezi_shenxuan.length && get.distance(i, player, 'attack') <= 1) {
							for (var j = 0; j < i.storage.gezi_shenxuan.length; j++) {
								if (get.type(i.storage.gezi_shenxuan[j]) != 'equip') list.push(i.storage.gezi_shenxuan[j].name);
							}
						}
					}
					for (let i = 0; i < list.length; i++) {
		...
		gezi_zhenhun: {
			...
			filter(event, player) {
				...
				if (player.storage.gezi_zhenhun && player.storage.gezi_zhenhun.length) {
					for (let i = 0; i < player.storage.gezi_zhenhun.length; i++) {
		...
		gezi_zhenhun_mark: {
			...
			content() {
				...
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						if (get.position(i) == 'd') {
							player.storage.gezi_zhenhun.push(i);
						}
					}
		...
		gezi_reshenxuan_viewAs: {
			...
			chooseButton: {
				dialog(event, player) {
					...
					var list = [];
					for (const i of players) {
						if (i.hasSkill('gezi_reshenxuan') && i.storage.mingzhi.length && get.distance(i, player, 'attack') <= 1) {
							for (var j = 0; j < i.storage.mingzhi.length; j++) {
								if (get.type(i.storage.mingzhi[j]) != 'equip') list.push(i.storage.mingzhi[j].name);
							}
						}
					}
					for (let i = 0; i < list.length; i++) {
		...
		gezi_rezhenhun: {
			...
			filter(event, player) {
				...
				if (player.storage.gezi_rezhenhun && player.storage.gezi_rezhenhun.length) {
					for (let i = 0; i < player.storage.gezi_rezhenhun.length; i++) {
		...
		gezi_rezhenhun_mark: {
			...
			content() {
				...
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						if (get.position(i) == 'd') {
							player.storage.gezi_rezhenhun.push(i);
						}
					}
		...
		gezi_hezou_2: {
			...
			content() {
				...
				for (const i of players) {
					if (!trigger.player.canUse('sha', i)) players.remove(i);
				}
				var list = ['该【杀】无效'];
			...
			content() {
				...
				if (target) {
					if (trigger.name == 'equip') {
						...
						if (Array.isArray(trigger.cards))
							for (const i of trigger.cards) {
								if (i.original == 'h' && trigger.player == player && !player.storage.gezi_mingjian) {
									var es = target.getCards('e');
									for (var j = 0; j < es.length; j++) {
										var info = get.info(es[j]);
										if (info.skills) {
		...
		gezi_huanzou: {
			...
			filter(event, player) {
				...
				var list = [];
				for (let i = 0; i < event.player.storage.gezi_mingjian4.length; i++) list.push(event.player.storage.gezi_mingjian4[i].name);
		...
		gezi_rehuanzou2: {
			...
			filter(event, player) {
				if (_status.currentPhase == event.player) {
					if (Array.isArray(event.cards))
						for (const i of event.cards) {
							if (event.player.storage.mingzhi && event.player.storage.mingzhi.includes(i)) return true;
						}
				}
				return false;
			},
		...
		gezi_mingguan_viewAs: {
			...
			filterCard(card, player) {
				...
				var list = [];
				for (const i of players) {
					if (i.hasSkill('gezi_mingguan') && get.distance(i, player, 'attack') <= 1 && i.storage.gezi_mingguan.length) {
						for (var j = 0; j < i.storage.gezi_mingguan.length; j++) list.push(i.storage.gezi_mingguan[j].name);
					}
				}
		...
		gezi_remingguan_viewAs: {
			...
			filterCard(card, player) {
				...
				var list = [];
				for (const i of players) {
					if (i.hasSkill('gezi_remingguan') && get.distance(i, player, 'attack') <= 1 && i.storage.mingzhi) {
						for (var j = 0; j < i.storage.mingzhi.length; j++) list.push(i.storage.mingzhi[j].name);
					}
				}
				return list.includes(card.name);
			},
		...
		gezi_shiqu2: {
			...
			forced: true,
			filter(event, player) {
				if (event.parent.name == 'gezi_shiqu2') return false;
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.hasSkill('gezi_shiqu2') && i != player) return true;
				}
				return false;
			},
			content() {
		...
		gezi_yinhuashan: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_yinhuashan.gezi_cost;
			},
			content() {
				player.loselili(lib.skill.gezi_yinhuashan.gezi_cost);
				player.Fuka();
				player.say('<符卡>六根清净斩!');
			},
		},
		gezi_yinhuashan2: {
			...
			filter(event, player) {
				return event.card && event.card.name == 'sha' && player.lili;
			},
			forced: true,
			content() {
				'step 0';
				...
				if (result.control) {
					if (result.control == '重置一闪') {
						player.loselili();
						player.getStat('triggerSkill').gezi_yishan--;
					}
		...
		gezi_youdie: {
			...
			content() {
				...
				for (const i of players) {
					if (nmax > 1 && i.hp == nmax && get.attitude(player, i) > 0) num -= 2;
					if (nmax == 1 && i.hp == nmax && get.attitude(player, i) > 0) num--;
					if (nmax > 1 && i.hp == nmax && get.attitude(player, i) <= 0) num++;
					if (nmax == 1 && i.hp == nmax && get.attitude(player, i) <= 0) num += 2;
				}
				player
		...
		gezi_zhaixing: {
			...
			intro: {
				content(storage, player) {
					var str = '';
					for (let i = 0; i < player.storage.gezi_zhaixing.length; i++) {
		...
		gezi_zhaixing_mark: {
			...
			content() {
				if (!player.storage.gezi_zhaixing.includes(trigger.card.suit)) {
					player.storage.gezi_zhaixing.push(trigger.card.suit);
					player.gainlili();
				}
		...
		gezi_lanyue: {
			...
			filterTarget(card, player, target) {
				...
				var length = 0;
				for (const i of players) {
					if (get.distance(player, i, 'attack') > 1) players.remove(i);
					else if (get.distance(player, i) > length) length = get.distance(player, i);
				}
				...
				if (result.control) {
					...
					for (var i in cards) {
						switch (result.control) {
							case '牌名长度':
								valid.push(get.translation(i.name).length);
								break;
							case '花色':
								valid.push(i.suit);
								break;
							case '点数':
								...
								break;
							case '种类':
								valid.push(get.type(i));
								break;
							case '颜色':
								valid.push(get.color(i));
								break;
						}
					}
						...
						.chooseCard('是否交给' + get.translation(player) + '一张与' + get.translation(result.cards) + '不同' + result.control + '的牌？', 'he', function (card) {
							switch (result.control) {
								case '牌名长度':
									...
									break;
								case '花色':
									return !valid.includes(card.suit);
									break;
								case '点数':
									return !valid.includes(card.number);
									break;
								case '种类':
		...
		gezi_poxiao: {
			...
			content() {
				...
				if (Array.isArray(cards))
					for (const i of cards) {
						if (i.suit && !suits.includes(i.suit)) suits.push(i.suit);
					}
		...
		gezi_richuguo: {
			...
			filter(event, player) {
				...
				if (Array.isArray(cards))
					for (const i of cards) {
						if (get.type(i) && !types.includes(get.type(i))) types.push(get.type(i));
					}
			...
			check(event, player) {
				...
				var t = false;
				for (const i of players) {
					if (get.recoverEffect(i, player, player) > 0 && (i.hp < 2 || i.Maxhp - i.hp >= 2)) {
						t = true;
					}
				}
				return t;
		...
		gezi_yuhuo: {
			...
			subSkill: {
				...
				count: {
					...
					silent: true,
					content() {
						if (!player.storage.gezi_yuhuo) player.storage.gezi_yuhuo = [trigger.card.suit];
						else player.storage.gezi_yuhuo.push(trigger.card.suit);
					},
					forced: true,
					popup: false,
				},
			...
			filter(event, player) {
				...
				if (player.getStat('triggerSkill').gezi_yuhuo && player.getStat('triggerSkill').gezi_yuhuo >= player.maxHp - player.hp) return false;
				return true;
			},
			filterCard(card, player) {
				if (!player.storage.gezi_yuhuo) return true;
				return !player.storage.gezi_yuhuo.includes(card.suit);
			},
			position: 'he',
			viewAs: {
				name: 'sha',
			...
			onuse(event, player) {
				if (!player.getStat('triggerSkill').gezi_yuhuo) {
					player.getStat('triggerSkill').gezi_yuhuo = 1;
				} else {
					player.getStat('triggerSkill').gezi_yuhuo++;
				}
			},
		...
		gezi_huanshi: {
			...
			content() {
				...
				for (var i in lib.card) {
					...
					for (let i = 0; i < player.storage.gezi_huanshi_4.length; i++) list.remove(player.storage.gezi_huanshi_4[i]);
				}
				for (let i = 0; i < list.length; i++) {
					list[i] = [get.type(list[i]), '', list[i]];
				}
			...
			ai: {
				...
				result: {
					player(player, target) {
						var players = game.filterPlayer();
						for (const i of players) {
							if (!i.hasSkill('gezi_huanshi_3')) return 1;
						}
					},
				},
				threaten: 1.5,
		...
		gezi_huanshi_2: {
			...
			forced: true,
			filter(event, player) {
				return event.parent.parent.name == 'gezi_huanshi';
			},
		...
		gezi_kaiyun: {
			...
			filter(event, player) {
				...
				for (const i of players) {
					...
					if (i.storage._gezi_mubiao) return false;
				}
				return player.countCards('he');
			},
			forced: true,
		...
		gezi_kaiyun_1: {
			...
			prompt: '交给帝一张牌,获得一张【神佑】,本回合不能对你和她以外的人用牌',
			content() {
				'step 0';
				player.chooseCardTarget({
					prompt: '交给帝一张牌,获得一张【神佑】,本回合不能对你和她以外的人用牌',
		...
		gezi_mitu: {
			...
			filter(event, player) {
				...
				if (get.distance(player, event.target, 'attack') > 1) return false;
				for (let i = 0; i < player.storage.gezi_mitu.length; i++) {
					if (event.card.name == player.storage.gezi_mitu[i].name || event.card.suit == player.storage.gezi_mitu[i].suit) {
						return true;
					}
				}
			},
			...
			content() {
				...
				player
					...
					.set('filterButton', function (button) {
						return button.name == trigger.card.name || button.suit == trigger.card.suit;
					})
					.set('ai', function (button) {
						return choice;
					})
		...
		gezi_mitu_2: {
			...
			filter(event, player) {
				return player.storage.gezi_mitu && player.storage.gezi_mitu.length;
			},
			forced: true,
			content() {
				'step 0';
		...
		gezi_yingguang: {
			...
			usable: 3,
			filter(event, player) {
				if (!player.hasSkill('gezi_yechong1') && player.getStat('triggerSkill').gezi_yingguang && player.getStat('triggerSkill').gezi_yingguang >= 1) return false;
				return true;
			},
			content() {
				player.gainlili();
			...
			content() {
				...
				if (result.bool && result.links && result.links.length) {
					if (get.type(result.links[0]) == 'jinengpai') {
						...
						for (const i of cards) {
							if (i && i.name == result.links[0].name) {
								event.targets[event.num].removeJudgen(i);
								break;
							}
						}
		...
		gezi_guihang_cost: {
			...
			forced: true,
			filter(event, player) {
				return event.parent.skill == 'gezi_guihang';
			},
			content() {
		...
		gezi_wujian: {
			...
			content() {
				...
				for (const i of players) {
					...
					if (get.attitude(player, i) > 0) choice--;
					if (get.attitude(player, i) <= 0) choice++;
				}
				player
					.chooseControl(list, function (event, player) {
		...
		gezi_chunmian: {
			...
			content() {
				...
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_chunmian) {
						i.draw();
						i.storage.gezi_chunmian = 0;
					}
				}
			},
		},
		...
		gezi_huayuan: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_huayuan.gezi_cost;
			},
			content() {
				player.loselili(lib.skill.gezi_huayuan.gezi_cost);
				player.say('<符卡>毒气花园!');
				player.Fuka();
			},
			check(event, player) {
		...
		gezi_kuanglan: {
			...
			filter(event, player) {
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_kuanglan && player != i) return true;
				}
				return false;
			},
			forced: true,
		...
		gezi_kuanglan_4: {
			...
			filter(event, player) {
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_kuanglan) return true;
				}
				return false;
			},
			content() {
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_kuanglan) delete i.storage.gezi_kuanglan;
				}
			},
		},
		gezi_kuanglan_5: {
		...
		gezi_fengmi: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_fengmi.gezi_cost;
			},
			check(event, player) {
				return player.lili > 1;
			},
			content() {
				player.loselili(lib.skill.gezi_fengmi.gezi_cost);
				player.Fuka();
				player.say('「幻想风靡」!');
			},
		},
		...
		gezi_nianxie: {
			...
			content() {
				...
				player.judge(function (card) {
					if (!player.storage.gezi_nianxie_storage.includes(card.suit)) return -1;
					return 1;
				});
				('step 1');
				event.card = result.card;
				if (player.storage.gezi_nianxie_storage.includes(result.card.suit)) {
					player
		...
		gezi_nianxie_storage: {
			...
			intro: {
				content(storage, player) {
					var str = '';
					for (let i = 0; i < player.storage.gezi_nianxie_storage.length; i++) {
					...
					for (const i of trigger.cards) {
						if (get.position(i) == 'd') {
							if (!player.storage.gezi_nianxie_storage) {
								player.storage.gezi_nianxie_storage = [i.suit];
							} else {
								if (!player.storage.gezi_nianxie_storage.includes(i.suit)) {
									player.storage.gezi_nianxie_storage.push(i.suit);
								}
							}
						}
					}
		...
		gezi_lianxu2: {
			...
			selectCard: 1,
			filterCard(card, player) {
				return player.storage.gezi_lianxu3.includes(card.number) || player.storage.gezi_lianxu3.includes(card.suit);
			},
			content() {
				if (lib.config.background_audio) {
					game.playlili('shutter');
		...
		gezi_lianxu3: {
			...
			intro: {
				content(storage, player) {
					var str = '';
					for (let i = 0; i < player.storage.gezi_lianxu3.length; i++) {
					...
					for (const i of trigger.cards) {
						if (get.position(i) == 'd') {
							if (!player.storage.gezi_lianxu3) {
								player.storage.gezi_lianxu3 = [i.number, i.suit];
							} else {
								if (!player.storage.gezi_lianxu3.includes(i.number)) {
									player.storage.gezi_lianxu3.push(i.number);
								}
								if (!player.storage.gezi_lianxu3.includes(i.suit)) {
									player.storage.gezi_lianxu3.push(i.suit);
								}
							}
						}
					}
		...
		library_mengji2: {
			...
			content() {
				...
				if (player.storage._tanpai) {
					...
					for (let i = 0; i < get.info(name).incidentskills.length; i++) {
						player.removeSkill(get.info(name).incidentskills[i]);
					}
					for (let i = 0; i < get.info(name).skills.length; i++) {
		...
		saiqian_die: {
			...
			filter(event, player) {
				...
				if (Array.isArray(event.cards))
					for (const i of event.cards) {
						if (i.name == 'gezi_saiqianxiang' && get.position(i) == 'd') return true;
					}
				return false;
			},
			content() {
		...
		gezi_mengxiangtiansheng: {
			...
			content() {
				var players = game.filterPlayer();
				for (const i of players) {
					if (!player.canUse('sha', i, false)) players.remove(i);
				}
				players.remove(player);
				player.loselili();
				player.useCard(
		...
		gezi_juewang_1: {
			...
			filter(event, player) {
				return get.tag(event.card, 'damage') == 0 && !player.storage.gezi_juewang_1.includes(event.card.suit) && event.card.suit;
			},
			content() {
				player.storage.gezi_juewang_1.push(event.card.suit);
			},
		...
		gezi_juewang_2: {
			...
			content() {
				'step 0';
				event.players = game.filterPlayer(function (current) {
					return current.isEnemiesOf(player) && current.maxHp <= 2;
				});
				('step 1');
		...
		gezi_lianyu1: {
			...
			subSkill: {
				kill: {
					...
					forced: true,
					filter(event, player) {
						if (event.player.hasSkill('revive_boss') && game.me.storage.reskill && game.me.storage.reskill.length) return false;
						return true;
					},
					content() {
						'step 0';
		...
		gezi_zuanshi: {
			...
			content() {
				...
				if (Array.isArray(result) && result.length) {
					...
					if (Array.isArray(cards))
						for (const i of cards) {
							player.storage.gezi_zuanshi.push(i.name);
						}
		...
		gezi_shiting: {
			...
			forced: true,
			content() {
				for (let i = 0; i < player.storage.gezi_shiting.length; i++) {
		...
		gezi_shiting_mark: {
			...
			content() {
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						if (!player.storage.gezi_shiting) player.storage.gezi_shiting = [i];
						else player.storage.gezi_shiting.push(i);
					}
		...
		gezi_jingjie: {
			init(player) {
				for (const i of game.players) {
					if (!i.storage.gezi_jingjie) i.storage.gezi_jingjie = 0;
				}
			},
			intro: {
				content(storage) {
			...
			subSkill: {
				...
				fengyinRelease: {
					onremove(player, skill) {
						//alert(1);
						console.warn('player.storage.gezi_yazhi_fengyinRelease:', player.storage.gezi_yazhi_fengyinRelease);
		...
		gezi_huozhong: {
			init(player) {
				for (const i of game.players) {
					if (!i.storage.gezi_huozhong) i.storage.gezi_huozhong = 0;
				}
			},
			intro: {
				content: 'mark',
		...
		gezi_bazhi: {
			...
			content() {
				...
				var num = 0;
				for (const i of game.players) {
					if (i.storage.gezi_huozhong) {
						num += i.storage.gezi_huozhong;
						delete i.storage.gezi_huozhong;
					}
				}
				player.maxHp = num;
		...
		gezi_miaohui: {
			...
			content() {
				...
				const list = ['game.me.draw()<br>你抽一张牌', 'game.me.gainlili()<br>你获得1点灵力', 'game.players[1].damage()<br>对下家造成1点伤害'];
				if (player.hp < player.maxHp) {
					list.push('game.me.recover()<br>你回复1点体力');
				}
				if (get.mode() == 'identity' || get.mode() == 'old_identity') {
					list.push('game.players[1].setIdentity(game.players[1].identity)<br>展示下家的身份');
				}
				if (get.mode() != 'identity') {
					list.push("game.me.addIncident(game.createCard('gezi_scarlet'))<br>你获得【红月】异变牌");
				}
				if (Object.keys(lib.config.monika).length >= 5) {
					list.push("lib.skill.gezi_miaohui.usable = Infinity<br>【描绘】改为'一回合无限次'");
				}
				if (Object.keys(lib.config.monika).length >= 10) {
					list.push("game.removeCard('sha')<br>移除牌堆里的所有【杀】");
				}
				if (Object.keys(lib.config.monika).length >= 15) {
					list.push('game.me.insertPhase()<br>你进行一个额外的回合');
				}
				if (Object.keys(lib.config.monika).length >= 20) {
					list.push('game.over(true)<br>你获得游戏胜利');
				}
				if (event.isMine()) {
					game.pause();
					...
					const str = list.randomGet();
					const str2 = str.split('<br>')[0];
					eval(str2);
				}
			},
		},
		gezi_kehua: {
			...
			content() {
				...
				if (result.index == 0) {
					...
					var list = [];
					for (let i = 0; i < lib.character[event.character][3].length; i++) {
				...
				if (event.index == 0) {
					...
					if (result.control) {
						...
						for (const i of game.players) {
							if (i.name == event.character) {
								for (var j = 0; j < i.skills.length; j++) {
									if (get.translation(i.skills[j]) == result.control) {
										i.removeSkill(i.skills[j]);
									}
								}
							}
						}
		...
		gezi_huanri: {
			...
			filter(event, player) {
				return event.num > 0 && ui.cardPile.childNodes.length;
			},
			check(event, player) {
		...
		gezi_zhonger: {
			init(player) {
				...
				lib.translate.gezi_shenyou_info = '锁定技,你的判定牌生效前,你移除一张【神佑】,令之视为♥️️;你受到伤害时,若伤害大于你的体力值,你移除一张【神佑】,防止该伤害.</br><u>特殊效果:回复一点体力.若你的手牌为全场最小或之一,你摸2张牌.</u>';
		...
		gezi_tiequan: {
			...
			content() {
				...
				player.loselili(num);
				if (num > 1) trigger.parent.directHit.push(trigger.target);
				if (num > 0) {
		...
		gezi_mengsui: {
			...
			content() {
				...
				trigger.cancel();
				player.node.avatar.setBackgroundImage('extension/东方project/image/gezi_FSakuraawake.jpg');
				var tgs = game.filterPlayer((target) => target != player);
				for (const i of tgs) i.addSkillLog('gezi_jiushuying');
		...
		gezi_xuwu: {
			...
			forced: true,
			filter(event, player) {
				return game.filterPlayer((current) => current.inRangeOf(player)).length;
			},
			content() {
				var tgs = game.filterPlayer((current) => current.inRangeOf(player));
				for (const i of tgs) i.damage('thunder');
			...
			chooseButton: {
				...
				filter(button, player) {
					...
					for (var k of numlist) {
						if (_status.event.parent.filterCard({ name: button.link[2] }, player, _status.event.parent) && list[k][0] <= get.translation(button.link[2]).length && get.translation(button.link[2]).length <= list[k][1]) return true;
					}
				},
				check(button) {
					var player = _status.event.player;
		...
		gezi_jinbei: {
			...
			filter(event, player) {
				...
				if (event.source != undefined && event.source != player && event.cards && event.cards.length && event.source.countCards('hej') > 0) return true;
				return false;
			},
			content() {
		...
		gezi_daomo: {
			...
			forced: true,
			filter(event, player) {
				return event.card && event.card.name == 'sha' && event.targets.length == 1 && player.countCards('hs', (card) => player.hasUseTarget(card) && !get.tag(card, 'damage')) > 0;
			},
			content() {
				'step 0';
				player.chooseToUse(
		...
		gezi_chuancheng: {
			...
			//你强化牌时,可以令灵力大于你的角色选择是否代替你消耗灵力;灵力小于你的其他角色使用牌强化时,其可以选择令你代替消耗灵力;以此法强化过的牌结算后,你摸一张牌
			async contentx(event, trigger, player) {
				for (const target of event.targets) {
					const { bool } = await target
						.chooseBool('是否替' + get.translation(player) + '强化' + get.translation(card.name))
						.set('ai', () => get.attitude(target, player) > 0)
						.forResult();
					if (bool) {
						break;
						target.loselili(get.info(event.card).enhance);
						game.log(get.translation(player) + '强化了' + get.translation(event.card.name) + '');
						if (!player.storage._enhance) {
							player.storage._enhance = 1;
						} else {
							player.storage._enhance += 1;
						...
						event.card.storage.enhance = true;
					}
				}
			}, //QQQ
			ai: {
			...
			subSkill: {
				draw: {
					...
					filter(event, player) {
						return event.card && event.card.storage && event.card.storage.enhance;
					},
					forced: true,
					content() {
						player.draw();
		...
		gezi_chaoyue: {
			...
			nobracket: true,
			filter(event, player) {
				return player.lili > lib.skill.gezi_chaoyue.gezi_cost - player.storage.gezi_chaoyue;
			},
			content() {
				player.loselili(lib.skill.gezi_chaoyue.gezi_cost - player.storage.gezi_chaoyue);
				player.Fuka();
				player.storage.gezi_chaoyue = 0;
		...
		gezi_huanzhao1: {
			...
			content() {
				...
				for (const i of game.players) {
					player.storage.gezi_huanzhao.list.remove([i.name]);
					player.storage.gezi_huanzhao.list.remove([i.name1]);
					player.storage.gezi_huanzhao.list.remove([i.name2]);
				}
		...
		gezi_huanzhao2: {
			...
			content() {
				...
				('step 6');
				if (result.bool) {
					for (let i = 1; i < lib.character[event.char][3].length; i++) {
		...
		gezi_touying: {
			...
			filter(event, player) {
				if (player.getStat('triggerSkill').gezi_touying_target && player.getStat('triggerSkill').gezi_touying_target >= 1) return false;
			...
			content() {
				...
				if (result.cards) {
					...
					player.equip(card1);
					if (!player.getStat('triggerSkill').gezi_touying_target) {
						player.getStat('triggerSkill').gezi_touying_target = 1;
					} else {
						player.getStat('triggerSkill').gezi_touying_target++;
					}
				}
			},
			ai: {
		...
		gezi_touying_target: {
			audio: 'ext:东方project/audio:2', //QQQ
			trigger: {
				target: 'useCardToBegin',
			},
			filter(event, player) {
		...
		gezi_touying_recast: {
			...
			filter(event, player) {
				...
				if (Array.isArray(cards))
					for (const i of cards) {
						if (i && i.dataset.cardType == 'touying') {
							return true;
						}
					}
			},
		...
		gezi_heyi: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_heyi.gezi_cost;
			},
			content() {
				player.loselili(lib.skill.gezi_heyi.gezi_cost);
				player.Fuka();
			},
			check(event, player) {
		...
		gezi_heyi_skill: {
			...
			init(player) {
				if (!player.getStat('triggerSkill').gezi_touying_target) {
					player.getStat('triggerSkill').gezi_touying_target = -2;
				}
			},
			onremove(player) {
				if (player.getStat('triggerSkill').gezi_touying_target > -2) {
					player.getStat('triggerSkill').gezi_touying_target = 1;
				} else {
					player.getStat('triggerSkill').gezi_touying_target = 0;
				}
			},
			filter(event, player) {
				return player.countCards('e');
		...
		gezi_muqi: {
			...
			filter(event, player) {
				...
				for (const i of lib.inpile) {
					if (event.filterCard({ name: i }, player, event)) {
						if (get.type(i) == 'basic') list.push(['基础', '', i]);
						if (get.type(i) == 'trick') list.push(['锦囊', '', i]);
					}
				} //QQQ
				return player.countCards('he') && _status.currentPhase == player && list.length;
			},
			chooseButton: {
				dialog(event, player) {
					const list = [];
				...
				setTimeout(function () {
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							...
							setTimeout(function () {
								player.say('赞颂吧!黄金的剧场!!');
								player.$skill('黄金剧场', null, null, true);
								ui.background.setBackgroundImage('extension/东方project/image/nero.jpg');
								game.resume();
							}, 1500);
						}, 1500);
					}, 1500);
			...
			chooseButton: {
				...
				backup(links, player) {
					return {
						...
						content() {
							player.addJudgen(game.createCard(event.parent.parent._result.links[0][2], '', ''));
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							...
							setTimeout(function () {
								...
								ui.background.setBackgroundImage('extension/东方project/image/nerobridge.png');
								game.resume();
							}, 1500);
						}, 1500);
					}, 1500);
		...
		gezi_ruizhi: {
			...
			content() {
				...
				if (Array.isArray(event.cards))
					for (const i of event.cards) {
						if (i.suit == 'spade') {
							if (!list.includes('♠️:获得2点灵力')) list.push('♠️:获得2点灵力');
						} else if (i.suit == 'club') {
							if (!list.includes('♣️:将一名角色的一张牌置于牌堆顶')) list.push('♣️:将一名角色的一张牌置于牌堆顶');
						} else if (i.suit == 'diamond') {
				...
				player
					.chooseControl(event.list)
					.set('ai', function () {
						if (event.list.includes('♣️:将一名角色的一张牌置于牌堆顶')) return '♣️:将一名角色的一张牌置于牌堆顶';
						if (event.list.includes('♢:视为使用一张【杀】')) return '♢:视为使用一张【杀】';
						if (event.list.includes('♠️:获得2点灵力') && player.lili < 2) return '♠️:获得2点灵力';
						return event.list.randomGet();
					})
					.set('prompt', str);
				('step 5');
				event.control = result.control;
				if (result.control == '♠️:获得2点灵力') {
					player.gainlili(2);
				} else if (result.control == '♣️:将一名角色的一张牌置于牌堆顶') {
					player
				...
				('step 8');
				if (event.card && event.control == '♣️:将一名角色的一张牌置于牌堆顶') {
		...
		gezi_mojing0: {
			init() {
				lib.translate.gezi_ruizhi_info = '准备阶段,你可以判定3次,选择所有项:若结果中有♠️️,你获得2点灵力;若有♦️️,你视为使用一张无视距离的【杀】;若有♣️️,你将一张角色的一张牌置于牌堆顶';
			},
		},
		gezi_mojing1: {
			trigger: {
		...
		gezi_shuitian_1: {
			...
			content() {
				...
				if (player.storage.gezi_shuitian) {
					...
					var players = game.filterPlayer();
					for (const i of players) {
						delete i.storage.shuihp;
						delete i.storage.shuilili;
					}
				}
			},
		},
			...
			content() {
				if (trigger.name == 'phaseJieshu') {
					if (
						...
						for (const i of game.players) i.removeSkill('gezi_yuanfen_yuan');
					}
		...
		gezi_haruhi1: {
			...
			content() {
				...
				if (result.bool) {
					...
					let mode = 'standard';
					for (const i of result.links) {
						var card = [i.suit, i.number.toString(), i.name];
		...
		gezi_huoli: {
			...
			forced: true,
			filter(event, player) {
				if (player.getStat('triggerSkill').gezi_huoli) return false;
				return player.countCards('he');
			},
			content() {
				'step 0';
				...
				('step 1');
				if (result.bool) {
					if (!player.getStat('triggerSkill').gezi_huoli) {
						player.getStat('triggerSkill').gezi_huoli = 1;
					} else {
						player.getStat('triggerSkill').gezi_huoli++;
					}
		...
		gezi_shenyuan: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_shenyuan.gezi_cost;
			},
			content() {
				player.loselili(lib.skill.gezi_shenyuan.gezi_cost);
				player.Fuka();
			},
			check(event, player) {
				return (
		...
		gezi_honglian: {
			...
			filter(event, player) {
				if (event.player == player) return false;
				if (_status.currentPhase != player) return false;
				if (player.getStat('triggerSkill').gezi_honglian) return false;
				return event.player.isAlive();
			},
			forced: true,
			content() {
				...
				if (event.list[result.index] == '视为对' + get.translation(trigger.player) + '出一张杀') {
					...
					if (!player.getStat('triggerSkill').gezi_honglian) {
						player.getStat('triggerSkill').gezi_honglian = 1;
					} else {
						player.getStat('triggerSkill').gezi_honglian++;
					}
				}
		...
		sbrs_liansuo: {
			...
			filter(event, player) {
				if (event.player == player) return false;
				if (_status.currentPhase != player) return false;
				if (player.getStat('triggerSkill').sbrs_liansuo) return false;
				if (event.parent.name == 'discard' || event.parent.name == 'gain' || event.getParent(2).name == 'gainPlayerCard') {
					return true;
				}
				return false;
			},
			content() {
				...
				if (event.list[result.index] == '视为对' + get.translation(trigger.player) + '出一张杀') {
					...
					if (!player.getStat('triggerSkill').sbrs_liansuo) {
						player.getStat('triggerSkill').sbrs_liansuo = 1;
					} else {
						player.getStat('triggerSkill').sbrs_liansuo++;
					}
				}
				if (event.list[result.index] == '视为对距离内的' + get.translation(event.players) + '出一张杀') {
					for (const i of event.players) {
						i.addTempSkill('sbrs_liansuo_2');
					}
					player.useCard(
						{
							name: 'sha',
		...
		sbrs_liansuo_2: {
			...
			filter(event, player) {
				return event.parent.parent.name == 'sbrs_liansuo';
			},
			forced: true,
			content() {
				'step 0';
		...
		gezi_explosion: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_explosion.gezi_cost;
			},
			forced: true,
			content() {
				'step 0';
				...
				('step 1');
				if (result.bool) {
					player.loselili(lib.skill.gezi_explosion.gezi_cost);
					player.Fuka();
					player.skip('phaseUse');
					player.skip('phaseDiscard');
		...
		ng_pinjian3: {
			...
			filter(event, player) {
				if (!(event.type == 'compare' && !event.directresult && player.getLili() > 0)) {
					return false;
				}
				const evt = event.parent;
				if (evt.player && evt.target) {
					if (player == evt.target) {
						return evt.player.countCards('h') > 0 && evt.player.countCards('hej') > 1;
					}
		...
		ClarentBloodArthur: {
			...
			nobracket: true,
			content() {
				player.loselili(lib.skill.ClarentBloodArthur.gezi_cost);
				player.Fuka();
		...
		gezi_daxianzhe: {
			...
			content() {
				...
				if (map && map.skills && map.skills.length) {
					for (const i of map.skills) player.addTempSkill(i);
				}
			},
		},
		//阿尔托莉雅
		gezi_shengguang: {
			...
			async content(event, trigger, player) {
				...
				const { bool } = await player.chooseToCompare(pl).forResult();
			...
			subSkill: {
				glory: {
					...
					content() {
						...
						('step 4');
						player.chooseBool('是否令' + get.translation(event.current) + '获得一点灵力？').set('ai', () => get.attitude(player, event.current) > 0); //QQQ
						('step 5');
		...
		gezi_zuoshibaozhan: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return player.lili > lib.skill.gezi_zuoshibaozhan.gezi_cost;
			},
			check(event, player) {
			...
			content() {
				'step 0';
				player.loselili(lib.skill.gezi_zuoshibaozhan.gezi_cost);
				player.Fuka();
				('step 1');
				if (player.countCards('h') < 3) player.draw(3 - player.countCards('h'));
				('step 2');
		...
		gezi_solomon: {
			...
			filter(event, player) {
				if (player.node.fuka) return false;
				return (player.hp < 3 && player.lili > 0) || player.lili > lib.skill.gezi_solomon.gezi_cost;
			},
			check(event, player) {
				return true;
			},
			content() {
				...
				if (player.hp < 3) {
					...
					player.loselili(lib.skill.gezi_solomon.gezi_cost);
				}
				player.Fuka();
				('step 1');
		...
		WLD1: {
			...
			prompt: '先弃置任意张牌,消耗任意点灵力,摸等量轩辕剑拓展的牌',
			ai: {
				order: 9,
				result: {
					player: 1,
		...
		gezi_wulin: {
			...
			check(event, player) {
				...
				var players = game.filterPlayer();
				for (const i of players) {
					if (-get.attitude(player, i) && i.countCards('h') > 3) bad_att = true;
				}
		...
		gezi_maria: {
			...
			trigger: { player: 'phaseBegin' },
			filter(event, player) {
				return player.getLili() > lib.skill.gezi_maria.gezi_cost;
			},
			nobracket: true,
			content() {
				player.loselili(lib.skill.gezi_maria.gezi_cost);
				player.Fuka();
			},
			check(event, player) {
		...
		gezi_shishu: {
			...
			filter(event, player) {
				...
				if (player.storage.gezi_shishu3 && player.storage.gezi_shishu3.length) {
					for (let i = 0; i < player.storage.gezi_shishu3.length; i++) {
			...
			content() {
				...
				if (result.links) {
					...
					for (const i of result.links) {
						ui.discardPile.remove(i);
						player.storage.gezi_shishu3.remove(i);
					}
				}
				('step 4');
		...
		gezi_shishu3: {
			...
			filter(event, player) {
				...
				if (Array.isArray(event.cards))
					for (const i of event.cards) {
						if (get.type(i) != 'jinjipai' && get.type(i) != 'equip') continue;
						if (get.type(i) == 'equip' && event.parent.name == 'useCard' && event.parent.card.name == i.name) continue;
						if (get.position(i) == 'd') {
							return true;
						}
					}
			...
			content() {
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						if (get.type(i) != 'jinjipai' && get.type(i) != 'equip') continue;
						if (!player.storage.gezi_shishu3) player.storage.gezi_shishu3 = [i];
						else player.storage.gezi_shishu3.push(i);
					}
		...
		gezi_lvtu: {
			...
			filter(event, player) {
				...
				for (let i = 0; i < event.num.length; i++) {
					if (event.num[i] == low) {
		...
		gezi_weimo: {
			...
			roundi: true,
			filter(event, player) {
				return player.lili > lib.skill.gezi_weimo.gezi_cost;
			},
			nobracket: true,
			content() {
				player.loselili(lib.skill.gezi_weimo.gezi_cost);
				player.Fuka();
			},
			check(event, player) {
				return player.hp < 2;
		...
		gezi_guyin_3: {
			...
			forced: true,
			filter(event, player) {
				return event.card && event.card.name == 'shan' && !event.skill && event.cards.length == 1 && event.cards[0].name == 'shan';
			},
			content() { },
		},
		gezi_guyin_4: {
			...
			forced: true,
			filter(event, player) {
				return event.card && event.card.name == 'tao' && !event.skill && event.cards.length == 1 && event.cards[0].name == 'jiu';
			},
			content() { },
		},
		gezi_tianze: {
		...
		gezi_tianze2: {
			...
			filter(event, player) {
				if (!event.target) return false;
				return event.card && event.card.suit == 'heart' && player.lili > 1;
			},
			content() {
				'step 0';
				player.loselili();
			...
			ai: {
				effect: {
					target(card, player, target) {
						if (target.lili > 1 && card.suit == 'heart') {
							if (get.attitude(player, target) > 0) {
								return 1.4;
							} else {
								return 0.8;
		...
		gezi_yueding1: {
			...
			prompt2: '你使用【杀】指定目标后,可以弃置目标一张牌;若目标没有装备牌,你摸一张牌.',
			ai: {
				expose: 0.2,
			},
			check(event, player) {
		...
		gezi_huanrao_3: {
			...
			content() {
				...
				if (Array.isArray(trigger.cards))
					for (const i of trigger.cards) {
						player.storage.gezi_huanrao.add(i);
					}
				player.showCards(trigger.cards);
			},
		},
		gezi_huanrao_4: {
			...
			forced: true,
			filter(event, player) {
				return event.card && event.card.name == 'sha' && !event.skill && event.cards.length == 1 && player.storage.gezi_huanrao.includes(event.cards[0]);
			},
			content() { },
		},
		sliver_arrow: {
			...
			filter(event, player) {
				return player.lili > lib.skill.sliver_arrow.gezi_cost;
			},
			check(event, player) {
				return game.countPlayer(function (current) {
			...
			content() {
				player.loselili(lib.skill.sliver_arrow.gezi_cost);
				player.Fuka();
			},
		},
		sliver_arrow_2: {
		...
		gezi_time: {
			...
			content() {
				...
				var sort;
				for (let i = 0; i < lib.config.gameRecord.gezi_homura.length; i++) {
		...
		gezi_time2: {
			...
			content() {
				var gezi_homura = [];
				for (let i = 0; i < player.storage.gezi_time.length; i++) {
					gezi_homura.push({
			...
			chooseButton: {
				...
				backup(links, player) {
					return {
						selectCard: 0,
						content() {
							var cards = event.parent.parent._result.links;
							player.$gain(cards);
							player.gain(cards);
							if (Array.isArray(cards))
								for (const i of cards) {
									player.storage.gezi_time.remove(i);
								}
							// 无可奈何的改法,以后希望可以有别的方法改
							event.parent.parent.goto(0);
							var gezi_homura = [];
							for (let i = 0; i < player.storage.gezi_time.length; i++) {
								gezi_homura.push({
									name: player.storage.gezi_time[i].name,
		...
		gezi_zhongzou: {
			...
			content() {
				...
				var players = game.filterPlayer();
				for (const i of players) {
					if (i.storage.gezi_zhongzou == true) targets.push(i);
				}
				if (targets.length)
					player.useCard(
						{
			...
			check(event, player) {
				...
				var players = game.filterPlayer();
				for (const i of players) {
					if (!i.storage.gezi_zhongzou) continue;
					num += get.effect(
						i,
						{
							name: 'sha',
		...
		gezi_moxin: {
			...
			content() {
				...
				if (trigger.player.lili < trigger.player.maxlili) {
					list.push('令' + get.translation(trigger.player) + '回复灵力');
				}
				list.push('摸一张牌,可以交给' + get.translation(trigger.player) + '一张牌');
				event.list = list;
				player.chooseControlList(event.list, function (event, player) {
					if (get.attitude(player, trigger.player) <= 0) return event.list.indexOf('摸一张牌,可以交给' + get.translation(trigger.player) + '一张牌');
					if (!_status.currentPhase.node.fuka && _status.currentPhase.lili < 3) return event.list.indexOf('令' + get.translation(trigger.player) + '回复灵力');
					return event.list.indexOf('摸一张牌,可以交给' + get.translation(trigger.player) + '一张牌');
				});
				('step 1');
				if (event.list[result.index] == '令' + get.translation(trigger.player) + '回复灵力') {
					trigger.player.gainlili();
				} else if (event.list[result.index] == '摸一张牌,可以交给' + get.translation(trigger.player) + '一张牌') {
					player.draw();
					if (trigger.player != player) {
						player.chooseCard('he', '可以交给' + get.translation(trigger.player) + '一张牌').set('ai', function (card) {
							var player = _status.event.player;
		...
		gezi_lunwu: {
			...
			content() {
				...
				for (const i of players) {
					if (i == player) continue;
					if (target.inRange(i)) i.damage('thunder');
				}
			},
			prompt: '低音炮向谁发射呢？',
			ai: {
		...
		gezi_qingqu: {
			...
			forced: true,
			filter(event, player) {
				return player.lili > 0 && player.getCards('h', (card) => !player.getShownCards().includes(card)).length;
			},
			content() {
				'step 0';
		...
		gezi_geying: {
			...
			async content(event, trigger, player) {
				const { cards } = await player.chooseCard('h', true).forResult();
		...
		gezi_wuyan: {
			...
			intro: {
				...
				content(storage, player) {
					...
					cards.forEach((card) => suit[card.suit]++);
		...
		shrine4: {
			...
			content() {
				...
				if (
					...
					var players = game.filterPlayer();
					for (const i of players) {
						if (i.name == 'gezi_reimu') i.gain(result.cards);
					}
				}
			},
		},
	...
	const translate = {
		...
		gezi_huanjinglili_info: '一名角色的准备阶段开始时,你可以弃置一张牌,展示牌堆底的一张牌;<br><li>若该角色可以成为该牌的目标,将之对其使用;<br><li>若为装备牌,将之置于其装备区内;否则,弃置之.',
		//魔理沙
		gezi_liuxing: '流星',
		...
		//芙兰
		gezi_kuangyan: '狂宴',
		gezi_kuangyan_info: '出牌阶段开始时,或你受到伤害后,你可以获得一点灵力并弃置攻击范围内的所有其他角色区域内各一张牌;<br><li>对其中没有手牌的角色各造成1点伤害;<br><li>你发动此技能后,此技能改为锁定技,直到一名角色死亡.<br><li>若你装备了魔剑莱瓦丁,你可以选择是否发动.',
		gezi_jiesha: '皆杀',
		...
		//小恶魔
		gezi_qishu: '奇术',
		gezi_qishu_info: '准备阶段,你可以选择摸牌阶段、出牌阶段、弃牌阶段中的一个阶段并获得一点灵力,指定一名其他角色;<br><li>若如此做,你此回合跳过该阶段,你的回合结束后,该角色获得1点灵力,并进行一个以此法跳过的阶段.',
		gezi_qishu2: '奇术',
		gezi_qishu2_info: '',
		gezi_anye: '暗夜',
		...
		//美铃
		gezi_xingmai: '星脉',
		gezi_xingmai_info: '你可以将一张【杀】当作一种基本牌使用;调换描述中一张【杀】与一种基本牌的位置.',
		gezi_xingmai2: '星脉',
		gezi_xingmai2_info: '你可以将一种基本牌当作一张【杀】使用或打出;调换描述中一张【杀】与一种基本牌的位置.',
		gezi_dizhuan: '地转',
		gezi_dizhuan_info: '你攻击范围内的一名其他角色成为【杀】的目标时,若目标不包括你,你可以交给目标你的区域内一张牌,将目标转移给你.<br><li>你受到【杀】造成的伤害后,获得1点灵力.',
		gezi_dizhuan2: '地转',
		gezi_dizhuan2_info: '',
		gezi_jicai: '极彩',
		...
		gezi_miingyunleimi_info: '出牌阶段开始时,或你受到伤害后,你可以摸X张牌且获得一点灵力,并将等量牌置于牌堆顶(X为你的攻击范围);<br><li>直到结束阶段:一名角色使用非装备牌指定目标后,若该牌不是以此技能使用,其取消一个目标并判定.<br><li>若判定牌可以对目标使用(无视距离和次数限制),其将判定牌对目标使用<br><li>若你装备冈格尼尔,你不会进行目标判定.',
		gezi_mingyunleimi2: '命运',
		gezi_mingyunleimi2_info: '',
		gezi_hongmo: '红魔',
		gezi_hongmo_info: '觉醒技,准备阶段开始时,若你体力不大于一,且你没有装备武器牌,你减一点体力上限,获得技能【神枪】.',
		gezi_gungirs: '神枪',
		gezi_gungirs_info: '锁定技,你创造并装备一张【冈格尼尔】.<br><li>当你失去此装备时,销毁之,创造并装备一张【冈格尼尔】.<br><li>当你不因此技能装备武器前,取消之,改为获得其武器技能.',
		gezi_gungirs2: '神枪',
		gezi_gungirs2_info: '',
		gezi_feise: '绯色',
		...
		//咲夜
		gezi_huanzang: '幻葬',
		gezi_huanzang_info: '你成为其他角色的卡牌的唯一目标后,或其他角色于你的回合内使用卡牌指定目标后,你可以将一张花色或点数相同的牌置于弃牌堆,令该牌对目标无效,你获得一点灵力.',
		gezi_huanzang_1: '幻葬',
		...
		gezi_world: '世界',
		gezi_world_info: '符卡技1(永续)<br><li>你的回合开始时,可以消耗一点灵力发动符卡技,直到你的下个回合开始.<br><li>效果:一回合一次,当前回合角色使用带有伤害标签的牌指定目标时,你可以消耗1点灵力(装备月时针则不消耗),取消目标,并获得一名角色的一张牌,你可以使用获得牌.',
		...
		gezi_hanghourai_info: '符卡技1(永续)<br><li>你的回合开始时,可以消耗一点灵力,将任意张手牌扣置为<手办>,并摸等量牌,发动符卡,直到你的下个回合开始.<br><li>一名角色的结束阶段,你可以交给其一张<手办>;<br><li>若其可以使用该牌,你可以令其使用之,目标由你指定.',
		//橙
		gezi_mingdong: '鸣动',
		gezi_mingdong2: '鸣动',
		...
		//蕾蒂
		gezi_shuangjiang: '霜降',
		gezi_shuangjiang_info: '结束阶段,你可以对本回合成为过牌的目标,且没有使用/打出过牌的一名角色造成1点雷电伤害,你获得一点灵力.',
		...
		//莉莉白
		gezi_chunxiao: '春晓',
		gezi_chunxiao_info: '准备阶段,若你的灵力值不小于体力值(或灵力未定义),你可以令所有角色各摸一张牌,各弃置与其最近的一名角色一张牌.',
		gezi_mengya: '萌芽',
		gezi_mengya_info: '一回合两次,出牌阶段,你可以选择一项:<br><li>1,摸一张牌并获得1点灵力,弃置一张牌;<br><li>2,消耗1点灵力,摸两张牌.',
		//露娜萨
		gezi_shenxuan: '神弦',
		...
		gezi_kuangxiang: '狂想',
		gezi_kuangxiang_info: '一回合一次,体力不大于你的一名角色成为【杀】的目标时,你可以重铸你与其区域内各一张牌;<br><li>若目标不包括你,将目标转移给你.',
		...
		//紫
		gezi_huanjing: '幻境',
		gezi_huanjing_info: '一名角色的准备阶段开始时,你可以弃置一张牌,展示牌堆底的一张牌;<br><li>若该角色可以成为该牌的目标,将之对其使用;若为装备牌,将之置于其装备区内;否则,弃置之.',
		gezi_mengjie: '梦界',
		...
		gezi_fanhundie2: '反魂蝶',
		gezi_fanhundie_info: '',
		gezi_fanhundie_die_info: '符卡技1(终语)<br><li>你的回合开始时,或你进入濒死时,可以消耗一点灵力发动符卡技,直到没有灵力或当前回合结束.符卡结束时,若体力不大于0,你进入濒死.<br><li>符卡持续时,你不会死亡<br><li>当前回合的结束阶段,你需弃置一名角色的一张牌,其以此法失去最后的手牌后,其流失1点体力<br><li>你须消耗1点灵力并重复此流程,直到灵力为1,或重复第X次(X为你已受伤值).',
		//永琳
		gezi_zhaixing: '摘星',
		...
		gezi_lanyue: '揽月',
		gezi_lanyue_info: '一回合一次,出牌阶段,你可以令攻击范围内离你最远的一名角色选择:体力值或卡牌数量中与你不同的一项,将该项调整至与你相同.',
		gezi_tianwen: '天文',
		gezi_tianwen_info: '符卡技X<br><li>你的回合开始时,你可以消耗X点灵力值(X为任意值且至少为1),发动符卡技.<br><li>你观看牌堆顶两倍数量的牌,以任意顺序置于牌堆顶,进行两次判定:<br><li>你获得其中一张,使用该牌后,若另一张效果能对所有指定的目标使用,可以视为你使用了一张与另一张效果相同的牌,直到回合结束.',
		...
		//辉夜
		gezi_nanti: '难题',
		gezi_nanti_info: '一回合一次,出牌阶段,你可以展示任意张手牌,并声明一项:牌名的字数,花色,点数,属性,或颜色,令一名其他角色选择一项:<br><li>1,交给你一张该项与你展示的牌均不同的牌,你将展示的牌置于弃牌堆;<br><li>2,令你重铸其与展示的牌等量张牌,并对其造成1点雷电伤害.',
		gezi_poxiao: '破晓',
		...
		//慧音
		gezi_jiehuo: '解惑',
		gezi_jiehuo_info: '一回合一次,一名角色的出牌阶段,一张牌因你使用/打出进入弃牌堆时,你可以令一名角色获得之,你获得一点灵力.',
		gezi_richuguo: '日出',
		...
		gezi_businiao_die_info: '符卡技1(终语)<br><li>你的回合开始时,或你进入濒死时,可以消耗一点灵力发动符卡技,直到没有灵力或当前回合结束.<br><li>符卡效果:你不死亡;<br><li>你每扣减1点体力,若体力不大于0,失去等量的灵力;<br><li>当前回合的结束阶段,你可以使用一张【杀】并消耗一点灵力;<br><li>你可以重复此流程任意次直到你没有灵力值;<br><li>你须消耗所有灵力,将体力回复至1,并将手牌补至3张.',
		...
		//米斯蒂娅
		gezi_shiming: '失明',
		gezi_shiming_info: '准备阶段,或你受到伤害后,你可以获得一点灵力,令一名角色获得以下效果,直到当前回合结束:<br><li>其不能以此技能以外的方式使用牌;<br><li>其需要使用牌时,可以洗混其手牌;<br><li>其不能查看其中暗置牌;<br><li>其展示其中一张:若可以使用,本次结算中其可以使用该牌;否则,其弃置之,并可以重复此流程.',
		gezi_shiming_2: '失明',
		gezi_shiming_2_info: '',
		gezi_shiming_3: '失明',
		gezi_shiming_3_info: '',
		...
		gezi_huanshi_5_info: '',
		gezi_zhenshi: '真实',
		gezi_zhenshi_info: '符卡技1(永续)<br><li>你的回合开始时,可以消耗一点灵力发动符卡技,直到你的下个回合开始.<br><li>你攻击范围内角色成为带有伤害标签的牌的唯一目标时,你可以消耗一点灵力并将一张牌置于弃牌堆,<br><li>将包括其的至多3名角色牌扣置并洗混;来源明置一张,将目标转移给明置的角色;将这些牌调整为原状态.',
		gezi_zhenshi_1: '真实之月',
		gezi_zhenshi_1_info: '',
		//帝
		gezi_kaiyun: '开运',
		gezi_kaiyun_info: "一名角色的出牌阶段结束时,若你和其之外的存活角色未成为过卡牌的目标,<br><li>其可以交给你一张牌,你可以使用'神佑(挖坑)',其贴上一张【神佑】技能牌.",
		gezi_kaiyun1: '开运',
		gezi_kaiyun1_info: '一名角色的出牌阶段开始时,其可以交给你一张牌,<br><li>其贴上一张【神佑】技能牌,且其不能对你或其以外的角色使用牌,直到回合结束.',
		gezi_kaiyun_1: '开运',
		gezi_kaiyun_1_info: '',
		gezi_kaiyun_3: '开运',
		gezi_kaiyun_3_info: '',
		...
		gezi_kaiyun_4_info: '',
		gezi_mitu: '迷途',
		gezi_mitu_info: "<挖坑>弃牌阶段开始时,你摸一张牌,可以将一张手牌扣置于角色牌上并获得一点灵力,称为'坑'.<br><li>你攻击范围内的一名角色成为牌的目标后,你可以展示同名或同花色'坑',令来源判定;<br><li>若为黑色,弃置'坑'(若你已发动【远古】则可以额外发动一次【迷途(挖坑)】),弃置来源一张牌,<br><li>且若目标为你,令该牌对你无效.",
		...
		//莉格露
		gezi_yingguang: '萤光',
		gezi_yingguang_info: '一回合一次,你使用牌后,可以展示一张技能牌并获得一点灵力,令一名角色贴上.',
		gezi_yechong: '夜虫',
		gezi_yechong_info: '符卡技2(永续)<br><li>你的回合开始时,可以消耗两点灵力发动符卡技,直到你的下个回合开始.<br><li>【萤光】中的<一回合一次>改为<一回合三次>;<br><li>准备阶段,你弃置有牌的所有其他角色一张牌或移除其一张技能牌;对所有以此法移除技能牌,且手牌区和装备区牌数不小于你的角色造成1点雷电伤害.',
		gezi_yechong1: '夜虫风暴',
		gezi_yechong1_info: '',
		//萃香
		gezi_cuiji: '萃集',
		gezi_cuiji_info: '准备阶段,或你造成伤害后,你可以选择一项:<br><li>1. 失去1点体力,获得2点灵力;<br><li>2. 消耗2点灵力值,回复1点体力.<br><li>若你的灵力大于体力(或灵力未定义),一回合一次,你可以将一张黑色牌当作【杀】使用/打出.<br><li>你以此法使用的【杀】指定目标前本回合出【杀】次数+1.',
		gezi_cuiji_buff: '萃集',
		...
		gezi_jidong_2_info: '',
		gezi_bingbi: '冰壁',
		gezi_bingbi_info: '你成为一名角色的【杀】的目标后,可以打出一张【杀】,令之对你无效;<br><li>你摸X张牌(X为其本回合对你使用的牌数量)并获得一点灵力.',
		gezi_dongjie: '冻结',
		...
		//大妖精
		gezi_zhufu: '祝福',
		gezi_zhufu_info: '一回合一次,出牌阶段,你可以消耗1点灵力(灵力值未定义则不消耗),令一名体力值不大于你的角色回复1点体力.若你没有灵力,将灵力补至当前体力值.',
		gezi_zhufu2: '祝福',
		gezi_zhufu2_info: '',
		gezi_zhufu3: '祝福',
		gezi_zhufu3_info: '',
		gezi_zhiyue: '织月',
		gezi_zhiyue_info: '一回合一次,出牌阶段,你可以交给一名其他角色一张牌;你可以消耗任意点灵力,摸1+X张牌.(X为你消耗的灵力值).<br><li>若你以此法获得至少2张牌且灵力消耗为1,令所有角色回复一点体力.',
		//映姬
		gezi_huiwu: '悔悟',
		gezi_huiwu_info: '锁定技,一名角色的回合结束时,其须重铸一张牌;<br><li>若其本回合造成过伤害,该牌由你指定.<br><li>以此法重铸牌后,其获得一点灵力.',
		gezi_caijue: '裁决',
		gezi_caijue_info: '准备阶段开始时,你可以展示一名角色的手牌:弃置其中所有【杀】,其失去等量灵力,<br><li>你对其造成1点雷电伤害;<br><li>若其中没有【杀】,【裁决】无效,直到你受到伤害后.',
		gezi_caijue2: '裁决',
		gezi_caijue2_info: '',
		gezi_shenpan: '审判',
		gezi_shenpan_info: '符卡技X(限定)<br><li>限定技,你的回合开始时,你可以消耗X点灵力(X为你当前体力值)发动符卡,直到当前回合结束.<br><li>准备阶段,你视为使用一张【令避之间】.<br><li>结束阶段,你可以对场上体力最高或之一的角色造成１点雷电伤害,依次对卡牌数,手牌数,击杀数重复此流程.',
		gezi_shenpan_1: '最终审判',
		gezi_shenpan_1_info: '',
		//小町
		gezi_guihang: '归航',
		...
		gezi_wujian_info: '符卡技X<br><li>你的回合开始时,可以消耗X点灵力(X为任意值且至少为1),直到当前回合结束.<br><li>一回合限X次,出牌阶段,你可以令攻击范围内的所有角色各失去1点灵力;<br><li>若其中有角色灵力不大于2(或灵力未定义),你获得其区域内一张牌.<br><li>你以此法获得牌后,结束此符卡技,获得等同本回合未使用此技能次数的灵力值.',
		gezi_wujian_skill2: '无间',
		gezi_wujian_skill2_info: '',
		//莉莉黑
		gezi_chunmian: '春眠',
		gezi_chunmian_info: '准备阶段,若你的灵力值不大于体力值(或灵力未定义),你可以令所有角色各弃置与其最近的一名角色一张牌;<br><li>所有以此法失去牌的角色各摸一张牌.',
		gezi_bamiao: '拔苗',
		gezi_bamiao_info: '一回合两次,出牌阶段,你可以:获得1点灵力,重铸1张牌;或消耗1点灵力,摸2张牌.',
		//梅蒂欣
		gezi_zaidu: '灾毒',
		...
		gezi_zaidu2_info: '',
		gezi_zhanfang: '绽放',
		gezi_zhanfang_info: '觉醒技,准备阶段,若你手牌大于体力值:<br><li>若你未受伤,将【灾毒】中的<对其造成1点伤害>改为<对其造成1点雷电伤害>;否则,改为<回复1点体力>;<br><li>你增加１点体力上限.',
		gezi_huayuan: '花园',
		...
		gezi_xiaofeng: '啸风',
		gezi_xiaofeng_info: '<u>若你的体力不大于2,消耗-2.</u>符卡技4<br><li>你的回合开始时,可以消耗四点灵力发动符卡技.<br><li>符卡发动时,视为使用一张【花之祝福】,符卡效果内,你获得以下效果:<br><li>你的攻击范围无限,你造成伤害时,该伤害+1,你拼点时的点数均视为Q.',
		gezi_xiaofeng1: '啸风弄月',
		gezi_xiaofeng1_info: '',
		gezi_xiaofeng2: '啸风弄月',
		gezi_xiaofeng2_info: '',
		...
		gezi_fengmi: '风靡',
		gezi_fengmi_1: '幻想风靡',
		gezi_fengmi_info: '符卡技0<br><li>你的回合开始时,你可以发动符卡直到当前回合结束.<br><li>你的摸牌/出牌/弃牌阶段开始前,可以消耗1点灵力,或跳过该阶段,视为使用一张【过河拆桥】.',
		//果
		gezi_nianxie: '念写',
		...
		//莲子
		gezi_xingdu: '星读',
		gezi_xingdu_info: '准备阶段,你可以观看牌堆顶的两张牌,你可以令你的摸牌改为从牌堆底摸,直到回合结束;<br><li>选择完成后,摸一张牌.',
		gezi_xingdu_1: '星读',
		gezi_xingdu_1_info: '',
		gezi_sihuan: '似幻',
		...
		//梅莉
		gezi_xijian: '隙见',
		gezi_xijian_info: '结束阶段,你可以观看牌堆底的一张牌,选择一项:将之交给一名角色,或使用之.',
		gezi_rumeng: '如梦',
		gezi_rumeng_2: '如梦(梦境与现实的诅咒)',
		gezi_rumeng_info: '你的第一个准备阶段开始时,须视为使用一张目标数为2的【幻想之扉】;<br><li>你体力变化时,增加1点体力上限(上限至多为5);<br><li>准备阶段结束时,你可以扣减1点体力上限,无视消耗发动一次【梦境与现实的诅咒】;<br><li>发动后若你体力上限不大于3,失去此技能.',
		gezi_dshift: '幻想之扉',
		...
		gezi_jiqiang: '冰柱机枪',
		gezi_jiqiang_info: '锁定技,所有其他角色的手牌上限-2;一名其他角色的回合结束时,若其手牌数小于你,摸一张牌,对其造成1点雷属性伤害,你可以对其使用一张牌',
		gezi_zuanshi: '钻石风暴',
		...
		//dio
		gezi_zhipei: '支配',
		gezi_zhipei_info: '一名角色使用卡牌指定目标后,你可以将一张花色或点数相同的牌置于弃牌堆,令该牌对目标无效,你获得一点灵力.',
		gezi_shiting: '时停',
		...
		gezi_the_world: '世界',
		gezi_the_world_info: '一回合一次,一名角色使用带有伤害标签的牌指定目标时,你可以消耗1点灵力,取消目标,并获得一名角色的一张牌,你可以使用获得牌.',
		//妹红boss
		...
		gezi_bazhi: '八咫',
		gezi_bazhi_info: '限定技,濒死阶段,可以弃置场上所有<火>标记,将体力上限变为标记数量,并回复至体力上限.你失去技能〖火种〗.',
		//莫妮卡
		gezi_miaohui: '描绘',
		gezi_miaohui_info: '一回合一次,出牌阶段,你可以输入一行代码并执行',
		gezi_kehua: '刻画',
		gezi_kehua_info: '出牌阶段,你可以指定一名角色(包括不在场上的角色),选择一项:为该角色:增加技能;删除技能;更改起始灵力值;更改灵力上限;更改体力上限;或删除该角色;此改动在以后所有非联机模式的游戏中有效',
		//发牌姬
		gezi_huanri: '换日',
		...
		gezi_xuwu: '虚无',
		gezi_xuwu_info: '锁定技,你视为拥有【皆杀】异变牌,所有其他角色的胜利条件失效(该效果仅异变模式生效).结束阶段,所有攻击范围内的角色受到一点雷属性伤害,其中所有灵力为0的角色受到一点伤害.',
		gezi_chunhui: '春回',
		...
		//多萝西
		gezi_zhuanhuan: '次元转换',
		gezi_zhuanhuan_info: '出牌阶段限一次,你可以弃置所有手牌,摸X张牌(X为你当前灵力值).',
		gezi_moli: '次元魔力',
		gezi_moli_info: '出牌阶段限一次,你可以弃置一张牌,并弃置至多三名角色各一张牌,你观看牌堆底等量牌,并依次交给这些角色各一张.',
		gezi_chaoyue: '次元超越',
		...
		//伊莉雅
		gezi_huanzhao: '幻召',
		gezi_huanzhao_info: '游戏开始时,你获得两张角色牌并暗置,称为<梦幻>;<br><li>你击杀一名角色后,将其角色牌暗置加入<梦幻>;<br><li>你视为拥有明置<梦幻>的第一项技能;<br><li>准备阶段开始时,你可以明置一张<梦幻>并暗置其余,可以获得该<梦幻>的所有技能,直到回合结束.',
		...
		gezi_huanzhao4_info: '',
		gezi_wuxian: '无限',
		gezi_wuxian_info: '锁定技,准备阶段,你获得1点灵力;<br><li>:若你的灵力等于上限(或灵力未定义),你摸一张牌.',
		gezi_quintette_fire: '炮击',
		gezi_quintette_fire_info: '符卡技7<br><u>此符卡消耗-X(X为<梦幻>的数量);</u><br><li>你的回合开始时,可以发动此符卡.<br><li>符卡发动时,你对一名角色造成3点伤害,你弃置所有手牌.',
		...
		gezi_wenmo: '吻魔',
		gezi_wenmo_info: '一回合一次,出牌阶段,你可以展示一张手牌,令一名其他角色展示一张牌,交换这两张牌;<br><li>若颜色相同,你获得1点灵力(无法获得灵力改为摸一张牌).',
		gezi_heyi: '鹤翼',
		gezi_heyi_skill: '鹤翼三连',
		gezi_heyi_info: '符卡技1<br><li>你的回合开始时,可以消耗一点灵力发动此符卡,直到当前回合结束.<br><li>【投影】中的<一次>视为<三次>;<br><li>你使用【杀】指定目标后,可以重铸装备区内的任意张牌,弃置目标等量牌.',
		//尼禄
		gezi_muqi: '幕启',
		...
		gezi_AestusDomusAurea_info: '符卡技2<永续><br><li>准备阶段,你将手牌数补至手牌上限;<br><li>出牌阶段,你可以弃置一张手牌,声明一种技能牌,获得之;<br><li>符卡结束时,你可以消耗1点灵力,令符卡不结束.',
		ADA2: '黄金剧场',
		ADA2_info: '',
		ADA3: '招荡的黄金剧场',
		ADA3_info: '',
		...
		gezi_shanyao: '闪耀',
		gezi_shanyao_info: '出牌阶段,你可以视为使用一张普通锦囊牌,消耗等于此牌合法目标数的灵力;当你使用普通锦囊牌指定目标时,可以消耗任意点灵力为此牌指定等量合法目标.',
		//斯卡哈
		gezi_ruizhi: '智慧',
		gezi_ruizhi_info: '准备阶段,你可以判定3次,选择一项:<br><li>若结果中有♠️️,你获得2点灵力;<br><li>若有♦️️,你视为使用一张无视距离的【杀】;<br><li>若有♣️️,你将一张角色的一张牌置于牌堆顶.',
		gezi_mojing: '魔境',
		gezi_mojing_info: '符卡技2<br><li>你的回合开始时,可以发动此符卡,直到你的回合结束.<br><li>符卡发动时,你视为使用了一张【死境之门】;<br><li>永久效果:【智慧】中的<选择一项>改为<选择所有项>;<br><li>符卡效果:一名角色因弃置而失去♥️️牌后,若你灵力大于1,你需消耗1点灵力令所有其他角色各失去1点体力.',
		gezi_mojing0: '魔镜',
		gezi_mojing0_info: '',
		gezi_mojing1: '魔境之门',
		gezi_mojing1_info: '',
		...
		sbrs_liansuo_2_info: '',
		gezi_explosion: '爆裂',
		gezi_explosion_info: '符卡技4<br><li>你的回合开始时,可以消耗四点灵力发动符卡,直到回合结束.<br><li>你跳过你的出牌阶段与弃牌阶段,对一名其他角色造成2点火焰伤害,并弃置其装备区内所有牌.',
		...
		ng_pinjian3: '？',
		ng_wenhao2: '？',
		ng_wenhao2_info: '限定技,准备阶段,若你的灵力大于1,你可以摸X张牌(X为你已受伤值),获得并发动你的符卡技',
		ClarentBloodArthur: '向端丽的吾父发起叛逆',
		ClarentBloodArthur_skill: '向端丽的吾父发起叛逆',
		ClarentBloodArthur_info: '符卡技1<永续><br><li>你的回合开始时,可以消耗一点灵力发动符卡,直到你的下个回合开始.<br><li>你的灵力上限为无限,你的牌点数均为K,符卡结束时,你对一名角色造成X点弹幕伤害,消耗所有灵力(X为你的灵力值)',
		CBA2: '向端丽的吾父发起叛逆',
		CBA3: '向端丽的吾父发起叛逆',
		//利姆鲁
		gezi_bushizhe: '捕食者',
		...
		gezi_daxianzhe: '大贤者',
		gezi_daxianzhe_info: '准备阶段,你可以消耗2点灵力,选择一名你对其发动过【捕食者】的角色,视为拥有其一个不为符卡技的技能直到回合结束',
		//阿尔托莉雅
		gezi_shengguang: '胜光',
		...
		gezi_shijian_info: '符卡技(1)<永续>你的手牌上限和灵力值上限视为无限;符卡结束时,对一名角色造成X点雷属性伤害(X为灵力值),消耗所有灵力',
		//阿尔托莉雅lily
		gezi_hualu: '花路',
		gezi_hualu_info: '你使用带有伤害标签的牌指定目标后,或成为带有伤害标签牌的目标后,可以与目标/来源拼点;赢的角色摸一张牌;你拼点前,你可以令其他角色选择是否代替你打出拼点牌;你可以令选择是的角色获得1点灵力',
		gezi_jinjian: '金剑',
		gezi_jinjian_spell: '胜利黄金之剑',
		gezi_jinjian_use: '胜利黄金之剑',
		gezi_jinjian_info: '符卡技(1)<永续><仪式>你的手牌上限和灵力值上限视为无限:符卡结束时,你选择X次(X为灵力值):令一名角色:受成1点雷属性伤害,或回复1点体力;消耗所有灵力',
		//时雨
		kc_yuzhi: '雨至',
		...
		//夕立
		gezi_hongxi: '轰袭',
		gezi_hongxi_info: '你可以将一张牌当作【杀】使用;<br><li>该【杀】指定目标后,你令对方摸一张牌,按照原牌类型执行对应效果:<br><li>基本～你摸一张牌,与目标拼点:若你赢,该【杀】不能成为【闪】的目标;<br><li>锦囊～获得目标角色一张牌;<br><li>其他牌～目标不能使用或打出手牌直到该【杀】结算完成.',
		gezi_hongxi_2: '轰袭',
		gezi_hongxi_2_info: '',
		gezi_hongxi_3: '轰袭',
		gezi_hongxi_3_info: '',
		...
		//开膛手杰克
		gezi_wulin: '雾临',
		gezi_wulin_info: '限定技,一名角色的回合开始时,你可以令你攻击范围内的一名角色获得以下效果,直到回合结束:<details><summary>其只能以随机选择手牌的方式使用/打出牌;弃置选择的不合理的牌.</summary><p>其不能以此法以外的方式使用牌;其需要使用牌时,可以扣置并洗混其手牌,展示其中一张:若可以使用,本次结算中其可以使用该牌;否则,其弃置之,并可以重复此流程.</details>其首次成为【杀】的目标后,令之不计次数',
		gezi_yejiang: '夜降',
		...
		//童谣
		gezi_lvtu: '旅兔',
		gezi_lvtu_info: '准备阶段,你可以将体力上限,灵力值,手牌数,技能牌数中不为最少的一项调整至与其中最少相同;将另一项调整至与其中最多相同',
		gezi_mengjin: '梦镜',
		gezi_mengjin_info: '出牌阶段限一次,你可以将体力值调整至已受伤值,手牌数调整至(3-手牌数,至少为0),灵力值调整至(5-灵力值);此技能失效直到你进入濒死状态',
		gezi_weimo: '为某人所写的故事',
		...
		gezi_bingfeng: '冰封禁制',
		gezi_bingfeng_info: '一名角色使用可强化牌时,若该牌未强化,你可以弃置一张牌,为该牌追加描述<强化(-1):对目标造成１点雷属性伤害>,强化之',
		gezi_aoshu: '奥术光环',
		gezi_aoshu_lili: '奥术光环(获得灵力)',
		gezi_aoshu_info: '一回合一次,出牌阶段,你可将灵力消耗到0,并为任意名角色分配等量灵力;直到你的准备阶段,一名角色发动符卡时,你获得1点灵力',
		//七宫
		gezi_guyin: '孤樱',
		gezi_guyin_info: '锁定技,你使用牌时,无视消耗强化之;<br><li>你使用闪可响应的牌指定角色为目标时,或成为该种牌的目标时,取消之,<br><li>来源视为对原目标使用一张【决斗】【南蛮入侵】或【惊雷闪】;<br><li>你的【闪】视为【无懈可击】,你的【酒】均视为【桃】.',
		gezi_guyin_2: '孤樱',
		gezi_guyin_2_info: '',
		gezi_guyin_3: '孤樱',
		gezi_guyin_3_info: '',
		gezi_tianze: '天则',
		gezi_tianze_info: '锁定技,你受到伤害后,须获得一点灵力,否则摸一张牌;<br><li>你成为♥️️牌的目标时,须消耗1点灵力,视为你对自己使用一张可强化的牌.',
		gezi_tianze2: '天则',
		gezi_tianze2_info: '',
		//十香
		gezi_iphone3: '尘杀',
		...
		//2B
		gezi_qiyue: '契约',
		gezi_qiyue_info: '你使用【杀】指定目标后,可以弃置目标一张牌;<br><li>若目标没有手牌/装备牌,你依次获得1点灵力(不能获得改为摸一张牌).',
		gezi_yueding: '约定',
		gezi_yueding_info: '符卡技2<br><li>你的回合开始时,可以消耗两点灵力发动此符卡,直到当前回合结束.<br><li>一回合一次,你可以将一张牌当作【杀】使用;<br><li>你使用【杀】的次数上限+1;<br><li>你使用【杀】指定目标后,可以弃置目标一张牌;<br><li>若目标没有装备牌,你摸一张牌.',
		gezi_yueding1: '白之约定',
		...
		//诗乃
		gezi_yanju: '燕狙',
		gezi_yanju_info: '出牌阶段开始时,你可以选择一项:造成伤害后摸一张牌,无视装备效果,或无法闪避;<br><li>视为使用一张持有该效果的无视次数和距离的【杀】.',
		gezi_yanju1: '燕狙',
		gezi_yanju1_info: '',
		gezi_yanju3: '燕狙',
		gezi_yanju3_info: '',
		gezi_shangtang: '上膛',
		gezi_shangtang_info: '准备阶段,若你的手牌数小于4或灵力小于3,你可以重置该数值,获得以下效果,直到回合结束开始:<br><li>你不能对其他角色使用杀以外的牌;你的手牌上限至少为4.',
		...
		gezi_moxin: '默心',
		gezi_moxin_info: '一名角色的结束阶段,若其本回合:<br><li>没有使用过杀且没有造成过伤害,<br><li>你可以令其获得一点灵力,或摸一张牌交给其一张牌.',
		//由理
		gezi_chongzou: '重奏',
		...
		gezi_qixin: '齐心',
		gezi_qixin_info: '一名角色的结束阶段,<br><li>若其本回合使用过杀且造成过伤害,<br><li>你可以令其获得一点灵力,或摸一张牌可以交给其一张牌.',
		//美九
		gezi_duzou: '独奏',
		gezi_duzou_info: '一回合一次,出牌阶段,你可以观看一名角色手牌,明置其中一张;若其没有灵力,其于回合结束后进行一个额外的出牌阶段,该阶段内其由你控制',
		gezi_lunwu: '轮舞',
		gezi_lunwu_info: '一回合一次,出牌阶段,你可以交给一名角色一张手牌,并明置之;对其攻击范围内除你以外的所有角色各造成１点雷属性伤害',
		gezi_tiaoxian: '调弦',
		...
		gezi_wuyan: '舞燕',
		gezi_wuyan_info: '所有角色根据你明置手牌和装备区内的牌中数量最多的花色获得以下效果:<br><li>♠️️:准备阶段获得一点灵力;<br><li>♣️️:摸牌阶段额外摸一张牌;<br><li>♥️️:每回合限一次,可以将一张牌当作【桃】使用;<br><li>♦️️:弃牌阶段开始时,可以交给你一张牌.',
		gezi_stage: '舞台',
		...
		//集气石
		jiqitou: '集气？',
		jiqitou_info: '距离两格以内的一名角色回合结束时,若其已受伤,其回复1点体力,弃置两张牌',
		jiqitou2: '集气',
		jiqitou2_info: '',
		//流星雨
		paimaiyunshi: '拍卖陨石',
		paimaiyunshi_info: '距离两格以内的一名角色回合结束时,若其体力值大于1,其受到1点弹幕伤害,摸两张牌',
		paimaiyunshi2: '拍卖陨石',
		paimaiyunshi2_info: '',
		//黑洞
		shenmiR18: '啊——呜——',
		...
		const mingzhi = {
			...
			character: {
				re_lunasa: {
					...
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_reshenxuan', 'gezi_rezhenhun', 'gezi_hezou'],
				},
				re_merlin: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_remingguan', 'gezi_kuangxiang', 'gezi_hezou'],
				},
				re_lyrica: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_remingjian', 'gezi_rehuanzou', 'gezi_hezou'],
				},
				gezi_miku: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_geying', 'gezi_wuyan', 'gezi_stage'],
				},
				gezi_diva: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_duzou', 'gezi_lunwu', 'gezi_tiaoxian'],
				},
				gezi_aqua: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_yuyin', 'gezi_qingqu'],
				},
				gezi_rumia: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_heiguan', 'gezi_yuezhi'],
				},
				gezi_meiling: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_xingmai', 'gezi_dizhuan', 'gezi_jicai'],
				},
				gezi_koakuma: {
					sex: 'female',
					hp: 3,
				...
				gezi_patchouli: {
					...
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_qiyao', 'gezi_riyin2', 'gezi_xianzhe'],
				},
				gezi_sakuya: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_huanzang', 'gezi_shijing', 'gezi_world'],
				},
				gezi_remilia: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_miingyunleimi', 'gezi_hongmo', 'gezi_feise'],
				},
				gezi_flandre: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_kuangyan', 'gezi_jiesha'],
				},
				gezi_cirno: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_jidong', 'gezi_bingbi', 'gezi_dongjie'],
				},
				gezi_daiyousei: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_zhufu', 'gezi_zhiyue'],
				},
				gezi_suika: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_cuiji', 'gezi_baigui'],
				},
				gezi_wriggle: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_yingguang', 'gezi_yechong'],
				},
				gezi_mystia: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_shiming', 'gezi_wuye'],
				},
				gezi_keine: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_jiehuo', 'gezi_richuguo'],
				},
				gezi_reimu: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_yinyang', 'gezi_mengdie', 'gezi_mengxiang'],
				},
				gezi_marisa: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_liuxing', 'gezi_xingchen', 'gezi_stardust'],
				},
				gezi_tewi: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_kaiyun', 'gezi_mitu', 'gezi_yuangu'],
				},
				gezi_reisen: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_huanshi', 'gezi_zhenshi'],
				},
				gezi_eirin: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_zhaixing', 'gezi_lanyue', 'gezi_tianwen'],
				},
				gezi_kaguya: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_nanti', 'gezi_poxiao', 'gezi_yongye_die'],
				},
				gezi_mokou: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_yuhuo', 'gezi_businiao_die'],
				},
				gezi_huolingmeng: {
					sex: 'female',
					hp: 8,
					maxHp: 8,
					group: 'shen',
					skills: ['gezi_fenggui', 'gezi_lingji', 'gezi_mengxiangtiansheng'],
					isBoss: true,
					isBossAllowed: true,
				},
				gezi_letty: {
					sex: 'female',
					group: 'wu',
					skills: ['gezi_shuangjiang', 'gezi_baofengxue'],
				},
				gezi_chen: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_mingdong', 'gezi_shihuo', 'gezi_shuanggui'],
				},
				gezi_lilywhite: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_chunxiao', 'gezi_mengya'],
				},
				gezi_lunasa: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_shenxuan', 'gezi_zhenhun', 'gezi_hezou'],
				},
				gezi_merlin: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_mingguan', 'gezi_kuangxiang', 'gezi_hezou'],
				},
				gezi_lyrica: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_mingjian', 'gezi_huanzou', 'gezi_hezou'],
				},
				gezi_alice: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_huanfa', 'gezi_mocai', 'gezi_hanghourai'],
				},
				gezi_youmu: {
					sex: 'female',
					group: 'wu',
					skills: ['gezi_yishan', 'gezi_yinhuashan'],
				},
				gezi_yuyuko: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_youdie', 'gezi_moyin', 'gezi_fanhundie_die'],
				},
				gezi_ran: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_jiubian', 'gezi_shiqu', 'gezi_tianhugongzhu'],
				},
				gezi_yukari: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_huanjing', 'gezi_mengjie', 'gezi_mengjing'],
				},
				gezi_lilyblack: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_chunmian', 'gezi_bamiao'],
				},
				gezi_medicine: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_zaidu', 'gezi_zhanfang', 'gezi_huayuan'],
				},
				gezi_yuuka: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_zanghua', 'gezi_xiaofeng'],
				},
				gezi_komachi: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_guihang', 'gezi_wujian'],
				},
				gezi_eiki: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_huiwu', 'gezi_caijue', 'gezi_shenpan'],
				},
				gezi_aya: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_kuanglan', 'gezi_fengmi'],
				},
				gezi_hetate: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_nianxie', 'gezi_jilan', 'gezi_lianxu'],
				},
				gezi_renko: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_xingdu', 'gezi_sihuan'],
				},
				gezi_meribel: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_xijian', 'gezi_rumeng'],
				},
				akyuu: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shen',
					skills: ['library_mengji', 'library_yixiang'],
				},
				gezi_daria: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_zhuanhuan', 'gezi_moli', 'gezi_chaoyue'],
				},
				gezi_rylai: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_tanxue', 'gezi_bingfeng', 'gezi_aoshu'],
				},
				gezi_jack: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_wulin', 'gezi_yejiang', 'gezi_maria'],
				},
				gezi_yuuko: {
					sex: 'female',
					hp: 3,
				...
				gezi_haruhi: {
					...
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_haruhi1', 'gezi_haruhi2'],
				},
				gezi_nurseryrhyme: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_lvtu', 'gezi_mengjin', 'gezi_weimo'],
				},
				gezi_tohka: {
					sex: 'female',
					group: 'wu',
					skills: ['gezi_iphone3', 'gezi_Halvanhelev'],
				},
				gezi_kanade: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'key',
					skills: ['gezi_zhongzou', 'gezi_moxin'],
				},
				gezi_nero: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_muqi', 'gezi_AestusDomusAurea'],
				},
				gezi_nerowedding: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_muqi', 'gezi_FaxCaelestis'],
				},
				gezi_illyasviel: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_huanzhao', 'gezi_wuxian', 'gezi_quintette_fire'],
				},
				gezi_bb: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_shiguan', 'gezi_jinbei', 'gezi_ccc'],
				},
				gezi_kurumi: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_kedan', 'gezi_shishu', 'gezi_shishi'],
				},
				gezi_sinon: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_yanju', 'gezi_shangtang'],
				},
				gezi_scathach: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_ruizhi', 'gezi_mojing'],
				},
				gezi_kuro: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_touying', 'gezi_wenmo', 'gezi_heyi'],
				},
				gezi_twob: {
					sex: 'female',
					group: 'wu',
					skills: ['gezi_qiyue', 'gezi_yueding'],
				},
				gezi_homura: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_time3', 'gezi_time', 'gezi_homuraworld'],
				},
				gezi_shigure: {
					sex: 'female',
					group: 'shu',
					skills: ['kc_yuzhi', 'gezi_zuoshibaozhan'],
				},
				gezi_yudachi: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_hongxi', 'gezi_solomon'],
				},
				gezi_m4a1: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_huoli', 'gezi_zhihui', 'gezi_shenyuan'],
				},
				gezi_megumin: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_honglian', 'sbrs_liansuo', 'gezi_explosion'],
				},
				gezi_satone: {
					sex: 'female',
					hp: 3,
				...
				gezi_yuri: {
					...
					maxHp: 3,
					group: 'key',
					skills: ['gezi_chongzou', 'gezi_qixin'],
				},
				gezi_priestress: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_xiaoyu', 'gezi_jinhua', 'gezi_shengbi'],
				},
				gezi_tamamo: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_liyu', 'gezi_zhoufa', 'gezi_shuitian'],
				},
				gezi_aliceWLD: {
					sex: 'female',
					hp: 3,
				...
				gezi_arisa: {
					...
					maxHp: 3,
					group: 'qun',
					skills: ['gezi_yaowu', 'gezi_huanrao', 'sliver_arrow'],
				},
				gezi_tsubaki: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_xiangyi', 'gezi_chunse'],
				},
				gezi_rimuru: {
					sex: 'none',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_bushizhe', 'gezi_daxianzhe'],
				},
				niuzhanshi: {
					sex: 'female',
					group: 'shu',
					skills: ['niguang', 'ng_wenhao2'],
				},
				mordred: {
					sex: 'female',
					group: 'shu',
					skills: ['niguang', 'ClarentBloodArthur'],
				},
				blackmagicgirl: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_daomo', 'gezi_chuancheng', 'gezi_dba'],
				},
				gezi_rin: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_cuican', 'gezi_shanyao'],
				},
				gezi_FSakura: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'wei',
					skills: ['gezi_jiushu', 'gezi_mengsui'],
				},
				gezi_saber: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_shengguang', 'gezi_shijian'],
				},
				gezi_saberlily: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_hualu', 'gezi_jinjian'],
				},
				gezi_JoanofArc: {
					sex: 'female',
					group: 'shu',
					skills: ['gezi_chouzi', 'gezi_yuanfen', 'gezi_huiqiong'],
				},
				aventurinepurple: {
					sex: 'female',
					group: 'wei',
					skills: ['gezi_genyuanzhini', 'gezi_jingjie', 'gezi_yazhi'],
				},
				gezi_lingliaolukong: {
					sex: 'female',
					hp: 3,
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_chizhimu', 'gezi_jibian', 'gezi_huozhong', 'gezi_bazhi'],
				},
				gezi_hechenghequ: {
					sex: 'female',
					group: 'qun',
					skills: ['qianghua_qianghuagezi'],
				},
				gezi_liuhua: {
					sex: 'female',
					hp: 3,
				...
				gezi_dio: {
					...
					maxHp: 8,
					group: 'shen',
					skills: ['gezi_zhipei', 'gezi_shiting', 'gezi_the_world'],
					isBoss: true,
					isBossAllowed: true,
				},
				stg_bookshelf: {
				...
				gezi_fapaiji: {
					...
					maxHp: 3,
					group: 'wu',
					skills: ['gezi_huanri', 'gezi_toutian'],
					isBoss: true,
				},
				boss_cirno: {
					sex: 'female',
					...
					maxHp: 9,
					group: 'shen',
					skills: ['gezi_jidong', 'bianshen_cirno'],
					isBoss: true,
				},
				boss_cirno2: {
					sex: 'female',
					group: 'shen',
					skills: ['gezi_jiqiang', 'gezi_zuanshi', 'gezi_jubing'],
					isHiddenBoss: true,
					isBossAllowed: true,
				},
				boss_mokou: {
					sex: 'female',
					group: 'shen',
					skills: ['gezi_huoniao1', 'bianshen_mokou'],
					isBoss: true,
				},
				boss_mokou2: {
					sex: 'female',
					...
					maxHp: 8,
					group: 'shen',
					skills: ['gezi_huoniao2', 'businiao_boss'],
					isHiddenBoss: true,
					isBossAllowed: true,
				},
				gezi_monika: {
					...
					maxHp: 3,
					group: 'shu',
					skills: ['gezi_miaohui', 'gezi_kehua'],
				},
				boss_nianshou: {
					sex: 'female',
					hp: 10000,
				...
				gezi_xiandai: {
					sex: 'female',
					group: 'qun',
					skills: ['gezi_tiequan', 'gezi_zhicai'],
				},
			},
			characterTitle: {
			...
			characterIntro: {
				re_lunasa: '全名露娜萨·普莉兹姆利巴.骚灵三姐妹中的大姐,因此也担任乐团的领队.有些阴沉,但又不喜欢拐弯抹角,且很容易较真的性子.使用乐器为小提琴,演奏的曲调带有令观众镇静,低落,甚至忧郁的效果.</b>',
				re_merlin: '全名梅露兰·普莉兹姆利巴.骚灵三姐妹中的二姐.很是开朗,但是感觉上有点神经质.力量上是姐妹中最强的.使用乐器为小号,演奏的曲调带有令观众激动,激昂,甚至抓狂的效果.1</b>',
				re_lyrica: '全名莉莉卡·普莉兹姆利巴.骚灵三姐妹中的三妹.聪明,但是总是想用小聪明去赚姐姐们的便宜.使用乐器为键盘,打击乐器也可以使用.</b>',
				gezi_miku: '全名初音未来,我们天下第一的公主大人~<br>出自:Vocaloid<b>画师:saberii</b>',
				gezi_diva: '1. 进入男主的一群萌妹的后宫<br>2. 亮出自己百合的身份<br>3. ???<br>4. 发了发了!<br><b>出自:Date-A-Live! 画师:干物A太</b>',
				gezi_aqua: '阿库娅绝对是个受智障诅咒的名字<br><b>出自:火焰纹章 if 画师:輪回666</b>',
				gezi_rumia: '以吃人为生的一只宵暗妖怪.天真浪漫还有点笨蛋的萝莉.她可以以自己为中心制造一个完全黑暗的地带,但是她在其中也看不见,其他人也看不见她,结果她还经常会撞树上.<br> <b>画师:こけこっこ</b>',
				gezi_meiling: '全名红美铃,红魔馆的门卫.擅长中国功夫,据说还会放元气弹？因为红魔馆的门卫好像就她一个人24小时值班,所以几乎24小时都可以看到她在门口打盹……<br> <b>画师:もねてぃ</b>',
				gezi_koakuma: '红魔馆图书馆的管理员(？).没有名字没有设定没有官方插画,留下来的只有小恶魔这一个称呼,和对她的主人帕秋莉大人的忠贞不渝.<br> <b>画师:Dhiea</b>',
				gezi_patchouli: '全名帕秋莉·萝雷姬.红魔馆图书馆的馆长(？).持有贤者之石,并会操纵全七种元素魔法的大魔女.但是同时也是个怎么都不肯出门的病弱家里蹲.<br> <b>画师:忘川の泉眼</b>',
				gezi_sakuya: '全名十六夜咲夜.红魔馆的女仆长.持有操控时间和空间的能力,并用来做家务,因此被称作<完美而潇洒的女仆长>.似乎和蕾米过去有什么孽缘……？<br> <b>画师:ds</b>',
				gezi_remilia: '全名蕾米莉亚·斯卡雷特.红魔馆的女主人.已经活了500年的萝莉吸血鬼,持有传说中的神枪冈格尼尔,和影响他人命运的能力……但是总是被红魔馆的其他人欺负着玩.<br> <b>画师:こぞう</b>',
				gezi_flandre: '全名芙兰朵露·斯卡雷特.是蕾米小5岁的妹妹.破坏力在幻想乡里是数一数二的,但是因为精神不稳定,被姐姐关在了红魔馆的地下室495年.最近好像渐渐和解了,加入了调戏姐姐的红魔馆大家里.<br> <b>画师:黑果冻dog</b>',
				gezi_cirno: '冰之妖精,在众多妖精中是能力最强的!(然而对于其他人来说只是杂鱼)是个喜欢到处游荡和搞恶作剧的笨蛋.<br><b>画师:茨乃</b>',
				gezi_daiyousei: '不知名不知能力不知长相的一只妖精.跟琪露诺的关系很不错(大部分时候充当她的监护人……)<br><b>画师:伊崎</b>',
				gezi_suika: '全名伊吹萃香.不知道什么时候出现在幻想乡的鬼,现住在博丽神社.几乎什么时候都在喝酒.虽然是萝莉身材,但是筋力非常强大,并持有控制密度的能力.<br> <b>画师:あいに</b>',
				gezi_wriggle: '全名莉格露·奈特巴格.萤火虫妖怪,并且是虫王,可以操纵大量的各种虫子(和听起来一样恶心).但是惧怕虫子的人越来越少,妖力下降到只是个萝莉,还是个笨蛋.<br> <b>画师:羽々斩</b>',
				gezi_mystia: '全名米斯蒂娅·萝蕾拉.夜雀妖怪,可以通过歌声让人疯狂或是变成夜盲.以前以吃人为生,现在因种种原因在开烧烤店.<br> <b>画师:鶖(かしどり)</b>',
				gezi_keine: '全名上白泽慧音.虽然是妖怪,平时与人类住在一起,并给人类孩子们教书.在月圆之夜会变身成兽人,做些……不可描述的事情.<br> <b>画师:にしもん</b>',
				gezi_reimu: '全名博丽灵梦.东方project的主角,博丽神社的巫女,符卡规则的创建人.因为是巫女,在幻想乡是绝对权威势力.但是平常懒到连异变都不去解决……<br> <b>画师:萩原</b>',
				gezi_marisa: '全名雾雨魔理沙.东方project的主角.住在魔法森林里的人类魔法使(魔炮流派).总是抢在巫女之前冲去解决异变.因为种种原因与多名少女有说不清楚的关系.<br> <b>画师:フアルケン</b> <br><s>据说其实是创星神的分身？</s>',
				gezi_tewi: '全名因蟠帝.住在迷途竹林里的兔妖.据说在竹林里见到她的人能获得好运.虽然,从竹林里活着出来的人并没有发现他们怎么好运了.<br> <b>画师:ねこぜ</b>',
				gezi_reisen: '全名铃仙·优昙华院·因蟠.从月亮逃下,躲入幻想乡的一只月兔妖.因为在永远亭中是最下级的位置,总是被其他人欺负.持有令人解释不清楚程度的能力.<br> <b>画师:k2pudding</b>',
				gezi_eirin: '全名八意永琳.不老不死的药师,月都创立人之一.数千年前从月亮逃出,带着辉夜躲入幻想乡.最近才冒出来,并开设了一个诊所.<br> <b>画师:minusT</b>',
				gezi_kaguya: '全名蓬莱山辉夜.不老不死的月亮的公主,数千年前从月亮逃出,与永琳一同躲入幻想乡,最近才冒出来.<br> <b>画师:Riv</b>',
				gezi_mokou: '全名藤原妹红.原本是人类,数千年前因为辉夜成为了不老不死.一段时间前流浪入了幻想乡,最近住在竹林里.<br> <b>画师:palinus</b>',
				gezi_huolingmeng: '不要在灵梦面前提钱,不要动灵梦的赛钱箱,不要对博丽神社做任何事情.<br>——来自造成了目前整个事态的某个魔法师的灵梦三戒律<br>画师:Ran',
				gezi_letty: '全名蕾蒂·霍瓦特洛克.在冬天才会出来的雪女.能力是操纵寒气,也可以强化冬天的效果.<br> <b>画师:国家飯</b>',
				gezi_chen: '一只妖怪猫化作的,八云蓝的式神.因为是式神的式神所以比较弱,习性也更接近猫而不是妖怪.能力是使用妖术的能力.<br> <b>画师:水佾</b>',
				gezi_lilywhite: '在春天才会出现的,宣告春天到来的妖精.<br> <b>画师:oninoko</b>',
				gezi_lunasa: '全名露娜萨·普莉兹姆利巴.骚灵三姐妹中的大姐,因此也担任乐团的领队.有些阴沉,但又不喜欢拐弯抹角,且很容易较真的性子.使用乐器为小提琴,演奏的曲调带有令观众镇静,低落,甚至忧郁的效果.<br><b>画师:中島楓</b>',
				gezi_merlin: '全名梅露兰·普莉兹姆利巴.骚灵三姐妹中的二姐.很是开朗,但是感觉上有点神经质.力量上是姐妹中最强的.使用乐器为小号,演奏的曲调带有令观众激动,激昂,甚至抓狂的效果.<br><b>画师:中島楓</b>',
				gezi_lyrica: '全名莉莉卡·普莉兹姆利巴.骚灵三姐妹中的三妹.聪明,但是总是想用小聪明去赚姐姐们的便宜.使用乐器为键盘,打击乐器也可以使用.<br><b>画师:中島楓</b>',
				gezi_alice: '全名爱丽丝·玛格特罗伊德.住在魔法森林中,以操纵人偶出名的魔法使.因为是人偶使,大部分人不太敢接近,所以比较孤僻的样子.顺便也有着收集各种道具的坏习惯.<br><b>画师:藤原</b>',
				gezi_youmu: '全名魂魄妖梦.冥界白玉楼的庭师(同时也是女仆,厨师,剑术指导).是半人半幽灵的混血,因此持有更长的寿命和更强的身体能力.使用楼观剑和白楼剑的二刀流剑豪.<br><b>画师:daiaru</b>',
				gezi_yuyuko: '全名西行寺幽幽子.冥界白玉楼的大小姐.冥界的管理者,可以控制死亡,也可以控制已死的幽灵.已经成为亡灵千年以上,并且没有生前的记忆,因而很是乐天和无忧无虑.<br><b>画师:.SIN</b>',
				gezi_ran: '全名八云蓝.八云紫的式神,传说中的九尾妖狐.因为紫长期睡觉,还需要冬眠,所以工作从日常打理到大结界的维护都是蓝来进行.<br> <b>设计:无限连的陆伯言  画师:ルリア</b>',
				gezi_yukari: '全名八云紫.持有如同GM权限的控制境界的能力,力量深不可测.幻想乡的创始人之一.大部分时候都在睡觉,或者是用隙间做些很逗比的事情.<br> <b>画师:Shionty</b>',
				gezi_lilyblack: '似乎就是莉莉白换了身衣服？<br><b>画师:Cloudy.R</b>',
				gezi_medicine: '全名梅蒂欣·梅兰可莉.被人类抛弃,受铃兰之毒多年后妖怪化的人偶.<br><b>画师:はるとき</b>',
				gezi_yuuka: '全名风间幽香.太阳花园之主.虽然只是种花的,但是实际战力是顶尖级别,还是战狂性格.不过最近比较满足于种花和养老.<br><b>画师:萩原</b>',
				gezi_komachi: '全名小野塚小町.负责将死者灵魂带入冥界的,冥河上摆渡的死神.虽然是至关重要的工作,但是因为太懒怠工疑似弄出了异变来.<br><b>画师:MAY☆嵐(G.H)</b>',
				gezi_eiki: '全名四季映姬.幻想乡的裁判长/阎魔王.负责判断死者灵魂是转生还是进入地狱永劫不复.职业病非常非常非常严重.<br><b>画师:DomotoLain</b>',
				gezi_aya: '全名射命丸文,妖怪之山的天狗记者.以持有幻想乡最快的报导速度而闻名(也以报导的片面性和捏造闻名).<br><b>画师:スプライト</b>',
				gezi_hetate: '全名姬海棠果,妖怪之山的天狗记者.以不用出门就能写报导的念写能力而闻名(也以报导的过期性闻名).<br><b>画师:o-bow</b>',
				gezi_renko: '从幻想乡外界进来的女高中生,秘封俱乐部成员之一.为了通过世界的缝隙而穿越去其他世界而行动着.<br><b>画师:An2a</b>',
				gezi_meribel: '从幻想乡外界进来的女高中生,秘封俱乐部成员之一.似乎能够看见和感觉到世界的缝隙？<br><b>画师:An2a</b>',
				akyuu: '全名稗田阿求,将毕生奉献于记载幻想乡的历史的稗田家的现任家主.持有过目不忘的记忆能力.<br><b>画师:渡瀬　玲<br></b><br>现因一些原因,被赋予了幻想乡的管理员权限.不过依然是和平常一样做着记录屋的工作',
				gezi_daria: '先手,5费,巨像突击,守护者巨像,命运的指引,智慧之光,剑仙,使徒,魔法剑,魔法剑,奇美拉,多萝西,引导,啊,这3个大眼下不去了,到你了吧<br>出自:影之诗 <b>设计:雪樱  画师:アカトネ</b>',
				gezi_rylai: '<b>出自:dota2 设计:路人orz  画师:forest</b>',
				gezi_jack: '同时是杀人狂和暴露狂的幼女？快收住你奇怪的想法……<br>出自:Fate/Apocrypha <b>画师:オウカ</b>',
				gezi_yuuko: '<br>出自:神的记事本 <b>画师:岸田メル 设计:伶</b>',
				gezi_haruhi: '如果有一个疯子,但是他说的狂话最后都成真了,那他还算一个疯子吗？<br><b>出自:凉宫春日的忧郁 画师:まとけち</b>',
				gezi_nurseryrhyme: '<br>出自:Fate/Extra <b>画师:十八三</b>',
				gezi_tohka: '传说中的有史以来最敷衍最没有良心的技能组!<br>就跟原作的能力设置差不多.<br>出自:Date-A-Live!<b>画师:mmrailgun</b>',
				gezi_kanade: '啊,是天使,我死了——<br>全名立华奏,死后世界的学校中的学生会长,标准的无口萌妹.<br>出自:Angel Beats! <b>画师:sua(スア)</b>',
				gezi_nero: '全名尼禄·克劳狄乌斯·凯萨·奥古斯都·日耳曼尼库斯.古罗马的皇帝,比起皇帝更像个偶像,奢华浪费和浮夸无人能出其右.<br>出自:Fate/Extra <b>画师:demercles</b>',
				gezi_nerowedding: '全名尼禄·克劳狄乌斯·凯萨·奥古斯都·日耳曼尼库斯.古罗马的皇帝,比起皇帝更像个偶像,奢华浪费和浮夸无人能出其右.<br>出自:Fate/Extra CCC<b>画师:Kirra</b>',
				gezi_illyasviel: '全名伊莉雅丝菲尔·冯·爱因兹贝伦,在日本的动漫中十分常见的那种使用特殊能力帮助他人或对抗恶役的女孩子<br>出自:Fate/kaleid liner 魔法少女☆伊莉雅 <b>画师:永恒之舞MK_2',
				gezi_bb: '<所以就算你喜欢我也没用哦？哪怕你被BB亲迷得神魂颠倒,也绝对无法抓住我啦!>BB一脸自豪地如是说.<br>出自:Fate/Extra CCC<b>画师:swd3e2(隋文帝)</b>',
				gezi_kurumi: '全名时崎狂三.咕咕咕<br>出自:Date-A-Live! <b>画师:kyuriin</b>',
				gezi_sinon: '被称为<冰之狙击手>的GGO玩家.<死枪>事件后在桐谷和人的邀请下转换到新生了ALO中,扮演擅长使用弓箭狙击的猫妖精.<br>出自:刀剑神域<b>画师:PCManiac88</b>',
				gezi_scathach: '影之国的女王,当女王当了几千年了.<br>出自:Fate/Grand Order <b>设计:冰茶\t画师:saberii</b>',
				gezi_kuro: '全名克洛伊·冯·爱因兹贝伦,小腹上的那个不是○纹,不要问了!<br>出自:Fate/kaleid liner 魔法少女☆伊莉雅 <b>画师:トミフミ</b>',
				gezi_twob: '——机器人会梦见电子绵羊吗？<br>——不,机器人会梦见和小男朋友一起去商城买T恤衫————<br>出自:NieR:Automata <b>设计:雪樱   画师:saberii</b>',
				gezi_homura: '问题:如果你目睹你最喜欢的人死亡,要她死多少次你才会疯掉？<br><b>出自:魔法少女小圆 画师:Capura.L</b>',
				gezi_shigure: '白露级驱逐舰2号舰,名是从前代神风级驱逐舰10号舰所继承.<br>出自:舰队collection <b>画师:konomi★きのこのみ</b>',
				gezi_yudachi: '白露级驱逐舰4号舰,以初春级为基础,加固了舰体,提高了稳定性.<br>出自:舰队collection <b>画师:ﾏｸｰ</b>',
				gezi_m4a1: '<br>出自:少女前线 <b>画师:怠惰姬空白</b>',
				gezi_megumin: '以<艺术就是爆炸>为人生信条的小萝莉红魔法师.<br>出自:为美好的世界献上祝福!<b>画师:seki</b>',
				gezi_satone: '全名索菲亚琳·SP·撒旦七世,传说中的魔法魔王少女,可以召唤天使!<br>出自:中二病也要谈恋爱! <b>设计:子规    画师:あかつき聖</b>',
				gezi_yuri: '全名仲村由理,死后世界的学校中,死后世界战线的领导者,旨在与神,与神的天使对抗.<br>出自:angel beats! <b>画师:戦-G</b>',
				gezi_priestress: '女神官 是日本轻小说<哥布林杀手>及改编作品中的女主角,身材娇小的金发少女,会使用信仰地母神的教会法术.协助哥布林杀手一起不停地讨伐哥布林',
				gezi_tamamo: '玉藻前,TYPE-MOON的游戏<Fate/EXTRA>、<Fate/EXTRA CCC>、<Fate/EXTELLA>、<Fate/EXTELLA LINK>及<Fate/Grand Order>中登场的Servant',
				gezi_aliceWLD: '<br><b>出自:爱丽丝漫游仙境 画师:夕凪セシナ</b>',
				gezi_arisa: '守护森林的妖精弓手.(不过森林里的东西比她要恐怖多了)<br>出自:暗影之诗 <b>画师:黒井ススム</b>',
				gezi_tsubaki: '全名朱雀院椿.<这是你为我锻造的翅膀,我将用它们尽情翱翔.>...椿姐赛高!<br>出自:牵绊闪耀的恋之伊吕波 <b>画师:ぺろ</b>',
				gezi_rimuru: '据说某b阿姨发明了史莱公,那应该是这个吧.<br>出自:关于我转生变成史莱姆这档事 <b>画师:8bit 设计:伶酱</b>',
				blackmagicgirl: '自从出现后就没停止过出本子的劳模.<br>出自:游戏王<b> 画师:Seinen 设计:黑白葱</b>',
				gezi_rin: '我去练撑杆跳能找到这么萌的妹子吗？<br>出自:Fate/StayNight <b>画师:基月</b>',
				gezi_FSakura: '笑容最美丽反而内心是最痛苦的<br>出自:Fate/StayNight <b>画师:bomhat</b>',
				gezi_saber: '你们光炮太多我很为难诶<br>出自:Fate/StayNight <b>画师:冬ゆき</b>',
				gezi_saberlily: '<br>出自:Fate/Grand Order <b>画师:未知</b>',
				gezi_JoanofArc: '法国村姑被背刺后一缕真灵不灭自灰烬中重生的亵神之人,誓要将神的光辉从世上抹除.<人类不需要神明的注视,我要将高踞御座的神明一一焚净>,好吧我乱编的<br>出自:Fate/Grand Order <b>画师:未知</b>',
				aventurinepurple: '八云紫是日本的<东方Project>系列弹幕游戏及其衍生作品的登场角色之一.八云紫是个力量非常强大的妖怪,她持有与根源相关的危险能力,这能力能和神明对抗.她是幻想乡的创始人之一,和历代博丽巫女一同维护着博丽大结界.全东方系列唯一的PH(Phantasm)关卡头目(蓝则是<妖妖梦>的EX关卡的Boss) 主要剧情 东方求闻史纪 要说最像妖怪的妖怪,就必须要提到八云紫的名字.这个妖怪,由于其危险的与根源相关的能力,神出鬼没而性格不近人情,行为原理与人类完全相异等等的原因,是一个没人愿意与她为敌的妖怪. 外表和人类没有区别.喜欢华丽的衣服,撑着一把大阳伞. 主要活动时间是夜晚,白天睡觉.是个典型的妖怪(×1最近在夜晚活动的人类增加的同时,昼间活动的妖怪也增加了.).另外,尽管都说冬天一直在冬眠,因为只是其本人所谈及而且实际在何处居住无法确认,所以真伪的程度无法确定. 古时候,幻想乡缘起阿一所著(×2初代撰写的的幻想乡缘起,有超过一千二百年的历史了.)的妖怪录中也有着这种相似的妖怪登场.那个时代她的身影就已经出现了. 能力 操纵境界的能力是能够从根源开始颠覆一切事物的恐怖的能力.众所周知,物的存在是建立在境界的存在之上的.没有水面的话,湖是不存在的.没有山脊的话,山和天空也是不存在的吧.没有幻想乡的大结界的话,幻想乡也不会存在.如果全部物的境界都不存在的话,那世界将会是一个巨大的整体吧. 也就是说,操纵境界的能力是逻辑的创造与破坏的能力.从本质上创造新的存在,从本质上否定已有的存在.妖怪所持有的能力中能够与神之力相匹敌的、最危险的能力之一. 还有,也能通过空间的裂缝从任何地方瞬间移动,或身体的一部分移动到别的场所.据说不仅仅是物理的空间,也能够在画中、梦中和故事中等移动. 另外,还拥有超人的头脑(×3因为是妖怪所以超越人类是当然的.),尤其是数学很强.而且,因为活了很久的缘故,知识和经验都很丰富. 日常 她本人意外地健谈,告诉别人各种各样的关于自己和幻想乡相关的事情.只是,哪一个都没有被确认过,实际上都是没有办法去确认,所以真假无从判断. 住所和博丽神社一样,据说是建在幻想乡和外边的世界的境界之处,但没有人见过那个建筑物.因为利用空间的断开处,从不知何处出现并消失去了无法得知的地方.没法从后面追赶,真伪的程度无从判定(×4也有传言说家是在外面的世界.). 另外,平时自己行动很少,而是操纵妖兽作为式神,杂事全部都由这匹妖兽来做.特别是在白天和冬天,睡觉的时候这个式神会代替紫来行动. 还有就是,同样寿命很长的妖怪朋友也有很多.住在冥界的西行寺幽幽子(后述)、鬼族的伊吹萃香(后述)等,大部分的朋友都是最强一类的妖怪. 与幻想乡的关系 幻想乡自身,依靠大结界与外界相隔离.这个结界也是分开内外境界线. 只是,外面的世界和幻想乡在地理上是接续的,所以可以说是结界的境界从逻辑上面创造了幻想乡这个地方. 也就是说,只要境界之妖怪还存在,要创造或破坏幻想乡这样的地方都是可能的.被境界所环绕的幻想乡里,有着不知道居住在何处的境界之妖怪的存在,可以说是和幻想乡的成立有极重要的关系. 不可思议的是,这个妖怪很少袭击人类.因为不袭击人的妖怪不算是妖怪,可能还是在某个地方有袭击过人的吧.由于能够自由地前往外面的世界,那个地方说不定就是外面的世界了吧(×5有传言说是外面的世界的妖怪回到幻想乡来游玩.冬季与昼间等,据说是在睡觉的时候,可能就是在外面的世界度过的也说不定,也有的说法是紫的梦中世界就是外面的世界.). 关于这个妖怪的逸闻 幻想月面战争骚动 此为一千多年前的秘史.据说她玩弄着实与虚的境界,跃入了映照在湖上的月亮并向其发动进攻.尽管聚集了大量的妖怪,在月面的近代兵器面前很快遭到惨败.从那以后,妖怪们很少再去进攻不属于自己的领土了.以这个骚动作为契机,这个境界妖怪的力量在人类之间也广为流传. 妖怪扩张计划 超过五百年以前的逸事了.由于人口的增加,为了回复被人类压迫的妖怪在幻想乡的势力,紫策划并实行的. 在这以前的幻想乡仅仅是在远离人类村落的深山里的地方,这个计划是幻想乡周围创造了幻与实的境界,在逻辑上创造了这个世界. 幻想乡内为幻之世界,外面的世界为实体世界这般,在外面的世界里势力变弱了的妖怪会自动地被幻想乡召唤进来这样换时代的事件. 从此以后,变成了从日本以外的国家也有妖怪移民进来.境界的效现在仍在持续着. 此外,这个计划的高明之处在于,外面世界妖怪消失得越多,幻想乡里的妖怪就越强.外面的世界是人类的天下,幻想乡内还是妖怪的天下. 大结界骚动 理所当然般存在着的博丽大结界(×6分隔幻想乡与外面世界的境界.这是合乎逻辑的境界,是据说人类和妖怪都不能轻易地往来的,强大的境界.).据说是这个妖怪是提议大结界并创建的幻想乡的贤者之一.但是,最初是大量的妖怪联合起来反对,她与妖怪之间为此发生了争斗.在这期间,妖怪都没有去袭击人类,幻想乡的人类们开始对大结界大为称赞. 　　结果,大结界的好处传到了妖怪们那里,反对这个结界的人几乎不存在了. 幽灵骚动 冥界与显界的境界变薄了,发生了幽灵会出显界、生者容易前往冥界这样的骚动.已经确认是这个妖怪的所作所为.因为是贤明的妖怪做的事,可能会有关系到妖怪社会的崇高的目的性所在(×7在我有生之年是无法确认了.).这个异变仍然进行着没有被解决. 目击者报告 没注意到的时候放在眼前的食物就这样消失了,希望她别这样了.(博丽灵梦) 据说能够趁人不备把手伸进空间的断开处,并把远处的东西取走. 求你平时别突然出现在我家里了,好好的从门口进来不行么？(雾雨魔理沙) 神出鬼没也要有个限度. 对策 出现在人类村庄的情况很少.只要不发生特殊情况,是不会袭击普通人类的. 无论对自己的能力多么地有自信,最好也别去退治这个妖怪.但是,只要不是恶性战斗的话,她会配合这边的力量而手下留情. 总之,对于境界的能力的防御方法和弱点都不存在.而且,其智力也使远远凌驾于人类的想象之上的,当然身体能力也是和妖怪一样.万分之一的胜利希望也不会有吧.除了以绅士般的态度面对之外没有其他的有效对策. 东方妖妖梦 一到春天便会醒来的隙间妖怪.  主要拥有操纵所有境界程度的能力. 跟幽幽子是故知的关系,但并不是幽灵.是在幻想乡中,静静栖息在最边境的妖怪.从这里,边看着博丽大结界边生活着. 偶尔会有人类混进幻想乡,也是由于紫有时去摇晃境界的结果.人类所称的神隐这种现象,如果被知晓其实不是什么神明所为, 只是这名妖怪少女捣的鬼的话,什么神秘性都会失去吧. 紫平时不怎么活动,一般操纵式神的蓝来生活. 但因为基本都是在睡,其间都丢着蓝不管,所以基本不知道蓝在做什么. 当得知这位式神蓝被什么人打败了的时候,稍微惊讶了下但还是去睡回笼觉了. 自回笼觉醒来的时候,总算是察觉到周围已然是一片春色. 其实大约十天前就已经是春天了,但每每睡去又忘记,醒来又惊奇. 在蓝回复之前,没法安心睡觉呀,这样想着但还是睡过去了. 这个时侯,幽幽子来拜托希望修复因为这次骚动而稀薄了的幽明的境界. 你自己叫它变弱的还管什么修复不修复呀,这样想着揉揉睡眼便出发了. 久违到访冥界的紫,因为异常热闹的气氛而感到奇怪.西行寺家不是更寂静的地方吗？虽然这么想,但也许是幽幽子换了个嗜好吧. 这样自顾自地理解了. 途中遇见了庭师妖梦,详细地了解了情况.根据妖梦所描述似乎是倒了大霉,但紫怎么听都觉得是自作自受. 所有人,看起来都因为那棵妖怪樱而失常了.那棵樱花树吸入的人类的精气太过量了.一直都觉得很危险.尽管这样想,但也很清楚,凭自己的力量是什么都做不到的. 幽幽子生前的时候,紫就知晓了.也知道,其实,并不是幽幽子被妖怪樱所封印,而是幽幽子的亡骸封印着妖怪樱这件事.[5] 东方萃梦想 幻想乡的妖怪.如同字面一样既妖又怪.住在幻想乡的境界的屋子,但是那个场所并无人知道. 不知道为何对于外面的世界很熟知.看起来非常的可疑. 性格是无法理解型,但是绝对不是好东西这点是大家都认同的地方. 都认为如果可以的话真不想见到她. 一般来说除了灵梦之外没有认真和她相处的人. 体术,采取和人类标准相异的行动.既不快也不迟钝. 到底是强是弱也是不可知. 紫的行动让自己看起来仿佛被耍了一样,就算赢了她也会生气. 但是,恐怕也没有像她一样真挚地接受幻想乡、爱着幻想乡的人了. 东方永夜抄 居住在幻想乡边境某处的妖怪. 鉴于境界本身不是一般人所能见,因此无法判定其住在哪里. 只能肯定不是在同一个边境上的神社里. 于幻想乡的妖怪中,她是从很久以前就已经存在,且力量也相当强大. 眼下仍然清楚了解幻想乡本身的人物,即使在妖怪当中亦极其稀少. 她看起来形迹可疑,那是因为她缺少人味,让人不能理解她的行动. 如果她有那个心思,要毁掉整个幻想乡是易如反掌,非常危险的妖怪. 东方绯想天 性格:冷静,经常避开他人. 东方地灵殿 平常根本搞不清楚她人在哪的神出鬼没的妖怪. 地灵殿中八云紫的支援子弹和BOMB 这次制作了能够对话的阴阳玉. 因为地底和地上的妖怪们所构筑的是完全不同的社会,因此并不想过多去干涉.对突然出现的怨灵感到很困惑. 因为怕异变扩大之后会很讨厌,而拜托影响力比较小的灵梦前往调查. 东方绀珠传 雾雨魔理沙打败纯狐后从月球回来后正在琢磨月之都的力量石的时候八云紫突然出现,在魔理沙同意下研究了下力量石,她指出从这石头能够得到各种情报和妖力会让幻想乡越来越变化多端,魔理沙始终被八云紫的语言压制着,她猜不透八云紫所说的变化到底是啥,更不知道八云紫是为什么而出现(或许是给新作做伏笔吧)',
				gezi_lingliaolukong: '灵乌路空 灵乌路空(れいうじ うつほ/Reiuji Utsuho),系列作品<东方Project>中的角色,于<东方地灵殿>首次登场.她有操控核融合程度的能力. 中文名 灵乌路空 其他名称 阿空(おくう)炽热焦躁的神之火 登场作品 <东方地灵殿>、<东方非想天则> 性别 女 血型 不明 日文名 れいうじ うつほ 种族 地狱鸦(体内寄宿着神灵八咫鸟) 能力 操控核融合程度的能力 英文名 Reiuji Utsuho 主题曲 霊知の太阳信仰～Nuclear Fusion 登场作品 <东方地灵殿>第6面Boss <东方非想天则>可用角色、早苗线4面BOSS、琪露诺线4面BOSS <Double Spoiler ～ 东方文花帖>Lv8 <东方心绮楼>地灵殿观众 设定文档 求闻口授立绘 东方求闻口授 栖息于旧地狱最深处的地狱鸦.住在旧地狱的妖怪基本上都是因为原住地成了废墟,继而迁居于此,但地狱鸦本来就住在地狱,是这里最早的住民. 与地面之上产生了瓜葛,因而被确认为是一种固有种(×1 在被隔离的特定环境里繁衍生息,只存在于那里的动物,妖怪们.).自地狱的黑暗诞生而来的鸟,话虽如此,却没什么作为妖怪的特点可言. 不过,她仍旧是个特殊的存在.她的身体里寄宿着神灵.寄宿着神灵的话,就变成了类似移动分社一样的东西.对一般的人类跟妖怪来说这一定很难吧.无名无个性,对自己不加执着,且拥有接受一切的包容力,这显得非常必要.并且也需要一定的努力.当然这其中最简单的捷径应该是脑袋空空吧. 她总是一副异样的姿态.可疑的存在着的「分解之足」,看起来很重的「融合之足」,手上还安着一门大炮一样的东西,这好像是「第三足」,胸部有着深红的「赤之目」,这全都是寄宿在他身体里的神灵,八咫乌的影响. 很少到地面上去,也很少与人们接触,所以性格不明.唯一明确的就是她的脑袋空空的. 八咫乌　寄于她体内的神灵八咫乌,是出现在古事记中的高等神灵.　八咫乌被称作是栖息于太阳的乌鸦.用肉眼来看会比较困难,但在地面上还是可以观测到它们的身姿.八咫乌的身姿正是被人们称作是太阳黑子的那种东西.大约每隔十一年有一次力量上升,可以看到很多的黑点.　这位神明的力量是太阳之力,神之火.原封不动地成为了她的力量.　神之火这种东西,在超高温的前提下就算放着不管也会持续燃烧,所以控制起来非常困难.她的力量与其说是排出体内的神之火,倒不如说是花费力气控制神之火呢.　将此神灵附在她身上的人,好像是八坂神奈子.　明明具备了太阳这种至高无上的神格,却又被分灵到旧地狱这种尽是居住着被厌恶者的地方,这时候的八咫乌会怎么想呢. 对策　不知道算不算是危险.　没怎么去过地面上,只用记住这种妖怪都是住在地下的就好了.　顺带一提拜她所赐有温泉涌出来了的样子.这也算是给予的恩惠吧. 地灵殿立绘 东方地灵殿 外表大放异彩的乌鸦. 左足乃『分解之足』 右足乃『融合之足』 右手所持的是控制它们的『第三足』 据说她就是靠这三足来操纵究极的能量. 在沉入永眠的火焰地狱遗迹上有一座地灵殿. 在地底世界已经不再是地狱的现在,已经不再会有罪人被贬入此地, 现在只有以前就住在这里的地狱鸦,负责搬运尸体的火车,其他一些 怪异的妖怪们,以及只靠怨恨行动的怨灵们住在这里. 觉呢,把这块属于自己的土地分配给宠物们居住. 空也和燐一样,是觉的一只宠物. 燐负责怨灵,而空则负责看守火焰地狱遗迹防止其失控. 她的工作就是,发现火焰变旺了就打开中庭的天窗,火力不足了就 将燐送来的尸体扔进其中,借此来调整火力. 每天每天,虽然都重复着同样的事情,但也算过着平稳充实的生活. 但是,从某天开始这种日子被打破了. 有什么人从地上,没有被任何人发现一口气进入了地狱火焰遗迹当中. 对她说了什么. 那声音的内容刺激了她的好奇心. 『在地狱火焰遗迹中隐藏有诞生出人类能够获得的最后一种能量的 秘密. 以及,你这只栖息在火焰中的乌鸦. 你应该可以让那究极的力量寄宿于自己身体中. 那样一来不仅是对于地底,也可以给地上带来更多的希望吧』 她被光芒所包围,感觉到有什么人进入到了自己的身体当中. 等回过神来便发现自己的样子产生了巨大的变化. 空对于使用获得的力量感到非常有趣,回过神来发现灼热地狱遗迹 已经回复了以前的热度. 那温度烧热了地下水,当水蒸气的压力到达极限时便会变成间歇泉 一口气喷出地面. 从地上来访的使者. 那究竟是什么人呢,以及目的是什么. 空小小的头脑中,立刻说出了那样的疑问. ――八咫乌 进入她身体里的力量是一种被称为八咫乌的神. 有三只脚的乌鸦,据说居住在太阳上. 八咫乌所拥有的究极力量,便是创造原子的力量,核融合. 核能,这是留给未来的究极幻想. 非想天则立绘 东方非想天则 东方非想天则没有相关的文字设定. 符卡 地灵殿 中文名 日文名 难度 核热「Nuclear Fusion」(核聚变) 核热「ニュークリアフュージョン」 Easy / Normal 核热「Nuclear Excursion」(核功率骤增) 核热「ニュークリアエクスカーション」 Hard 核热「核反应失控」 核热「核反応制御不能」 Lunatic 非想天则 中文名 日文名 备注 「地狱的人造太阳」 「地狱の人工太阳」 对战使用 控制 「Self Tokamak」(自动托卡马克装置) 制御「セルフトカマク」 对战使用 遮光「核热护罩」 遮光「核热バイザー」 对战使用 Double Spoiler ～ 东方文花帖 中文名 日文名 等级 射命丸文的评价\t姬海棠果的评价 炉解「Melting White」(消熔的纯白) 炉解「メルティングホワイト」 Level 8-2 [1]拍到了正在进行核聚变反应的乌鸦 　　可是逆光太强了,拍出来的照片都是纯白色 　　怎样才能把太阳拍得更清晰一些呢？ 啊啊,这就是传说中的第二个太阳呀.温泉涌出来使 　　核聚变这个词广为人知,虽然不明白原理但是感觉很厉害 　　像是连外面的世界都无法实现的强大力量呢 巨星「Red Giant」(红色巨人) 巨星「レッドジャイアント」 Level 8-4 好热啊.而且因为逆光所以拍不到好照片 　　问河童的话,回答是「若是逆光的话调整一下曝光度就好了」 　　本来就很热,再加一件衣服的话真有点…… 不停乱动的太阳! 　　如果这样迅速运转的话一天就会变短,会很有意思啊 　　就会有更多快乐的日子能够享受了! 星符「巨星陨落」 星符「巨星坠つ」 Level 8-6 我已经放弃拍摄空的照片了 　　所以我考虑把一张全白的照片登在报纸上 　　写上「这就是灵乌路空」 真是热死人啦—— 　　烧温泉也很辛苦啊—— 　　以后一定要很小心地进去.把酒也带上吧 七星「Septentrion」(北斗星辰) 七星「セプテントリオン」 Level 8-8 北斗七星？啊啊,说的就是天龙呢 　　虽然如果龙真的出来闹了绝对不止这种程度吧 　　但是,在七星背面跟着的那颗小星星到底是……？ 北斗七星吗——？那个是巨人的勺子吧？ 　　当那把勺子翻过来的时候,大量的流星就会倾注而下 　　就这样概括一下写成新闻么——',
				gezi_hechenghequ: '河城荷取(かわしろ にとり,Kawashiro Nitori)是系列作品<东方project>中的角色,初登场于<东方风神录>.能力是操纵水程度的能力,主要活动场所是玄武之泽等.<br><b>画师:ZYANNA</b>',
				gezi_liuhua: '爆裂吧,现实!粉碎吧,精神!放逐这个世界!邪王真眼是最强的!<br>出自:中二病也要谈恋爱!<b>画师:雪降</b>',
				gezi_dio: 'ko no dio da!!!',
				stg_bookshelf: '帕秋莉最喜欢的书架...大概',
				gezi_fapaiji: "本次四轰鬼抽由Love Live!赞助提供.<br>谢谢来自μ's的东条希的友情出演!<br>………等等,什么？<br>画师:醐味屑",
				boss_cirno: '要我说几遍啊,我不是什么⑨!我是幻想乡最强的!<br>画师:原悠衣',
				boss_cirno2: '虽然成功的获得了超越常人的力量,但是这力量对于超越常人的家伙们来说……还是⑨级别的.<br>画师:しがらき',
				gezi_monika: '问题:如果其他人已经不再是人了,那对她们做多残忍的事情都是没问题的,对吧？<br><b>出自:心跳文学部 画师:はっく</b>',
				boss_nianshou: '比起加一堆没人想要的大杂烩设定,把本来欢乐的活动变成一个累死人的掀桌活动,还是回到最开始的简单欢乐日子好',
				gezi_xiandai: '并没有在原作游戏中登场过,只在<上海爱丽丝通信Ver2.7>提及其存在以及名字被遗忘之事,人设和擅长体术的设定初登场于MUGEN人物,同人作品<博丽二人>中也有出现',
			},
		};
		for (const i in mingzhi.character) {
			const info = mingzhi.character[i];
			if (!info.hp) {
				info.hp = 4;
			}
			if (!info.maxHp) {
			...
			info.trashBin = [`ext:东方project/image/${i}.jpg`];
			info.dieAudios = [`ext:东方project/audio/${i}.mp3`];
		}
		lib.config.characters.add('TouhouProject');
		lib.config.all.characters.add('TouhouProject');
	...
	lib.init.js(
		'extension/东方project/source/module',
		'KJ',
		function () {
