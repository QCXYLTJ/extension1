      ...
      content(config, pack) {
        lib.element.player.changePhasen = function (name) {
          ...
          this.phase = function (skill) {
            ...
            if (get.mode() == 'guozhan') next.setContent(this.name1 + 'changePhasen');else
            next.setContent(this.name + 'changePhasen');
          ...
          intro: {
            content(content, player) {
              ...
              if (player.changePhasenordermarks) {
                ...
                for (let i = 1; i < player.changePhasenordermarks.length; i++) {
      ...
      precontent() {
        game.import('character', function () {
          var KJ2 = {
            ...
            connect: false,
            character: {
              gezi_Nyaruko: ['female', 'wei', 3, ['gezi_hundun', 'gezi_bianshenjiamian'], ['ext:东方project/image/gezi_Nyaruko.jpg', 'des:我是始终面带微笑潜行到你身边的混沌,奈亚拉托提普向你问好.<br>出自:潜行吧,奈亚子!']],
              gezi_jiamian: ['male', 'wei', 3, ['yinghun', 'xinyongsi', 'jigong', 'nzry_kuizhu', 'xinfu_zuilun'], ['ext:东方project/image/gezi_jiamian.jpg', 'des:<假面骑士>系列是由石森章太郎原作、东映株式会社制作的日本特摄系列英雄故事,至今共有36位(重置系列和番外系列未被算入其中)主角骑士.']],
              gezi_kezi: ['female', 'wei', 4, ['gezi_chiyan', 'gezi_huoling'], ['ext:东方project/image/gezi_kezi.jpg', 'des:奈亚子的青梅竹马,病娇似地爱着奈亚子,自称是她的妻子.<br>出自:潜行吧,奈亚子!']],
              gezi_nep: ['female', 'shu', 4, ['gezi_ziling'], ['ext:东方project/image/gezi_nep.jpg', 'des:涅普迪努,又称:涅普缇努(妮普禔努),涅普顿.是<超次元游戏:海王星>、<超次元游戏:海王星mk2>等海王星系列游戏中紫耀之都Planeptune的守护女神.']],
              gezi_Noire: ['female', 'shu', 4, ['gezi_heiling'], ['ext:东方project/image/gezi_Noire.jpg', 'des:诺瓦露,<超次元游戏:海王星>、<神次元游戏:海王星V>等海王星系列游戏的角色之一.Lastation的守护女神,喜欢Cosplay(动画中部分COS角色为今井麻美在其他作品内的角色),是个典型的努力型女神.']]
            },
            skill: {
              gezi_ziling: {
              ...
              gezi_chiyan: {
                ...
                content() {
                  ...
                  for (let i = 0; i < Math.min(cards.length, players.length); i++) {
                    //if (i.isAlive()) {
                    game.otherUseCardTo({ name: i }, i);
                    //}
                  }
                }
              },
          ...
          lib.config.all.characters.add('KJ2');
          lib.config.characters.add('KJ2');
