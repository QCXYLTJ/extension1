					...
					skill: {
						...
						归心: {
							...
							check(event, player) {
								...
								var num = 0;
								for (var i = 0; i < game.players.length; i++) {
									if (game.players[i].num('he') && game.players[i] != player && get.attitude(player, game.players[i]) <= 0) {
										num++;
									}
									if (game.players[i].num('j') && game.players[i] != player && get.attitude(player, game.players[i]) > 0) {
										num++;
									}
						...
						反噬: {
							...
							filter(event, player) {
								if (event.parent.name != '雷袭') {
									for (var i = 0; i < game.players.length; i++) {
										if (game.players[i].get('e') && game.players[i] != event.player) return true;
									}
								}
								return false;
							},
							content() {
								for (var i = 0; i < game.players.length; i++) {
									if (game.players[i].get('e') && game.players[i] != trigger.player) {
										game.players[i].damage(1, 'thunder');
									}
								}
							},
						...
						妆戎: {
							...
							content() {
								...
								('step 2');
								player.draw(Math.abs(player.maxHp - player.num('h')));
						...
						决洪: {
							...
							content() {
								'step 0';
								player.discardPlayerCard(trigger.player.num('he'), trigger.player);
								('step 1');
						...
						灵鲲: {
							...
							subSkill: {
								cancel: {
									...
									filter(event, player) {
										//QQQ
										return !event.source || event.source.num('h') < player.maxHp;
									},
									content() {
										trigger.cancel();
										player.recover();
									...
									target(player, target) {
										...
										switch (ui.selected.cards[0].suit) {
											case 'heart':
												...
												return 0.1;
											case 'diamond':
												return 1 + Math.sqrt(target.num('h'));
											case 'club':
												return -target.num('h') - Math.sqrt(target.num('h', 'sha'));
											case 'spade':
							...
							chooseButton: {
								...
								backup(links, player) {
									return {
										...
										precontent() {
											'step 0';
											player.chooseToDiscard('he', true, [1, player.num('he')]);
											('step 1');
											var list = game.filterPlayer(function (current) {
												return current.isFriendsOf(player);
											});
						...
						逐师2: {
							...
							content() {
								...
								//player.changeHujia();
								trigger.player.discard(trigger.player.get('he').randomGets());
			...
			card: {
				card: {
					...
					guohefencheng: {
						...
						content() {
							'step 0';
							player.discardPlayerCard(target.num('he'), target, 'he', true);
							('step 1');
