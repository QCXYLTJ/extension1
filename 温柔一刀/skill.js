...
const skill = {
  bug: {
    ...
    init(player) {
      ...
      if (window.ceshiskill) {
        game.bug = window.ceshiskill.unique().filter((Q) => !['QQQ_Marikashichui', 'QQQ_biaoji', 'QQQ_shichui', 'QQQ_bianshen'].includes(Q)); //改用这个直接获取技能
    ...
    async content(event, trigger, player) {
      const skills = game.bug.slice(270, 400); //(0, 50)改为要测的区间
  ...
  东皇钟: {
    ...
    async content(event, trigger, player) {
      const { cards } = await player
        .chooseCard('he', '重铸一张牌', lib.filter.cardRecastable)
  ...
  盘古斧: {
    ...
    subSkill: {
      1: {
        ...
        content() {
          const cards = trigger.cards.filter((Q) => Q.HQ('毁'));
  ...
  女娲石: {
    ...
    async content(event, trigger, player) {
      const { links } = await player
        .chooseButton(['将一名已死亡的角色复活为随从', game.dead])
  ...
  天谴: {
    ...
    forced: true,
    async content(event, trigger, player) {
      const card = game.createCard('shandian', 'spade', 5);
      trigger.cancel();
      trigger.result = {
        card: card,
  ...
  QQQ_人皇幡: {
    ...
    async content(event, trigger, player) {
      if (['delay', 'equip'].includes(get.type(trigger.card))) {
        ...
        const { targets } = await player
          .chooseTarget(`为${get.translation(trigger.card)}增加或减少一个目标`)
  ...
  募集: {
    ...
    async content(event, trigger, player) {
      const { cards } = await player
        .chooseToDiscard('弃置1张牌或失去一点体力', 'he')
      ...
      const { links } = await player
        .chooseButton(['选择要获得的牌', trigger.cards], num, true)
  ...
  治军: {
    ...
    async content(event, trigger, player) {
      const num = Math.ceil(trigger.cards.length / 2);
      const { links } = await player
        .chooseButton(['选择防止失去的牌', trigger.cards], num, true)
        .set('ai', (button) => get.value(button.link))
        .forResult();
      if (links?.length) {
  ...
  神裁: {
    ...
    async content(event, trigger, player) {
      const { targets } = await player
        .chooseTarget('令一名其他角色获得神裁标记', (card, player, target) => !target.hasSkill('徒'))
        .set('ai', (t) => -get.attitude(player, t))
        .forResult();
      if (targets?.length) {
  ...
  无矩: {
    ...
    content() {
      'step 0';
      const card = player.getCards('h', (card) => !card.hasGaintag('无矩'));
      player.recast(card);
  ...
  QQQ_longjing: {
    ...
    async content(event, trigger, player) {
      ...
      if (list.length) {
        const { cards } = await player
          .chooseCard('he', (c) => list.some((q) => get.cardNameLength(c) == get.cardNameLength(q[2])))
          .set('ai', (card) => 999 - get.value(card))
          .forResult(); //填小了会无限循环
        if (cards?.length) {
          list = list.filter((q) => get.cardNameLength(cards[0]) == get.cardNameLength(q[2]));
          const { links } = await player
            .chooseButton(['视为使用或打出牌名字数相同的牌', [list, 'vcard']])
            .set('ai', (button) => {
              ...
              if (['blood', 'kami'].includes(button.link[3])) {
  ...
  掠夺: {
    ...
    async content(event, trigger, player) {
      game.countPlayer(function (Q) {
        ...
        const card = Q.getCards('hej').filter((E) => E.suit == 'diamond');
        if (card && card.length) {
  ...
  漫卷: {
    ...
    async content(event, trigger, player) {
      if (trigger.name == 'gain') {
        const cardsx = trigger.cards.map((card) => {
          ...
          cardx._cardid1 = card.cardid;
          return cardx;
        });
        player.directgains(cardsx, null, get.translation(trigger.player));
      } else {
        for (const card of trigger.cards) {
          const cardx = player.getCards('s').find((q) => q._cardid1 == card.cardid);
          if (cardx) {
            cardx.delete();
          }
        }
    ...
    subSkill: {
      1: {
        ...
        firstDo: true,
        filter(event, player) {
          return event.cards?.some((card) => card._cardid1);
        },
        async content(event, trigger, player) {
          for (const card of trigger.cards.filter((card) => card._cardid1)) {
            const cardx = card.owner.getCards('h').find((q) => q.cardid == card._cardid1);
            if (cardx) {
              cardx.delete();
            }
          }
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set('filterOk', function (moved) {
          let sumx = 0;
          for (const c of moved[1]) {
            sumx += c.number;
          }
          return sum <= sumx;
        })
        .set('processAI', function (list) {
          const card = list[2][1].concat(list[3][1]);
  ...
  影火: {
    ...
    async content(event, trigger, player) {
      ...
      cards.forEach((card1, index, arr) => {
        ...
        if (!card2) {
          ...
          card2._cardid2 = card1.cardid;
          player.directgains([card2], null, '影火');
        }
        if (card2._cardid2 != card1.cardid) {
          card2.init(card1);
          card2._cardid2 = card1.cardid;
        }
      });
    ...
    subSkill: {
      1: {
        ...
        firstDo: true,
        filter(event, player) {
          return event.cards?.some((card) => card._cardid2);
        },
        async content(event, trigger, player) {
          const cards = Array.from(ui.cardPile.childNodes).slice(0, 4);
          for (const card of trigger.cards) {
            const cardx = cards.find((q) => q.cardid == card._cardid2);
            if (cardx) {
              cardx.delete();
            }
          }
  ...
  武绝: {
    ...
    async content(event, trigger, player) {
      ...
      if (result.targets?.length) {
        ...
        const name = result.targets[0].name;
        const position = result.targets[0].dataset.position;
    ...
    async content(event, trigger, player) {
      ...
      if (list1.length) {
        ...
        if (result.links?.length) {
          ...
          if (list.length) {
            const { links } = await player
              .chooseButton(['视为使用或打出对应基本牌/锦囊牌', [list, 'vcard']])
              .set('ai', (button) => {
                const num = player.getUseValue(
                  {
                    name: button.link[2],
                ...
                const result2 = await result1.targets[0]
                  ...
                  .set('filterButton', (button) => {
                    if (ui.selected.buttons.length) {
                      let num = 0;
                      let num1 = 0;
                      for (const q of result1.targets[0].getCards('he')) {
  ...
  QQQ_taye: {
    ...
    async content(event, trigger, player) {
      ...
      if (result.links?.length) {
        ...
        const card = get.cards(result.links.length);
        let card1 = [];
        game.cardsGotoOrdering(card);
        player.showCards(card);
        for (const i of card) {
  ...
  QQQ_yaoyi: {
    ...
    async content(event, trigger, player) {
      const cards = player.getCards('esjx');
      const evt = event.getParent(2);
      const card = player.getCards('h', 'ybsl_107xiaohu')[0];
      if (card) {
        cards.push(card);
      }
      if (cards[0]) {
  ...
  QQQ_xiangyun: {
    ...
    subSkill: {
      1: {
        ...
        async content(event, trigger, player) {
          let prompt = '将至少一张花色各不相同的牌置入<香>,摸等量的牌';
  ...
  QQQ_pianan: {
    ...
    async content(event, trigger, player) {
      const cards = player.getCards('hejsx', (card) => get.tag(card, 'damage'));
      if (cards[0]) {
  ...
  QQQ_zhuiyi: {
    ...
    subSkill: {
      1: {
        mod: {
          cardUsable(card, player, num) {
            if (card.storage && card.storage.QQQ_zhuiyi) {
              return Infinity;
            }
          },
          targetInRange(card, player) {
    ...
    subSkill: {
      ...
      2: {
        ...
        ai: {
          directHit_ai: true,
          skillTagFilter(player, tag, arg) {
            if (arg.card.name == 'sha' && arg.target.countCards('h', 'shan') < 2 && !arg.target.getEquip(2)) {
              return true;
            }
            return false;
          },
