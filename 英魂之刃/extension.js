						...
						init() {
							...
							lib.skill._jilueduijueRE = {
								...
								content() {
									...
									if (player == game.me) {
										if (lib.config.jilueduijueND != 'easy') player.discard(player.getCards('hej'));
				...
				const nyhzrlj = {
					...
					skill: {
						...
						nyhzr迷藏印记: {
							...
							filter(event, player) {
								return player.countCards('h') >= 1 && player.storage.nyhzr仙灵伙伴 >= 1;
							},
							content() {
								player.storage.nyhzr仙灵伙伴 -= 1;
						...
						nyhzr天之罚: {
							...
							filter(event, player) {
								return player.hasSkill('nyhzr光明圣礼') && player.countCards('h') >= 1;
							},
							content() {
								player.discardPlayerCard(1, player, true);
						...
						nyhzr骄阳: {
							content() {
								...
								('step 1');
								if (!event.current.hasSkill('nyhzr嗜血') || !event.current.hasSkill('nyhzr血祭启示录')) {
									event.current.removeSkill(event.current.getCards('s').randomGets(event.current.skills.length - 1));
								}
								('step 2');
								if (game.players.length < 5) {
									if (event.current.next != player) {
						...
						nyhzr龙之咆哮: {
							...
							filter(event, player) {
								return !player.hasSkill('nyhzr真龙化身') && player.countCards('h') > 2;
							},
							content() {
						...
						nyhzr钻能射线: {
							...
							filter(event, player) {
								for (var i of game.players) {
									//QQQ
									return i.countCards('h') > 0 && player.storage.nyhzr钻能光球 >= 2;
								}
							},
							prompt: '请选择1名角色',
							content() {
						...
						nyhzr千叶斩: {
							...
							content() {
								...
								if (qyzsha && player.countCards('h') > player.hp + 1 && target.hp > 0) {
						...
						nyhzr迷藏印记ol: {
							...
							filter(event, player) {
								return player.countCards('h') >= 1 && player.storage.nyhzr仙灵伙伴ol >= 2;
							},
							content() {
						...
						nyhzr歃血为神ol: {
							...
							content() {
								...
								player.chooseControl(['弃牌回复体力', '将现有体力转化为护盾值']).ai = function () {
									if (player.countCards('h') > 1) return '弃牌回复体力';
								};
								('step 1');
								...
								for (var i of game.players) {
									...
									if (i != player) {
										if (i.getEquips(3)) {
											...
											i.discard(i.getEquips(3));
										}
										if (i.getEquips(4)) {
											player.storage.nyhzrhorse++;
											i.discard(i.getEquips(4));
										}
									}
								}
						...
						nyhzr火之箭矢ol: {
							...
							subSkill: {
								...
								2222: {
									...
									forced: true,
									filter(event, player) {
										return event.target && event.target != player && event.target.storage.nyhzr火之箭矢ol > 0 && event.target.countCards('h') > 0;
									},
									content() {
						...
						nyhzr元素转化ol: {
							...
							content() {
								...
								player.chooseControl('火', '雷', '无属性', ui.create.dialog('请选择转化后的属性', 'hidden')).ai = function (event, player) {
									if (trigger.player.getEquips(2) && trigger.player.getEquips(2).name == 'tengjia') return '火';
								};
								('step 1');
								if (result.control == '火') {
									trigger.nature = 'fire';
						...
						nyhzr时光缓行ol: {
							...
							content() {
								...
								if (get.color(result.card) == 'black' && trigger.source) {
									trigger.source.disableSkill('', trigger.source.getCards('s'));
						...
						nyhzr魔能法球ol: {
							...
							content() {
								...
								pl.showHandcards();
								if (p.getCards('h', 'shan').length) player.useCard({ name: 'sha' }, pl, false);
							},
							ai: {
								result: {
									player: 1,
						...
						nyhzr树灵爆发ol: {
							...
							usable: 1,
							content() {
								if (!player.getEquips(2)) player.draw();
								player.useCard({ name: 'sha' }, player, false);
							},
							ai: {
								result: {
									player(player) {
										if (player.getEquips(2)) return 0;
										return player.hp - 1;
									},
								},
							},
						...
						nyhzr凝水之盾ol: {
							...
							filter(event, player) {
								return event.source && event.source.countCards('h') > 0 && player.countCards('h') > 0;
							},
							content() {
								'step 0';
						...
						nyhzr鬼雄ol: {
							...
							subSkill: {
								...
								3333: {
									...
									forced: true,
									filter(event, player) {
										return player.storage.nyhzr鬼雄ol1 != 1 && event.source && event.source.countCards('h') > 0;
									},
									content() {
										'step 0';
										...
										if (showTwoCard[0].number + showTwoCard[1].number >= 10) {
											player.chooseCardButton('选择' + get.translation(trigger.source) + '的一张手牌弃置', trigger.source.getCards('h'));
										} else {
											event.finish();
										}
										('step 1');
						...
						nyhzr腐肉吸噬ol: {
							...
							content() {
								'step 0';
								event.cards = get.cards(player.hp - player.countCards('h'));
						...
						nyhzr通天法眼ol: {
							...
							content() {
								...
								('step 2');
								player.chooseCardButton(get.translation(event.target) + '的手牌', event.target.getCards('h'));
								('step 3');
								targetCards = event.target.getCards('h');
						...
						nyhzr圣光法球ol: {
							...
							filter(event, player) {
								return player.countCards('h') >= 1 && player != event.player;
							},
							forced: true,
							content() {
								'step 0';
								player.chooseBool('是否弃置一张手牌并令' + get.translation(trigger.player) + '进行一个额外的摸牌阶段？').ai = function (event, player) {
									return get.attitude(player, trigger.player) > 0 && trigger.player.countCards('h') < 3;
								};
								('step 1');
								if (result.bool) {
									player.chooseToDiscard(1, 'h', true);
						...
						nyhzr欲望灾火ol: {
							...
							content() {
								'step 0';
								if (targets[0].countCards('h') == 0) targets[0].draw();
								if (targets[1].countCards('h') == 0) targets[1].draw();
						...
						nyhzr生存本能ol: {
							...
							mod: {
								targetEnabled(card, player, target) {
									if (player != target && target.hp < target.maxHp && target.countCards('h') == 0) return false;
								},
							},
							trigger: {
								player: 'damageAfter',
						...
						nyhzr地狱之躯ol: {
							...
							mod: {
								targetEnabled(card, player, target) {
									if (card.name == 'sha' && player.getEquips(1) && target.storage.nyhzr地狱之躯ol != undefined && !target.storage.nyhzr地狱之躯ol.includes(player)) return false;
								},
							},
							init(player) {
						...
						'nyhzr♠️️暗杀ol': {
							...
							content() {
								...
								for (var i = 0; i < player.getCards('h').length; i++) {
									if (player.storage.nyhzr爆头ol == undefined) player.storage.nyhzr爆头ol = [];
									player.storage.nyhzr爆头ol.push(player.getCards('h')[i]);
									player.markSkill('nyhzr爆头ol');
								}
								setTimeout(function () {
									game.broadcastAll(function (player) {
							...
							content() {
								...
								if (result.control == '获得技能') {
									...
									for (var i of game.players) {
										//QQQ
										if (i != player) {
											for (var j = 0; j < i.getCards('s').length; j++) {
												if (!player.hasSkill(i.getCards('s')[j])) skill.push(i.getCards('s')[j]);
											}
										}
									}
						...
						nyhzr火眼金睛ol: {
							...
							content() {
								...
								player.chooseCardButton(get.translation(event.target) + '的手牌', event.target.getCards('h'));
								('step 3');
								for (var i = 0; i < event.target.getCards('h').length; i++) {
									if (event.target.getCards('h')[i].name == 'sha') {
										player.storage.nyhzr七十二变ol++;
									}
								}
								if (event.targe.getCards('h', 'sha').length == 0) player.draw();
							},
						},
						_nyhzr七十二变ol: {
							trigger: {
						...
						nyhzr弹药填装ol: {
							...
							content() {
								...
								player.draw(2);
								if (nyhzr弹药填装oltarget.countCards('h') == 0) nyhzr弹药填装oltarget.draw();
						...
						nyhzr勾魂大法ol: {
							...
							content() {
								for (var i of game.players) {
									//QQQ
									if (i != player && i.sex == 'male' && i.countCards('h') > 0) {
						...
						nyhzr宿敌之剑ol: {
							...
							content() {
								...
								if (result.control == '弃牌或流失体力') {
									var list = ['流失一点体力'];
									if (target.countCards('h') >= 2) list.push('弃置两张手牌');
									target.chooseControl(list).ai = function () {
										if (target.countCards('h') >= 2) return '弃置两张手牌';
										return '流失一点体力';
									};
								}
						...
						_nyhzr颠覆之海ol: {
							...
							content() {
								...
								player.turnOver = function (all) {
									if (player.countCards('h') > 0) player.chooseToDiscard(1, 'h', true);
									return game.kong;
								};
							},
						},
						nyhzr银箭ol: {
							...
							forced: true,
							content() {
								var skill = trigger.player.getCards('s').randomGet();
						...
						nyhzr逐阳之弓ol1: {
							...
							content() {
								var cards = [];
								for (var i = 0; i < trigger.player.getCards('hej').length; i++) {
									if (get.number(trigger.player.getCards('hej')[i]) - trigger.card.number <= 2 && get.number(trigger.player.getCards('hej')[i]) - trigger.card.number >= -2) cards.push(trigger.player.getCards('hej')[i]);
								}
									...
									if (i.storage.nyhzr青莲刃ol > 0) {
										for (var j = 0; j < i.storage.nyhzr青莲刃ol; j++) {
											if (Math.random() <= 0.1) {
												...
												if (i.countCards('h') > 0) i.discard(i.getCards('h').randomGet());
											}
											game.log(i, '被扇骨刺伤');
						...
						_nyhzr弦月ol: {
							...
							content() {
								if (Math.random() <= 0.2 && trigger.player.countCards('he') > 0) trigger.player.chooseToDiscard(1, 'he', true);
							},
						},
						_nyhzr凸月ol: {
							trigger: {
						...
						nyhzr月潮引力ol: {
							...
							filter(event, player) {
								...
								for (var i of game.players) {
									//QQQ
									if (i != player && i.countCards('h')) return player.storage.nyhzr月之轮回ol > 3;
								}
								return false;
							},
							content() {
								...
								if (num > target.getCards('h').length) num = target.getCards('h').length;
								...
								content(storage, player, skill) {
									...
									for (var i of game.players) {
										...
										if (player != i) {
											...
											if (i.getCards('h').length == 0) str += '无手牌';
											for (var j = 0; j < i.getCards('h').length; j++) {
												str += lib.translate[i.getCards('h')[j].name] + ',';
											}
											str += '<br>';
										}
									}
						...
						nyhzr月之惩罚ol1: {
							content() {
								...
								var TRF = false;
								for (var i = 0; i < player.getCards('h').length; i++) {
									if (player.getCards('h')[i].name == 'nyhzr月饼ol') TRF = true;
								}
				...
				const QQQ = {
					...
					skill: {
						...
						bumo: {
							...
							forced: true,
							content() {
								player.discard(player.getCards('hej'), 999);
							},
						},
						shenglitiaojian1: {
							trigger: {
						...
						nyhzr千叶斩Chessboss: {
							...
							content() {
								...
								if (qyzsha && player.countCards('h') > player.countCards('e') && target.hp > 0) {
									player.gain(qyzsha, 'gain2');
									player.useCard(qyzsha, target);
									player.chooseToDiscard(1, 'h', true);
								} else {
					...
					skill: {
						...
						lielongshilan: {
							...
							content() {
								'step 0';
								var list = ['流失一点体力'];
								if (trigger.player.countCards('h') >= 1) list.push('弃置一张手牌');
								trigger.player.chooseControl(list).set('ai', function () {
									if (trigger.player.countCards('h') >= 1) return '弃置一张手牌';
									return '流失一点体力';
								});
								('step 1');
						...
						lielongqianniao: {
							...
							filter(event, player) {
								return event.source && _status.currentPhase != player && player.countCards('h') >= 2;
							},
							content() {
								player.chooseToDiscard(2, 'h', true);
						...
						lielongshenggen: {
							...
							content() {
								...
								target.loseHp();
								player.storage.lielongshenggen1 = target.countCards('h');
								player.discard(player.getCards('h'));
								player.recover(2);
				...
				const QQQ = {
					...
					skill: {
						...
						nyhzr冰天雪地1: {
							...
							forced: true,
							filter(event, player) {
								return player.countCards('h') > 0 && !player.hasSkill('nyhzr三重卡组');
							},
							content() {
								player.chooseToDiscard(1, 'h', true);
							},
							...
							content() {
								for (var i of game.players) {
									...
									if (i != player) {
										...
										i.storage.nyhzr道标1 = i.getCards('h');
										i.lose(i.getCards('h'));
