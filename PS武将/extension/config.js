...
export const CONFIG = {
	...
	/* "PS_jiaqun": {
		name: '交流群<img style="vertical-align: text-top; transition: all .8s; linear; transform: rotate(-90deg); width:16px;" src=extension/PS武将/image/other/T2.png>',
		clear: true,
		onclick () {
			if (this.jiaqun == undefined) {
				this.jiaqun = ui.create.div('.PSjiaqun');
				this.icon = this.querySelector('img');
				var more = ui.create.div('.PSjiaqun-content', `<a href="https://qm.qq.com/q/Lm30YLypeq"><img src="extension/PS武将/image/other/QQgroup.jpg" style="width:100%; vertical-align:bottom;"></a>`);
	...
	PS_splash: {
		...
		textMenu(node, link) {
			...
			if (link === 'default') {
				...
				node.innerHTML = `<div class="PSselect-item" style="background:url(extension/PS武将/image/splash/${link}.jpg) no-repeat right center/cover"><span style="font-family: xingkai, xinwei;">${node.innerText}</span></div>`;
			}
		},
	},
	/* "PS_prefix": {
...
textMenu (node, link) {
	   ...
	   node.innerHTML = `<div class="PSselect-item" style="width: 121px; height: 144px; border-radius: 12px; padding-top: 60px; background:url(extension/PS武将/image/prefix/${link}.jpg) no-repeat right center/cover"><span style="font-family: xingkai, xinwei;font-size:20px" class="firetext">${node.innerText}</span></div>`;
	 },
   }, */
	//切换BGM
	Background_Music: {
		name: `背景音乐<div class="PSmusic-container"><div class="PSneedle" style="background: url(extension/PS武将/image/music/needle.png) no-repeat 0 0/cover"></div><div class="PSrecord-box"><div class="PSrecord" style="background: url(extension/PS武将/image/music/coverall.png) no-repeat -140px -580px"></div><div class="PSrecord-img" style="background: url(extension/PS武将/image/music/${lib.config.extension_PS武将_Background_Music || '1'}.jpg) no-repeat 0 0/cover"></div></div></div>`,
		clear: true,
		...
		visualMenu(node, link) {
			...
			node.style.cssText = `width: 94px; height: 80px; box-sizing: border-box; border-radius: 10px 0 0 10px; margin: 8px; background: url(extension/PS武将/image/music/coverall.png) no-repeat -240px -1120px`;
			node.innerHTML = `<div style="width: 80px; height: 80px; box-sizing: border-box; border-radius: 10px; font-family: xingkai, xinwei; padding: 3px; background:url(extension/PS武将/image/music/${link}.jpg) no-repeat right center/cover ">${node.innerText}</div>`;
		},
	},
	PS_spCharacter: {
		name: '特殊武将',
