		...
		precontent() {
			...
			if (lib.skill._chenhuodajie) {
				lib.skill._chenhuodajie.filter = function (event, player) {
					...
					if (player.get('s').concat('SE_qiyuan')) return true;
					return false;
				};
			}//QQQ
			lib.group.push('zha');
			...
			lib.skill.SE_xuanyan = {
				...
				check(event, player) {
					...
					if (att > 2 && !event.player.num('h', 'tao') && !event.player.num('h', 'jiu')) return true;
					return false;
				},
				filter(event, player) {
			...
			lib.skill.SE_xuanyan2 = {
				...
				content() {
					...
					player.chooseControl('basic', 'equip', 'trick', 'delay').ai = function (event) {
						if (player.num('h', { type: 'basic' }) < 2) return 'basic';
						if (!player.num('h', { type: 'trick' })) return 'trick';
					...
					var num = Math.abs(player.num('h') - player.maxHp);
					var cards = [];
			...
			lib.skill.SE_poxiao = {
				...
				filter(event, player) {
					var num = player.storage.wumai.length;
					return num > player.num('h') && !player.storage.SE_poxiao;
				},
				init(player) {
			...
			lib.skill.wumai = {
				...
				trigger: { player: 'damageEnd' },
				filter(event, player) {
					return event.num > 0 && event.source && event.source.num('hej');
				},
				init(player) {
				...
				content() {
					'step 0';
					player.chooseCardButton(trigger.source.get('hej'), '选择' + trigger.num + '张牌置于你的武将牌上', trigger.num).ai = function (button) {
			...
			lib.skill.SE_sheyin = {
				...
				forced: true,
				filter(event, player) {
					return player.num('h') > 0 && event.num > 0 && event.source && event.source.classList.contains('linked');
				},
				content() {
					'step 0';
			...
			lib.skill.SE_xinxianshi = {
				...
				content() {
					...
					player.chooseControl('抵消伤害', '继续结算', ui.create.dialog('请选择一项', 'hidden')).ai = function () {
						if (player.num('h', { name: 'shan' }) >= 1 && get.type(event.card) == 'basic') return '继续结算';
						return '抵消伤害';
					};
					('step 1');
			...
			lib.skill.SE_juexian = {
				...
				content() {
					...
					player.chooseControl('red', 'black', ui.create.dialog('请选择一种颜色', 'hidden')).ai = function (event) {
						if (target.num('h', { color: 'red' }) > target.num('h', { color: 'black' })) return 'red';
						return 'black';
					};
					('step 1');
					...
					('step 2');
					if (event.choice == 'red') {
						target.discard(target.get('h', { color: 'red' }));
						player.addTempSkill('SE_juexian3', { player: 'phaseBegin' });
					} else {
						target.discard(target.get('h', { color: 'black' }));
			...
			lib.skill.SE_luxian = {
				...
				_priority: -20,
				content() {
					if (trigger.player.num('h') > trigger.player.hp) {
						var num = trigger.player.num('h') - trigger.player.hp;
						trigger.player.chooseToDiscard(num, 'h', true);
					} else if (trigger.player.num('h') < trigger.player.hp) {
			...
			lib.skill.SE_yuzhi = {
				...
				content() {
					...
					event.choice = result.control;
					player.popup(event.choice);
					event.cards = target.get('he').randomGet();
			...
			lib.skill.SE_xuyu = {
				...
				filter(event, player) {
					...
					return event.player != player && event.player.num('h') > player.hp;
				},
				content() {
					'step 0';
					var num = trigger.player.num('h') - player.hp;
			...
			lib.skill.SE_yongheng = {
				...
				check(event, player) {
					...
					if (player.num('h') < player.maxHp - 1) return false;
			...
			lib.skill.SE_yongheng2 = {
				...
				content() {
					var num = player.num('h') - player.maxHp;
			...
			lib.skill.SE_qiyuan12 = {
				...
				content() {
					...
					var num = target.num('h', { type: 'trick' });
					var cards = target.get('h', { type: 'trick' });
					if (num > 0) {
			...
			lib.skill.SE_qiyuan3 = {
				...
				content() {
					...
					if (targets.length) {
						...
						var dialog = ui.create.dialog('请选择需要声明的1张基本牌', [list, 'vcard']);
						player.chooseButton(dialog, function (button) {
							if (!player.getStat().card.sha && player.num('h', { name: 'sha' })) return button.link[2] == 'tianxianjiu' ? 1 : -1;
							if (player.hp == 1 && !player.num('h', { name: 'tao' })) return button.link[2] == 'pantao' ? 1 : -1;
							if (!player.num('h', { name: 'jiu' }) && player.num('h', { name: 'sha' })) return button.link[2] == 'jiu' ? 1.5 : -1.5;
							if (!player.num('h', { name: 'sha' }) && !player.getStat().card.sha) return button.link[2] == 'shihuawuqi' ? 1 : -1;
							if (player.num('h') > player.hp) return button.link[2] == 'jihuocard' ? 1 : -1;
							if (player.num('h') < 2) return button.link[2] == 'xingjiegoutong' ? 1 : -1;
					...
					('step 4');
					var target = event.target;
					var num = target.num('h', { type: 'basic' });
					var cards = target.get('h', { type: 'basic' });
					if (num > 0) {
						target.lose(cards)._triggered = null;
						target.$throw(cards);
						game.log(target, '弃置了', cards);
			...
			lib.skill.SE_kuilei2 = {
				...
				content() {
					...
					if (trigger.source && trigger.source != player) {
						var cards = trigger.source.get('h', 'sha');
						if (cards.length) {
			...
			lib.skill.SE_hougong = {
				...
				trigger: { global: 'phaseBegin' },
				filter(event, player) {
					return event.player.sex == 'female' && event.player.num('h') > 0;
				},
				content() {
			...
			lib.skill.SE_xianjing = {
				...
				content() {
					...
					var num = Math.min(player.num('h'), num2);
			...
			lib.skill.SE_xianjing2 = {
				...
				content() {
					...
					('step 10');
					if (player.get('h').includes(event.card)) {
			...
			lib.skill.SE_zhenlizhimeng = {
				...
				content() {
					...
					('step 1');
					var num = trigger.player.get('hej');
			...
			lib.skill.SE_shenpan = {
				...
				filterTarget(card, player, target) {
					return player != target && target.num('h') > 0 && player.num('h') > 0;
				},
				selectTarget: [1, 3],
				selectCard: 3,
				check(card) {
			...
			lib.skill.SE_shuihuofu = {
				...
				filterTarget(card, player, target) {
					var num = ui.selected.cards.length;
					return player != target && target.num('he') >= num;
				},
				check(card) {
					if (_status.event.player.hp == _status.event.player.maxHp) {
						return 10 - get.value(card);
			...
			lib.skill.SE_jixian = {
				...
				content() {
					...
					event.target = game.players.randomGet(player);
					('step 2');
					var num = event.target.get('h');
					player.gain(event.target.get('h'));
					event.target.$give(event.target.get('h'), player);
				},
			};
						...
						if (Array.isArray(event.cards)) for (var i of event.cards) {
							if (i.original == 'h') {
								for (var j = 0; j < game.players.length; j++) {
									...
									if (game.players[j].num('h')) return true;
								}
							}
						}
						return false;
					...
					if (!player.num('e')) {
						...
						if (Array.isArray(event.cards)) for (var i of event.cards) {
							if (i.original == 'e') {
								for (var j = 0; j < game.players.length; j++) {
									if (game.players[j] == player) continue;
									if (game.players[j].num('e')) return true;
								}
							}
						}
						return false;
					...
					if (!player.num('j')) {
						if (Array.isArray(event.cards)) for (var i of event.cards) {
							if (i.original == 'j') {
								for (var j = 0; j < game.players.length; j++) {
									if (game.players[j] == player) continue;
									if (game.players[j].num('j')) return true;
								}
							}
						}
						return false;
					...
					effect: {
						target(card, player, target) {
							if (get.type(card) == 'trick') {
								if (player.hp == 1 && !player.num('h', 'tao')) {
									return [0, 1, 0, -10];
								}
								if (player.hp == player.maxHp) {
									return [0, 1, 0, -1];
			...
			lib.skill.SE_wuyuzhiye = {
				...
				forced: true,
				filter(event, player) {
					return player.num('h', { type: 'delay' }) || player.num('h', { type: 'trick' });
				},
				content() {
					'step 0';
					player.showHandcards();
					('step 1');
					var cards0 = player.get('h');
					var cards1 = [];
					...
					var num = player.num('h', { type: 'trick' }) + player.num('h', { type: 'delay' });
			...
			lib.skill.SE_duansheng = {
				...
				content() {
					...
					if (result.bool) {
						event.target = result.targets[0];
						var num0 = event.target.num('e');
						var num1 = player.num('e');
						if (num1 >= num0) {
			...
			lib.skill.se_youhuo = {
				...
				content() {
					...
					target.chooseControl('获得你的牌', '流失体力', ui.create.dialog('请选择一项', 'hidden')).ai = function () {
						if (target.num('he') < 2) return '获得你的牌';
						if (target.num('h', 'tao') || target.num('h', 'jiu')) return '流失体力';
						if (target.hp == 1 && (!target.num('h', 'tao') || !target.num('h', 'jiu'))) return '获得你的牌';
					...
					('step 2');
					var num = Math.min(game.players.length, target.num('he'));
					if (target.num('he') > 0) {
